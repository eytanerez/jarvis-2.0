...
This document describes Jarvis's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/jarvis-brain/security/advisories/new)
...
- Environment details (`jarvis version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Jarvis access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Jarvis's documentation
...
Jarvis supports two OS-level isolation postures. They address
...
Jarvis supports this in two ways:

- **Jarvis's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Jarvis's in-process heuristics
...
Jarvis filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Jarvis
itself. Bugs in Jarvis's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Jarvis does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Jarvis's own documentation, or reasonable
  operator expectations would predict — including cases where
  Jarvis has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  jarvis-home security, and similar. Reports against those
