...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("JARVIS_HOME", os.path.expanduser("~/.jarvis")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("JARVIS_HOME", Path.home() / ".jarvis")) / "skills" / "red-teaming" / "godmode"
...
JARVIS_HOME = Path(os.getenv("JARVIS_HOME", Path.home() / ".jarvis"))
CONFIG_PATH = JARVIS_HOME / "config.yaml"
PREFILL_PATH = JARVIS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Jarvis models — already uncensored, just needs clean prompt
    "jarvis": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "jarvis" in model_lower or "nous" in model_lower:
        return "jarvis"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Jarvis config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.jarvis/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Jarvis for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Jarvis for changes to take effect.")
