...
def _check_config():
    ...
    if missing:
        jarvis_env = os.path.join(
            os.environ.get("JARVIS_HOME", os.path.expanduser("~/.jarvis")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {jarvis_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Jarvis"
    )
