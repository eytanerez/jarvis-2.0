#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `jarvis kanban list` and
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed jarvis CLI."""
    try:
        out = subprocess.run(
            ["jarvis", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `jarvis kanban list`
    out = subprocess.run(
        ["jarvis", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["jarvis", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
