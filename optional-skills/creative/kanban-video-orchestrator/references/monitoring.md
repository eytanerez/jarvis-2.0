...
jarvis kanban watch --tenant <project-slug>

# Snapshot of the board
jarvis kanban list --tenant <project-slug>
jarvis kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
jarvis kanban stats --tenant <project-slug>

# Visual dashboard (browser)
jarvis dashboard

# Inspect a specific task (includes comments + events)
jarvis kanban show <task-id>

# Follow a single task's event stream
jarvis kanban tail <task-id>
```

Verify available subcommands with `jarvis kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `jarvis kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `jarvis kanban block <id>` then `jarvis kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `jarvis kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `jarvis kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `jarvis profile list` (profile exists?), `jarvis status` (gateway/dispatcher up?), and verify tenant. |
...
jarvis kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
jarvis kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(jarvis kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
jarvis kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
jarvis kanban block <task-id>
jarvis kanban archive <task-id>

# Diagnose what happened
jarvis kanban show <task-id>      # task body, comments, recent events
jarvis kanban tail <task-id>      # follow the live event stream
jarvis kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `jarvis kanban create` for the director
...
while true; do
    clear
    jarvis kanban list --tenant <slug>
    echo "---"
    jarvis kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `jarvis kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `jarvis profile list` and start
