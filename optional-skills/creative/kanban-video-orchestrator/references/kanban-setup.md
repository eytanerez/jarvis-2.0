...
the actual `setup.sh` that creates the project workspace, configures Jarvis
...
2. **Create profiles** — `jarvis profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.jarvis/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `jarvis kanban create` assigned to the director
...
```bash
jarvis profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.jarvis/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.jarvis/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Jarvis
...
Each profile gets a `SOUL.md` at `~/.jarvis/profiles/<name>/SOUL.md` that
...
```bash
jarvis kanban create "Direct production of <video title>" \
    --assignee director \
...
the Jarvis `.env` (`${JARVIS_HOME:-$HOME/.jarvis}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _jarvis_env="${JARVIS_HOME:-$HOME/.jarvis}/.env"
    if grep -q "^${var}=" "$_jarvis_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_jarvis_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_jarvis_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY jarvis ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY jarvis OPENROUTER_API_KEY || exit 1
# ...
```
