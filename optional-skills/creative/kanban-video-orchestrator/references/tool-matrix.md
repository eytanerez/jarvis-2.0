...
Maps each role archetype to the Jarvis skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public jarvis-agent
...
## Jarvis skills relevant to video production

### Visual / rendering skills (`jarvis-brain/skills/creative/`)
...
### Design / pre-production skills (`jarvis-brain/skills/creative/`)
...
### Audio / media skills (`jarvis-brain/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`jarvis-brain/skills/devops/`)
...
These are **not** Jarvis skills but external CLIs / APIs that profiles invoke.
...
## Built-in Jarvis tools for media review

These are native Jarvis tools — not invoked via terminal but through their own
...
key is present in `${JARVIS_HOME:-~/.jarvis}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Jarvis profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${JARVIS_HOME:-~/.jarvis}/.env` → environment variable.
...
When a new Jarvis-public video skill ships:
