...
Usage (via cron with --no-agent):
    ...
    jarvis cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$JARVIS_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Jarvis-Watcher/1.0"})
