...
_UNSET = object()
_JARVIS_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_JARVIS_HOME_OVERRIDE", default=_UNSET
)


def set_jarvis_home_override(path: str | Path | None) -> Token:
    """Set a context-local Jarvis home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _JARVIS_HOME_OVERRIDE.set(value)


def reset_jarvis_home_override(token: Token) -> None:
    """Restore the previous context-local Jarvis home override."""
    _JARVIS_HOME_OVERRIDE.reset(token)


def get_jarvis_home_override() -> str | None:
    """Return the active context-local Jarvis home override, if any."""
    override = _JARVIS_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_jarvis_home() -> Path:
...
def get_jarvis_home() -> Path:
    """Return the Jarvis home directory (default: platform-native path).

    Reads JARVIS_HOME env var, falls back to the platform-native default.
    ...
    ``jarvis_cli/kanban_db.py``).  See https://github.com/NousResearch/jarvis-brain/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_jarvis_home()
    ...
    return _get_platform_default_jarvis_home()


def get_default_jarvis_root() -> Path:
    """Return the root Jarvis directory for profile-level operations.

    In standard deployments this is the platform-native Jarvis home
    (``~/.jarvis`` on POSIX, ``%LOCALAPPDATA%\\jarvis`` on native Windows).
    ...
    """
    native_home = _get_platform_default_jarvis_home()
    ...
    try:
        ...
        # JARVIS_HOME is under ~/.jarvis (normal or profile mode)
        return native_home
    except ValueError:
        pass
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Jarvis is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_jarvis_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_jarvis_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        ...
        4. ``<JARVIS_HOME>/skills`` last-resort
    """
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/NousResearch/jarvis-brain/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    ...
    jarvis_home = get_jarvis_home_override() or (env or {}).get("JARVIS_HOME") or os.getenv("JARVIS_HOME")
    if not jarvis_home:
        return None
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Jarvis profile HOME.

    ``JARVIS_HOME`` scopes Jarvis state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={JARVIS_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{JARVIS_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{JARVIS_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Jarvis' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["JARVIS_REAL_HOME"] = real_home
...
def is_container() -> bool:
    ...
    See: NousResearch/jarvis-agent#47111
    """
...
def get_config_path() -> Path:
    ...
    in 7+ files (skill_utils.py, jarvis_logging.py, jarvis_time.py, etc.).
    """
    return get_jarvis_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under JARVIS_HOME."""
    return get_jarvis_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under JARVIS_HOME."""
