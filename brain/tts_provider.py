...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Jarvis with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.jarvis/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Jarvis
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``jarvis tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``jarvis setup`` for
