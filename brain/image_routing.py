...
def _supports_vision_override(
    ...
    # (jarvis_cli/runtime_provider.py:_resolve_named_custom_runtime), so the
...
# ``run_brain._try_shrink_image_parts_in_messages`` to shrink + retry if
...
def _file_to_data_url(path: Path) -> Optional[str]:
    ...
    (``run_brain._try_shrink_image_parts_in_messages``) shrinks on the
...
def build_native_content_parts(
    ...
    once — see ``run_brain._try_shrink_image_parts_in_messages``.
