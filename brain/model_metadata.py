...
Pure utility functions with no AIBrain dependency. Used by ContextCompressor
and run_brain.py for pre-flight context checks.
"""
...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Jarvis also honours JARVIS_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("JARVIS_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_jarvis_home() / "cache" / "openrouter_model_metadata.json"
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Jarvis from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Jarvis' system prompt, but mapping
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Jarvis grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from jarvis_cli.models import get_copilot_model_context
    ...
    if effective_provider:
        from brain.models_dev import lookup_models_dev_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Jarvis sends to providers:
