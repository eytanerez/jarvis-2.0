...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Jarvis starts and finds ``~/.openclaw/``.

    Points users at ``jarvis claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``jarvis claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Jarvis, run "
        "`jarvis claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `jarvis claw cleanup` (renames it to ~/.openclaw.pre-migration — "
