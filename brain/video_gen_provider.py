...
as ``kind: backend``) or ``~/.jarvis/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).

Mirrors the ``image_gen`` provider design (``brain/image_gen_provider.py``) so
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``jarvis tools``
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$JARVIS_HOME/cache/videos/``.
