"""Anthropic Messages API adapter for Jarvis.

Translates between Jarvis's internal OpenAI-style message format and
...
# Jarvis effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    from brain.azure_identity_adapter import build_bearer_http_client
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    path in ``run_brain.py`` when a subscription rejects the beta; leave at
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            # and chmod. Mirrors brain/google_oauth.py (#19673) and
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Jarvis historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Jarvis)
    ...
    # 1. Jarvis-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Jarvis configs.
...
# ── Jarvis-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.jarvis/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_JARVIS_OAUTH_FILE = get_jarvis_home() / ".anthropic_oauth.json"
...
def run_jarvis_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print()
    print("Authorize Jarvis with your Claude Pro/Max subscription.")
    print()
...
def read_jarvis_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if _JARVIS_OAUTH_FILE.exists():
        try:
            data = json.loads(_JARVIS_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Jarvis OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See jarvis-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for m in result:
        if m["role"] == "assistant" and isinstance(m["content"], list):
            ...
            # replaying it verbatim.  See jarvis-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See jarvis-agent#13848 (Kimi) and
    jarvis-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Jarvis", "Claude Code")
                text = text.replace("Jarvis agent", "Claude Code")
                text = text.replace("jarvis-agent", "claude-code")
        ...
        #      a) bare Jarvis-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Jarvis surfaces in its CLI. We
