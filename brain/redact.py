...
# `export JARVIS_REDACT_SECRETS=false`) cannot disable redaction
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Jarvis — used by
    ``jarvis config``, ``jarvis status``, ``jarvis dump``, and anywhere
...
def redact_sensitive_text(text: str, *, force: bool = False, code_file: bool = False) -> str:
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical jarvis log line
