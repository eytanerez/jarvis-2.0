...
Pure functions extracted from ``run_brain.py`` so the AIBrain module can
...
re-exports from ``run_brain`` remain in place so existing imports
``from run_brain import _sanitize_surrogates`` keep working.
"""
...
# below as well as by run_brain and the CLI for paste-from-clipboard
# scrubbing.
