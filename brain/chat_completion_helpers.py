...
Extracted from :class:`AIBrain` for cleanliness — bodies of the
...
Each function takes the parent ``AIBrain`` as its first argument
(``agent``).  :class:`AIBrain` keeps thin forwarder methods so call
sites unchanged.  Symbols that tests patch on ``run_brain`` (e.g.
...
from jarvis_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from jarvis_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
from brain.error_classifier import FailoverReason
from brain.model_metadata import is_local_endpoint
...
def _ra():
    """Lazy ``run_brain`` reference.

    Used to honor test patches like
    ``patch("run_brain.cleanup_vm")`` / ``patch("run_brain.cleanup_browser")``
    that target symbols imported into ``run_brain``'s namespace.
    """
    import run_brain
    return run_brain
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # Request-local cancellation flag. Distinct from brain._interrupt_requested
    ...
    # AIBrain instances per session). Tracks whether THIS specific request was
    ...
    # activity. Operators can tune via JARVIS_CODEX_TTFB_TIMEOUT_SECONDS and
    # JARVIS_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    # JARVIS_CODEX_TTFB_TIMEOUT_SECONDS=0 to disable this watchdog entirely.
    _ttfb_enabled = _codex_watchdog_enabled
    _ttfb_timeout = _env_float("JARVIS_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float("JARVIS_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 25_000.0)
        _ttfb_strict = os.environ.get("JARVIS_CODEX_TTFB_STRICT", "").strip().lower() in {
            "1", "true", "yes", "on"
        }
        if (
            not _ttfb_strict
            ...
            logger.info(
                ...
                "Set JARVIS_CODEX_TTFB_STRICT=1 to force early reconnects.",
                f"{_est_tokens_for_codex_watchdog:,}",
                _ttfb_disable_above,
            )
        else:
            _ttfb_cap = _env_float("JARVIS_CODEX_TTFB_MAX_SECONDS", 120.0)
            if _ttfb_cap > 0 and _ttfb_timeout > _ttfb_cap:
                logger.info(
                    "Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                    "(context=~%s tokens). Set JARVIS_CODEX_TTFB_MAX_SECONDS to tune.",
                    _ttfb_timeout,
                    _ttfb_cap,
                    f"{_est_tokens_for_codex_watchdog:,}",
                )
    ...
    _codex_idle_timeout = _env_float(
        "JARVIS_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    try:
        from brain.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE
    ...
    if (_is_or or _is_nous) and "claude" in (agent.model or "").lower():
        try:
            from brain.anthropic_adapter import _get_anthropic_max_output
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "jarvis",
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # Respects JARVIS_REDACT_SECRETS via redact_sensitive_text — no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
    ...
    # brain/transports/anthropic.py and brain/anthropic_adapter.py.
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        if not fb_api_key_hint:
            ...
            # _normalize_custom_provider_entry in jarvis_cli/config.py).
        ...
        if fb_api_mode == "anthropic_messages":
            ...
            from brain.anthropic_adapter import build_anthropic_client, resolve_anthropic_token, _is_oauth_token
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    try:
        ...
        for msg in messages:
            ...
            # and every Jarvis-internal underscore-prefixed scaffolding key.
        ...
        summary_extra_body = {}
        try:
            from brain.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE as _OMIT_TEMP
        except Exception:
        ...
        if _is_nous:
            from brain.portal_tags import nous_portal_tags as _portal_tags
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions():
        ...
        # wins over the JARVIS_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else float(os.getenv("JARVIS_API_TIMEOUT", 1800.0))
        )
        # Read timeout: config wins here too.  Otherwise use
        # JARVIS_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = float(os.getenv("JARVIS_STREAM_READ_TIMEOUT", 120.0))
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode JARVIS_STREAM_READ_TIMEOUT.
    ...
    def _call_anthropic():
        ...
        from brain.anthropic_adapter import sanitize_anthropic_kwargs
    ...
    def _call():
        ...
        _max_stream_retries = env_int("JARVIS_STREAM_RETRIES", 2)
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = float(os.getenv("JARVIS_STREAM_STALE_TIMEOUT", 180.0))
    # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
    # for prefill on large contexts.  Disable the stale detector unless
    # the user explicitly set JARVIS_STREAM_STALE_TIMEOUT.
