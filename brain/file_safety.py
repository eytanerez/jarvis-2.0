...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    jarvis_home = _jarvis_home_path()
    jarvis_root = _jarvis_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(jarvis_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(jarvis_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved JARVIS_WRITE_SAFE_ROOT path, or None if unset."""
...
def is_write_denied(path: str) -> bool:
    ...
    jarvis_dirs = []
    for base in (_jarvis_home_path(), _jarvis_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in jarvis_dirs:
                jarvis_dirs.append(real)
        except Exception:
            continue

    for base_real in jarvis_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Jarvis path.

    Three categories are blocked:

      * Internal Jarvis cache files under ``JARVIS_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under JARVIS_HOME and the global Jarvis
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.jarvis/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active JARVIS_HOME (profile-aware) AND the global
    # Jarvis root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (JARVIS_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    jarvis_dirs: list[Path] = []
    for base in (_jarvis_home_path(), _jarvis_root_path()):
        try:
            real = base.resolve()
            if real not in jarvis_dirs:
    ...
    for hd in jarvis_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Jarvis cache file "
    ...
    for hd in jarvis_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Jarvis credential store "
    ...
    for hd in jarvis_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Jarvis MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Jarvis MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Jarvis profiles are separate JARVIS_HOME dirs under
...
#
# Reference: May 2026 incident where a jarvis-security profile session
# edited skills under both ``~/.jarvis/profiles/jarvis-security/skills/``
# AND ``~/.jarvis/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under JARVIS_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from JARVIS_HOME.
    ...
    """
    try:
        home_real = _jarvis_home_path().resolve()
        root_real = _jarvis_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Jarvis scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Jarvis entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Jarvis profile {info['target_profile']!r}, but the "
...
#
#   <JARVIS_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.jarvis/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.jarvis/…`` segment and warns
# regardless of which Jarvis profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.jarvis/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.jarvis`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.jarvis/…`` and returns the
    index where the inner Jarvis-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .jarvis / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".jarvis":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Jarvis state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.jarvis``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.jarvis`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Jarvis resolver to
    succeed, so it works correctly even when called from contexts where
    JARVIS_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Jarvis process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real JARVIS_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.jarvis/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.jarvis/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host JARVIS_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Jarvis state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Jarvis "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real JARVIS_HOME. Use the "
