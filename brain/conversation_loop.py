"""The agent conversation loop — extracted from ``run_brain.AIBrain``.

This is the biggest single chunk pulled out of ``run_brain.py``: the
...
The function takes the parent ``AIBrain`` instance as its first
argument (``agent``) and accesses its state via attribute lookup.
``_ra().AIBrain.run_conversation`` is now a thin forwarder.

Symbols that production code or tests patch on ``run_brain`` directly
...
from brain.codex_responses_adapter import _summarize_user_message_for_log
from brain.display import KawaiiSpinner
from brain.error_classifier import FailoverReason, classify_api_error
from brain.iteration_budget import IterationBudget
from brain.turn_context import build_turn_context
from brain.turn_retry_state import TurnRetryState
from brain.memory_manager import build_memory_context_block
from brain.message_sanitization import (
...
from brain.prompt_caching import apply_anthropic_cache_control
from brain.retry_utils import jittered_backoff
from brain.trajectory import has_incomplete_scratchpad
from brain.usage_pricing import estimate_usage_cost, normalize_usage
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Jarvis tool use: "
    ...
    return (
        ...
        f"context, but Jarvis needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Jarvis config, set `model.ollama_num_ctx: 65536` "
...
def _ra():
    ...
    ``run_brain.handle_function_call`` / ``run_brain._set_interrupt`` /
    ``run_brain.OpenAI`` and have those patches reach this code path.
    """
    import run_brain
    return run_brain
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    silent prefix-cache misses are visible in ``brain.log``:
    ...
    (which constructs a fresh ``AIBrain`` per turn and depends on this
    DB roundtrip).
    """
    stored_prompt = None
    ...
    # gateway path (fresh AIBrain per turn → reads from this row every
    # subsequent turn).
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `jarvis fallback add`."
)
...
def run_conversation(
    ...
    # all run inside Codex). Default Jarvis path is bypassed entirely.
    # See brain/transports/codex_app_server_session.py for the adapter
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        if _pre_api_steer:
            ...
            for _si in range(len(messages) - 1, -1, -1):
                ...
                if isinstance(_sm, dict) and _sm.get("role") == "tool":
                    from brain.prompt_builder import format_steer_marker
        ...
        from brain.agent_runtime_helpers import repair_message_sequence_with_cursor
        ...
        # cache prefix.  The system prompt is reserved for Jarvis internals.
        #
        # Jarvis invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Jarvis tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            if agent.provider == "nous":
                try:
                    from brain.nous_rate_guard import (
            ...
            try:
                ...
                try:
                    from jarvis_cli.middleware import apply_llm_request_middleware
                ...
                if env_var_enabled("JARVIS_DUMP_REQUESTS"):
                ...
                from jarvis_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Jarvis/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    if agent._session_db and agent.session_id:
                        try:
                            ...
                            # visible in brain.log — silent loss here is
                ...
                if agent.provider == "nous":
                    try:
                        from brain.nous_rate_guard import clear_nous_rate_limit
                ...
                # which Jarvis' system prompt + tool schemas alone exceed.
        ...
        try:
            ...
            # brain.log AND errors.log.  Previously this was logged at DEBUG,
                ...
                if msg.get("role") == "assistant" and msg.get("tool_calls"):
                    ...
                    for tc in msg["tool_calls"]:
                        ...
                        if tc["id"] not in answered_ids:
                            err_msg = {
                                "role": "tool",
                                "name": _ra().AIBrain._get_tool_call_name_static(tc),
                                "tool_call_id": tc["id"],
    ...
    # Post-loop turn finalization extracted to brain/turn_finalizer.finalize_turn
    # (god-file decomposition Phase 1 step 4). Behavior-neutral: the assembled
    # result dict is returned exactly as before.
    from brain.turn_finalizer import finalize_turn
