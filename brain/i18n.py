"""Lightweight internationalization (i18n) for Jarvis static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Jarvis itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``JARVIS_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``JARVIS_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    2. ``<repo-root>/locales`` -- source checkouts and ``pip install -e .``,
       where the working tree sits next to ``brain/``.
    ...
    """
    override = os.getenv("JARVIS_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "JARVIS_BUNDLED_LOCALES points to a non-directory path (%s); "
            "falling back to bundled/source locale resolution",
            override,
        )

    # brain/i18n.py -> brain/ -> repo root (source checkout, editable install)
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a jarvis dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`jarvis_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("JARVIS_LANGUAGE")
    if env_lang:
