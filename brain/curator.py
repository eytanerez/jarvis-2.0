...
``interval_hours`` ago, ``maybe_run_curator()`` spawns a forked AIBrain to do
the review.
...
def _state_file() -> Path:
    return get_jarvis_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.jarvis/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `jarvis update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``jarvis curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``jarvis update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``jarvis curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `jarvis curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Jarvis' background skill CURATOR. This is an "
    ...
    "into ~/.jarvis/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: jarvis-config-*, jarvis-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.jarvis/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.jarvis/logs/curator/``)
    alongside ``brain.log`` and ``gateway.log`` so it's found by anyone
    looking for operational telemetry, not mixed in with the user's
    authored skill data in ``~/.jarvis/skills/``.

    ``ensure_jarvis_home()`` pre-creates this dir on every CLI launch and
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every jarvis install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: jarvis curator status
        keep an umbrella stable: jarvis curator pin document-tools

    Cap is 10 entries so a 50-skill consolidation doesn't blow up
    brain.log; the full list is always in REPORT.md. The pin hint only
    ...
    lines.append("full report: jarvis curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: jarvis curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.jarvis/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.jarvis/skills/.archive/` for "
            "safety and can be restored via `jarvis curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.jarvis/skills/.archive/`. "
            "Restore any via `jarvis curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `jarvis curator restore <name>`")
    lines.append("- All archives live under `~/.jarvis/skills/.archive/` and are recoverable by `mv`")
...
# ---------------------------------------------------------------------------
# Orchestrator — spawn a forked AIBrain for the LLM review pass
# ---------------------------------------------------------------------------
...
def run_curator_review(
    ...
    Steps:
      ...
      2. If consolidation is enabled AND there are agent-created skills, spawn
         a forked AIBrain that runs the LLM review prompt against the current
         candidate list.
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``jarvis curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    if dry_run:
        ...
        try:
            from brain import curator_backup
    ...
    # real pass out. We still record a summary so `jarvis curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `jarvis update` only if "
        ...
        # recorded in state so `jarvis curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``jarvis model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    """Spawn an AIBrain fork to run the curator review prompt.
    ...
    try:
        ...
        result_meta["error"] = f"AIBrain import failed: {e}"
    ...
    # "No models provided"). AIBrain() without explicit provider/model
    ...
    # (canonical aux-task slot, wired through `jarvis model` → auxiliary
