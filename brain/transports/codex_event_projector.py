"""Projects codex app-server events into Jarvis' messages list.

The translator that lets Jarvis' memory/skill review keep working under the
...
`brain/curator.py` already knows how to read.
...
Jarvis' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Jarvis
...
Counters tracked alongside projection:
  - tool_iterations: ticks once per completed tool-shaped item. Used by
    AIBrain._iters_since_skill (skill nudge gate, default threshold 10).
"""
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Jarvis' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Jarvis stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Jarvis already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Jarvis' messages store text only.
        text_parts: list[str] = []
