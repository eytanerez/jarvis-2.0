...
and returns a clean turn result that AIBrain.run_conversation() can splice
...
call is synchronous and behaves like AIBrain's existing chat_completions loop.
"""
...
from brain.codex_responses_adapter import _format_responses_error
from brain.redact import redact_sensitive_text
from brain.transports.codex_app_server import (
    CodexAppServerClient,
    CodexAppServerError,
)
from brain.transports.codex_event_projector import CodexEventProjector
...
# Jarvis' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Jarvis/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Jarvis session, lifetime owned by AIBrain.

    Not thread-safe — one caller drives it at a time, matching how AIBrain's
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _JARVIS_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("JARVIS_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="jarvis",
            client_title="Jarvis",
            client_version=_get_jarvis_version(),
        )
    ...
    def request_interrupt(self) -> None:
        ...
        and unwind. Called by AIBrain's _interrupt_requested path."""
    ...
    def run_turn(
        ...
        *,
        ...
        # up to AIBrain.run_conversation.
        result = TurnResult()
        try:
            self.ensure_started()
        ...
        # supports rich content but Jarvis' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Jarvis' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own jarvis-tools callback we
            # auto-accept — the user already approved Jarvis' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Jarvis approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Jarvis returns 'once', 'session', 'always', or 'deny'.
