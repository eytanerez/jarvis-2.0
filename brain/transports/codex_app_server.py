...
projection into Jarvis' display, approval bridging, transcript projection into
AIBrain.messages, plugin migration) live in sibling modules.

Status: optional opt-in runtime gated behind `model.openai_runtime ==
"codex_app_server"`. Jarvis' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    Intentionally NOT async. AIBrain.run_conversation() is synchronous and
    ...
    def __init__(
        ...
        if spawn_env.get("JARVIS_KANBAN_TASK"):
            kanban_db = spawn_env.get("JARVIS_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "JARVIS_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("JARVIS_HOME", os.path.expanduser("~/.jarvis")),
                        "kanban",
                    ),
                )
            )
    ...
    def take_notification(self, timeout: float = 0.0) -> Optional[dict]:
        ...
        AIBrain turn loop to interleave reads with interrupt checks."""
        try:
            if timeout <= 0:
