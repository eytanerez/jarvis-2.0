...
from brain.auxiliary_client import call_llm, _is_connection_error
...
# heuristics. See https://github.com/NousResearch/jarvis-brain/issues/38389
#
# Underscore-prefixed ON PURPOSE: the wire sanitizers
# (brain/transports/chat_completions.py convert_messages and the summary-path
# mirror in brain/chat_completion_helpers.py) strip every top-level message
...
def _strip_historical_media(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    shape the jarvis compressor emits).
    """
    if not messages:
        return messages
...
def _summarize_tool_result(tool_name: str, tool_args: str, tool_content: str) -> str:
    ...
    Returns strings like::
        ...
        [search_files] content search for 'compress' in brain/ -> 12 matches
    """
    try:
...
class ContextCompressor(ContextEngine):
    ...
    def should_defer_preflight_to_real_usage(self, rough_tokens: int) -> bool:
        ...
        overestimates schema-heavy requests so Jarvis compresses before a
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _generate_summary(
        ...
        # user's configured timezone via jarvis_time.now(). The compaction summary
        ...
        try:
            ...
            _today_str = _jarvis_now().strftime("%Y-%m-%d")
