...
This adapter lets Jarvis use the ``google-gemini-cli`` provider as if it were
...
- ``GeminiCloudCodeClient`` exposes ``.chat.completions.create(**kwargs)``
  mirroring the subset of the OpenAI SDK that ``run_brain.py`` uses.
...
from brain.google_code_assist import (
    ...
    def _create_chat_completion(
        ...
        **_: Any,
        ...
        headers = {
            ...
            "User-Agent": "jarvis-agent (gemini-cli-compat)",
            "X-Goog-Api-Client": "gl-python/jarvis",
...
def _gemini_http_error(response: httpx.Response) -> CodeAssistError:
    ...
    if status == 429 and error_reason == "MODEL_CAPACITY_EXHAUSTED":
        ...
        message = (
            ...
            f"not a Jarvis issue). Try a different Gemini model or set a "
        ...
        message = (
            ...
            f"retired. Check jarvis_cli/models.py for the current list."
        )
    elif err_message:
