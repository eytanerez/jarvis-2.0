"""System-prompt assembly for :class:`AIBrain`.

The agent's system prompt is built once per session and reused across all
turns — only context compression triggers a rebuild.  This keeps the
upstream prefix cache warm.  See ``jarvis-agent-dev``'s
...
Pure helpers that read the agent's state.  AIBrain keeps thin forwarders.
"""
...
from brain.runtime_cwd import resolve_context_cwd


def _ra():
    """Lazy reference to the ``run_brain`` module.
    ...
    imported into ``run_brain``'s namespace.  Many tests
    ``patch("run_brain.load_soul_md", ...)``; if we imported them
    directly here those patches would not reach us.  Looking them up
    through ``run_brain`` on every call preserves the patch contract.
    """
    import run_brain
    return run_brain
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIBrain.  Jarvis never re-renders parts of this string mid-
    session — that's the only way to keep upstream prompt caches
    warm across turns.
    """
    # Local import to avoid pulling model_tools at module load.  Tests
    # patch ``run_brain.get_toolset_for_tool`` and similar helpers, so
    ...
    # Some execution modes (cron) still want JARVIS_HOME persona while keeping
    ...
    # Pointer to the jarvis-agent skill + docs for user questions about Jarvis itself.
    stable_parts.append(JARVIS_AGENT_HELP_GUIDANCE)
    ...
    # JARVIS_KANBAN_TASK env var). Normal chat sessions never see
    ...
    if has_skills_tools:
        ...
        try:
            from brain.coding_context import coding_compact_skill_categories
    ...
    # Coding posture (base Jarvis, any interactive coding surface in a code
    # workspace — see brain/coding_context.py). The operating brief + the live
    ...
    if agent.valid_tool_names:
        try:
            from brain.coding_context import coding_system_blocks
    ...
    # Active-profile hint — names the Jarvis profile the agent is running
    # under so it doesn't conflate ~/.jarvis/skills/ (default profile) with
    # ~/.jarvis/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Jarvis profile: default. Other profiles (if any) live "
            "under ~/.jarvis/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Jarvis profile: {active_profile}. This session reads "
            f"and writes ~/.jarvis/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.jarvis/skills/, ~/.jarvis/plugins/, "
            f"~/.jarvis/cron/, ~/.jarvis/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Jarvis never rebuilds or reinjects parts of it
