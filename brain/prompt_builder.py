...
All functions are stateless. AIBrain._build_system_prompt() calls these to
...
JARVIS_AGENT_HELP_GUIDANCE = (
    "You run on Jarvis (by Nous Research). When the user needs help with "
    "Jarvis itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://jarvis-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `jarvis-agent` skill with skill_view(name='jarvis-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.jarvis/kanban.db`. Your task id is in "
    "`$JARVIS_KANBAN_TASK`; your workspace is `$JARVIS_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $JARVIS_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `jarvis kanban <verb>` for board operations. Use "
...
#
# The jarvis-agent runtime already executes a batch of tool calls
# concurrently when they are independent (read-only tools always; path-scoped
# file ops when their targets don't overlap — see
# run_brain._execute_tool_calls / tool_dispatch_helpers). The missing piece
...
# from Cline's TypeScript tool-surface guidance to jarvis-agent's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Jarvis "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Jarvis WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Jarvis itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Jarvis runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Jarvis itself is running. The host OS, home, and cwd "
                f"of the Jarvis process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Jarvis "
    ...
    # Jarvis desktop GUI — any agent running under the desktop app should know
    # it. JARVIS_DESKTOP marks the backend powering the chat; JARVIS_DESKTOP_TERMINAL
    # marks a jarvis launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("JARVIS_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("JARVIS_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Jarvis desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Jarvis
    ...
    extra = (os.getenv("JARVIS_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
# Collect truncation warnings so the caller (run_brain) can surface them.
...
def _skills_prompt_snapshot_path() -> Path:
    return get_jarvis_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.jarvis/skills/`` directory.  External dirs
    are read-only — they appear in the index but new skills are always created
    in the local dir.  Local skills take precedence when names collide.

    ``compact_categories`` (e.g. from the coding posture — see
    brain/coding_context.py) demotes whole categories to a names-only line in
    ...
    _platform_hint = (
        os.environ.get("JARVIS_PLATFORM")
        or get_session_env("JARVIS_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Jarvis itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `jarvis-agent` skill "
            "first. It has the actual commands (e.g. `jarvis config set …`, `jarvis tools`, "
            "`jarvis setup`) so you don't have to guess or invent workarounds.\n"
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from JARVIS_HOME and return its content, or None.
...
def _load_jarvis_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    ...
    try:
        ...
        return _truncate_content(
            result, ".jarvis.md", context_length=context_length,
            read_path=str(jarvis_md_path),
        )
    except Exception as e:
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .jarvis.md / JARVIS.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from JARVIS_HOME is independent and always included when present.
    ...
    project_context = (
        _load_jarvis_md(cwd_path, context_length)
    ...
    # SOUL.md from JARVIS_HOME only — skip when already loaded as identity
    if not skip_soul:
