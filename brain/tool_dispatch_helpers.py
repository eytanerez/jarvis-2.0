...
Pure module-level utilities extracted from ``run_brain.py``:
...
All helpers are stateless.  ``run_brain`` re-exports each name so existing
``from run_brain import ...`` imports in tests and other modules keep
working unchanged.
"""
