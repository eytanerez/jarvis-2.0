...
Jarvis pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.jarvis/.env``.
...
* The access token is stored in ``~/.jarvis/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``jarvis`` invocations don't
  hammer the API.
* Failures NEVER block Jarvis startup.  Missing binary, no network,
...
# In-process cache so repeated load_jarvis_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `jarvis chat -q ...`
...
# <jarvis_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.jarvis/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under jarvis_home/cache/.

    `home_path` is what `load_jarvis_dotenv()` already resolved; falling back
    to `$JARVIS_HOME` / `~/.jarvis` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("JARVIS_HOME", Path.home() / ".jarvis"))
...
def install_bws(*, force: bool = False) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="jarvis-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<jarvis_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from jarvis_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_jarvis_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `jarvis secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `jarvis secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `jarvis secrets bitwarden setup` to install."
        )
        return result
