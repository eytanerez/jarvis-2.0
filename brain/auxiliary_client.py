...
#       `patch("brain.auxiliary_client.OpenAI", ...)` — tests see the proxy,
...
# See tests/brain/test_auxiliary_client.py for patch patterns this supports.
if TYPE_CHECKING:
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Jarvis triggers summarization.  Higher values delay
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    agent client (``AIBrain._apply_user_default_headers``). This lets a
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``JARVIS_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``JARVIS_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("JARVIS_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("JARVIS_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "JarvisBrain",
}
...
#
# The tags are computed from brain.portal_tags so the client= marker stays
...
# see brain/portal_tags.py for the rationale.
from brain.portal_tags import nous_portal_tags as _nous_portal_tags
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Jarvis)",
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        # (brain/transports/codex.py). Maintaining a private conversion loop
        ...
        from brain.codex_responses_adapter import _chat_messages_to_responses_input
        ...
        # brain/transports/codex.py::build_kwargs() so auxiliary callers
        ...
        if isinstance(extra_body, dict):
            ...
            if isinstance(reasoning_cfg, dict):
                if reasoning_cfg.get("enabled") is False:
                    ...
                    # Truthy-only check mirrors brain/transports/codex.py
        ...
        try:
            ...
            from brain.codex_runtime import _consume_codex_event_stream
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        from brain.anthropic_adapter import build_anthropic_kwargs, create_anthropic_message
        ...
        if temperature is not None:
            from brain.anthropic_adapter import _forbids_sampling_params
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``jarvis_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _maybe_wrap_anthropic(
    ...
    if _safe_isinstance(client_obj, CodexAuxiliaryClient):
        return client_obj
    try:
        from brain.gemini_native_adapter import GeminiNativeClient
        if _safe_isinstance(client_obj, GeminiNativeClient):
            return client_obj
    except ImportError:
        pass
    try:
        from brain.copilot_acp_client import CopilotACPClient
...
def _nous_api_key(provider: dict) -> str:
    ...
    from jarvis_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``jarvis auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``jarvis_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Jarvis auth store.
    ...
    try:
        from jarvis_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from jarvis_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from jarvis_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if provider_id == "gemini":
                from brain.gemini_native_adapter import GeminiNativeClient, is_native_gemini_base_url
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from brain.nous_rate_guard import nous_rate_limit_remaining
...
def _read_main_model() -> str:
    ...
    Runtime override: when an AIBrain is active with a CLI/gateway-provided
...
# Process-local override set by AIBrain at session/turn start. Single-threaded
...
def set_runtime_main(
    ...
    *,
    ...
    """Record the live runtime provider/model/credentials for the current AIBrain.

    Called by ``run_brain.AIBrain._sync_runtime_main_for_aux_routing`` (or
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `jarvis setup` or `jarvis model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_anthropic(explicit_api_key: str = None) -> Tuple[Optional[Any], Optional[str]]:
    try:
        from brain.anthropic_adapter import build_anthropic_client, resolve_anthropic_token
    except ImportError:
        return None, None
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second jarvis
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``jarvis config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from jarvis_cli.nous_account import get_nous_portal_account_info
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "anthropic":
            from brain.anthropic_adapter import read_claude_code_credentials, _refresh_oauth_token, resolve_anthropic_token
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Jarvis' built-in discovery
...
def _resolve_auto(
    ...
    #    scenario where a user switches providers via `jarvis model` but the
    #    old OPENAI_BASE_URL lingers in ~/.jarvis/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: jarvis model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.jarvis/.env",
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        from brain.gemini_native_adapter import GeminiNativeClient, AsyncGeminiNativeClient
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        try:
            from brain.model_metadata import _infer_provider_from_url
...
def resolve_provider_client(
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: jarvis model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: jarvis model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from jarvis_cli.runtime_provider import _get_named_custom_provider
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: jarvis doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("jarvis_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "external_process":
        ...
        try:
            from brain.bedrock_adapter import has_aws_credentials, resolve_bedrock_region
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`jarvis_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _build_call_kwargs(
    ...
    if tools:
        ...
        # injection paths (run_brain.py) already dedup, but this guard
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `jarvis model`."
                )
