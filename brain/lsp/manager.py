...
from brain.lsp import eventlog
from brain.lsp.client import (
    DIAGNOSTICS_DOCUMENT_WAIT,
    LSPClient,
)
from brain.lsp.servers import (
    ServerContext,
    find_server_for_file,
    language_id_for,
)
from brain.lsp.workspace import (
    clear_cache,
    resolve_workspace_for_file,
)

logger = logging.getLogger("brain.lsp.manager")
...
class LSPService:
    ...
    def create_from_config(cls) -> Optional["LSPService"]:
        """Build a service from ``jarvis_cli.config`` settings.
    ...
    def enabled_for(self, file_path: str) -> bool:
        ...
        timeout cost — until the service is restarted (``jarvis lsp
        ...
        if delta:
            ...
            if baseline:
                if line_shift is not None:
                    ...
                    from brain.lsp.range_shift import shift_baseline
    ...
    # ------------------------------------------------------------------
    # status / introspection (used by ``jarvis lsp status``)
    # ------------------------------------------------------------------
