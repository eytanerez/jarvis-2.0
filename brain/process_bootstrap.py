"""Process-level bootstrap helpers for ``run_brain``.

Three concerns, all tied to ``AIBrain`` boot-time / runtime IO setup:
...
1. **Lazy OpenAI SDK import** — ``_load_openai_cls`` + ``_OpenAIProxy``
   ...
   ``patch("run_brain.OpenAI", ...)`` test patterns.
...
``run_brain`` re-exports every name so existing
``from run_brain import _get_proxy_from_env`` imports keep working
unchanged.
"""
...
class _SafeWriter:
    ...
    When jarvis-agent runs as a systemd service, Docker container, or headless
...
# ``from brain.process_bootstrap import OpenAI`` (or re-exported via
# ``run_brain`` for legacy tests).
OpenAI = _OpenAIProxy()
