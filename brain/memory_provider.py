...
Lifecycle (called by MemoryManager, wired in run_brain.py):
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - jarvis_home (str): The active JARVIS_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.jarvis``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "jarvis").
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        reassigns ``AIBrain.session_id`` without tearing the provider down.
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'jarvis memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], jarvis_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'jarvis memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``jarvis_home`` is the active JARVIS_HOME directory path.
