...
Pure display functions and classes with no AIBrain dependency.
Used by AIBrain._execute_tool_calls for CLI feedback.
"""
...
from brain.tool_result_classification import file_mutation_result_landed
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in Jarvis' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("JARVIS_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
