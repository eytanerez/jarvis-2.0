...
as ``kind: backend``) or ``~/.jarvis/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
provider design (``brain/video_gen_provider.py``) so the two surfaces
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``jarvis tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``jarvis tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``jarvis tools`` picker.
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``jarvis tools``
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$JARVIS_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$JARVIS_HOME/cache/images/``.
