...
Jarvis keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for Jarvis's multi-turn
brain/tool loop (auth churn, tool-call replay quirks, thought-signature
...
from brain.gemini_schema import sanitize_gemini_tool_parameters
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with Jarvis)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). Jarvis typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Jarvis retry/error classification."""
...
def build_gemini_request(
    *,
    ...
    if max_tokens is not None:
        ...
        # finishReason=MAX_TOKENS, truncating tool calls mid-stream (Jarvis
...
def translate_stream_event(event: Dict[str, Any], model: str, tool_call_indices: Dict[str, Dict[str, Any]]) -> List[_GeminiStreamChunk]:
    ...
    if finish_reason_raw:
        ...
        # loop in run_brain.py can record token counts (mirrors the
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.jarvis/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `jarvis setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": "jarvis-agent (gemini-native)",
        }
