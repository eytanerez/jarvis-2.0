...
Extracted from run_brain.py to isolate Responses API-specific logic from the
...
# output items.  Jarvis injects xAI's native ``web_search`` for the xAI
# transport (see brain/transports/codex.py); the rest are listed so the
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on Jarvis threading encrypted reasoning back across
    ...
    ``AIBrain._disable_codex_reasoning_replay`` which both strips cached
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a Jarvis-side metadata key and not part
    ...
    *,
    ...
    if tools is not None:
        ...
        for idx, tool in enumerate(tools):
            ...
            # brain/transports/codex.py for where xAI's native web_search is
            # injected.
...
def _normalize_codex_response(
    ...
    *,
    ...
    # handled in run_brain.py) gets a chance to re-elicit a proper
