...
# Matches ${JARVIS_SKILL_DIR} / ${JARVIS_SESSION_ID} tokens in SKILL.md.
# Tokens that don't resolve (e.g. ${JARVIS_SESSION_ID} with no session) are
# left as-is so the user can debug them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(JARVIS_SKILL_DIR|JARVIS_SESSION_ID)\}")
...
def substitute_template_vars(
    ...
    """Replace ${JARVIS_SKILL_DIR} / ${JARVIS_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "JARVIS_SKILL_DIR" and skill_dir_str:
            return skill_dir_str
        if token == "JARVIS_SESSION_ID" and session_id:
