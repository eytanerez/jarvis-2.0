...
from brain.secret_scope import get_secret as _get_secret
from brain.credential_persistence import (
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``jarvis model`` / ``jarvis auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Jarvis process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``jarvis`` invocation that defaults to the active provider
        (e.g. setup wizard, ``jarvis auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                from brain.anthropic_adapter import refresh_anthropic_oauth_pure

                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("jarvis_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if entry.source == "claude_code":
                    try:
                        from brain.anthropic_adapter import _write_claude_code_credentials
                ...
                # refresh_token — single-use tokens consumed by another Jarvis
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("jarvis_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Jarvis process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Jarvis profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `jarvis model` / `jarvis auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `jarvis model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``jarvis auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `jarvis auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `jarvis auth remove <provider> <N>` is stable across all source types.
    try:
        from jarvis_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Jarvis PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `jarvis setup` ("Claude
        ...
        # Claude Code CLI, jarvis_pkce creds from a previous OAuth login)
        ...
        # User-Agent header (`brain/anthropic_adapter.py:2128`).  Users who
        # explicitly opted into the API-key path are explicitly opting OUT of
        # that masquerade.  Prefer ~/.jarvis/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `jarvis setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"jarvis_pkce", "claude_code"}
            ]
        ...
        from brain.anthropic_adapter import read_claude_code_credentials, read_jarvis_oauth_credentials

        for source_name, creds in (
            ("jarvis_pkce", read_jarvis_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `jarvis auth add nous --label <name>`).  Fall back to the
        ...
        # Jarvis auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from jarvis_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.jarvis/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `jarvis auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `jarvis auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Jarvis auth store.  Without this gate the removal is instantly
        ...
        # Jarvis owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``jarvis model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``jarvis auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.jarvis/.env over os.environ — the user's config file is the
    # authoritative source for Jarvis credentials. Stale env vars from parent
    ...
    # Honour user suppression — `jarvis auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.jarvis/.env.
    ...
    try:
        from jarvis_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from jarvis_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.jarvis/.env over os.environ
