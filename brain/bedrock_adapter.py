"""AWS Bedrock Converse API adapter for Jarvis.
...
def _require_boto3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or install Jarvis with Bedrock support: pip install -e '.[bedrock]'"
        )
    # converse() / converse_stream() were added in boto3 1.34.59.
    # When Jarvis is installed editable into system Python, the system boto3
...
def normalize_converse_response(response: Dict) -> SimpleNamespace:
    ...
    The agent loop in ``run_brain.py`` expects responses shaped like
...
# Used by run_brain.py to trigger context compression instead of retrying.
...
# context window sizes.  Used by brain/model_metadata.py when dynamic
