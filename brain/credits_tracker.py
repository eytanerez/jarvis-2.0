...
live consumers (run_brain, tui_gateway) — not a dev-only shim.
...
class AgentNotice:
    ...
    The agent fires these via ``AIBrain.notice_callback`` (and clears them via
...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    ...
    2. A peek into the in-process pricing cache in ``jarvis_cli.models``
    ...
    if not base_url:
        return False
    try:
        from jarvis_cli.models import _is_model_free, _pricing_cache
...
def evaluate_credits_notices(
    ...
    *,
    ...
    Pure function — no I/O, no brain/run_brain imports.
    """
    ...
    try:
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update Jarvis",
                    version_val,
                )
            return None
        ...
        # response in brain.log, without needing a dev flag.
        logger.debug("credits ▸ parse_credits_headers raised (fail-open miss)", exc_info=True)
        return None


# ── Dev test fixtures (JARVIS_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set JARVIS_DEV_CREDITS_FIXTURE to either a
...
# JARVIS_DEV_CREDITS scaffolding.
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for JARVIS_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag JARVIS_DEV_CREDITS
    is also on, so a stray JARVIS_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("JARVIS_DEV_CREDITS")):
        return None
    raw = os.environ.get("JARVIS_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
...
def seed_credits_at_session_start(agent) -> bool:
    ...
    try:
        ...
        # turn / desktop), so a dead session-open seed is diagnosable in brain.log.
