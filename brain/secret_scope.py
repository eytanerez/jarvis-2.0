...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the JARVIS_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "JARVIS_MAX_ITERATIONS", "JARVIS_MAX_TOKENS", "JARVIS_API_TIMEOUT",
...
def build_profile_secret_scope(jarvis_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(jarvis_home) / ".env")
