...
Storage
-------
Bundles live in ``~/.jarvis/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``JARVIS_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``jarvis bundles``)
...
# Slug normalization — matches brain/skill_commands.py so a bundle and a
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under JARVIS_HOME.

    Honors ``JARVIS_BUNDLES_DIR`` for tests; falls back to
    ``<JARVIS_HOME>/skill-bundles``.
    """
    override = os.environ.get("JARVIS_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_jarvis_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `jarvis bundles` CLI subcommand.
# ---------------------------------------------------------------------------
