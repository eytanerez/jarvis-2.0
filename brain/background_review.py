...
After every turn, ``AIBrain.run_conversation`` may call
:func:`spawn_background_review` to fire off a daemon thread that replays
the conversation snapshot in a forked :class:`AIBrain` and asks itself
...
See the ``jarvis-agent-dev`` skill (``references/self-improvement-loop.md``)
...
# the user-message that the forked review agent receives.  AIBrain exposes
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Jarvis, e.g. 'jarvis-agent').\n"
    "  • Hub-installed skills (installed via 'jarvis skills install').\n"
    "Pinned skills (marked via 'jarvis curator pin') CAN be improved — "
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("JARVIS_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def _run_review_in_thread(
    ...
    Spawns a forked ``AIBrain`` inheriting the parent's runtime, runs the
    ...
    try:
        with open(os.devnull, "w", encoding="utf-8") as _devnull, \
            ...
            # AIBrain.__init__ re-runs auto-resolution from env vars,
            ...
            # skill_manage). Those tools require Jarvis' own dispatch,
            # which the codex_app_server runtime bypasses entirely
            # (it runs the turn inside codex's subprocess). So when
            # the parent is on codex_app_server, downgrade the review
            # fork to codex_responses — same auth/credentials, but
            # talks to the OpenAI Responses API directly so Jarvis
            ...
            # scratch (fresh _jarvis_now() timestamp, fresh
...
def spawn_background_review_thread(
    ...
    Returns a ``(target, prompt)`` tuple.  The caller (``AIBrain._spawn_background_review``)
    owns the actual ``threading.Thread`` construction so test-level patches
    of ``run_brain.threading.Thread`` keep working.
    """
