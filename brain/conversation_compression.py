"""Context compression — extract the AIBrain methods that drive summarisation.
...
* :func:`replay_compression_warning` — re-emit a stored warning through
  ...
  set after :class:`AIBrain` construction).
...
``run_brain`` keeps thin wrappers for each so existing call sites
...
def check_compression_model_feasibility(agent: Any) -> None:
    ...
    Called during ``AIBrain.__init__`` so CLI users see the warning
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `jarvis setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Jarvis "
...
def compress_context(
    ...
    *,
    ...
    Args:
        agent: The owning :class:`AIBrain`.
    ...
    # at AIBrain.__init__. Saves ~400ms cold off every short session that
    ...
    # AIBrain instances that share the same session_id (most commonly the
    # parent-turn agent and its background-review fork — see
    # ``brain/background_review.py``: ``review_agent.session_id =
    ...
    # long-lived ``jarvis_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `jarvis update`) to resync.",
    ...
    if agent._session_db:
        try:
            ...
            # from ``jarvis_logging._session_context`` (set once per turn in
            ...
            agent._session_db.create_session(
                ...
                source=agent.platform or os.environ.get("JARVIS_SESSION_SOURCE", "cli"),
                model=agent.model,
    ...
    # compression (not a fresh /new). Plugin engines (e.g. jarvis-lcm) use
    # boundary_reason="compression" to preserve DAG lineage across the
    # rollover instead of re-initializing fresh per-session state.
    # See jarvis-lcm#68. Built-in ContextCompressor ignores kwargs.
    try:
    ...
    if _cc >= 2:
        agent._vprint(
            f"{brain.log_prefix}⚠️  Session compressed {_cc} times — "
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="jarvis_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
