"""Implementation of :meth:`AIBrain.__init__` — extracted as a module function.

``AIBrain.__init__`` is one of the longest methods in the codebase (60+
...
Keeping it in ``run_brain.py`` bloats that file with code that's mostly
"setup state, then forget".

After this extraction the body lives here as ``init_agent(agent, ...)``
and :meth:`AIBrain.__init__` is a thin wrapper that calls
...
Symbols that tests patch on ``run_brain.*`` (``OpenAI``, ``cleanup_vm``,
...
from brain.memory_manager import StreamingContextScrubber
...
from brain.process_bootstrap import _install_safe_stdio
from brain.subdirectory_hints import SubdirectoryHintTracker
from brain.think_scrubber import StreamingThinkScrubber
...
# Use the same logger name as run_brain so tests patching ``run_brain.logger``
# capture our warnings.  (run_brain.py also does
# ``logger = logging.getLogger(__name__)``, which resolves to "run_brain"
# from inside that module.)
logger = logging.getLogger("run_brain")


def _ra():
    """Lazy reference to ``run_brain`` so callers can patch
    ``run_brain.OpenAI`` / ``run_brain.cleanup_vm`` / ... and have those
...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: jarvis config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.jarvis/SOUL.md as the primary
    ...
    # AIBrain is created for every gateway request, so without the guard
    ...
    agent._active_children = []      # Running child AIBrains (for interrupt propagation)
    ...
    agent._cache_ttl = "5m"
    try:
        from jarvis_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind JARVIS_DEV_CREDITS.
    ...
    # Centralized logging — brain.log (INFO+) and errors.log (WARNING+)
    # both live under ~/.jarvis/logs/.  Idempotent, so gateway mode
    # (which creates a new AIBrain per message) won't duplicate handlers.
    ...
    if agent.verbose_logging:
        ...
        # root logger's file handlers (brain.log, errors.log) from
        # ever seeing the records, because Python checks
        # logger.isEnabledFor() before handler propagation. We rely
        # on the fact that jarvis_logging.setup_logging() does not
        ...
        # handler level inside jarvis_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from jarvis_cli.auth import build_minimax_oauth_token_provider
            ...
            from brain.anthropic_adapter import _is_oauth_token as _is_oat
        ...
        try:
            from jarvis_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                from brain.auxiliary_client import build_or_headers
                client_kwargs["default_headers"] = build_or_headers()
            elif base_url_host_matches(effective_base, "integrate.api.nvidia.com"):
                from brain.auxiliary_client import build_nvidia_nim_headers
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `jarvis model` to "
                        "select a provider, or run `jarvis setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff JARVIS_KANBAN_TASK is set).
    # Resolving the ~835-token block once here avoids re-running the
    # membership test + reference on every system-prompt rebuild
    # (init + each context compression).
    from brain.prompt_builder import KANBAN_GUIDANCE
    ...
    try:
        ...
        os.environ["JARVIS_SESSION_ID"] = agent.session_id
    ...
    # reads the JSON files directly.  See run_brain._save_session_log.
    agent._session_json_enabled = False
    try:
        from jarvis_cli.config import load_config as _load_sess_cfg
    ...
    # brain/conversation_loop.py's invalid_encrypted_content retry branch.
    ...
    # In-memory todo list for task planning (one per brain/session)
    from tools.todo_tool import TodoStore
    agent._todo_store = TodoStore()
    
    # Load config once for memory, skills, and compression sections
    try:
        from jarvis_cli.config import load_config as _load_agent_config
    ...
    if not skip_memory:
        try:
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                from brain.memory_manager import MemoryManager as _MemoryManager
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "jarvis"
                    except Exception:
                        pass
    ...
    from brain.memory_manager import inject_memory_provider_tools as _inject_memory_provider_tools
    ...
    # Lets an enterprise admin append to or replace Jarvis' built-in
    ...
    # Stored verbatim; resolution happens in brain/system_prompt.py against
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from jarvis_cli.plugins import get_plugin_context_engine
