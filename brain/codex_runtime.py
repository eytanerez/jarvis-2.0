...
Extracted from :class:`AIBrain` to keep the agent loop file focused.
Each function takes the parent ``AIBrain`` as its first argument
(``agent``).  AIBrain keeps thin forwarder methods for backward
compatibility.
...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into Jarvis accounting.

    Codex app-server reports usage via thread/tokenUsage/updated as:
    inputTokens, cachedInputTokens, outputTokens, reasoningOutputTokens,
    totalTokens.

    Jarvis' canonical prompt bucket includes uncached input + cached input.
    The Codex app-server protocol does not currently expose cache-write tokens,
    so that bucket remains zero on this runtime.

    Even when Codex omits usage for a turn, Jarvis should still count that turn
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into Jarvis'
    messages list so memory/skill review keep working.

    Called from run_conversation() when agent.api_mode == "codex_app_server".
    Returns the same dict shape as the chat_completions path.
    """
    from brain.transports.codex_app_server_session import CodexAppServerSession

    # Lazy session: one CodexAppServerSession per AIBrain instance.
    # Spawned on first turn, reused across turns, closed at AIBrain
    # shutdown (see _cleanup hook).
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        cwd = getattr(agent, "session_cwd", None) or os.getcwd()
        # Approval callback: defer to Jarvis' standard prompt flow if a
...
def _raise_stream_error(event: Any) -> None:
    ...
    pull in ``run_brain`` (e.g. plugin code, doc tools).
    """
