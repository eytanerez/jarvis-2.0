...
collect that info and emit it both to ``brain.log`` (full detail) and to
the user-facing status line (compact).

All helpers are extracted from :class:`AIBrain` for cleanliness.
``run_brain`` keeps thin forwarder methods so existing call sites and
tests that patch ``run_brain.<helper>`` keep working.
"""
...
# on the dict from brain.log for free-form post-hoc analysis.
STREAM_DIAG_HEADERS = (
    "cf-ray",
    "cf-cache-status",
...
def log_stream_retry(
    ...
    *,
    ...
    """Record a transient stream-drop and retry to ``brain.log``.
...
def emit_stream_drop(
    ...
    *,
    ...
    WARNING to ``brain.log`` via :func:`log_stream_retry` with the full
    ...
    mid-flight.  Full diagnostic detail goes to ``brain.log`` only —
    ``jarvis logs --level WARNING | grep "Stream drop"`` to inspect.
    """
