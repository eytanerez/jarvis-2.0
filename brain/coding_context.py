"""Coding-context awareness — base Jarvis, every interactive surface.

When the user runs Jarvis inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Jarvis shifts into a **coding posture**. This module is the
...
domain reads the same resolved object instead of probing git/config itself:
  ...
  * **System prompt** — ``RuntimeMode.system_blocks()`` → the operating brief +
    a live git/workspace snapshot (``brain/system_prompt.py``).
...
logger = logging.getLogger("jarvis.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "jarvis", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _JARVIS_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``JARVIS_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Jarvis should operate in its coding posture right now."""
