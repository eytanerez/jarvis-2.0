...
from brain.model_metadata import estimate_tokens_rough
...
_SENSITIVE_JARVIS_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    blocked_exact.add(jarvis_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(jarvis_home / rel for rel in _SENSITIVE_JARVIS_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Jarvis path and cannot be attached")
