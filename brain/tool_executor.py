...
Both AIBrain methods (``_execute_tool_calls_sequential`` and
``_execute_tool_calls_concurrent``) live here as module-level
functions that take the parent ``AIBrain`` as their first argument.

``run_brain`` keeps thin wrappers so existing call sites work; tests
that patch ``run_brain._set_interrupt`` are honored because the
extracted functions reach back through the ``run_brain`` module via
``_ra()`` for that symbol.
"""
...
from brain.tool_guardrails import ToolGuardrailDecision
...
# Mirrors the constant in ``run_brain`` for tests/imports that look here.
_MAX_TOOL_WORKERS = 8


def _ra():
    """Lazy reference to ``run_brain`` so patches like ``run_brain._set_interrupt`` work."""
    import run_brain
    return run_brain
...
def _run_brain_tool_execution_middleware(
    agent,
    *,
    function_name: str,
    function_args: dict,
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    def _run_tool(index, tool_call, function_name, function_args, middleware_trace):
        ...
        # to it — see AIBrain.interrupt().  Must happen first thing, and
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if _block_msg is not None:
            ...
            function_result, function_args = _run_brain_tool_execution_middleware(
                agent,
                function_name=function_name,
                function_args=function_args,
                effective_task_id=effective_task_id,
