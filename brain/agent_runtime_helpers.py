"""Assorted AIBrain runtime helpers — moved out of run_brain.py for clarity.
...
``drop_thinking_only_and_merge_users``) which are stateless.  AIBrain
...
from brain.tool_dispatch_helpers import _trajectory_normalize_msg, make_tool_result_message
from brain.trajectory import convert_scratchpad_to_think
from brain.credential_pool import STATUS_EXHAUSTED
...
def _ra():
    """Lazy ``run_brain`` reference for test-patch routing."""
    import run_brain
    return run_brain
...
def sanitize_tool_call_arguments(
    ...
    *,
    ...
    repaired = 0
    marker = _ra().AIBrain._TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER
...
def drop_thinking_only_and_merge_users(
    ...
    *,
    ...
    kept = [
        m for m in messages
        if not _ra().AIBrain._is_thinking_only_assistant(
            m,
...
def restore_primary_runtime(agent) -> bool:
    ...
    In long-lived CLI sessions a single AIBrain instance spans multiple
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    from brain.auxiliary_client import _validate_base_url, _validate_proxy_env_urls
    ...
    # Tests in ``tests/run_brain/test_create_openai_client_reuse.py`` and
    # ``tests/run_brain/test_sequential_chats_live.py`` pin this invariant.
    if "http_client" not in client_kwargs:
        keepalive_http = agent._build_keepalive_http_client(client_kwargs.get("base_url", ""))
        if keepalive_http is not None:
            client_kwargs["http_client"] = keepalive_http
    # Uses the module-level `OpenAI` name, resolved lazily on first
    # access via __getattr__ below. Tests patch via `run_brain.OpenAI`.
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from jarvis_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
...
def repair_tool_call(agent, tool_name: str) -> str | None:
    ...
    # tests/run_brain/test_repair_tool_call_name.py).
    for _xml_sep in ('"', "'", "<", ">"):
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for msg in messages:
        role = msg.get("role")
        if role not in _ra().AIBrain._VALID_API_ROLES:
            _ra().logger.debug(
    ...
    for msg in messages:
        if msg.get("role") == "assistant":
            for tc in msg.get("tool_calls") or []:
                cid = _ra().AIBrain._get_tool_call_id_static(tc)
                if cid:
