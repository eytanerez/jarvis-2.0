...
const ENV_KEYS = ['JARVIS_AUTO_HEAPDUMP', 'JARVIS_HEAPDUMP_DIR', 'JARVIS_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'jarvis-heapdump-test-'))
    process.env.JARVIS_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without JARVIS_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without JARVIS_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when JARVIS_AUTO_HEAPDUMP=1', async () => {
    process.env.JARVIS_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.JARVIS_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of JARVIS_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.JARVIS_HEAPDUMP_DIR
    savedMax = process.env.JARVIS_HEAPDUMP_MAX_BYTES
    delete process.env.JARVIS_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'jarvis-heapdump-prune-'))
    process.env.JARVIS_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {delete process.env.JARVIS_HEAPDUMP_DIR}
    else {process.env.JARVIS_HEAPDUMP_DIR = savedDir}

    if (savedMax === undefined) {delete process.env.JARVIS_HEAPDUMP_MAX_BYTES}
    else {process.env.JARVIS_HEAPDUMP_MAX_BYTES = savedMax}
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.JARVIS_HEAPDUMP_MAX_BYTES = String(2 * 1024)
