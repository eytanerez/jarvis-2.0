...
const logDir = join(process.env.JARVIS_HOME?.trim() || join(homedir(), '.jarvis'), 'logs')
...
// a real ~/.jarvis (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('jarvis-tui: parent lifecycle log unavailable\n')
    }
  }
}
