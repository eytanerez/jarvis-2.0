...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.JARVIS_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Jarvis on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via JARVIS_TUI_TRUECOLOR=1.
