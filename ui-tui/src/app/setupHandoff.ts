import type { RunExternalProcess } from '@jarvis/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`jarvis ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching jarvis: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`jarvis ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
