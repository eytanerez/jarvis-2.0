...
React + Ink terminal UI for Jarvis. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `JARVIS_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.jarvis/.jarvis_history` or under `JARVIS_HOME`.
...
```text
ui-tui/
  packages/jarvis-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      jarvis-ink.d.ts    type declarations for @jarvis/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent JarvisCLI subprocess for slash commands
```
