...
await build({
  ...
  // Skip the prebuilt @jarvis/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@jarvis/ink': resolve(root, 'packages/jarvis-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The jarvis_cli launcher
