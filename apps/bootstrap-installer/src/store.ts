...
export const $jarvisHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, jarvisHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_jarvis_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $jarvisHome.set(jarvisHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Jarvis" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Jarvis-Setup.exe --update);
...
export async function launchJarvisDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_jarvis_desktop', { installRoot })
}
