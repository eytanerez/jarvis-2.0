...
//!
//! Mirrors `jarvis_constants.get_jarvis_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\jarvis
//!   macOS:   ~/.jarvis
//!   Linux:   ~/.jarvis  (override via $JARVIS_HOME)
//!
//! NOTE (macOS): Python's get_jarvis_home(), scripts/install.sh, and the
//! Electron desktop's resolveJarvisHome() ALL use ~/.jarvis on macOS — there
...
/// Returns the canonical Jarvis home directory, respecting $JARVIS_HOME if set.
pub fn jarvis_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("JARVIS_HOME") {
    ...
    {
        // %LOCALAPPDATA%\jarvis — matches scripts/install.ps1's $JarvisHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("jarvis");
        }
    }

    // macOS + Linux + fallback: ~/.jarvis (matches Python get_jarvis_home(),
    // install.sh, and the Electron desktop's resolveJarvisHome()).
...
pub fn bootstrap_cache_dir() -> PathBuf {
    jarvis_home().join("bootstrap-cache")
}
...
/// JARVIS_HOME so it survives repo checkout deletion (unlike anything under
/// jarvis-brain/).
///
/// On Windows this is `%LOCALAPPDATA%\jarvis\jarvis-setup.exe`; on other
...
pub fn copy_self_to_jarvis_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to JARVIS_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_JARVIS_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".jarvis-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under JARVIS_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[jarvis-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("JARVIS_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_jarvis_home() -> String {
    jarvis_home().to_string_lossy().into_owned()
}
