...
//! JARVIS_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for JARVIS_HOME. Tests use this; production
...
/// Spawn the locally-built Jarvis desktop binary, then close the installer
...
pub async fn launch_jarvis_desktop(
    ...
    let exe_path = resolve_jarvis_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Jarvis desktop at {}. The desktop build step \
             may have been skipped or failed. Run `jarvis desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Jarvis desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Jarvis
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_jarvis_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Jarvis.exe"),
            ("win-arm64-unpacked", "Jarvis.exe"),
        ]
...
pub(crate) fn resolve_jarvis_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_jarvis_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
...
/// path so a bare re-open just opens Jarvis instead of re-running setup.
pub(crate) fn jarvis_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".jarvis-bootstrap-complete").exists()
        && resolve_jarvis_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_jarvis_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Jarvis desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_jarvis_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::jarvis_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $JarvisHome which Stage-Repository clones
    // the repo INTO at $JarvisHome\jarvis-agent. Mirrors jarvis_constants.
    let jarvis_home = args
        .jarvis_home
        .clone()
        .unwrap_or_else(|| crate::paths::jarvis_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&jarvis_home).join("jarvis-agent");

    // Copy ourselves to JARVIS_HOME/jarvis-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_jarvis_home() {
        tracing::warn!(?err, "failed to copy installer into JARVIS_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    jarvis_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, jarvis_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Jarvis"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Jarvis.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Jarvis.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("jarvis");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Jarvis.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_jarvis_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_jarvis_desktop_app(&root)
    ...
    #[test]
    fn resolve_jarvis_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_jarvis_desktop_app(&root).is_none(),
