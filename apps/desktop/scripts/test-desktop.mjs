...
// no longer bundles the Jarvis Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Jarvis.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Jarvis'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Jarvis.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'jarvis'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default JARVIS_HOME for non-sandboxed runs -- matches main.cjs's
// resolveJarvisHome(). On Windows it's %LOCALAPPDATA%\jarvis; elsewhere
// it's ~/.jarvis. The fresh-install sandbox launchFresh() sets its own
// JARVIS_HOME and never touches this.
const DEFAULT_JARVIS_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'jarvis')
  }
  return path.join(os.homedir(), '.jarvis')
})()
const VENV_ROOT = path.join(DEFAULT_JARVIS_HOME, 'jarvis-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'jarvis-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.JARVIS_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Jarvis-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Jarvis-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Jarvis-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Jarvis-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.JARVIS_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.JARVIS_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const jarvisHome = path.join(sandbox, 'jarvis-home')
  ...
  env.JARVIS_DESKTOP_CWD = cwd
  env.JARVIS_DESKTOP_IGNORE_EXISTING = '1'
  env.JARVIS_DESKTOP_TEST_MODE = 'fresh-install'
  env.JARVIS_DESKTOP_USER_DATA_DIR = userDataDir
  env.JARVIS_HOME = jarvisHome
  delete env.JARVIS_DESKTOP_JARVIS
  delete env.JARVIS_DESKTOP_JARVIS_ROOT
  ...
  return { runtimeRoot: path.join(jarvisHome, 'jarvis-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Jarvis Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of jarvis_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'jarvis-agent', 'jarvis_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Jarvis
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + JARVIS_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  JARVIS_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
