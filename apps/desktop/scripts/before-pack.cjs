 ...
 * product name (`Jarvis`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Jarvis'
 *
 * This is a hard failure with no obvious cause for the user — `jarvis desktop`
 ...
 * (the mac-unpacked Jarvis.app bundle) and Windows (win-unpacked), so we
