...
import { formatRefValue, jarvisDirectiveFormatter } from './directive-text'
...
describe('jarvisDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = jarvisDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = jarvisDirectiveFormatter.parse('@file:src/main.tsx the entry point')
