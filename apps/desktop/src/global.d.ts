...
declare global {
  interface Window {
    jarvisDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<JarvisConnection>
      ...
      profile: {
        ...
        // backend under the new JARVIS_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: JarvisApiRequest) => Promise<T>
      notify: (payload: JarvisNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<JarvisReadFileTextResult>
      selectPaths: (options?: JarvisSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<JarvisPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<JarvisPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: JarvisTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<JarvisReadDirResult>
      ...
      worktrees?: (cwds: string[]) => Promise<Record<string, JarvisWorktreeInfo | null>>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: JarvisTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<JarvisTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: JarvisWindowState) => void) => () => void
...
export interface JarvisTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface JarvisTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `jarvis update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  jarvisRoot?: string
}
...
export interface JarvisTitleBarTheme {
  background: string
  foreground: string
}

export interface JarvisWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface JarvisPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface JarvisReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface JarvisPreviewWatch {
  id: string
  path: string
}

export interface JarvisWorktreeInfo {
...
export interface JarvisReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface JarvisReadDirResult {
  entries: JarvisReadDirEntry[]
  error?: string
}

export interface JarvisPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface JarvisSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
