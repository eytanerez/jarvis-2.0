...
const SKIN_KEY = 'jarvis-desktop-theme-v2'
const MODE_KEY = 'jarvis-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'jarvis-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'jarvis-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'jarvis-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.jarvisTheme = skinName
  root.dataset.jarvisMode = rendered
  ...
  window.jarvisDesktop?.setTitleBarTheme?.({
  ...
  try {
    window.localStorage.setItem('jarvis-boot-background', c.background)
    window.localStorage.setItem('jarvis-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.jarvisThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.jarvisDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Jarvis backend theme on connect. */
