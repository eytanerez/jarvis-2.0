...
import {
  getElevenLabsVoices,
  getJarvisConfigDefaults,
  getJarvisConfigRecord,
  getJarvisConfigSchema,
  saveJarvisConfig
} from '@/jarvis'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, JarvisConfigRecord } from '@/types/jarvis'
...
export function ConfigSettings({
  ...
  const [_defaults, setDefaults] = useState<JarvisConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getJarvisConfigRecord(), getJarvisConfigDefaults(), getJarvisConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: JarvisConfigRecord) => {
