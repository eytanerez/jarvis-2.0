...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: JarvisConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: JarvisConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // JARVIS_QWEN_ and JARVIS_GEMINI_ both share the JARVIS_ stem.
      expect(providerGroup('JARVIS_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('JARVIS_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
