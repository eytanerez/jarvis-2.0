...
import type { EnvVarInfo, OAuthProvider } from '@/types/jarvis'
...
//      in jarvis_cli/provider_catalog.py) — the SAME provider identity
//      `jarvis model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Jarvis can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Jarvis can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Jarvis never deletes creds another tool owns behind a silent API call.
