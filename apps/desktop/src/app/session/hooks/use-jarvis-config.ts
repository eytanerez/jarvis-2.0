...
import { getJarvisConfig, getJarvisConfigDefaults } from '@/jarvis'
...
interface JarvisConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useJarvisConfig({ activeSessionIdRef, refreshProjectBranch }: JarvisConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshJarvisConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getJarvisConfig(), getJarvisConfigDefaults().catch(() => ({}))])
  ...
  return { refreshJarvisConfig, sttEnabled, voiceMaxRecordingSeconds }
}
