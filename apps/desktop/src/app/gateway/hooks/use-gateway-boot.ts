...
import type { RpcEvent } from '@/types/jarvis'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.jarvisDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: JarvisGateway | null) => void
  refreshJarvisConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: JarvisConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Jarvis...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Jarvis…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Jarvis gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshJarvisConfig().catch(() => undefined)
    ...
    const gateway = new JarvisGateway()
