...
// bug that drives the REAL useGatewayBoot hook + REAL JarvisGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshJarvisConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { jarvisDesktop?: unknown }).jarvisDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { jarvisDesktop?: unknown }).jarvisDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForJarvis) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startJarvis()'s remote branch awaits waitForJarvis() for 45s before it
    ...
    ;(window as { jarvisDesktop?: unknown }).jarvisDesktop = desktop
    ...
    // After ~45s waitForJarvis gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Jarvis backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
