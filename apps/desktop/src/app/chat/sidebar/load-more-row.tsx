...
export function SidebarLoadMoreRow({ step, onClick, loading = false }: SidebarLoadMoreRowProps) {
  ...
  return (
    <button
      className="flex min-h-5 items-center gap-1.5 self-start rounded-[3px] border border-transparent bg-transparent py-0.5 pl-2 pr-2 text-left text-[0.6875rem] text-(--ui-text-tertiary) transition-[background-color,border-color,color] duration-150 ease-out hover:border-[color-mix(in_srgb,var(--jarvis-hairline)_36%,transparent)] hover:bg-(--ui-row-hover-background) hover:text-white disabled:cursor-default disabled:opacity-60 disabled:hover:border-transparent disabled:hover:bg-transparent disabled:hover:text-(--ui-text-tertiary)"
      disabled={loading}
      onClick={onClick}
      type="button"
    >
