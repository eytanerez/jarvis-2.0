...
import type { JarvisWorktreeInfo } from '@/global'
...
describe('workspaceGroupsFor', () => {
  it('groups by full cwd, not by basename — same-named folders are separate groups', () => {
    const groups = workspaceGroupsFor(
      [makeSession('/a/jarvis-brain/apps/desktop'), makeSession('/a/jarvis-agent-wt-rtl/apps/desktop')],
      'No workspace'
    )
  ...
  it('disambiguates colliding basenames by walking up the path', () => {
    expect(
      labels([makeSession('/a/jarvis-brain/apps/desktop'), makeSession('/a/jarvis-agent-wt-rtl/apps/desktop')])
    ).toEqual(['jarvis-brain/apps/desktop', 'jarvis-agent-wt-rtl/apps/desktop'])
  })

  it('leaves a unique basename as its short label', () => {
    expect(labels([makeSession('/a/jarvis-brain/apps/desktop'), makeSession('/b/heval-py')])).toEqual([
      'desktop',
      'heval-py'
    ])
  })
...
const info = (
  over: Partial<JarvisWorktreeInfo> & Pick<JarvisWorktreeInfo, 'repoRoot' | 'worktreeRoot'>
): JarvisWorktreeInfo => ({
  branch: null,
...
describe('workspaceTreeFor', () => {
  it('heuristic nests `<repo>-wt-<branch>` under its sibling repo', () => {
    const tree = workspaceTreeFor(
      [makeSession('/www/jarvis-agent'), makeSession('/www/jarvis-agent-wt-rtl')],
      'No workspace'
    )

    expect(tree).toHaveLength(1)
    expect(tree[0].label).toBe('jarvis-agent')
    expect(tree[0].groups.map(g => g.label).sort()).toEqual(['jarvis-agent', 'rtl'])
  })
  ...
  it('git metadata is authoritative — worktrees group by repoRoot regardless of directory naming', () => {
    const resolver: WorktreeResolver = cwd => {
      if (cwd === '/www/jarvis-agent') {
        return info({
          repoRoot: '/www/jarvis-agent',
          worktreeRoot: '/www/jarvis-agent',
          isMainWorktree: true,
          branch: 'main'
        })
      }
      ...
      if (cwd === '/elsewhere/ha-rtl') {
        return info({ repoRoot: '/www/jarvis-agent', worktreeRoot: '/elsewhere/ha-rtl', branch: 'rtl' })
      }
    ...
    const tree = workspaceTreeFor(
      [makeSession('/www/jarvis-agent'), makeSession('/elsewhere/ha-rtl')],
      'No workspace',
      resolver
    )
    ...
    expect(tree[0].groups.map(g => g.label)).toEqual(['jarvis-agent', 'rtl'])
  })
