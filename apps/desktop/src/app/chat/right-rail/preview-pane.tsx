        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.jarvisDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Jarvis to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.jarvisDesktop?.watchPreviewFile ||
      !window.jarvisDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.jarvisDesktop.onPreviewFileChanged(payload => {
    ...
    void window.jarvisDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.jarvisDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.jarvisDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:jarvis-preview')
