...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { jarvisDesktop: { readDir: typeof readDir } }).jarvisDesktop = { readDir }
})
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { jarvisDesktop: unknown }).jarvisDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: JarvisReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<JarvisReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { jarvisDesktop: unknown }).jarvisDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.jarvisDesktop is missing', async () => {
    delete (window as unknown as { jarvisDesktop?: unknown }).jarvisDesktop
