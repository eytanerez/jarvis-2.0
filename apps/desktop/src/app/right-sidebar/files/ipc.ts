...
function ignoredBy(rules: GitignoreRule[], entry: JarvisReadDirEntry) {
...
async function filterIgnored(entries: JarvisReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<JarvisReadDirResult> {
