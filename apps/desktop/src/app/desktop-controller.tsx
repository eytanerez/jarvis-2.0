...
import { useJarvisConfig } from './session/hooks/use-jarvis-config'
...
const MenuOverlay = lazy(async () => ({ default: (await import('./menu/menu-overlay')).MenuOverlay }))
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.jarvisDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.jarvisDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.jarvisDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.jarvisDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.jarvisDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshJarvisConfig, sttEnabled, voiceMaxRecordingSeconds } = useJarvisConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshJarvisConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshJarvisConfig])
  ...
  const overlays = (
    <>
      ...
      {menuOpen && (
        <Suspense fallback={null}>
          <MenuOverlay onClose={closeOverlayToPreviousRoute} />
        </Suspense>
      )}
    </>
  )
