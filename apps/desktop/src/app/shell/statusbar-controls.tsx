...
const STATUSBAR_ACTION_CLASS =
  'inline-flex h-full items-center gap-1 rounded-none px-1.5 font-label text-[0.6875rem] text-(--ui-text-tertiary) transition-colors hover:bg-(--chrome-action-hover) hover:text-foreground disabled:cursor-default disabled:opacity-45'
...
function StatusbarItemView({ item, navigate }: { item: StatusbarItem; navigate: ReturnType<typeof useNavigate> }) {
  ...
  if (item.variant === 'text' && !item.onSelect && !item.to && !item.href) {
    return (
      <div
        className={cn(
          'inline-flex h-full items-center gap-1 px-1.5 font-label text-[0.6875rem] text-(--ui-text-tertiary)',
          item.className
        )}
      >
        {content}
