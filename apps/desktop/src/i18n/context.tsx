...
import { getJarvisConfigRecord, type JarvisConfigRecord, saveJarvisConfig } from '@/jarvis'
...
export interface I18nConfigClient {
  getConfig: () => Promise<JarvisConfigRecord>
  saveConfig: (config: JarvisConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.jarvisDesktop?.api) {
      return Promise.resolve({})
    }

    return getJarvisConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.jarvisDesktop?.api) {
    ...
    return saveJarvisConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: JarvisConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: JarvisConfigRecord, locale: Locale): JarvisConfigRecord {
