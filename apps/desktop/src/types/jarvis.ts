...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Jarvis doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in jarvis_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `jarvis model` uses — instead of
...
export type JarvisConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `jarvis model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface ToolProvider {
   ...
   *  CLI `jarvis tools` active-provider detection). */
  is_active: boolean
}
...
/** Shape of `GET /api/jarvis/update/check` — the backend's own update state.
