import type { ConnectionState, GatewayEvent } from '@jarvis/shared'
...
const isOpen = (gateway: JarvisGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<JarvisGateway | null>(null)
...
let primaryGateway: JarvisGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: JarvisGateway | null, profile = 'default'): void {
...
export function activeGateway(): JarvisGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<JarvisGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
