...
import type { SessionInfo, UsageStats } from '@/types/jarvis'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'jarvis.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'jarvis.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'jarvis.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'jarvis.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'jarvis.desktop.composer.fast'

let configuredDefaultProjectDir = ''

function workspaceCwdKey(connection: JarvisConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.jarvisDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.jarvisDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<JarvisConnection | null>(null)
...
export const setConnection = (next: Updater<JarvisConnection | null>) => updateAtom($connection, next)
