...
import type { BackendUpdateCheckResponse } from '@/types/jarvis'
...
const UPDATE_TOAST_SNOOZE_KEY = 'jarvis:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'jarvis:backend-skew-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: {
      label: translateNow('notifications.updateJarvis'),
      onClick: () => {
        snoozeSkewToast()
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.jarvisDesktop?.getVersion?.()
...
export async function checkBackendUpdates(): Promise<DesktopUpdateStatus | null> {
  ...
  try {
    const status = mapBackendCheck(await checkJarvisUpdate(true))
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  const bridge = window.jarvisDesktop?.updates
...
export async function applyBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    const started = await updateJarvis()
