...
import { getProfiles, setApiRequestProfile } from '@/jarvis'
...
import type { ProfileInfo } from '@/types/jarvis'
...
// /api/profiles/active `current`). "default" is the root ~/.jarvis. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'jarvis.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'jarvis.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.jarvisDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new JARVIS_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.jarvisDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.jarvisDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'jarvis.desktop.showAllProfiles'
...
// Switch to the default (root ~/.jarvis) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.jarvisDesktop?.touchBackend?.(target).catch(() => undefined)
}
