...
const remoteConn = (over: Partial<JarvisConnection> = {}): JarvisConnection =>
  ({ baseUrl: 'https://jarvis-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as JarvisConnection

const localConn = (over: Partial<JarvisConnection> = {}): JarvisConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as JarvisConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<JarvisConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { jarvisDesktop: { getConnection } })
})
