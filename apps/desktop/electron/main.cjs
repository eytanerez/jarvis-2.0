...
const USER_DATA_OVERRIDE = process.env.JARVIS_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.JARVIS_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with JARVIS_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[jarvis] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[jarvis] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[jarvis] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[jarvis] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// JARVIS_HOME — the user-facing root for everything Jarvis-related. Mirrors
// scripts/install.ps1's $JarvisHome and scripts/install.sh's $JARVIS_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\jarvis (matches install.ps1)
//   macOS / Linux: ~/.jarvis (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.jarvis directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\jarvis yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// JARVIS_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// JARVIS_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.jarvis / %LOCALAPPDATA%\jarvis.
function resolveJarvisHome() {
  if (process.env.JARVIS_HOME) return normalizeJarvisHomeRoot(process.env.JARVIS_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'jarvis-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a JARVIS_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\jarvis and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('JARVIS_HOME')
    if (fromRegistry) return normalizeJarvisHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'jarvis')
    const legacy = path.join(app.getPath('home'), '.jarvis')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.jarvis setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.jarvis')
}
...
const ACTIVE_JARVIS_ROOT = path.join(JARVIS_HOME, 'jarvis-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_JARVIS_ROOT, 'venv')
...
// boots, going straight to `resolveJarvisBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_JARVIS_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_JARVIS_ROOT, '.jarvis-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which Jarvis profile the desktop launches its
// local backend as. When set, startJarvis() passes `jarvis --profile <name>
// dashboard …`, which deterministically pins JARVIS_HOME (see
// _apply_profile_override in jarvis_cli/main.py) and bypasses the sticky
// ~/.jarvis/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors jarvis_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// jarvisDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under JARVIS_HOME/logs/ so it sits next to brain.log,
// errors.log, gateway.log produced by jarvis_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(JARVIS_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (jarvis_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.JARVIS_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.JARVIS_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// 'jarvis:native-theme' ('dark' | 'light' | 'system'); we pin
...
// Seed the native About panel with the live Jarvis version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `jarvis update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveJarvisVersion(),
...
const MEDIA_PROTOCOL = 'jarvis-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(jarvis-media://…)`.
...
// (the desktop's launch profile) stays managed by jarvisProcess +
// connectionPromise + startJarvis(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.JARVIS_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.JARVIS_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startJarvis() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Jarvis backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[jarvis] ${line}`)
  jarvisLog.push(...lines)
  if (jarvisLog.length > 300) {
    jarvisLog.splice(0, jarvisLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-jarvis-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('jarvis:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the jarvis:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('jarvis:bootstrap:event', ev)
}
...
function isJarvisSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'jarvis_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.JARVIS_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Jarvis deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Jarvis' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\jarvis\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'jarvis', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'jarvis', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\jarvis\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'jarvis', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'jarvis', 'git', 'bin', 'git.exe'))
  }
...
function recentJarvisLog() {
  return jarvisLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's jarvis root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_JARVIS_ROOT.
// JARVIS_DESKTOP_JARVIS_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.JARVIS_DESKTOP_JARVIS_ROOT && path.resolve(process.env.JARVIS_DESKTOP_JARVIS_ROOT),
    !IS_PACKAGED && isJarvisSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isJarvisSourceRoot(ACTIVE_JARVIS_ROOT) ? ACTIVE_JARVIS_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_JARVIS_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('jarvis:updates:progress', merged)
  }
}
...
// JARVIS_HOME/jarvis-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_jarvis_home). That binary owns
// ALL repo mutation — running `jarvis update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'jarvis-setup.exe' : 'jarvis-setup'
  const candidate = path.join(JARVIS_HOME, name)
...
// Path to the venv shim whose lock decides whether `jarvis update` can write
// fresh entry points. On Windows this is the file the running backend
// `jarvis.exe` holds open; on POSIX it's never mandatory-locked.
function venvJarvisShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'jarvis.exe')
    : path.join(updateRoot, 'venv', 'bin', 'jarvis')
}
...
// only signals the direct child, so on Windows a backend `jarvis.exe` that
// spawned its own grandchildren (a `jarvis` REPL, a pty terminal session, the
...
// `jarvisProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `jarvis.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a jarvis
// REPL, a pty terminal, the gateway) keep `jarvis.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (jarvisProcess && Number.isInteger(jarvisProcess.pid)) pids.push(jarvisProcess.pid)
  ...
  const shim = venvJarvisShimPath(updateRoot)
...
// `jarvis update`). Instead we spawn the staged Jarvis-Setup binary with
// --update and quit, so it can run `jarvis update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri jarvis-setup. Unlike Windows
      ...
      // whole update itself: `jarvis update` (backend) + `jarvis desktop
    ...
    if (!updater) {
      ...
      // `jarvis desktop`, never the Tauri installer that self-copies
      // jarvis-setup.exe into JARVIS_HOME). They DO have a working `jarvis`
      ...
      // checkout they're on — bare `jarvis update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'jarvis update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `jarvis update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `jarvis update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, jarvisRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Jarvis updater…', percent: 100 })
    ...
    // jarvis.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `jarvis update` will run (the venv shim is locked while we live).
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvJarvis = path.join(venvBin, IS_WINDOWS ? 'jarvis.exe' : 'jarvis')
  const updaterArgs = fileExists(venvJarvis) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
// macOS/Linux in-app update: backend (`jarvis update`) + OS-aware GUI rebuild
// (`jarvis desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp() {
  ...
  // _kill_stale_dashboard_processes reads JARVIS_DESKTOP_CHILD_PID and excludes
  ...
  if (jarvisProcess && Number.isInteger(jarvisProcess.pid)) {
    desktopChildPids.push(jarvisProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.JARVIS_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Jarvis to retry.',
  ...
  const rebuiltApp = [
    ...
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Jarvis.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Jarvis to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.jarvis-update-new"; then
    rm -rf "$DST.jarvis-update-old" 2>/dev/null || true
    mv "$DST" "$DST.jarvis-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.jarvis-update-new" "$DST"
    rm -rf "$DST.jarvis-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `jarvis-desktop-update-${Date.now()}.sh`)
  try {
    fs.writeFileSync(scriptPath, swapScript, { mode: 0o755 })
  } catch (err) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Jarvis to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `jarvis
  ...
  return isJarvisSourceRoot(ACTIVE_JARVIS_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set JARVIS_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: jarvis desktop --force-build`
  )
  return candidates[0]
}
...
function resolveJarvisCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Jarvis.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.JARVIS_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveJarvisCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'jarvis_cli.main', ...dashboardArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_JARVIS_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Jarvis at ${ACTIVE_JARVIS_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'jarvis_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      jarvisHome: JARVIS_HOME,
      pythonPathEntries: [ACTIVE_JARVIS_ROOT],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_JARVIS_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveJarvisBackend(dashboardArgs) {
  // 1. Explicit override -- JARVIS_DESKTOP_JARVIS_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.JARVIS_DESKTOP_JARVIS_ROOT && path.resolve(process.env.JARVIS_DESKTOP_JARVIS_ROOT)
  if (overrideRoot && isJarvisSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Jarvis source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `jarvis` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isJarvisSourceRoot.)
  if (!IS_PACKAGED && isJarvisSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Jarvis source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_JARVIS_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\jarvis\jarvis-agent (Windows) or ~/.jarvis/jarvis-agent.
  ...
  //    to spawning jarvis. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `jarvis update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `jarvis` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    JARVIS_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.JARVIS_DESKTOP_IGNORE_EXISTING !== '1') {
    let jarvisCommand = null
    const jarvisOverride = process.env.JARVIS_DESKTOP_JARVIS

    if (jarvisOverride) {
      const resolvedOverride = findOnPath(jarvisOverride)
      if (resolvedOverride) {
        jarvisCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(jarvisOverride, { isWsl: IS_WSL })) {
        jarvisCommand = jarvisOverride
      } else {
        rememberLog(`Ignoring Windows Jarvis override under WSL: ${jarvisOverride}`)
      }
    } else {
      jarvisCommand = findOnPath('jarvis')
    }

    if (jarvisCommand) {
      if (looksLikeDesktopAppBinary(jarvisCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Jarvis CLI: ${jarvisCommand}`)
        jarvisCommand = null
      }
    }

    if (jarvisCommand) {
      // Smoke-test the candidate before trusting it. A `jarvis` shim
      ...
      const shellForProbe = isCommandScript(jarvisCommand)
      if (verifyJarvisCli(jarvisCommand, { shell: shellForProbe })) {
        return {
          label: `existing Jarvis CLI at ${jarvisCommand}`,
      ...
      rememberLog(
        `Ignoring existing Jarvis CLI at ${jarvisCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed jarvis_cli module via system Python.
  ...
  if (python) {
    ...
    // having jarvis_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\jarvis\jarvis-agent\venv.
    if (canImportJarvisCli(python)) {
      return {
        kind: 'python',
        label: `installed jarvis_cli module via ${python}`,
        command: python,
        args: ['-m', 'jarvis_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: jarvis_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveJarvisBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Jarvis not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveJarvisBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Jarvis install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error(
        'Jarvis recovery was handed off to Jarvis Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      jarvisHome: JARVIS_HOME,
      logRoot: path.join(JARVIS_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Jarvis install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Jarvis bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(JARVIS_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startJarvis() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // jarvis:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveJarvisBackend(backend.args))
  }
  ...
  if (!isJarvisSourceRoot(ACTIVE_JARVIS_ROOT)) {
    throw new Error(
      `Jarvis install at ${ACTIVE_JARVIS_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Jarvis' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\jarvis\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `jarvis` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Jarvis on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Jarvis.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isJarvisSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Jarvis venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Jarvis at ${ACTIVE_JARVIS_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Jarvis backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Jarvis backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Jarvis backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Jarvis-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('jarvis:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveJarvisCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('jarvis:preview-file-changed', payload)
}
...
async function waitForJarvis(baseUrl, token) {
  ...
  throw new Error(`Jarvis backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('jarvis:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('jarvis:close-preview-requested')
}
...
function sendPowerResume() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('jarvis:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('jarvis:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('jarvis:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'jarvis:desktop:zoomLevel'
...
//
// Hosted Jarvis gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``jarvis_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / jarvis #37247).
//     Both are set as HttpOnly cookies (``jarvis_session_at`` ~15 min,
//     ``jarvis_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:jarvis-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Jarvis gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Jarvis backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root JARVIS_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.JARVIS_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.JARVIS_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by JARVIS_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (jarvis #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Jarvis gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Jarvis gateway is selected, but no session token is saved. ' +
...
//   2. env override (JARVIS_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.JARVIS_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.JARVIS_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'JARVIS_DESKTOP_REMOTE_URL is set but JARVIS_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Jarvis backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.JARVIS_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Jarvis
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startJarvis())
  ...
  // connect to Jarvis gateway". Mirror the renderer's connect here so the test
...
function resetJarvisConnection() {
...
// startJarvis() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetJarvisConnection() nulls jarvisProcess.
  const dying = jarvisProcess && !jarvisProcess.killed ? jarvisProcess : null
  resetJarvisConnection()
...
// profile to startJarvis() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startJarvis() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForJarvis(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
  ...
  // --profile wins over the inherited JARVIS_HOME env (see _apply_profile_override
  // step 3 in jarvis_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveJarvisBackend(dashboardArgs))
  const jarvisCwd = resolveJarvisCwd()
  const webDist = resolveWebDist()

  rememberLog(`Starting Jarvis backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: jarvisCwd,
        JARVIS_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        JARVIS_DESKTOP: '1',
        JARVIS_WEB_DIST: webDist
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Jarvis backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Jarvis backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Jarvis backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
...
async function startJarvis() {
  ...
  // process, every subsequent startJarvis() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Jarvis backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Jarvis backend at ${remote.baseUrl}`, 24)
      await waitForJarvis(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Jarvis backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.jarvis/active_profile file) and
    // resolves JARVIS_HOME the same way `jarvis -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Jarvis runtime', 28)
    const backend = await ensureRuntime(resolveJarvisBackend(dashboardArgs))
    const jarvisCwd = resolveJarvisCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting Jarvis backend via ${backend.label}`, 84)
    rememberLog(`Starting Jarvis backend via ${backend.label}`)
    ...
    jarvisProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin JARVIS_HOME for the child so Python's get_jarvis_home()
          // resolves to the SAME location our resolveJarvisHome() picked. Without
          // this pin, Python falls back to ~/.jarvis on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\jarvis, which differs from C:\Users\<u>\.jarvis.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets JARVIS_HOME via setx; the desktop
    ...
    jarvisProcess.stdout.on('data', rememberLog)
    jarvisProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    jarvisProcess.once('error', error => {
      rememberLog(`Jarvis backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Jarvis backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    jarvisProcess.once('exit', (code, signal) => {
      rememberLog(`Jarvis backend exited (${signal || code})`)
      jarvisProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Jarvis backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Jarvis backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentJarvisLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Jarvis backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([waitForDashboardPort(jarvisProcess), backendStartFailed])

    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Jarvis backend to become ready', 90)
    await Promise.race([waitForJarvis(baseUrl, token), backendStartFailed])
    backendReady = true
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null jarvisProcess when the child dies.
      childAlive: () => jarvisProcess !== null && jarvisProcess.exitCode === null && !jarvisProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Jarvis backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startJarvis().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('jarvis:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Jarvis…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('jarvis:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetJarvisConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Jarvis backend failed liveness probe; dropping stale connection.')
    resetJarvisConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('jarvis:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('jarvis:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('jarvis:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('jarvis:window:openNewSession', async () => {
  createNewSessionWindow()

  return { ok: true }
})
ipcMain.handle('jarvis:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startJarvis() call restarts the
...
ipcMain.handle('jarvis:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startJarvis() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('jarvis:bootstrap:cancel', async () => {
...
ipcMain.handle('jarvis:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('jarvis:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('jarvis:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('jarvis:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('jarvis:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('jarvis:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('jarvis:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('jarvis:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new JARVIS_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('jarvis:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('jarvis:requestMicrophoneAccess', async () => {
...
ipcMain.handle('jarvis:api', async (_event, request) => {
...
ipcMain.handle('jarvis:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('jarvis:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('jarvis:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('jarvis:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('jarvis:readFileText', async (_event, filePath) => {
...
ipcMain.handle('jarvis:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('jarvis:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('jarvis:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('jarvis:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('jarvis:saveClipboardImage', async () => {
...
ipcMain.handle('jarvis:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('jarvis:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('jarvis:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('jarvis:titlebar-theme', (_event, payload) => {
...
ipcMain.on('jarvis:native-theme', (_event, mode) => {
...
ipcMain.on('jarvis:translucency', (_event, payload) => {
...
ipcMain.handle('jarvis:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolveJarvisCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('jarvis:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveJarvisCwd()
}))

ipcMain.handle('jarvis:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('jarvis:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('jarvis:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('jarvis:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('jarvis:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('jarvis:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: jarvisLog.slice(-200) }))
...
function terminalShellCommand() {
  // JARVIS_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.JARVIS_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Jarvis' TUI into
  ...
  env.TERM_PROGRAM = 'Jarvis'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a jarvis/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from JARVIS_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.JARVIS_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `jarvis:terminal:${id}:${suffix}`
}
...
ipcMain.handle('jarvis:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('jarvis:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

ipcMain.handle('jarvis:fs:worktrees', async (_event, cwds) => worktreesForIpc(cwds))

ipcMain.handle('jarvis:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Jarvis.')
  }
...
ipcMain.handle('jarvis:terminal:write', (_event, id, data) => {
...
ipcMain.handle('jarvis:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('jarvis:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('jarvis:updates:check', async () =>
...
ipcMain.handle('jarvis:updates:apply', async (_event, payload) =>
...
ipcMain.handle('jarvis:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('jarvis:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Jarvis version (the one `release.py` bumps in
// jarvis_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Jarvis version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveJarvisVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'jarvis_cli', '__init__.py')
...
// Re-resolve the live Jarvis version and push it into the native About panel
// just before showing it, so an in-place `jarvis update` is reflected without
...
ipcMain.handle('jarvis:version', async () => ({
  appVersion: resolveJarvisVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  jarvisRoot: resolveUpdateRoot()
}))
...
// the actual removal via `jarvis uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (jarvis_cli/uninstall.py + jarvis_cli/gui_uninstall.py).
//
...
// bundled agent), hiding the brain/full options when there's nothing to remove.
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_JARVIS_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isJarvisSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'jarvis_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, JARVIS_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Jarvis agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import jarvis_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_JARVIS_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `jarvis-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `jarvis-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('jarvis:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('jarvis:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('jarvis:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('jarvis:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// jarvis:// deep links (e.g. jarvis://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const JARVIS_PROTOCOL = 'jarvis'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${JARVIS_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // jarvis://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
...
ipcMain.handle('jarvis:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${JARVIS_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(JARVIS_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(JARVIS_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `jarvis://` launch spawns a
