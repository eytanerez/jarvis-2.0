 ...
 * the three user-facing uninstall modes to the `jarvis uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m jarvis_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `jarvis_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'jarvis_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Jarvis.app/Contents/MacOS/Jarvis  → …/Jarvis.app
 *   Windows: …\Jarvis\Jarvis.exe                 → …\Jarvis  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/Jarvis.app/Contents/MacOS/Jarvis → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Jarvis.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs Jarvis.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]Jarvis$/i.test(dir) || /[\\/]jarvis-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/jarvis
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import jarvis_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, jarvisHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export JARVIS_HOME=${q(jarvisHome)}`
  ]
  if (pythonPath) {
 ...
 * modes + `pythonPath`=agentRoot so `import jarvis_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Jarvis install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "JARVIS_HOME=${String(jarvisHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
  if (pythonPath) {
