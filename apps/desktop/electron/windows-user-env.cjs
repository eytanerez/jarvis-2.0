...
// even though a fresh shell — and the Jarvis CLI — sees it immediately. The
// desktop's JARVIS_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\jarvis. Reading
...
//       JARVIS_HOME    REG_SZ    F:\Jarvis\data
//
