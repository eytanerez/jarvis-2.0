 ...
 *
 * Drives apps/desktop's first-launch install of Jarvis by spawning
...
function bootstrapCacheDir(jarvisHome) {
  return path.join(jarvisHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.jarvis/jarvis-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(jarvisHome) {
  if (!jarvisHome) return null
  const candidate = path.join(jarvisHome, 'jarvis-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(jarvisHome, commit) {
  return path.join(bootstrapCacheDir(jarvisHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(jarvisHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(jarvisHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, jarvisHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass JARVIS_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          JARVIS_HOME: jarvisHome || process.env.JARVIS_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, jarvisHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, jarvisHome }) {
  const args = ['--dir', activeRoot, '--jarvis-home', jarvisHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, jarvisHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, jarvisHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, jarvisHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, jarvisHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(jarvisHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, jarvisHome, emit })
