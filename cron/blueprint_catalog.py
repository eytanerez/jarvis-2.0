...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``jarvis://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``jarvis://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"jarvis://blueprint/{quote(blueprint.key)}{qs}"
