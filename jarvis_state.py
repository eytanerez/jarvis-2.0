...
DEFAULT_DB_PATH = get_jarvis_home() / "state.db"
...
# jarvis_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``jarvis_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see jarvis_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple jarvis processes (gateway + CLI sessions + worktree agents)
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `jarvis update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._jarvis_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._jarvis_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
            ...
            for col_name, col_type in declared_cols.items():
                if col_name not in live_cols:
                    ...
                    try:
                        ...
                        # Log at DEBUG so it's visible in brain.log.
                        logger.debug(
    ...
    # new). That mutation is local to the AIBrain instance — but ``state.db``
    # is shared across all instances. Two AIBrains that share the same
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Jarvis surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``jarvis sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Jarvis, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Jarvis session.

        A Jarvis session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Jarvis session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``JARVIS_DISABLE_FTS_TRIGRAM`` or not yet created), so
