...
Jarvis 以 ShareGPT 兼容的 JSONL 格式保存对话轨迹，用于训练数据、调试产物和强化学习数据集。

源文件：`brain/trajectory.py`、`run_brain.py`（搜索 `_save_trajectory`）、`batch_runner.py`
...
系统消息在保存时生成（不取自对话内容），遵循 Jarvis 函数调用 prompt 模板，包含：
