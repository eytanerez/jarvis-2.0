...
description: "使用 Nix 安装和部署 Jarvis——从快速 `nix run` 到完全声明式的 NixOS 模块（含容器模式）"
---

# Nix & NixOS 安装配置

Jarvis 提供了一个 Nix flake，支持三个层级的集成：
...
**对于非 NixOS 用户**，这只影响安装步骤。之后的操作（`jarvis setup`、`jarvis gateway install`、编辑配置）与标准安装完全相同。

**对于 NixOS 模块用户**，整个生命周期有所不同：配置存放在 `configuration.nix` 中，密钥通过 sops-nix/agenix 管理，服务是一个 systemd 单元，CLI 配置命令被屏蔽。管理 jarvis 的方式与管理其他 NixOS 服务相同。
:::
...
执行 `nix profile install` 后，`jarvis`、`jarvis-agent` 和 `jarvis-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`jarvis setup` 引导你完成提供商选择，`jarvis gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.jarvis/`。
...
就这些。`nixos-rebuild switch` 会创建 `jarvis` 用户、生成 `config.yaml`、连接密钥并启动 gateway——这是一个长期运行的服务，将 Agent 连接到消息平台（Telegram、Discord 等）并监听传入消息。
...
设置 `addToSystemPackages = true` 有两个作用：将 `jarvis` CLI 添加到系统 PATH，**并**在系统范围内设置 `JARVIS_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行 `jarvis` 会创建独立的 `~/.jarvis/` 目录。
:::

### 容器感知 CLI

:::info
当 `container.enable = true` 且 `addToSystemPackages = true` 时，主机上的**所有** `jarvis` 命令都会自动路由到托管容器中执行。这意味着你的交互式 CLI 会话在与 gateway 服务相同的环境中运行——可以访问所有容器内安装的包和工具。

- 路由是透明的：`jarvis chat`、`jarvis sessions list`、`jarvis version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
- 如果容器未运行，CLI 会短暂重试（交互式使用时显示 5 秒 spinner，脚本中静默等待 10 秒），然后以明确的错误退出——不会静默回退
- 对于在 jarvis 代码库上工作的开发者，设置 `JARVIS_DEV=1` 可绕过容器路由，直接运行本地检出版本

设置 `container.hostUsers` 可创建 `~/.jarvis` 到服务状态目录的符号链接，使主机 CLI 和容器共享会话、配置和记忆：
...
`hostUsers` 中列出的用户会自动加入 `jarvis` 组以获得文件权限访问。
...
CLI 会自动检测何时需要 sudo 并透明地使用它。没有此配置，你需要手动运行 `sudo jarvis chat`。
:::
...
:::note 模型命名
`settings.model.default` 使用你的提供商所期望的模型标识符。使用 [OpenRouter](https://openrouter.ai)（默认）时，格式如 `"anthropic/claude-sonnet-4"` 或 `"google/gemini-3-flash"`。如果直接使用提供商（Anthropic、OpenAI），请将 `settings.model.base_url` 指向其 API，并使用其原生模型 ID（例如 `"claude-sonnet-4-20250514"`）。未设置 `base_url` 时，Jarvis 默认使用 OpenRouter。
:::
...
这会完全绕过 `settings`——不合并，不生成。每次激活时，该文件会原样复制到 `$JARVIS_HOME/config.yaml`。
...
| 添加 API 密钥 | `environmentFiles` | `[ config.sops.secrets."jarvis-env".path ]` |
| 给 Agent 设置个性 | `${services.jarvis-agent.stateDir}/.jarvis/SOUL.md` | 直接管理该文件 |
...
| 覆盖 jarvis 包 | `package` | `inputs.jarvis-agent.packages.${system}.default.override { ... }` |
| 更改状态目录 | `stateDir` | `"/opt/jarvis"` |
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$JARVIS_HOME/.env` 中。Jarvis 在每次启动时读取此文件，因此更改在 `systemctl restart jarvis-agent` 后生效——无需重建容器。
...
```yaml
# secrets/jarvis.yaml（使用 sops 加密）
jarvis-env: |
...
`documents` 选项将文件安装到 Agent 的工作目录（即 `workingDirectory`，Agent 将其作为工作区读取）。Jarvis 按约定查找特定文件名：

- **`USER.md`** — 关于 Agent 正在交互的用户的上下文信息。
- 你放置在此处的任何其他文件对 Agent 都可见，作为工作区文件。

Agent 身份文件是独立的：Jarvis 从 `$JARVIS_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.jarvis-agent.stateDir}/.jarvis/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。
...
:::tip
`env` 值中的环境变量在运行时从 `$JARVIS_HOME/.env` 解析。使用 `environmentFiles` 注入密钥——切勿将 token 直接放入 Nix 配置。
:::
...
对于使用 OAuth 2.1 的服务器，设置 `auth = "oauth"`。Jarvis 实现了完整的 PKCE 流程——元数据发现、动态客户端注册、token 交换和自动刷新。
...
Token 存储在 `$JARVIS_HOME/mcp-tokens/<server-name>.json` 中，在重启和重建后持久保留。

<details>
<summary><strong>无头服务器上的初始 OAuth 授权</strong></summary>

首次 OAuth 授权需要基于浏览器的同意流程。在无头部署中，Jarvis 将授权 URL 打印到 stdout/日志，而不是打开浏览器。

**方案 A：交互式引导** — 通过 `docker exec`（容器）或 `sudo -u jarvis`（原生）运行一次流程：
...
# 确保：chown jarvis:jarvis，chmod 0600
```
...
当 jarvis 通过 NixOS 模块运行时，以下 CLI 命令会被**屏蔽**，并显示指向 `configuration.nix` 的描述性错误：

| 被屏蔽的命令 | 原因 |
|---|---|
| `jarvis setup` | 配置是声明式的——请在 Nix 配置中编辑 `settings` |
| `jarvis config edit` | 配置由 `settings` 生成 |
| `jarvis config set <key> <value>` | 配置由 `settings` 生成 |
| `jarvis gateway install` | systemd 服务由 NixOS 管理 |
| `jarvis gateway uninstall` | systemd 服务由 NixOS 管理 |

这可以防止 Nix 声明的内容与磁盘上实际内容之间产生漂移。检测使用两个信号：

1. **`JARVIS_MANAGED=true`** 环境变量——由 systemd 服务设置，对 gateway 进程可见
2. **`.managed` 标记文件**，位于 `JARVIS_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it jarvis-agent jarvis config set ...` 也会被屏蔽）
...
启用容器模式后，jarvis 在持久化 Ubuntu 容器内运行，Nix 构建的二进制文件以只读方式从主机绑定挂载：

```
主机                                    容器
────                                    ─────────
/nix/store/...-jarvis-agent-0.1.0  ──►  /nix/store/... (ro)
~/.jarvis -> /var/lib/jarvis/.jarvis       （符号链接桥接，按 hostUsers）
...
Nix 构建的二进制文件能在 Ubuntu 容器内运行，是因为 `/nix/store` 被绑定挂载——它携带自己的解释器和所有依赖，不依赖容器的系统库。容器入口点通过 `current-package` 符号链接解析：`/data/current-package/bin/jarvis gateway run --replace`。执行 `nixos-rebuild switch` 时，只更新符号链接——容器继续运行。

### 各事件的持久性

| 事件 | 容器重建？ | `/data`（状态） | `/home/jarvis` | 可写层（`apt`/`pip`/`npm`） |
|---|---|---|---|---|
| `systemctl restart jarvis-agent` | 否 | 保留 | 保留 | 保留 |
...
仅当容器的**身份哈希**发生变化时才会重建容器。哈希涵盖：schema 版本、镜像、`extraVolumes`、`extraOptions` 和入口点脚本。环境变量、settings、文档或 jarvis 包本身的变更**不会**触发重建。

:::warning 可写层丢失
当身份哈希发生变化（镜像升级、新卷、新容器选项）时，容器会被销毁并从 `container.image` 的全新拉取重建。可写层中通过 `apt install`、`pip install` 或 `npm install` 安装的包将丢失。`/data` 和 `/home/jarvis` 中的状态会保留（这些是绑定挂载）。

如果 Agent 依赖特定包，考虑将其烘焙到自定义镜像中（`container.image = "my-registry/jarvis-base:latest"`），或在 Agent 的 SOUL.md 中编写安装脚本。
:::

### GC Root 保护

`preStart` 脚本在 `${stateDir}/.gc-root` 创建一个指向当前 jarvis 包的 GC root。这可以防止 `nix-collect-garbage` 删除正在运行的二进制文件。如果 GC root 损坏，重启服务会重新创建它。

---

## 插件

NixOS 模块支持声明式插件安装——无需命令式的 `jarvis plugins install`。

### 目录插件（`extraPlugins`）

对于只包含 `plugin.yaml` + `__init__.py` 的源码树插件（例如 [jarvis-lcm](https://github.com/stephenschoettler/jarvis-lcm)）：
...
插件在激活时以符号链接方式安装到 `$JARVIS_HOME/plugins/`。Jarvis 通过其正常的目录扫描发现它们。从列表中移除插件并运行 `nixos-rebuild switch` 会删除符号链接。

### 入口点插件（`extraPythonPackages`）

对于通过 `[project.entry-points."jarvis_agent.plugins"]` 注册的 pip 打包插件（例如 [rtk-jarvis](https://github.com/ogallotti/rtk-jarvis)）：
...
该包的 `site-packages` 会添加到 jarvis wrapper 的 PYTHONPATH 中。`importlib.metadata` 在会话启动时发现入口点。

### 可选依赖组（`extraDependencyGroups`）

对于已在 jarvis-agent 的 `pyproject.toml` 中声明的可选 extras（例如 `hindsight` 或 `honcho` 等记忆提供商），使用 `extraDependencyGroups` 在构建时将其包含到封闭的 venv 中：
...
:::note
构建时冲突检查可防止插件包覆盖核心 jarvis 依赖。如果插件提供了封闭 venv 中已有的包，`nixos-rebuild` 会以明确的错误失败。
:::
...
nix build .#checks.x86_64-linux.managed-guard       # JARVIS_MANAGED 屏蔽变更操作
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `jarvis` 和 `jarvis-agent` 二进制文件存在且 `jarvis version` 可运行 |
| `entry-points-sync` | `pyproject.toml` 中 `[project.scripts]` 的每个条目在 Nix 包中都有对应的封装二进制文件 |
| `cli-commands` | `jarvis --help` 暴露 `gateway` 和 `config` 子命令 |
| `managed-guard` | `JARVIS_MANAGED=true jarvis config set ...` 打印 NixOS 错误 |
| `bundled-skills` | skills 目录存在，包含 SKILL.md 文件，wrapper 中设置了 `JARVIS_BUNDLED_SKILLS` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `enable` | `bool` | `false` | 启用 jarvis-agent 服务 |
| `package` | `package` | `jarvis-agent` | 使用的 jarvis-agent 包 |
| `user` | `str` | `"jarvis"` | 系统用户 |
| `group` | `str` | `"jarvis"` | 系统组 |
| `createUser` | `bool` | `true` | 自动创建用户/组 |
| `stateDir` | `str` | `"/var/lib/jarvis"` | 状态目录（`JARVIS_HOME` 的父目录） |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent 工作目录（`MESSAGING_CWD`） |
| `addToSystemPackages` | `bool` | `false` | 将 `jarvis` CLI 添加到系统 PATH 并在系统范围内设置 `JARVIS_HOME` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | 包含密钥的 env 文件路径。激活时合并到 `$JARVIS_HOME/.env` |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | `jarvis gateway` 的额外参数 |
| `extraPackages` | `listOf package` | `[]` | Agent 可用的额外包。添加到 jarvis 用户的每用户 profile，终端命令、skills 和 cron 任务均可见 |
| `extraPlugins` | `listOf package` | `[]` | 以符号链接方式安装到 `$JARVIS_HOME/plugins/` 的目录插件包。每个包必须包含 `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.jarvis` 符号链接（指向服务 stateDir）的交互式用户，自动加入 `jarvis` 组 |
...
```
/var/lib/jarvis/                     # stateDir（归 jarvis:jarvis 所有，权限 0750）
├── .jarvis/                         # JARVIS_HOME
...
| 容器路径 | 主机路径 | 模式 | 说明 |
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Jarvis 二进制文件 + 所有 Nix 依赖 |
| `/data` | `/var/lib/jarvis` | `rw` | 所有状态、配置、工作区 |
| `/home/jarvis` | `${stateDir}/home` | `rw` | 持久化 Agent home——`pip install --user`、工具缓存 |
...
| `jarvis version` 显示旧版本 | 容器未重启 | `systemctl restart jarvis-agent` |
| `/var/lib/jarvis` 权限拒绝 | 状态目录为 `0750 jarvis:jarvis` | 使用 `docker exec` 或 `sudo -u jarvis` |
| `nix-collect-garbage` 删除了 jarvis | GC root 缺失 | 重启服务（preStart 会重新创建 GC root） |
| `no container with name or ID "jarvis-agent"`（Podman） | Podman rootful 容器对普通用户不可见 | 为 podman 添加免密 sudo（参见[容器模式](#container-mode)章节） |
| `unable to find user jarvis` | 容器仍在启动中（入口点尚未创建用户） | 等待几秒后重试——CLI 会自动重试 |
| 通过 `extraPackages` 添加的工具在终端中找不到 | 需要 `nixos-rebuild switch` 更新每用户 profile | 重建并重启：`nixos-rebuild switch && systemctl restart jarvis-agent` |