...
sidebar_position: 1
title: "消息网关"
description: "通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Yuanbao、Microsoft Teams、LINE、Webhooks 或任何兼容 OpenAI 的前端与 Jarvis 对话 — 架构与配置概览"
---

# 消息网关

通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Feishu/Lark、WeCom、Weixin、BlueBubbles（iMessage）、QQ、Yuanbao、Microsoft Teams、LINE、ntfy 或浏览器与 Jarvis 对话。网关是一个单一后台进程，连接所有已配置的平台，管理会话，运行 cron 任务，并传递语音消息。

完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [Use Voice Mode with Jarvis](/guides/use-voice-mode-with-jarvis)。
...
每个平台适配器接收消息，通过每个聊天的会话存储进行路由，并将其分发给 AIBrain 处理。网关还运行 cron 调度器，每 60 秒触发一次以执行到期任务。
...
```bash
jarvis gateway setup        # 交互式配置所有消息平台
```
...
```bash
jarvis gateway              # 在前台运行
jarvis gateway setup        # 交互式配置消息平台
jarvis gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo jarvis gateway install --system   # 仅 Linux：安装开机启动的系统服务
jarvis gateway start        # 启动默认服务
jarvis gateway stop         # 停止默认服务
jarvis gateway status       # 检查默认服务状态
jarvis gateway status --system         # 仅 Linux：显式检查系统服务
```
...
在 `~/.jarvis/gateway.json` 中配置各平台的覆盖设置：
...
# 其他配对命令：
jarvis pairing list          # 查看待审核和已批准的用户
jarvis pairing revoke telegram 123456789  # 撤销访问权限
```
...
第一次在任意平台向繁忙的 agent 发送消息时，Jarvis 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示每次安装只触发一次——由 `onboarding.seen.busy_input_prompt` 下的标志锁定。删除该键可再次看到提示。

如果你觉得繁忙确认消息过多——尤其是使用语音输入或快速连续发送消息时——可设置 `display.busy_ack_enabled: false`。你的输入仍会正常排队/引导/中断，只是聊天回复被静默。

## 工具进度通知

在 `~/.jarvis/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.jarvis/config.yaml` 中的 `display.background_process_notifications` 控制：
...
```bash
jarvis gateway install               # 安装为用户服务
jarvis gateway start                 # 启动服务
jarvis gateway stop                  # 停止服务
jarvis gateway status                # 检查状态
journalctl --user -u jarvis-gateway -f  # 查看日志
...
除非你确实有此需要，否则避免同时安装用户和系统网关单元。Jarvis 检测到两者同时存在时会发出警告，因为 start/stop/status 行为会变得不明确。

:::info 多个安装
如果你在同一台机器上运行多个 Jarvis 安装（使用不同的 `JARVIS_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.jarvis` 使用 `jarvis-gateway`；其他安装使用 `jarvis-gateway-<hash>`。`jarvis gateway` 命令会自动针对当前 `JARVIS_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
jarvis gateway install               # 安装为 launchd agent
jarvis gateway start                 # 启动服务
jarvis gateway stop                  # 停止服务
jarvis gateway status                # 检查状态
tail -f ~/.jarvis/logs/gateway.log   # 查看日志
```

生成的 plist 文件位于 `~/Library/LaunchAgents/ai.jarvis.gateway.plist`。它包含三个环境变量：
...
- **JARVIS_HOME** — 将网关限定到你的 Jarvis 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `jarvis gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `JARVIS_HOME` 目录都有自己的 launchd 标签。默认的 `~/.jarvis` 使用 `ai.jarvis.gateway`；其他安装使用 `ai.jarvis.gateway-<suffix>`。
:::
...
|----------|---------|--------------|
| CLI | `jarvis-cli` | 完全访问 |
| Telegram | `jarvis-telegram` | 完整工具，包括终端 |
| Discord | `jarvis-discord` | 完整工具，包括终端 |
| WhatsApp | `jarvis-whatsapp` | 完整工具，包括终端 |
| Slack | `jarvis-slack` | 完整工具，包括终端 |
| Google Chat | `jarvis-google_chat` | 完整工具，包括终端 |
| Signal | `jarvis-signal` | 完整工具，包括终端 |
| SMS | `jarvis-sms` | 完整工具，包括终端 |
| Email | `jarvis-email` | 完整工具，包括终端 |
| Home Assistant | `jarvis-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `jarvis-mattermost` | 完整工具，包括终端 |
| Matrix | `jarvis-matrix` | 完整工具，包括终端 |
| DingTalk | `jarvis-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `jarvis-feishu` | 完整工具，包括终端 |
| WeCom | `jarvis-wecom` | 完整工具，包括终端 |
| WeCom Callback | `jarvis-wecom-callback` | 完整工具，包括终端 |
| Weixin | `jarvis-weixin` | 完整工具，包括终端 |
| BlueBubbles | `jarvis-bluebubbles` | 完整工具，包括终端 |
| QQBot | `jarvis-qqbot` | 完整工具，包括终端 |
| Yuanbao | `jarvis-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `jarvis-teams` | 完整工具，包括终端 |
| API Server | `jarvis-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `jarvis-webhook` | 完整工具，包括终端 |
...
1. **网关日志**（`~/.jarvis/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
