...
| | |
|---|---|
| 来源 | 可选 — 通过 `jarvis skills install official/productivity/canvas` 安装 |
...
4. 为 token 命名（例如 "Jarvis"），设置可选的过期时间，然后点击 **Generate Token**
5. 复制 token 并添加到 `~/.jarvis/.env`：
...
```bash
CANVAS="python $JARVIS_HOME/skills/productivity/canvas/scripts/canvas_api.py"
