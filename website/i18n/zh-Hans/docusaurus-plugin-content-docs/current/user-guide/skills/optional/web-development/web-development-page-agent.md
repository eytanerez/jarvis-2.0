...
- 用户希望 **Jarvis 本身驱动浏览器** → 使用 Jarvis 内置的浏览器工具（Browserbase / Camofox）。page-agent 是*相反*的方向。
...
- 存在白名单和数据脱敏 hook，用于限制 agent 可操作的范围——完整选项列表见 https://alibaba.github.io/page-brain/
...
| 包 | 路径 | 用途 |
|---------|------|---------|
| `page-agent` | `packages/page-brain/` | 带 UI 面板的主入口 |
| `@page-brain/core` | `packages/core/` | 核心 agent 逻辑，无 UI |
...
- 文档：https://alibaba.github.io/page-brain/
