...
| | |
|---|---|
| 来源 | 可选 — 通过 `jarvis skills install official/research/darwinian-evolver` 安装 |
| 路径 | `optional-skills/research/darwinian-evolver` |
| 版本 | `0.1.0` |
| 作者 | Bihruze (Asahi0x), Jarvis |
| 许可证 | MIT |
| 平台 | linux, macos |
...
调用来调用它（纯聚合方式）。**不得**将上游类导入 Jarvis 本身。
...
```bash
mkdir -p ~/.jarvis/cache/darwinian-evolver && cd ~/.jarvis/cache/darwinian-evolver
...
```bash
cd ~/.jarvis/cache/darwinian-evolver/darwinian_evolver \
...
在浏览器中打开 `~/.jarvis/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
并加载 JSON 日志，即可查看进化树。
...
SKILL_DIR=~/.jarvis/skills/research/darwinian-evolver
DE_DIR=~/.jarvis/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL 协议。** 永远不要在 Jarvis 核心中使用 `from darwinian_evolver import ...`。`~/.jarvis/skills/...` 下的自定义驱动脚本属于用户侧，没有问题。
