...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.jarvis/.env ] && grep -q "^GITHUB_TOKEN=" ~/.jarvis/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.jarvis/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${JARVIS_HOME:-$HOME/.jarvis}/skills/github/github-auth/scripts/gh-env.sh"
...
```bash
# 若无问题——批准
gh pr review $PR_NUMBER --approve --body "Reviewed by Jarvis. Code looks clean — good test coverage, no security concerns."
