...
This page covers all commands related to [Jarvis profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `jarvis profile` without a subcommand shows help.
...
```bash
jarvis profile use <name>
```

Sets `<name>` as the active profile. All subsequent `jarvis` commands (without `-p`) will use this profile.
...
jarvis profile use default
```

## `jarvis profile create`

```bash
jarvis profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `jarvis profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `jarvis update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.jarvis`), use `jarvis skills opt-out` / `jarvis skills opt-in`. |
...
jarvis profile create work2 --clone-from work

# Clone everything from a specific profile
jarvis profile create work2-backup --clone-from work --clone-all
```

## `jarvis profile describe`

```bash
jarvis profile describe [<name>] [options]
```
...
jarvis profile describe researcher

# Set it explicitly
jarvis profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
jarvis profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
jarvis profile describe --all --auto
```

## `jarvis profile delete`

```bash
jarvis profile delete <name> [options]
```
...
```bash
jarvis profile delete mybot
jarvis profile delete mybot --yes
```
...
```bash
jarvis profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Jarvis home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ jarvis profile show work
Profile: work
Path:    ~/.jarvis/profiles/work
...
## `jarvis profile alias`

```bash
jarvis profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Jarvis installation.
...
```bash
jarvis profile alias work
# Creates/updates ~/.local/bin/work

jarvis profile alias work --name mywork
# Creates ~/.local/bin/mywork

jarvis profile alias work --remove
# Removes the wrapper script
```

## `jarvis profile rename`

```bash
jarvis profile rename <old-name> <new-name>
```
...
```bash
jarvis profile rename mybot assistant
# ~/.jarvis/profiles/mybot → ~/.jarvis/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `jarvis profile export`

```bash
jarvis profile export <name> [options]
```
...
```bash
jarvis profile export work
# Creates work.tar.gz in the current directory

jarvis profile export work -o ./work-2026-03-29.tar.gz
```

## `jarvis profile import`

```bash
jarvis profile import <archive> [options]
```
...
```bash
jarvis profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

jarvis profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`jarvis profile export` / `import` are still the right commands for
...
### `jarvis profile install`

```bash
jarvis profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `jarvis -p telemetry`). |
...
jarvis profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
jarvis profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
jarvis profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
jarvis profile install ./telemetry/
```

### `jarvis profile update`

```bash
jarvis profile update <name> [--force-config] [--yes]
```
...
```bash
jarvis profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Jarvis version, author, env var requirements, the source URL/path, and
...
`jarvis profile list` also shows the distribution name and version in a
`Distribution` column, and `jarvis profile show <name>` / `delete <name>`
...
jarvis profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
jarvis profile install https://github.com/your-org/internal-assistant.git
```
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Jarvis can clone from.
3. Tell recipients to run `jarvis profile install <your-repo-url>`.
...
## `jarvis -p` / `jarvis --profile`

```bash
jarvis -p <name> <command> [options]
jarvis --profile <name> <command> [options]
```

Global flag to run any Jarvis command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
jarvis -p work chat -q "Check the server status"
jarvis --profile dev gateway start
jarvis -p personal skills list
jarvis -p work config edit
```

## `jarvis completion`

```bash
jarvis completion <shell>
```
...
- `jarvis profile <TAB>` — subcommands (list, use, create, etc.)
- `jarvis profile use <TAB>` — profile names
- `jarvis -p <TAB>` — profile names
