...
description: "Frequently asked questions and solutions to common issues with Jarvis"
---
...
### What LLM providers work with Jarvis?
...
- **Anthropic** — Claude models (direct API, OAuth via `jarvis auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `jarvis model` or by editing `~/.jarvis/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Yes, natively.** Jarvis supports native Windows via the PowerShell installer — no WSL required. Run in PowerShell:
...
WSL2 remains a fully supported alternative. To run Jarvis inside WSL2, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and use the standard install command:
...
- let Jarvis use the resulting MCP browser tools

This is more reliable than trying to force Jarvis core browser transport to attach directly across the WSL2/Windows boundary.
...
Yes — Jarvis now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Jarvis does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.jarvis/`.

### Can I use it offline / with local models?

Yes. Run `jarvis model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Jarvis minimum; set this to match your server's actual context window
```
...
Jarvis persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Jarvis — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Jarvis auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `JARVIS_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Jarvis itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Jarvis instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIBrain` class and use Jarvis programmatically:

```python
from run_brain import AIBrain

agent = AIBrain(model="anthropic/claude-opus-4.7")
...
#### `jarvis: command not found` after installation
...
**Cause:** Jarvis requires Python 3.11 or newer.
...
**Cause:** Jarvis builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Jarvis runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Jarvis auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.jarvis/config.yaml`:
...
**Solution:** Exit your session and use `jarvis model` from your terminal to add new providers:

```bash
# Exit the Jarvis chat session first (Ctrl+C or /quit)
...
After adding a new provider via `jarvis model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `jarvis model` (from terminal) |
| Enter/change API keys | `jarvis model` (from terminal) |
...
# Or set directly
jarvis config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.jarvis/.env` for conflicting entries.
:::
...
# Set a valid model
jarvis config set JARVIS_MODEL anthropic/claude-opus-4.7

# Or specify per-session
jarvis chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `jarvis chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Jarvis detected the wrong context length for your model.
...
jarvis chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Jarvis may have the wrong context length for your model. Check what it detected:
...
**Cause:** Jarvis detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Jarvis never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `jarvis chat`
...
cat ~/.jarvis/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `jarvis gateway setup`
- Check gateway logs: `cat ~/.jarvis/logs/gateway.log | tail -50`
...
Configure in `~/.jarvis/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "jarvis-agent[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `jarvis gateway start` fails
...
tmux new -s jarvis 'jarvis gateway run'
# Reattach later: tmux attach -t jarvis

# Option 3: Background via nohup
nohup jarvis gateway run > ~/.jarvis/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'jarvis gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `jarvis gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
jarvis gateway install    # Re-snapshots your current PATH
jarvis gateway start      # Detects the updated plist and reloads
```
...
**Solution:**
- Try a faster/smaller model: `jarvis chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `jarvis chat -t "terminal"`
...
Verify your `~/.jarvis/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
jarvis config show | grep -A 12 mcp_servers

# Restart Jarvis or reload MCP after config changes
jarvis chat
```

See also:
- [MCP (Model Context Protocol)](/user-guide/features/mcp)
- [Use MCP with Jarvis](/guides/use-mcp-with-jarvis)
...
:::warning
If an MCP server crashes mid-request, Jarvis will report a timeout. Check the server's own logs (not just Jarvis logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `jarvis profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

### What happens when I run `jarvis update`?

`jarvis update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `jarvis update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.jarvis/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Jarvis can route subagents to a different model automatically. Set this in `~/.jarvis/config.yaml`:
...
Now when you tell Jarvis "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Jarvis do this?

**Current limitation:** Jarvis profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Jarvis reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `jarvis skills config` settings don't seem to take effect.

**Solution:** Use `jarvis skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`jarvis gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Jarvis creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Jarvis to another machine
...
2. On the **source machine**, create a full backup:
   ...
   jarvis backup
   ```
   This creates a zip of your entire `~/.jarvis/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/jarvis-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/jarvis-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   jarvis import ~/jarvis-backup-<timestamp>.zip
   ```

4. On the new machine, run `jarvis setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
jarvis profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
jarvis profile import ./work-backup.tar.gz work
```
...
### `jarvis backup` vs `jarvis profile export`

| Feature | `jarvis backup` | `jarvis profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.jarvis` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='jarvis-agent' ~/.jarvis/ newmachine:~/.jarvis/
```

:::tip
`jarvis backup` produces a consistent snapshot even while Jarvis is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Jarvis installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Jarvis-specific issue — it's a shell config permissions problem.
...
jarvis config show | head -20

# Re-run model selection
jarvis model

# Or test with a known-good model
jarvis chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/jarvis-brain/issues)
