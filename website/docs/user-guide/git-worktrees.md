...
title: "Git Worktrees"
description: "Run multiple Jarvis agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Jarvis is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Jarvis so each session has a clean, isolated working directory.

## Why Use Worktrees with Jarvis?

Jarvis treats the **current working directory** as the project root:

- CLI: the directory where you run `jarvis` or `jarvis chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.jarvis/config.yaml`
...
git worktree add ../repo-feature feature/jarvis-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/jarvis-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Jarvis there:

```bash
cd ../repo-feature

# Start Jarvis in the worktree
jarvis
```
...
git worktree add ../repo-experiment-a feature/jarvis-a
git worktree add ../repo-experiment-b feature/jarvis-b
```
...
- Works on its own branch (`feature/jarvis-a` vs `feature/jarvis-b`).
...
- Jarvis checkpoint data under `~/.jarvis/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Jarvis experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/jarvis-checkpoints-docs`, `feature/jarvis-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Jarvis from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `jarvis -w` (Automatic Worktree Mode)

Jarvis has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `jarvis/jarvis-<hash>`).
...
```bash
jarvis -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `jarvis -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Jarvis session its own clean checkout.
