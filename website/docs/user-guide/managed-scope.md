...
`~/.jarvis/config.yaml`, `~/.jarvis/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/jarvis`:

```text
/etc/jarvis/
├── config.yaml     # managed config layer (wins over ~/.jarvis/config.yaml)
└── .env            # managed env layer (wins over ~/.jarvis/.env + shell)
```
...
The location can be relocated with the `JARVIS_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `JARVIS_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Jarvis.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export JARVIS_MANAGED_DIR=/opt/org/jarvis-policy
```

:::warning
A user who can set `JARVIS_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `jarvis doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/jarvis/config.yaml` (managed) | `/etc/jarvis/.env` (managed) |
| 2 | `~/.jarvis/config.yaml` (user) | `~/.jarvis/.env` (user) |
...
```bash
jarvis config        # shows a header naming the managed source + the pinned keys
jarvis doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Jarvis refuses and names the source:

```bash
$ jarvis config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/jarvis/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `jarvis config set` / setup will not write
...
sudo tee /etc/jarvis/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/jarvis/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/jarvis
sudo chmod 0644 /etc/jarvis/config.yaml /etc/jarvis/.env
```

Changes take effect on the next Jarvis start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`jarvis doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Jarvis as `root`), managed scope is advisory.
