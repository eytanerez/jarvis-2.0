...
If you only run one Jarvis agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Jarvis agents that should all
...
(`ai.jarvis.gateway-<name>.plist`) or systemd user service
(`jarvis-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
jarvis profile create coder
jarvis profile create personal-bot
jarvis profile create research
...
```bash
jarvis config set gateway.multiplex_profiles true
jarvis gateway restart
```

Equivalently, in the default profile's `~/.jarvis/config.yaml`:
...
You do **not** run `jarvis gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `jarvis gateway start` / `run` is a
...
home). `jarvis status` reports the multiplexer and the profiles it serves;
`jarvis status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/jarvis-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: jarvis-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    jarvis gateway "$action"
  else
    jarvis -p "$profile" gateway "$action"
  fi
}
...
```bash
jarvis-gateways start      # start every configured profile
jarvis-gateways stop       # stop every configured profile
jarvis-gateways restart    # restart all
jarvis-gateways status     # status across all
jarvis-gateways list       # delegates to `jarvis gateway list`
```

:::tip
The `default` profile is targeted with `jarvis gateway <action>` (no `-p`),
not `jarvis -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `jarvis -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.jarvis.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/jarvis-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.jarvis.gateway.plist` /
...
tail -f ~/.jarvis/logs/gateway.error.log

# Named profile
tail -f ~/.jarvis/profiles/<name>/logs/gateway.log
tail -f ~/.jarvis/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.jarvis/logs/gateway.log ~/.jarvis/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
jarvis logs -f                  # follow default profile
jarvis -p coder logs -f         # follow one profile
jarvis logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
jarvis profile list             # profiles + model + gateway state
jarvis-gateways status          # full status across every profile
launchctl list | grep jarvis    # macOS — PIDs and labels
systemctl --user list-units 'jarvis-gateway-*'   # Linux — units
```
...
```
~/.jarvis/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.jarvis/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
jarvis config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.jarvis/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=jarvis --why="gateways running" \
  sleep infinity &
...
`jarvis-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.jarvis/.env ~/.jarvis/profiles/*/.env
```

## Updating the code

`jarvis update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `jarvis gateway start` after a previous `jarvis gateway stop`. The
...
```bash
ps -ef | grep "jarvis_cli.*-p <profile>"
cat ~/.jarvis/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.jarvis.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.jarvis.gateway-<profile>.plist

# Linux
systemctl --user restart jarvis-gateway-<profile>.service
```

### Health check

```bash
jarvis doctor                  # default profile
jarvis -p <profile> doctor     # one profile
```
