...
Connect Jarvis to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Jarvis — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Jarvis runs it in a
...
jarvis photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Jarvis` project on your account.
...
Runtime credentials are written to `~/.jarvis/.env`
...
(device token, dashboard project id) lives in `~/.jarvis/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Jarvis
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Jarvis replies with a pairing code. Approve it with:

```bash
jarvis pairing approve photon <CODE>
```

Use `jarvis pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.jarvis/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.jarvis/.env`):
...
By default Jarvis responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Jarvis` and
`@Jarvis agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Jarvis will reply.

## Status & troubleshooting

```bash
jarvis photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Jarvis uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run jarvis photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `jarvis photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `jarvis photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `jarvis photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Jarvis sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Jarvis wake words  | JSON list / comma / newline regex patterns for group mentions |
