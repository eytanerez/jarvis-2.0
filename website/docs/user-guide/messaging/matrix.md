...
sidebar_position: 9
title: "Matrix"
description: "Set up Jarvis as a Matrix bot"
---

# Matrix Setup

Jarvis integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Jarvis pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Jarvis works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Jarvis behaves once it's connected.

## How Jarvis Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Jarvis responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Jarvis requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Jarvis supports Matrix threads (MSC3440). If you reply in a thread, Jarvis keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Jarvis auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages — this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | Jarvis accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; Jarvis normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, Jarvis isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
Jarvis sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
Jarvis now includes the current Matrix room name, room ID, topic, message ID,
...
2. Choose a username like `jarvis` — the full user ID will be `@jarvis:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `jarvis-bot`).

### Option C: Use Your Own Account

You can also run Jarvis as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Jarvis needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Jarvis the bot's user ID and password. Jarvis will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@jarvis:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

Jarvis uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
# MATRIX_USER_ID=@jarvis:matrix.example.org
...
Bridge and appservice deployments need extra loop protection. Jarvis always
...
Jarvis supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with jarvis extras
pip install 'jarvis-agent[matrix]'
```
...
When E2EE is enabled, Jarvis:

- Stores encryption keys in `~/.jarvis/platforms/matrix/store/` (legacy installs: `~/.jarvis/matrix/store/`)
...
In Matrix conversations, Jarvis exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when Jarvis knows who requested the action.

### Media Limits

Jarvis uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. Jarvis rejects arbitrary
...
Jarvis includes an opt-in Synapse harness for local validation:

```bash
docker compose -f tests/e2e/matrix_synapse_gateway/docker-compose.yml up -d
JARVIS_MATRIX_SYNAPSE_INTEGRATION=1 \
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Jarvis imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If Jarvis bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
If you delete `~/.jarvis/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

Jarvis detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@jarvis:your-server';
     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@jarvis:your-server';
     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@jarvis:your-server';
     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@jarvis:your-server';
   "
...
2. Delete the local crypto store and restart Jarvis:
   ```bash
   rm -f ~/.jarvis/platforms/matrix/store/crypto.db*
   # restart jarvis
   ```
...
Jarvis supports the same gateway commands in Matrix that it supports on other
...
Jarvis only normalizes `!command` when the command is known to the gateway, a
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/NousResearch/jarvis-brain/issues/12614).
...
If you previously used Jarvis with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@jarvis:your-server.org"},
       "password": "***",
       "initial_device_display_name": "Jarvis"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.jarvis/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.jarvis/platforms/matrix/store/crypto.db
   rm -f ~/.jarvis/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.jarvis/.env`:
...
a working E2EE setup with a previous version of Jarvis and are upgrading.
...
macOS (Host):
  └─ jarvis gateway
       ...
       ├─ AIBrain ← single source of truth
       ├─ Sessions, memory, skills
       └─ Local file access (Obsidian, projects, etc.)

Linux VM (Docker):
  └─ jarvis gateway (proxy mode)
...
services:
  jarvis-matrix:
    ...
    environment:
      ...
      MATRIX_DEVICE_ID: "JARVIS_BOT"

      # Proxy mode — forward to host agent
      GATEWAY_PROXY_URL: "http://192.168.1.100:8642"
      GATEWAY_PROXY_KEY: "your-secret-key-here"
    volumes:
      - ./matrix-store:/root/.jarvis/platforms/matrix/store
```
...
RUN pip install 'jarvis-agent[matrix]'
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Jarvis API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-Jarvis-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: Jarvis uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps Jarvis's own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Jarvis logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Jarvis gateway isn't running, or it failed to connect.

**Fix**: Check that `jarvis gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.jarvis/.env` and restart the gateway. Use the full `@user:server` format.
...
- **Media support**: Jarvis can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
