...
Enable `status_indicator` and Jarvis sets that short description to **Online**
...
The gateway extracts `MEDIA:/path/to/file` tags from brain replies and ships the referenced file as a platform-native attachment. Supported extensions across all gateway platforms:
...
**MarkdownV2 fallback.** When the rich path is unavailable for a message, Jarvis converts markdown to MarkdownV2. Since MarkdownV2 has no native table syntax, pipe tables are normalized:
