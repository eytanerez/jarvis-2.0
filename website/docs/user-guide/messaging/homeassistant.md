...
description: Control your smart home with Jarvis via Home Assistant integration.
...
Jarvis integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Jarvis"
5. Copy the token
...
Jarvis registers four tools for smart home control:
...
Configure which events the agent sees in `~/.jarvis/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Jarvis".
...
The adapter reads credentials from `~/.jarvis/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Jarvis profile
...
running Jarvis — `curl -H "Authorization: Bearer <token>" <url>/api/` should
