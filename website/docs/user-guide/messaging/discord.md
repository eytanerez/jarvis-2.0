...
- [Use Voice Mode with Jarvis](/guides/use-voice-mode-with-jarvis)
...
discord.py plays only one audio stream per connection, so Jarvis installs a software mixer on the outgoing stream that sums an ambient loop, acknowledgements, and TTS replies into that single stream — they overlap instead of cutting each other off.
