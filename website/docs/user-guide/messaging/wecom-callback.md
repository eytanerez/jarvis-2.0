...
Connect Jarvis to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Jarvis supports two WeCom integration modes:
...
> Run `jarvis gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Jarvis decrypts the message, queues it for the agent
...
(Use `jarvis gateway start` only after `jarvis gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Jarvis and the token the
...
in `~/.jarvis/.env` values around `=` will also break signature checks. After
fixing, restart `jarvis gateway run`.
...
Check `jarvis gateway run` logs for the bound host/port. If the adapter bound to
