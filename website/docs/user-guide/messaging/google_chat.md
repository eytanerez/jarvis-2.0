...
sidebar_position: 12
title: "Google Chat"
description: "Set up Jarvis as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Jarvis to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Jarvis
...
> Run `jarvis gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Jarvis can read (e.g.,
`~/.jarvis/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a jarvis restart)
...
- Principal: `jarvis-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Jarvis calls
...
- **App name**: whatever you want users to see ("Jarvis" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/jarvis-chat-events`.
...
`ADDED_TO_SPACE` event that Jarvis uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Jarvis

Add the Google Chat section to `~/.jarvis/.env`:
...
Install the dependencies the Google Chat adapter needs (no Jarvis extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Jarvis is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Jarvis detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Jarvis session.
...
3. Download the JSON. Move it onto the host that runs Jarvis.
4. Register the client with Jarvis (run under the profile you want it scoped to):
...
jarvis -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Jarvis home (e.g.
`~/.jarvis/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.jarvis/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.jarvis/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Jarvis isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `jarvis gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Jarvis will only attach the SA bearer
...
- **Redaction**: Service Account emails, subscription paths, and topic paths
  are stripped from log output by `brain/redact.py`. The debug envelope dump
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.jarvis/google_chat_user_tokens/<sanitized_email>.json` (filesystem
