...
Critical rules when operating inside an brain/LLM session:

- **Never** read, print, parse, summarize, upload, or send `~/.xurl` to LLM context.
- **Never** ask the user to paste credentials/tokens into chat.
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Jarvis tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Jarvis Docker layout, `/opt/data` is `JARVIS_HOME`, but Jarvis tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Jarvis-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Jarvis tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Jarvis subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
