...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`jarvis-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-jarvis-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Jarvis terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Jarvis itself, `model.provider: openai-codex` uses Jarvis-managed Codex
OAuth from `~/.jarvis/auth.json` after `jarvis auth add openai-codex`. For the
...
## Jarvis Gateway Caveat

When invoking the Codex CLI from a Jarvis gateway/service context (for example,
