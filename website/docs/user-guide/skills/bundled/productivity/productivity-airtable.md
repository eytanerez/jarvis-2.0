...
4. Store the token in `~/.jarvis/.env` (or via `jarvis setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.jarvis/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
