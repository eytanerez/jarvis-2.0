...
| | |
|---|---|
| Source | Optional — install with `jarvis skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Jarvis |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Jarvis terminal calls (desktop app flow only)
...
### Service Account (recommended for Jarvis)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.jarvis/.env` (the skill will prompt for this on first load).
...
## Jarvis Execution Pattern (desktop app flow)

Jarvis terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/jarvis-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/jarvis-op.sock"
