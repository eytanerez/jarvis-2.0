...
description: "Configure and use Honcho memory with Jarvis -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Jarvis -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `jarvis skills install official/autonomous-ai-agents/honcho` |
...
| Related skills | [`jarvis-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-jarvis-agent) |
...
# Honcho Memory for Jarvis

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Jarvis profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/jarvis#running-honcho-locally-with-jarvis

### Verify

```bash
jarvis honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Jarvis profile's AI peer
...
Honcho models conversations as interactions between **peers**. Jarvis creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Jarvis instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Jarvis run |
| `global` | Single session across all directories |

Manual override: `jarvis honcho map my-project-name`
...
Each Jarvis profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block jarvis.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `jarvis.coder` host block in `honcho.json`
...
```bash
jarvis honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Jarvis when Honcho memory is active.
...
Config file: `$JARVIS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `jarvis honcho setup`. Ensure `memory.provider: honcho` is in `~/.jarvis/config.yaml`.

### Memory not persisting across sessions
Check `jarvis honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `jarvis profile create <name> --clone`. For existing profiles: `jarvis honcho sync`.
...
| Command | Description |
|---------|-------------|
| `jarvis honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `jarvis honcho status` | Show resolved config, connection test, peer info for active profile |
| `jarvis honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `jarvis honcho disable` | Disable Honcho for the active profile |
| `jarvis honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `jarvis honcho peers` | Show peer identities across all profiles |
| `jarvis honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `jarvis honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `jarvis honcho sessions` | List known directory-to-session-name mappings |
| `jarvis honcho map <name>` | Map current working directory to a Honcho session name |
| `jarvis honcho identity` | Seed AI peer identity or show both peer representations |
| `jarvis honcho sync` | Create host blocks for all Jarvis profiles that don't have one yet |
| `jarvis honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Jarvis + Honcho |
| `jarvis memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `jarvis memory status` | Show active memory provider and config |
| `jarvis memory off` | Disable external memory provider |
