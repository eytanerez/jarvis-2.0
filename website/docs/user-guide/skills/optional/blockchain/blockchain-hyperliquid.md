...
| | |
|---|---|
| Source | Optional — install with `jarvis skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Jarvis |
| License | MIT |
...
The script reads `~/.jarvis/.env` for two optional defaults:
...
Helper script: `~/.jarvis/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `~/.jarvis/.env`.
