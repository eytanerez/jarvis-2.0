...
| | |
|---|---|
| Source | Optional — install with `jarvis skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Jarvis or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Jarvis. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.jarvis/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.jarvis/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Jarvis integration, either:

- configure the server in `~/.jarvis/config.yaml` using the `native-mcp` skill, or
...
### Jarvis cannot see the deployed server

The server-building part may be correct while the Jarvis config is not. Load the `native-mcp` skill and configure the server in `~/.jarvis/config.yaml`, then restart Jarvis.
