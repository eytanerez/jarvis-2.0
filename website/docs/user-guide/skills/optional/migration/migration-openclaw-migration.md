---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Jarvis"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Jarvis"
---
...
Migrate a user's OpenClaw customization footprint into Jarvis. Imports Jarvis-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `jarvis skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Jarvis (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Jarvis`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Jarvis Migration

Use this skill when a user wants to move their OpenClaw setup into Jarvis with minimal manual cleanup.
...
```bash
jarvis claw migrate              # Full interactive migration
...
jarvis claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `jarvis setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.
...
- merge OpenClaw command approval patterns into Jarvis `command_allowlist`
- migrate Jarvis-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Jarvis working-directory configuration
- copy OpenClaw skills into `~/.jarvis/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Jarvis workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.jarvis/tts/`
- archive non-secret docs that do not have a direct Jarvis destination
...
1. Prefer the installed path under `~/.jarvis/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Jarvis-compatible secrets, currently:
...
Jarvis CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Jarvis. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Jarvis workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Jarvis skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Jarvis unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Jarvis targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Jarvis destination. Unsupported auth blobs must still be reported as skipped.
...
- Jarvis persona state imported
- Jarvis memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.jarvis/skills/openclaw-imports/`
