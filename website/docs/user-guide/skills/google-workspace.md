...
Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration for Jarvis. Uses OAuth2 with automatic token refresh. Prefers the [Google Workspace CLI (`gws`)](https://github.com/googleworkspace/cli) when available for broader coverage, and falls back to Google's Python client libraries otherwise.

**Skill path:** `skills/productivity/google-workspace/`

## Setup

The setup is fully agent-driven — ask Jarvis to set up Google Workspace and it walks you through each step. The flow:
...
3. **Authorize** — Jarvis generates an auth URL, you approve in the browser, paste back the redirect URL
...
| Problem | Fix |
|---------|-----|
| `NOT_AUTHENTICATED` | Run setup (ask Jarvis to set up Google Workspace) |
