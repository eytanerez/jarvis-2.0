...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Jarvis's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Jarvis instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
jarvis memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `jarvis` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Jarvis syncs them back at session init.
...
The `jarvis honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `jarvis memory setup honcho` (or run `jarvis memory setup` and pick it from the list); the `jarvis honcho` subcommand then appears on the next invocation.

```bash
jarvis memory setup honcho    # Configure Honcho directly (works before activation)
jarvis honcho status          # Connection status, config, and key settings
jarvis honcho setup           # Redirects to `jarvis memory setup` (post-activation alias)
jarvis honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
jarvis honcho peer            # Show or update peer names + dialectic reasoning level
jarvis honcho mode            # Show or set recall mode (hybrid/context/tools)
jarvis honcho tokens          # Show or set token budget for context and dialectic
jarvis honcho identity        # Seed or show the AI peer's Honcho identity
jarvis honcho sync            # Sync Honcho config to all existing profiles
jarvis honcho peers           # Show peer identities across all profiles
jarvis honcho sessions        # List known Honcho session mappings
jarvis honcho map             # Map current directory to a Honcho session name
jarvis honcho enable          # Enable Honcho for the active profile
jarvis honcho disable         # Disable Honcho for the active profile
jarvis honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `jarvis honcho`

If you previously used the standalone `jarvis honcho setup`:
...
No re-login or re-setup needed. Run `jarvis memory setup` and select "honcho" — the wizard detects your existing config.
