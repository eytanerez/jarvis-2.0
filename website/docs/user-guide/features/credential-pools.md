...
Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Jarvis can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Jarvis-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
1. **`brain/credential_pool.py`** — Pool manager: storage, selection, rotation, cooldowns
...
4. **`run_brain.py`** — Error recovery: 429/402/401 → pool rotation → fallback
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Jarvis' credential store, so its token remains persistable.
