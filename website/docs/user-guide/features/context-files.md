...
Context files are loaded by `build_context_files_prompt()` in `brain/prompt_builder.py`:
...
`SubdirectoryHintTracker` in `brain/subdirectory_hints.py` watches tool call arguments for file paths:
