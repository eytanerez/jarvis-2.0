...
description: "Customize Jarvis's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Jarvis's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `JARVIS_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Jarvis is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Jarvis now seeds a default `SOUL.md` automatically in:

```text
~/.jarvis/SOUL.md
```

More precisely, it uses the current instance's `JARVIS_HOME`, so if you run Jarvis with a custom home directory, it will use:
...
- Jarvis creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Jarvis loads `SOUL.md` only from `JARVIS_HOME`
- Jarvis does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Jarvis falls back to a built-in default identity
...
If Jarvis loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `JARVIS_HOME`, the personality belongs to the Jarvis instance itself.

That also makes it easier to teach users:
- "Edit `~/.jarvis/SOUL.md` to change Jarvis' default personality."
...
- how Jarvis should handle uncertainty, disagreement, or ambiguity
...
## What Jarvis injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Jarvis falls back to a built-in default identity ("You are Jarvis, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Jarvis ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Jarvis, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Jarvis its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.jarvis/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.jarvis/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-jarvis)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Jarvis speaks
- `display.skin` and `/skin` affect how Jarvis looks in the terminal
