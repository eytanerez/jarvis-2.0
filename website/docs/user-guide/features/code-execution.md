...
### `JARVIS_*` variables in the child

The child process receives only a small, fixed set of operational `JARVIS_*`
...
(plus `JARVIS_RPC_DIR` / `JARVIS_RPC_SOCKET` / `TZ` / `HOME`, which Jarvis
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `JARVIS_`
through to the child. That broad prefix was removed for security hardening: it
could leak `JARVIS_*`-named configuration that doesn't match a secret substring
(for example `JARVIS_BASE_URL`, `JARVIS_KANBAN_DB`, or a `JARVIS_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `JARVIS_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (Jarvis-managed provider credentials can never
...
**Diagnosing it.** When the child drops one or more non-allowlisted `JARVIS_*`
variables, Jarvis emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`jarvis logs --level
DEBUG`, or check `~/.jarvis/logs/brain.log`) and look for
`execute_code: dropped N non-allowlisted JARVIS_* var(s)` if a script behaves
as though a `JARVIS_*` variable is missing.
