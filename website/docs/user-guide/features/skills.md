...
By default every profile is seeded with the bundled skill catalog, and each `jarvis update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.jarvis` profile):

```bash
curl -fsSL https://jarvis-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
jarvis profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
jarvis skills opt-out            # stop future seeding — nothing on disk is touched
jarvis skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
jarvis skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `jarvis update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `jarvis skills opt-in`) to re-enable.

:::note Safe by default
`jarvis skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Jarvis installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
Staged writes survive restarts under `~/.jarvis/pending/skills/` and are
...
These are maintained in the Jarvis repository itself and install with built-in trust.
...
[Skills Hub](https://jarvis-agent.nousresearch.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Jarvis-side changes required.
