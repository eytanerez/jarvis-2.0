...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Jarvis CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Jarvis CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.jarvis/skins/mytheme.yaml
```

Or set the default skin in `~/.jarvis/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Jarvis — gold and kawaii | `Jarvis` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Jarvis` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Jarvis` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Jarvis` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Jarvis` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Jarvis` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Jarvis! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` ⚕ Jarvis ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default JARVIS_AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.jarvis/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.jarvis/skins/mytheme.yaml
...
- Opens any skin into a visual editor with all Jarvis skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.jarvis/skins/`
- Activates a skin by updating `~/.jarvis/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/jarvis-mod.git
cd jarvis-mod/app
npm install
npm start
```
...
6. Click **Save** to write the skin YAML to `~/.jarvis/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Jarvis Mod respects the `JARVIS_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `jarvis_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.jarvis/skins/` take precedence over built-in skins with the same name.
