...
Jarvis can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `jarvis computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `jarvis tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
4. Start a session with the toolset enabled:
   ...
   ```
   or add `computer_use` to your enabled toolsets in `~/.jarvis/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Jarvis refreshes the binary in two
places so you don't get stuck on a stale release:

- **`jarvis update`** — when you update Jarvis itself, if `cua-driver` is
...
- **`jarvis computer-use install --upgrade`** — manual force-refresh.
...
`jarvis computer-use status` shows the installed version next to the
binary path.
...
Jarvis applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.jarvis/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Jarvis applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `JARVIS_CUA_DRIVER_VERSION`
...
`jarvis computer-use install` to fetch the cua-driver binary, or run
`jarvis tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/jarvis-brain/blob/main/skills/apple/macos-computer-use/SKILL.md)
