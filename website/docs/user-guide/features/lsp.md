...
Jarvis runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Jarvis
...
1. Jarvis captures a baseline of current diagnostics for the file.
...
…). Jarvis auto-detects the binary on PATH or in
`<JARVIS_HOME>/lsp/bin/`.
...
`node_modules` tree — Jarvis installs both packages together when you
run `jarvis lsp install typescript` or auto-install fires on first
use.

## CLI

```
jarvis lsp status          # service state + per-server install status
jarvis lsp list            # registry, optionally --installed-only
jarvis lsp install <id>    # eagerly install one server
jarvis lsp install-all     # try every server with a known recipe
jarvis lsp restart         # tear down running clients
jarvis lsp which <id>      # print resolved binary path
```

`jarvis lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <JARVIS_HOME>/lsp/bin
...
When `install_strategy: auto`, Jarvis installs binaries into
`<JARVIS_HOME>/lsp/bin/`. NPM packages land in
`<JARVIS_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Jarvis-owned and is
...
Servers are kept alive for the life of the Jarvis process. There's
...
**`jarvis lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<JARVIS_HOME>/lsp/bin/`. Run
`jarvis lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `jarvis lsp status`**
...
When `jarvis lsp status` shows a `Backend warnings` section, install
...
`~/.jarvis/logs/brain.log`.

**Server starts but never returns diagnostics**

Check `~/.jarvis/logs/brain.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `jarvis lsp restart` to clear the set;
