...
When Jarvis runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.jarvis/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.jarvis/config.yaml` (switchable per session
via `/personality`).
...
If you use Jarvis' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.jarvis/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Jarvis's deliverable
