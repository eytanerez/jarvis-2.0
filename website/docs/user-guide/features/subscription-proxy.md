...
chat completions — use your Jarvis-managed provider subscription as their
...
| Use case | "Use Jarvis as a chat backend" | "Use my Portal sub from another app" |
...
Model:      Jarvis-4-70B    # or Jarvis-4.3-36B, Jarvis-4-405B
```
...
interface in `jarvis_cli/proxy/adapters/`.
...
If you see `not logged in`, run `jarvis portal`. If you see
...
`jarvis_cli/proxy/adapters/<provider>.py` and registering it in
