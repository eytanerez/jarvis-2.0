...
Off-gateway these keys do nothing. `jarvis memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/NousResearch/jarvis-brain/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/jarvis).
...
echo "OPENVIKING_API_KEY=..." >> ~/.jarvis/.env
```
...
`OPENVIKING_AGENT` is Jarvis' peer ID in OpenViking for peer-scoped memories.
...
See [plugin README](https://github.com/NousResearch/jarvis-brain/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
| **Requires** | `pip install jarvis-memori` + `jarvis-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install jarvis-memori
jarvis-memori install
jarvis config set memory.provider memori
jarvis memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `jarvis-memori` | Tool-aware memory + structured recall |
