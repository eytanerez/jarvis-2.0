...
description: "Run Jarvis directly on an Android phone with Termux"
---

# Jarvis on Android with Termux

This is the tested path for running Jarvis directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Jarvis from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Jarvis now ships a Termux-aware installer path:
...
- links `jarvis` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `jarvis` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/jarvis" "$PREFIX/bin/jarvis"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `jarvis` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.jarvis/.env`.
...
### `jarvis doctor` says ripgrep or Node is missing
