...
:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `jarvis plugins`.
:::
...
Your plugin implements the `MemoryProvider` abstract base class from `brain/memory_provider.py`:
...
the `messages` parameter; Jarvis will continue calling them with the legacy
signature.
...
See `tests/brain/test_memory_provider.py` and adjacent memory tests (`tests/brain/test_memory_session_switch.py`, `tests/brain/test_memory_user_id.py`, `tests/run_brain/test_memory_provider_init.py`) for end-to-end patterns.
