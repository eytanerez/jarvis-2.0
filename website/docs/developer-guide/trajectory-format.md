...
Jarvis saves conversation trajectories in ShareGPT-compatible JSONL format
for use as training data, debugging artifacts, and reinforcement learning datasets.

Source files: `brain/trajectory.py`, `run_brain.py` (search for `_save_trajectory`), `batch_runner.py`
...
It follows the Jarvis function-calling prompt template with:
