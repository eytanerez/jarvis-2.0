...
`purpose` is a free-form audit string — it shows up in `brain.log`
...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Jarvis already has applies. The plugin never sees a
...
jarvis-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Jarvis provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Jarvis provider
        # (must be one Jarvis already knows about — same names as
        # `jarvis model` and config.yaml model.provider).
        ...
        # plugin sends — Jarvis does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.jarvis/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Jarvis' usual aggregator-aware fallback
...
* **Audit log.** Each call writes one INFO line to `brain.log` with
...
Existing `ctx.*` methods extend an existing Jarvis subsystem:
...
* Implementation: [`brain/plugin_llm.py`](https://github.com/NousResearch/jarvis-brain/blob/main/brain/plugin_llm.py)
* Tests: [`tests/brain/test_plugin_llm.py`](https://github.com/NousResearch/jarvis-brain/blob/main/tests/brain/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/jarvis-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/jarvis-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
