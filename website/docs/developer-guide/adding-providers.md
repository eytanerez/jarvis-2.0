...
description: "How to add a new inference provider to Jarvis — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Jarvis can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `jarvis model` menu entries
...
1. `jarvis_cli/auth.py` decides how credentials are found.
2. `jarvis_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
   - `source`
3. `run_brain.py` uses `api_mode` to decide how requests are built and sent.
4. `jarvis_cli/models.py` and `jarvis_cli/main.py` make the provider show up in the CLI. (`jarvis_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
5. `brain/auxiliary_client.py` and `brain/model_metadata.py` keep side tasks and token budgeting working.
...
- a provider adapter in `brain/`
- `run_brain.py` branches for request building, dispatch, usage extraction, interrupt handling, and response normalization
- adapter tests
...
1. `jarvis_cli/auth.py`
2. `jarvis_cli/models.py`
3. `jarvis_cli/runtime_provider.py`
4. `jarvis_cli/main.py`
5. `brain/auxiliary_client.py`
6. `brain/model_metadata.py`
7. tests
8. user-facing docs under `website/docs/`

:::tip
`jarvis_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `jarvis setup`.
:::

### Additional for native / non-OpenAI providers

10. `brain/<provider>_adapter.py`
11. `run_brain.py`
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$JARVIS_HOME/plugins/model-providers/` both get picked up.
...
7. `jarvis model` menu includes the provider
8. `jarvis setup` wizard delegates to `main.py` automatically
...
User plugins at `$JARVIS_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `jarvis model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `jarvis_cli/auth.py`
- `_PROVIDER_LABELS` in `jarvis_cli/models.py`
- `_PROVIDER_ALIASES` in both `jarvis_cli/auth.py` and `jarvis_cli/models.py`
- CLI `--provider` choices in `jarvis_cli/main.py`
...
## Step 2: Add auth metadata in `jarvis_cli/auth.py`
...
- What env vars should Jarvis check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `jarvis_cli/models.py`
...
## Step 4: Resolve runtime data in `jarvis_cli/runtime_provider.py`
...
Be careful with API-key precedence. Jarvis already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `jarvis_cli/main.py`

A provider is not discoverable until it shows up in the interactive `jarvis model` flow.

Update these in `jarvis_cli/main.py`:
...
:::tip
`jarvis_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `jarvis model` and `jarvis setup` automatically.
:::
...
### `brain/model_metadata.py`

Add context lengths for the provider's models so token budgeting, compression thresholds, and limits stay sane.

## Step 7: If the provider is native, add an adapter and `run_brain.py` support

If the provider is not plain chat completions, isolate the provider-specific logic in `brain/<provider>_adapter.py`.

Keep `run_brain.py` focused on orchestration. It should call adapter helpers, not hand-build provider payloads inline all over the file.
...
- normalize provider responses back into what `run_brain.py` expects
...
Also search `run_brain.py` for `self.client.`. Any code path that assumes the standard OpenAI client exists can break when a native provider uses a different client object or `self.client = None`.
...
When you add a native provider, double-check that Jarvis is only sending fields that provider actually understands.
...
- `tests/jarvis_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/jarvis_cli/test_model_switch_custom_providers.py` (and adjacent `tests/jarvis_cli/test_model_switch_*.py`)
- `tests/jarvis_cli/test_setup_model_provider.py`
- `tests/run_brain/test_provider_parity.py`
- `tests/run_brain/test_run_brain.py`
...
python -m pytest tests/jarvis_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/jarvis_cli/test_setup_model_provider.py tests/run_brain/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m jarvis_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m jarvis_cli.main model
...
- [ ] `ProviderConfig` added in `jarvis_cli/auth.py`
- [ ] aliases added in `jarvis_cli/auth.py` and `jarvis_cli/models.py`
- [ ] model catalog added in `jarvis_cli/models.py`
- [ ] runtime branch added in `jarvis_cli/runtime_provider.py`
- [ ] CLI wiring added in `jarvis_cli/main.py` (setup.py inherits automatically)
- [ ] aux model added in `brain/auxiliary_client.py`
- [ ] context lengths added in `brain/model_metadata.py`
...
- [ ] adapter added in `brain/<provider>_adapter.py`
- [ ] new `api_mode` supported in `run_brain.py`
...
### 5. Native-provider branches hiding in `run_brain.py`
...
### 7. Updating `jarvis model` but not `jarvis setup`
