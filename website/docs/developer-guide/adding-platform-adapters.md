...
This guide covers adding a new messaging platform to the Jarvis gateway. A platform adapter connects Jarvis to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.jarvis/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
```
User ↔ Messaging Platform ↔ Platform Adapter ↔ Gateway Runner ↔ AIBrain
```
...
The plugin system lets you add a platform adapter without modifying any core Jarvis code. Your plugin is a directory with two files:

```
~/.jarvis/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `jarvis config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-jarvis-config) below).
...
def register(ctx):
    """Plugin entry point — called by the Jarvis plugin system."""
...
| `jarvis config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `jarvis status` | Shows plugin platforms with `(plugin)` tag |
| `jarvis gateway setup` | Plugin platforms appear in setup menu |
| `jarvis tools` / `jarvis skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.jarvis/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `jarvis gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `jarvis cron run` separate from `jarvis gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `jarvis config`

`jarvis_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
version: 1.0.0
description: >
  My Platform gateway adapter for Jarvis.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #jarvis)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the Jarvis core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`jarvis_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`jarvis_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`jarvis_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`jarvis_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`jarvis_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`jarvis_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
**`brain/prompt_builder.py`** — If your platform has specific rendering limitations (no markdown, message length limits, etc.), add an entry to the `_PLATFORM_HINTS` dict. This injects platform-specific guidance into the system prompt:
...
| `website/docs/reference/toolsets-reference.md` | jarvis-newplat toolset |
