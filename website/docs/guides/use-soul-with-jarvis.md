---
sidebar_position: 7
title: "Use SOUL.md with Jarvis"
description: "How to use SOUL.md to shape Jarvis's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Jarvis

`SOUL.md` is the **primary identity** for your Jarvis instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Jarvis to feel like the same assistant every time you talk to it — or if you want to replace the Jarvis persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Jarvis should be
- what Jarvis should avoid stylistically
- how Jarvis should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Jarvis is and how Jarvis speaks
...
Jarvis now uses only the global SOUL file for the current instance:

```text
~/.jarvis/SOUL.md
```

If you run Jarvis with a custom home directory, it becomes:
...
Jarvis automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Jarvis does not overwrite it
- if the file exists but is empty, Jarvis adds nothing from it to the prompt

## How Jarvis uses it

When Jarvis starts a session, it reads `SOUL.md` from `JARVIS_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Jarvis feels.
...
Jarvis already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Jarvis should sound.

# Avoid
What Jarvis should not do.

# Defaults
How Jarvis should behave when ambiguity appears.
```
...
Then restart Jarvis or start a new session.
...
### I edited SOUL.md but Jarvis still sounds the same

Check:
- you edited `~/.jarvis/SOUL.md` or `$JARVIS_HOME/SOUL.md`
...
### Jarvis is ignoring parts of my SOUL.md
