...
```bash
jarvis teams-pipeline validate
```
...
```bash
jarvis teams-pipeline token-health
jarvis teams-pipeline token-health --force-refresh
```
...
```bash
jarvis teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
jarvis teams-pipeline maintain-subscriptions
jarvis teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Jarvis cron (recommended if you already run the Jarvis gateway)

Jarvis ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.jarvis/scripts/`. First create the script:

```bash
mkdir -p ~/.jarvis/scripts
cat > ~/.jarvis/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec jarvis teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.jarvis/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
jarvis cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/jarvis-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Jarvis Teams pipeline subscription maintenance
...
ExecStart=/usr/local/bin/jarvis teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/jarvis-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Jarvis Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now jarvis-teams-pipeline-maintain.timer
systemctl list-timers jarvis-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/jarvis teams-pipeline maintain-subscriptions >> /var/log/jarvis/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.jarvis/.env` at the top of a wrapper script that crontab calls.
...
```bash
jarvis teams-pipeline subscriptions   # should show expirationDateTime advanced
jarvis teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
jarvis teams-pipeline list
jarvis teams-pipeline list --status failed
jarvis teams-pipeline show <job-id>
```

### Replay a stored job

```bash
jarvis teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
jarvis teams-pipeline fetch --meeting-id <meeting-id>
jarvis teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `jarvis teams-pipeline maintain-subscriptions --dry-run`
- inspect `jarvis teams-pipeline list --status failed`
...
- run `jarvis teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `jarvis teams-pipeline run`
...
- [ ] `jarvis teams-pipeline validate` returns an OK snapshot
- [ ] `jarvis teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Jarvis cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
