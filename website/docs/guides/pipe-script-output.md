...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `jarvis send`."
---

# Pipe Script Output to Messaging Platforms

`jarvis send` is a small, scriptable CLI that pushes a message to any
messaging platform Jarvis is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | jarvis send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `jarvis
...
jarvis send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
jarvis send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
jarvis send --list

# Filter by platform
jarvis send --list telegram
```
...
Any platform Jarvis ships adapters for works as a target:
...
`jarvis send` resolves the message body in this order:

1. **Positional argument** — `jarvis send --to telegram "hi"`
2. **`--file PATH`** — `jarvis send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | jarvis send --to telegram`

When stdin is a TTY (no pipe), Jarvis does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  jarvis send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `jarvis send` reuses your Jarvis config, the same script works on
any host where Jarvis is installed — no need to export bot tokens into
...
`jarvis send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  jarvis send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | jarvis send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.jarvis/bin/jarvis send \
...
```bash
./train.py --epochs 200 && \
  jarvis send --to telegram "training done" || \
  jarvis send --to telegram "training failed (exit $?)"
```
...
jarvis send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(jarvis send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `jarvis send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `jarvis send` calls
the platform's REST endpoint directly using credentials from
`~/.jarvis/.env` and `~/.jarvis/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `jarvis gateway start` and retry.
...
# Machine-readable
jarvis send --list --json
```

The listing is built from `~/.jarvis/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`jarvis gateway
...
| Approach | Multi-platform | Reuses Jarvis creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `jarvis send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`jarvis send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `jarvis send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `jarvis send` shares with cron delivery.
