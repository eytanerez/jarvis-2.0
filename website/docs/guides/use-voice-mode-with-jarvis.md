---
sidebar_position: 8
title: "Use Voice Mode with Jarvis"
description: "A practical guide to setting up and using Jarvis voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Jarvis
...
- you want Jarvis sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Jarvis.
...
## Step 1: make sure normal Jarvis works first
...
Jarvis supports both local and cloud speech stacks.
...
### If you use `jarvis setup`

If you choose NeuTTS in the setup wizard, Jarvis checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Jarvis transcribes and responds
...
- asking Jarvis to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Jarvis loop.

## Tuning CLI behavior

### Silence threshold

If Jarvis starts/stops too aggressively, tune:
...
Jarvis stays a normal chat bot, but can speak replies.
...
- you want Jarvis to function like a portable research or ops assistant
...
Jarvis joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Jarvis detects speech boundaries
- transcripts are posted in the associated text channel
- Jarvis responds in text and audio
...
1. get text Jarvis working
2. install `jarvis-agent[voice]`
