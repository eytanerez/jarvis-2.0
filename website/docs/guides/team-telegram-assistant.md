...
This tutorial walks you through setting up a Telegram bot powered by Jarvis that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Jarvis installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.jarvis/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Jarvis itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Jarvis Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_jarvis_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Jarvis. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Jarvis Gateway...
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `jarvis gateway install` to update the plist.
:::
...
```bash
jarvis gateway stop && jarvis gateway start
```
...
jarvis pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.jarvis/.env`:
...
Control how much detail the bot shows when using tools. In `~/.jarvis/config.yaml`:
...
Customize how the bot communicates by editing `~/.jarvis/SOUL.md`:

For a full guide, see [Use SOUL.md with Jarvis](/guides/use-soul-with-jarvis).
...
```bash
# From the CLI
jarvis cron list          # View all scheduled jobs
jarvis cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u jarvis-gateway` (Linux) or `~/.jarvis/logs/gateway.log` (macOS) |
| Cron job output | `~/.jarvis/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.jarvis/cron/jobs.json` |
| Pairing data | `~/.jarvis/pairing/` |
| Session history | `~/.jarvis/sessions/` |
