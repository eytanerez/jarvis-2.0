...
2. `~/.jarvis/logs/brain.log` for scheduler messages (or `errors.log` for warnings)
...
Each cron job creates a fresh AIBrain session, which may involve provider authentication and model loading. For time-sensitive schedules, add buffer time (e.g., `0 8 * * *` instead of `0 9 * * *`).
...
2. Check `~/.jarvis/logs/brain.log` for scheduler messages and `~/.jarvis/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/jarvis-agent](https://github.com/NousResearch/jarvis-agent) with:
