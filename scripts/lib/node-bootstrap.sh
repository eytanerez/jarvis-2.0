...
#   2. ~/.jarvis/node/ from a prior Jarvis-managed install
...
#   JARVIS_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   JARVIS_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   JARVIS_HOME               (default: $HOME/.jarvis)
# ============================================================================

JARVIS_NODE_MIN_VERSION="${JARVIS_NODE_MIN_VERSION:-20}"
JARVIS_NODE_TARGET_MAJOR="${JARVIS_NODE_TARGET_MAJOR:-22}"
JARVIS_HOME="${JARVIS_HOME:-$HOME/.jarvis}"
JARVIS_NODE_AVAILABLE=false
...
# Redirect a Jarvis-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $JARVIS_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$JARVIS_HOME/node/etc/npmrc"
}
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$JARVIS_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $JARVIS_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$JARVIS_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$JARVIS_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $JARVIS_NODE_TARGET_MAJOR..."
    proto install node "$JARVIS_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $JARVIS_NODE_TARGET_MAJOR..."
    nvm install "$JARVIS_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$JARVIS_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${JARVIS_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${JARVIS_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${JARVIS_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${JARVIS_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${JARVIS_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $JARVIS_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $JARVIS_HOME/node/..."
    ...
    _link_dir="$(_nb_get_link_dir)"
    mkdir -p "$_link_dir"
    ln -sf "$JARVIS_HOME/node/bin/node" "$_link_dir/node"
    ln -sf "$JARVIS_HOME/node/bin/npm"  "$_link_dir/npm"
    ln -sf "$JARVIS_HOME/node/bin/npx"  "$_link_dir/npx"
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $JARVIS_HOME/node/"
    return 0
}
...
ensure_node() {
    ...
    if [ -x "$JARVIS_HOME/node/bin/node" ]; then
        export PATH="$JARVIS_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Jarvis-managed)"
            JARVIS_NODE_AVAILABLE=true
            return 0
        fi
    fi
    ...
    _nb_try_fnm   && { JARVIS_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { JARVIS_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { JARVIS_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { JARVIS_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { JARVIS_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { JARVIS_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $JARVIS_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
