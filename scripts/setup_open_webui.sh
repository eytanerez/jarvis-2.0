...
# Bootstrap Open WebUI against Jarvis's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.jarvis/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-jarvis.sh
...
JARVIS_ENV_FILE="${JARVIS_ENV_FILE:-$HOME/.jarvis/.env}"
JARVIS_API_PORT="${JARVIS_API_PORT:-8642}"
JARVIS_API_HOST="${JARVIS_API_HOST:-127.0.0.1}"
JARVIS_API_CONNECT_HOST="${JARVIS_API_CONNECT_HOST:-127.0.0.1}"
JARVIS_API_MODEL_NAME="${JARVIS_API_MODEL_NAME:-Jarvis}"
JARVIS_API_BASE_URL="http://${JARVIS_API_CONNECT_HOST}:${JARVIS_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-jarvis.sh"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$JARVIS_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$JARVIS_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.jarvis.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.jarvis</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.jarvis"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.jarvis"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-jarvis.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Jarvis
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-jarvis.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.jarvis/logs/openwebui.log
StandardError=append:%h/.jarvis/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-jarvis.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$JARVIS_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Jarvis API server is configured...'
  upsert_env API_SERVER_ENABLED true "$JARVIS_ENV_FILE"
  upsert_env API_SERVER_HOST "$JARVIS_API_HOST" "$JARVIS_ENV_FILE"
  upsert_env API_SERVER_PORT "$JARVIS_API_PORT" "$JARVIS_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$JARVIS_API_MODEL_NAME" "$JARVIS_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$JARVIS_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Jarvis gateway so API server settings take effect...'
  jarvis gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${JARVIS_API_CONNECT_HOST}:${JARVIS_API_PORT}/health" >/dev/null; then
    log 'Jarvis API server did not answer on the first check. Trying to start gateway in the background...'
    nohup jarvis gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${JARVIS_API_CONNECT_HOST}:${JARVIS_API_PORT}/health" >/dev/null
  ...
  log "Jarvis API endpoint: ${JARVIS_API_BASE_URL}"
