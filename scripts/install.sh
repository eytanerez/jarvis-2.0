...
# Python-driven tool session (e.g. Jarvis terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:eytanerez/jarvis-2.0.git"
REPO_URL_HTTPS="https://github.com/eytanerez/jarvis-2.0.git"
...
#   code at /usr/local/lib/jarvis-agent, command at /usr/local/bin/jarvis,
#   data still at /root/.jarvis (JARVIS_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$JARVIS_HOME/.no-bundled-skills so future"
            echo "                   'jarvis update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Jarvis.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.jarvis/jarvis-agent"
            echo "                   default (root, Linux): /usr/local/lib/jarvis-agent"
            echo "  --jarvis-home PATH  Data directory (default: ~/.jarvis, or \$JARVIS_HOME)"
            ...
            echo "  Existing installs at \$JARVIS_HOME/jarvis-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs jarvis setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `jarvis update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Jarvis-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Jarvis","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install jarvis command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `jarvis` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $JARVIS_HOME/jarvis-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $JARVIS_HOME/jarvis-agent
...
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in JARVIS_HOME.
    if is_termux; then
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$JARVIS_HOME/jarvis-brain/.git" ]; then
            INSTALL_DIR="$JARVIS_HOME/jarvis-agent"
        ...
        # /usr/local/bin/jarvis wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
...
# Point a Jarvis-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $JARVIS_HOME/node/bin — which is NOT on
...
# Jarvis's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Jarvis-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$JARVIS_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$JARVIS_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$JARVIS_HOME/node/etc/npmrc"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/eytanerez/jarvis-2.0/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Jarvis owns its own uv at $JARVIS_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (jarvis_cli/managed_uv.py) looks in the same
    # place, so install.sh and `jarvis update` stay in sync.
    local _managed_uv="$JARVIS_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $JARVIS_HOME/bin ..."
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/jarvis-uv-install.$$.log")"
    ...
    # directly into $JARVIS_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$JARVIS_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Jarvis-
...
check_node() {
    ...
    # Repair pre-existing Jarvis-managed installs where `npm install -g` lands
    ...
    if [ -x "$JARVIS_HOME/node/bin/node" ] && node_satisfies_build "$("$JARVIS_HOME/node/bin/node" --version)"; then
        export PATH="$JARVIS_HOME/node/bin:$PATH"
        log_success "Node.js $("$JARVIS_HOME/node/bin/node" --version) found (Jarvis-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Jarvis-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.jarvis/node/..."
    ...
    # Place into ~/.jarvis/node/ and symlink binaries into the same bin dir
    # the jarvis command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$JARVIS_HOME/node"
    mkdir -p "$JARVIS_HOME"
    mv "$extracted_dir" "$JARVIS_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$JARVIS_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$JARVIS_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$JARVIS_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$JARVIS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.jarvis/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Jarvis install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Jarvis itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `jarvis update` path
                # (#4735).
                ...
                local stash_name
                stash_name="jarvis-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Jarvis behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up jarvis command..."

    if [ "$USE_VENV" = true ]; then
        JARVIS_BIN="$INSTALL_DIR/venv/bin/jarvis"
    else
        JARVIS_BIN="$(which jarvis 2>/dev/null || echo "")"
        if [ -z "$JARVIS_BIN" ]; then
            log_warn "jarvis not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$JARVIS_BIN" ]; then
        log_warn "jarvis entry point not found at $JARVIS_BIN"
    ...
    # Create a user-facing shim for the jarvis command.
...
EOF
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_success "jarvis command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v jarvis' \
                >/dev/null 2>&1; then
    ...
    # Export for current session so jarvis works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.jarvis directory structure (config at top level, code in subdir)
    mkdir -p "$JARVIS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}
    ...
    if [ ! -f "$JARVIS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            ...
            log_success "Created ~/.jarvis/.env"
        fi
    else
    ...
    if [ ! -f "$JARVIS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$JARVIS_HOME/config.yaml"
            log_success "Created ~/.jarvis/config.yaml from template"
        fi
    else
        log_info "~/.jarvis/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$JARVIS_HOME/SOUL.md" ]; then
        cat > "$JARVIS_HOME/SOUL.md" << 'SOUL_EOF'
# Jarvis Persona
...
-->
SOUL_EOF
        log_success "Created ~/.jarvis/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.jarvis/"

    # Seed bundled skills into ~/.jarvis/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `jarvis update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'jarvis update'." \
            > "$JARVIS_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $JARVIS_HOME/.no-bundled-skills"
        log_info "  Future 'jarvis update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.jarvis/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.jarvis/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$JARVIS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$JARVIS_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.jarvis/skills/"
            fi
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Jarvis will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (jarvis --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `jarvis update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'jarvis setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m jarvis_cli.main setup < /dev/tty
    else
        python -m jarvis_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Jarvis to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$JARVIS_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'jarvis whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'jarvis gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $JARVIS_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $JARVIS_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: jarvis gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: jarvis gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.jarvis/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: jarvis gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: jarvis gateway"
    fi
}
...
print_success() {
    ...
    echo -e "   ${YELLOW}Data:${NC}      $JARVIS_HOME/cron/, sessions/, logs/"
    ...
    echo -e "   ${GREEN}jarvis config${NC}       View/edit configuration"
    echo -e "   ${GREEN}jarvis config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}jarvis gateway install${NC} Install gateway service (messaging + cron)"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$JARVIS_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$JARVIS_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$JARVIS_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$JARVIS_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$JARVIS_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Jarvis for pip install"
    ...
    JARVIS_CMD="$(command -v jarvis 2>/dev/null || echo "")"
    if [ -n "$JARVIS_CMD" ]; then
        log_info "Running jarvis setup..."
        "$JARVIS_CMD" setup
    else
        log_warn "jarvis command not found on PATH"
        log_info "Try: python -m jarvis_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Jarvis` rename
...
# _purge_electron_build_cache() used by `jarvis desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Jarvis-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `jarvis desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Jarvis" ]; then
            app="$desktop_dir/release/linux-unpacked/Jarvis"
        elif [ -x "$desktop_dir/release/linux-unpacked/jarvis" ]; then
            app="$desktop_dir/release/linux-unpacked/jarvis"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Jarvis.app" \
            "$desktop_dir/release/mac/Jarvis.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Jarvis is damaged and can't be opened").
    ...
    # checkout stays clean for the next `jarvis update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Jarvis-managed Node
            # provisioned during prerequisites/node-deps (at $JARVIS_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $JARVIS_HOME.
    # $JARVIS_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'jarvis update' on this host install.
