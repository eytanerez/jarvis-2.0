...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.jarvis', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (JARVIS_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.JARVIS_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.jarvis', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.JARVIS_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.jarvis', 'document_cache');
const AUDIO_CACHE_DIR = process.env.JARVIS_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.jarvis', 'audio_cache');
...
// `jarvis update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Jarvis*\n────────────\n';
...
async function startSocket() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Jarvis' own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `jarvis_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
