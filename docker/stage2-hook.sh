...
#
# Wired into the image as /etc/cont-init.d/01-jarvis-setup by the
...
JARVIS_HOME="${JARVIS_HOME:-/opt/data}"
INSTALL_DIR="/opt/jarvis"

# Drop to jarvis via s6-setuidgid, but skip it when already non-root.
as_jarvis() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid jarvis "$@"; }
...
# starts non-root. The baked install tree under /opt/jarvis is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $JARVIS_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass JARVIS_UID/JARVIS_GID — or the PUID/PGID aliases — which the
...
# `id -u jarvis` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# jarvis UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u jarvis)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-jarvis UID).
...
and the baked /opt/jarvis install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e JARVIS_UID=\$(id -u) -e JARVIS_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the jarvis user to that UID/GID at boot and chowns the data
...
# --- Bootstrap JARVIS_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid jarvis mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom JARVIS_HOME paths whose parents only
# root can create (e.g. `JARVIS_HOME=/home/jarvis/.jarvis` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for JARVIS_UID/JARVIS_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  JARVIS_UID/
# JARVIS_GID still win when both are set.  See #15290, salvages #25872.
JARVIS_UID="${JARVIS_UID:-${PUID:-}}"
JARVIS_GID="${JARVIS_GID:-${PGID:-}}"

if [ -n "${JARVIS_UID:-}" ] && validate_uid_gid "$JARVIS_UID" && [ "$JARVIS_UID" != "$(id -u jarvis)" ]; then
    echo "[stage2] Changing jarvis UID to $JARVIS_UID"
    usermod -u "$JARVIS_UID" jarvis
fi
if [ -n "${JARVIS_GID:-}" ] && validate_uid_gid "$JARVIS_GID" && [ "$JARVIS_GID" != "$(id -g jarvis)" ]; then
    echo "[stage2] Changing jarvis GID to $JARVIS_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$JARVIS_GID" jarvis 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised jarvis user (UID 10000)
...
# jarvis process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes jarvis. Idempotent across container
...
#   - jarvis is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G jarvis 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] jarvis already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" jarvis 2>/dev/null; then
        echo "[stage2] Added jarvis to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group jarvis failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When JARVIS_UID is remapped or the top-level $JARVIS_HOME isn't owned by
# the runtime jarvis UID, restore ownership to jarvis — but ONLY for the
# directories jarvis actually writes to. The full $JARVIS_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of jarvis-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_jarvis_uid=$(id -u jarvis)
needs_chown=false
if [ "$(stat -c %u "$JARVIS_HOME" 2>/dev/null)" != "$actual_jarvis_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $JARVIS_HOME (targeted) to jarvis ($actual_jarvis_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $JARVIS_HOME: chown the directory itself (not its contents)
    # so jarvis can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown jarvis:jarvis "$JARVIS_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $JARVIS_HOME failed (rootless container?) — continuing"
    # Jarvis-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by jarvis (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$JARVIS_HOME/$sub" ]; then
            chown -R jarvis:jarvis "$JARVIS_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $JARVIS_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi
...
# jarvis user. Hosted/container instances keep mutable state under
# $JARVIS_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# JARVIS_DISABLE_LAZY_INSTALLS=1. Keeping /opt/jarvis root-owned and
...
# Always reset ownership of $JARVIS_HOME/profiles to jarvis on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> jarvis …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as jarvis and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$JARVIS_HOME/profiles" ]; then
    chown -R jarvis:jarvis "$JARVIS_HOME/profiles" 2>/dev/null || true
fi

# Always reset ownership of $JARVIS_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged jarvis runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$JARVIS_HOME/cron" ]; then
    chown -R jarvis:jarvis "$JARVIS_HOME/cron" 2>/dev/null || true
fi

# Reset ownership of jarvis-owned top-level state files on every boot.
# The targeted data-volume chown above only covers jarvis-owned
# *subdirectories*; loose state files living directly under $JARVIS_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> jarvis …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged jarvis runtime then hits
...
# sweep so host-owned files in a bind-mounted $JARVIS_HOME are never
...
# entries of jarvis_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$JARVIS_HOME/$f" ]; then
        chown jarvis:jarvis "$JARVIS_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the jarvis runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$JARVIS_HOME/config.yaml" ]; then
    chown jarvis:jarvis "$JARVIS_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$JARVIS_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as jarvis user ---
# Run as jarvis via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $JARVIS_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_jarvis mkdir -p \
    "$JARVIS_HOME/cron" \
    "$JARVIS_HOME/sessions" \
    "$JARVIS_HOME/logs" \
    "$JARVIS_HOME/logs/gateways" \
    "$JARVIS_HOME/hooks" \
    "$JARVIS_HOME/memories" \
    "$JARVIS_HOME/skills" \
    "$JARVIS_HOME/skins" \
    "$JARVIS_HOME/plans" \
    ...
    "$JARVIS_HOME/home" \
    "$JARVIS_HOME/pairing" \
    "$JARVIS_HOME/platforms/pairing"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/jarvis/.install_method (see Dockerfile), NOT written here into
# $JARVIS_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $JARVIS_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.jarvis:/opt/data) and sometimes shared with a
...
# 'jarvis update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $JARVIS_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/jarvis stamp, so deleting the data-dir copy is safe).
if [ -f "$JARVIS_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$JARVIS_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$JARVIS_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$JARVIS_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_jarvis cp "$INSTALL_DIR/$src" "$JARVIS_HOME/$dest"
    fi
}
...
if [ -f "$JARVIS_HOME/.env" ]; then
    chown jarvis:jarvis "$JARVIS_HOME/.env" 2>/dev/null || true
    chmod 600 "$JARVIS_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $JARVIS_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `jarvis update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set JARVIS_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$JARVIS_HOME/config.yaml" ]; then
    s6-setuidgid jarvis "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$JARVIS_HOME/auth.json" ] && [ -n "${JARVIS_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$JARVIS_AUTH_JSON_BOOTSTRAP" > "$JARVIS_HOME/auth.json"
    chown jarvis:jarvis "$JARVIS_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$JARVIS_HOME/auth.json"
fi
...
# can set JARVIS_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# JARVIS_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$JARVIS_HOME/gateway_state.json" ] && \
        [ "${JARVIS_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$JARVIS_HOME/gateway_state.json"
    chown jarvis:jarvis "$JARVIS_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$JARVIS_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_jarvis "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/jarvis/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Jarvis spawns for the
...
# main-wrapper.sh propagates it into the supervised ``jarvis`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-jarvis, dashboard, etc.).
