...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Jarvis to work everywhere.
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `jarvis skills browse` (labeled "official") and install it with `jarvis skills install` (no third-party warning, built-in trust).
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.jarvis/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`brain/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(jarvis_home, config)` for setup-wizard integration
...
The installer creates the Jarvis venv, wires the `jarvis` command, stamps the
install method for `jarvis update`, and clones the full git project into
`$JARVIS_HOME/jarvis-agent` (usually `~/.jarvis/jarvis-agent`). That keeps your
...
Use this only if you intentionally do not want Jarvis' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `jarvis` entrypoint from this venv; running the
system `python3 -m jarvis_cli.main` can pick up unrelated system Python
packages.
...
echo "OPENROUTER_API_KEY=***" >> ~/.jarvis/.env
```

### Run

```bash
# The standard installer already put `jarvis` on PATH.
jarvis doctor
jarvis chat -q "Hello"
```

If you used the manual clone fallback, run `./jarvis` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/jarvis" ~/.local/bin/jarvis
```
...
```
jarvis-brain/
├── run_brain.py              # AIBrain class — core conversation loop, tool dispatch, session persistence
...
│
├── brain/                    # Agent internals (extracted modules)
...
│
├── jarvis_cli/               # CLI command implementations
...
### User configuration (stored in `~/.jarvis/`)
...
| `~/.jarvis/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.jarvis/cron/` | Scheduled job data |
| `~/.jarvis/whatsapp/session/` | WhatsApp bridge credentials |
...
```
User message → AIBrain._run_brain_loop()
...
Four fields are supported under `metadata.jarvis`:
...
The filtering happens at prompt build time in `brain/prompt_builder.py`. The `build_skills_system_prompt()` function receives the set of available tools and toolsets from the agent and uses `_skill_should_show()` to evaluate each skill's conditions.
...
The user may skip setup and keep loading the skill. Jarvis only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `jarvis setup` or update `~/.jarvis/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Jarvis tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Jarvis" is the secondary collaborator. If the contributor's commit shows "Jarvis" as author because they used Jarvis to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Jarvis tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `jarvis --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Jarvis uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.jarvis/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `jarvis_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `jarvis_cli/skin_engine.py` for the full schema and existing skins as examples.
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the jarvis wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `jarvis_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `jarvis_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `jarvis_cli/gateway_windows.py::
...
Jarvis has terminal access. Security matters.
...
2. **Test manually**: Run `jarvis` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/jarvis-brain/issues)
