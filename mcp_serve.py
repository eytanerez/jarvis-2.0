"""
Jarvis MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Jarvis-specific extra)

Usage:
    jarvis mcp serve
    jarvis mcp serve --verbose
...
logger = logging.getLogger("jarvis.mcp_serve")
...
class EventBridge:
    ...
    This is the Jarvis equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Jarvis MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "jarvis",
        instructions=(
            "Jarvis messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Jarvis MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
