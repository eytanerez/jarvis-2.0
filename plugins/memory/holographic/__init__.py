"""jarvis-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $JARVIS_HOME/config.yaml (profile-scoped):
  plugins:
    jarvis-memory-store:
      db_path: $JARVIS_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    ...
    try:
        ...
        return cfg_get(all_config, "plugins", "jarvis-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, jarvis_home):
        ...
        try:
            ...
            existing["plugins"]["jarvis-memory-store"] = values
