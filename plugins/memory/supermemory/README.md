...
Config file: `$JARVIS_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `jarvis` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `jarvis-{identity}` → `jarvis-coder`). |
...
All Supermemory API calls send `x-sm-source: jarvis`, and document writes stamp
`metadata.sm_source: jarvis`. This is a **functional routing key, not telemetry**:
it groups Jarvis-written memories into a dedicated "Jarvis" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Jarvis profile:

```json
{
  "container_tag": "jarvis-{identity}"
}
```

For a profile named `coder`, this resolves to `jarvis-coder`. The default profile resolves to `jarvis-default`. Without `{identity}`, all profiles share the same container.
