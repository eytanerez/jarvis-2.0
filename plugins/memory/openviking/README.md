...
connection values into Jarvis, or create a minimal `ovcli.conf` when one does
not exist.
...
| `OPENVIKING_AGENT` | `jarvis` | Jarvis peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Jarvis lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Jarvis sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
