...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Jarvis peer ID in OpenViking (default: jarvis)
...
from brain.message_content import flatten_message_text
from brain.memory_provider import MemoryProvider
from brain.skill_commands import extract_user_instruction_from_skill_message
...
_DEFAULT_AGENT = "jarvis"
_AGENT_PROMPT_LABEL = "Jarvis peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "JARVIS_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Jarvis slash-skill scaffolding before sending content to OpenViking.
...
def _load_jarvis_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("JARVIS_HOME", "")).expanduser() if os.environ.get("JARVIS_HOME") else Path.home() / ".jarvis"
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory disabled for this Jarvis run."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Jarvis session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Jarvis only", "write values only to Jarvis .env"),
    ...
    _save_jarvis_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Jarvis .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See jarvis-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_jarvis_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Jarvis peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, jarvis_home: str, config: dict) -> None:
        ...
        from jarvis_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        jarvis_home_path = Path(jarvis_home)
        env_path = jarvis_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_jarvis_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Jarvis run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Jarvis' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Jarvis canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See jarvis-agent#28296.
    ...
    def _tool_read(self, args: dict) -> str:
        ...
        # (e.g. viking://user/jarvis/.overview.md).
