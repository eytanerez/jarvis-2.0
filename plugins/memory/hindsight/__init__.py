...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: jarvis)
...
Or via $JARVIS_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Jarvis can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $JARVIS_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_jarvis_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "jarvis": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "jarvis"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Jarvis config."""
    profile = config.get("profile", "jarvis")
    return str(profile or "jarvis")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Jarvis profile name (from agent_identity)
      {workspace} — Jarvis workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``jarvis-{user}`` with no user becomes ``jarvis``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Jarvis and the User"
    ...
    def save_config(self, values, jarvis_home):
        """Write config to $JARVIS_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(jarvis_home) / "hindsight"
    ...
    def post_setup(self, jarvis_home: str, config: dict) -> None:
        ...
        from jarvis_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "jarvis")
        ...
        self.save_config(provider_config, jarvis_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(jarvis_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(jarvis_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "jarvis"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: jarvis-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Jarvis and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "jarvis"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "jarvis"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "jarvis", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "jarvis")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Jarvis and the User")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See jarvis-agent#6672.
    ...
    def shutdown(self) -> None:
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Jarvis created/used that client on the shared async loop,
        ...
        # loader creates a new provider per AIBrain, and the gateway
        # creates one AIBrain per concurrent chat session. Stopping the
