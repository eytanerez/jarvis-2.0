...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/jarvis>
...
```bash
jarvis memory setup honcho   # configure Honcho directly (works on a fresh install)
jarvis memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
jarvis config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.jarvis/.env
```

> `jarvis honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `jarvis memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$JARVIS_HOME/honcho.json` | Profile-local (isolated Jarvis instances) |
| 2 | `~/.jarvis/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Jarvis profile: `jarvis` (default) or `jarvis_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `jarvis honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Jarvis session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`jarvis-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`jarvis`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-jarvis-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening jarvis in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Jarvis session ID (timestamp + hex). Every `jarvis` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Jarvis profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`jarvis`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Jarvis profile: `jarvis` (default) or `jarvis_<profile>` (e.g. `jarvis -p coder` -> host key `jarvis_coder`). Older `jarvis.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `JARVIS_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `jarvis memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `jarvis honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `jarvis memory setup`) |
| `jarvis honcho status` | Show resolved config for active profile |
| `jarvis honcho enable` / `disable` | Toggle Honcho for active profile |
| `jarvis honcho mode <mode>` | Change recall or observation mode |
| `jarvis honcho peer --user <name>` | Update user peer name |
| `jarvis honcho peer --ai <name>` | Update AI peer name |
| `jarvis honcho tokens --context <N>` | Set context token budget |
| `jarvis honcho tokens --dialectic <N>` | Set dialectic max chars |
| `jarvis honcho map <name>` | Map current directory to a session name |
| `jarvis honcho sync` | Create host blocks for all Jarvis profiles |
