...
Resolution order for config file:
  1. $JARVIS_HOME/honcho.json  (instance-local, enables isolated Jarvis instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Jarvis profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Jarvis profile.

    Resolution order:
      1. JARVIS_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``jarvis.<profile>``
      3. Fallback: ``"jarvis"`` (default profile)
    """
    explicit = os.environ.get("JARVIS_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $JARVIS_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.jarvis/honcho.json          (default profile — shared host blocks live here)
    ...
    default_path = _get_default_jarvis_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Jarvis system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.jarvis
    ...
    def from_global_config(
        ...
        Resolution: $JARVIS_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Jarvis profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.jarvis block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Jarvis session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Jarvis session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Jarvis session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'jarvis honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        # the "go run jarvis honcho setup" hint they used to.
        try:
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `jarvis honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                jarvis_cfg = load_config()
                honcho_cfg = jarvis_cfg.get("honcho", {})
