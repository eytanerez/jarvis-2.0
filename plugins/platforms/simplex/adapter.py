"""SimpleX Chat platform adapter (Jarvis plugin).
...
This adapter ships as a Jarvis platform plugin under
``plugins/platforms/simplex/``. The Jarvis plugin loader scans the
...
Optional environment variables:
    ...
    JARVIS_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``jarvis setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Jarvis — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("JARVIS_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``jarvis setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.jarvis/.env`` via
    ``jarvis_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "jarvis_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.jarvis/.env"
        )
        return
