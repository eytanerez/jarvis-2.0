...
# ``model_tools`` import time, so ``jarvis status``, ``jarvis chat``, etc.
...
# ``jarvis_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Jarvis is thinking…" card during the small window between
...
def _redact_sensitive(text: str) -> str:
    ...
    email addresses. brain/redact.py handles log-level redaction elsewhere;
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a jarvis MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${JARVIS_HOME}/google_chat_thread_counts.json so
        ...
        self._thread_count_store = _ThreadCountStore(
            _jarvis_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Jarvis is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("JARVIS_HOME", str(_Path.home() / ".jarvis"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${JARVIS_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a jarvis MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Jarvis is thinking…' marker message.
        ...
        "Jarvis is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Jarvis is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Jarvis is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Jarvis is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Jarvis is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``jarvis setup``.

    The setup wizard at ``jarvis_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.jarvis/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. jarvis-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.jarvis/.env")
    print_info("Restart the gateway: jarvis gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``jarvis cron`` running as a
    separate process from ``jarvis gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Jarvis plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'jarvis-agent[google_chat]'",
        ...
        # the "Jarvis is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Jarvis is thinking…' marker message "
