"""ntfy platform adapter (Jarvis plugin).
...
``plugins/platforms/ntfy/``. The Jarvis plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "jarvis-in"              # subscribe topic (incoming)
          publish_topic: "jarvis-out"     # optional — defaults to topic
...
_ECHO_TAG = "jarvis-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``jarvis cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Jarvis dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `jarvis gateway status` without
