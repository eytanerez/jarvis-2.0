"""
IRC Platform Adapter for Jarvis.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Jarvis agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "jarvis-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Jarvis")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: jarvis_, jarvis_1, jarvis_2...
...
def check_requirements() -> bool:
    ...
    # jarvis setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `jarvis gateway setup` flow for the IRC platform.

    Lazy-imports ``jarvis_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Jarvis to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #jarvis — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.jarvis/.env")
    print_info("Restart the gateway for changes to take effect: jarvis gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "jarvis-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "jarvis-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Jarvis (cron)")
