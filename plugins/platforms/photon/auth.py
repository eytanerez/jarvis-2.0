...
Credential storage mirrors every other Jarvis channel:

    * runtime SDK creds  -> ``~/.jarvis/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.jarvis/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a jarvis dependency
...
# registers Jarvis as its own client_id.
...
# Default name of the project Jarvis provisions for the operator.
DEFAULT_PROJECT_NAME = "Jarvis"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of jarvis-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.jarvis/auth.json`` honouring the active Jarvis profile."""
    try:
        from jarvis_constants import get_jarvis_home
        return Path(get_jarvis_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.jarvis")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.jarvis/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.jarvis/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.jarvis/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: jarvis_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `jarvis photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `jarvis photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `jarvis photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `jarvis photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `jarvis photon setup`)"
