...
Small Node helper that bridges Jarvis to Photon's Spectrum SDK
(`spectrum-ts`).  Jarvis is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Jarvis-Sidecar-Token`)
...
The Jarvis plugin's `jarvis photon setup` command runs `npm install`
here automatically.
