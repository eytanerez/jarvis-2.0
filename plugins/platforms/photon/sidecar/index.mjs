// Jarvis — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Jarvis): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Jarvis -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Jarvis-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `jarvis photon telemetry`)
...
// instead of a cryptic module-resolution error during import. Apply Jarvis'
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-jarvis-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Jarvis supplied them so a known-good
