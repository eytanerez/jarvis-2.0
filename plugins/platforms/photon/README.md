...
This plugin connects Jarvis to iMessage (and other Spectrum
...
TypeScript-only, Jarvis runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Jarvis-Sidecar-Token`.
...
There is no separate `login` command; like every other Jarvis channel,
...
Run `jarvis photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.jarvis/.env` (the same place every other
...
Management metadata lives in `~/.jarvis/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `jarvis photon telemetry on\|off` (restart the gateway to apply) |
...
5. Verify end-to-end: `jarvis photon status`, a DM and a group roundtrip,
