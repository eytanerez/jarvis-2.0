...
Auto-tracks and cleans up ephemeral files created during Jarvis
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$JARVIS_HOME` and `/tmp/jarvis-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `JARVIS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `JARVIS_HOME` or `/tmp/jarvis-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$JARVIS_HOME/disk-cleanup/` is itself excluded
- `$JARVIS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
