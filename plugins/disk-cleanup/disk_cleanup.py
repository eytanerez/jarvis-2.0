"""disk_cleanup — ephemeral file cleanup for Jarvis.
...
Rules:
  ...
  - empty dirs    → always delete (under JARVIS_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly JARVIS_HOME and /tmp/jarvis-*
Never touches: ~/.jarvis/logs/ or any system directory.
"""
...
try:
    ...
    def get_jarvis_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("JARVIS_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".jarvis").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$JARVIS_HOME/logs/``."""
    return get_jarvis_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$JARVIS_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under JARVIS_HOME or ``/tmp/jarvis-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    jarvis_home = get_jarvis_home()
    try:
        path.resolve().relative_to(jarvis_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/jarvis-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("jarvis-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "jarvis-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $JARVIS_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so JARVIS_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        jarvis_home = get_jarvis_home()
        for parent in ("cron", "cronjobs"):
            base = jarvis_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside JARVIS_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under JARVIS_HOME, but never recurse into known
    # durable state trees.  Some installs place the Jarvis checkout, venv,
    # and desktop build under JARVIS_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    jarvis_home = get_jarvis_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in jarvis_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    jarvis_home = get_jarvis_home()
    try:
        rel = path.resolve().relative_to(jarvis_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "jarvis-agent",
        }:
            return None
        ...
        # Path isn't under JARVIS_HOME (e.g. /tmp/jarvis-*) — fall through.
        pass
