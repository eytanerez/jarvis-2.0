# Jarvis Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: jarvis-achievements plugin
...
### `GET /api/plugins/jarvis-achievements/achievements`
...
### `POST /api/plugins/jarvis-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/jarvis-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/jarvis-achievements/overview`
...
- Persist plugin data under:
  - `~/.jarvis/plugins/jarvis-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.jarvis/plugins/jarvis-achievements/scan_checkpoint.json`
...
- `~/.jarvis/plugins/jarvis-achievements/`
