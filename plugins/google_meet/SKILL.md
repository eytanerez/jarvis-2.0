...
jarvis meet install                 # pip deps + Chromium (transcribe only)
jarvis meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
jarvis meet auth                    # optional; skips guest-lobby wait
jarvis meet setup                   # preflight checks
```

`jarvis meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or JARVIS_MEET_REALTIME_KEY in ~/.jarvis/.env
```
...
pip install playwright websockets && python -m playwright install chromium
jarvis plugins enable google_meet
jarvis meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
jarvis meet node approve my-mac ws://<mac-ip>:18789 <token>
jarvis meet node ping my-mac                   # confirm reachable
```

Run `jarvis meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Jarvis agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `jarvis meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `JARVIS_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `jarvis_meet_src`). |
...
Local:
```
$JARVIS_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$JARVIS_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `jarvis meet node approve`.
