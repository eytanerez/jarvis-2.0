...
Let the jarvis agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where jarvis runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `jarvis meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `jarvis meet` CLI |
...
| `cli.py` | `jarvis meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$JARVIS_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `jarvis meet node {run,list,approve,remove,status,ping}` |
...
jarvis meet install                                      # pip + Chromium
jarvis meet setup                                        # preflight
jarvis meet auth                                         # optional
jarvis meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
jarvis meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.jarvis/.env
jarvis meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
jarvis meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
jarvis meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.jarvis/.env
jarvis meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, jarvis will **not** switch your system audio input automatically — the
...
jarvis plugins enable google_meet
jarvis meet node run --display-name my-mac --host 0.0.0.0 --port 18789
