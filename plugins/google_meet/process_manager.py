...
session-scoped state file under ``$JARVIS_HOME/workspace/meetings/.active.json``
...
def start(
    ...
    *,
    ...
    If a bot is already running for this jarvis install, leave it first —
    ...
    env = os.environ.copy()
    env["JARVIS_MEET_URL"] = url
    env["JARVIS_MEET_OUT_DIR"] = str(out)
    env["JARVIS_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["JARVIS_MEET_HEADED"] = "1"
    if auth_state:
        env["JARVIS_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["JARVIS_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if JARVIS_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["JARVIS_MEET_MODE"] = mode
    if realtime_model:
        env["JARVIS_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["JARVIS_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["JARVIS_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["JARVIS_MEET_REALTIME_KEY"] = realtime_api_key
