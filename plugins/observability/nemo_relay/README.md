...
Optional Jarvis observability plugin that maps Jarvis observer hooks to
...
not replace Jarvis's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Jarvis emit NeMo Relay lifecycle events for
the work Jarvis already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Jarvis can:

- Preserve Jarvis execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
jarvis plugins enable observability/nemo_relay
```

The `JARVIS_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `JARVIS_HOME` that the
agent run will use:

```bash
env JARVIS_HOME=/tmp/jarvis-nemo-relay-test \
  jarvis plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`JARVIS_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`JARVIS_HOME` is the Jarvis profile/config home used by both
`jarvis plugins enable ...` and the later `jarvis chat ...` run. If unset,
Jarvis uses the user's default home, usually `~/.jarvis`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export JARVIS_HOME=/tmp/jarvis-nemo-relay-test
jarvis plugins enable observability/nemo_relay
jarvis chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `jarvis` command you run is built from the
...
uv run jarvis plugins enable observability/nemo_relay
uv run jarvis chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/jarvis_agent-*.whl
...
The plugin can configure exporters directly from `JARVIS_NEMO_RELAY_*`
...
```bash
export JARVIS_NEMO_RELAY_ATOF_ENABLED=1
export JARVIS_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export JARVIS_NEMO_RELAY_ATIF_ENABLED=1
export JARVIS_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `JARVIS_NEMO_RELAY_ATOF_FILENAME`
- `JARVIS_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `JARVIS_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `JARVIS_NEMO_RELAY_ATIF_AGENT_NAME`
- `JARVIS_NEMO_RELAY_ATIF_AGENT_VERSION`
- `JARVIS_NEMO_RELAY_ATIF_MODEL_NAME`
- `JARVIS_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Jarvis at it:

```bash
export JARVIS_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `JARVIS_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`JARVIS_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`JARVIS_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Jarvis does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Jarvis keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Jarvis routes LLM and tool
...
For the full generic Jarvis middleware contract, see
...
export JARVIS_HOME=/tmp/jarvis-nemo-relay-docs/jarvis-home
mkdir -p "$JARVIS_HOME"

cat > "$JARVIS_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Jarvis session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export JARVIS_NEMO_RELAY_ATOF_ENABLED=1
export JARVIS_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/jarvis-nemo-relay-docs/subagent/atof
export JARVIS_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export JARVIS_NEMO_RELAY_ATOF_MODE=overwrite
export JARVIS_NEMO_RELAY_ATIF_ENABLED=1
export JARVIS_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/jarvis-nemo-relay-docs/subagent/atif
export JARVIS_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export JARVIS_NEMO_RELAY_ATIF_AGENT_NAME='Jarvis E2E'
export JARVIS_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export JARVIS_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"jarvis.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Jarvis E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Jarvis dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/jarvis-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/jarvis-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/jarvis-nemo-relay-docs/workdir/beta.txt
cd /tmp/jarvis-nemo-relay-docs/workdir

export JARVIS_NEMO_RELAY_ATOF_ENABLED=1
export JARVIS_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/jarvis-nemo-relay-docs/parallel/atof
export JARVIS_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export JARVIS_NEMO_RELAY_ATOF_MODE=overwrite
export JARVIS_NEMO_RELAY_ATIF_ENABLED=1
export JARVIS_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/jarvis-nemo-relay-docs/parallel/atif
export JARVIS_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export JARVIS_NEMO_RELAY_ATIF_AGENT_NAME='Jarvis E2E'
export JARVIS_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Jarvis sessions map to `agent` scopes.
- Jarvis API request hooks map to `llm` scope start/end events.
- Jarvis tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Jarvis execution middleware to hand
...
Enable it for Jarvis:

```bash
export JARVIS_NEMO_RELAY_PLUGINS_TOML=/tmp/jarvis-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Jarvis routes execution
through these boundaries:

```text
Jarvis provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Jarvis provider adapter next_call(...)

Jarvis tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Jarvis tool dispatcher next_call(...)
```
...
middleware for a local Jarvis run. This path requires a NeMo Relay runtime that
...
```bash
export JARVIS_HOME=/tmp/jarvis-middleware-test/jarvis-home
mkdir -p "$JARVIS_HOME" /tmp/jarvis-middleware-test/nemo-relay
...
cat > /tmp/jarvis-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/jarvis-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/jarvis-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Jarvis Middleware E2E"
agent_version = "local"
...
export JARVIS_NEMO_RELAY_PLUGINS_TOML=/tmp/jarvis-middleware-test/nemo-relay/plugins.toml
