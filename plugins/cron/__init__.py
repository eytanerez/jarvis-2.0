...
1. Bundled providers: ``plugins/cron/<name>/`` (shipped with jarvis-agent)
...
_USER_NAMESPACE = "_jarvis_user_cron"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_jarvis_user_cron.<name>``, a dotted
    ...
    ``ModuleNotFoundError: No module named '_jarvis_user_cron'`` — the same
...
def load_cron_scheduler(name: str) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    (``$JARVIS_HOME/plugins/<name>/``) directories. Bundled takes precedence
    on name collisions.
