...
``{title, url, description, position}`` rows every other Jarvis web
provider produces.
...
prefers Jarvis-managed xAI Grok OAuth (via ``jarvis auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.jarvis/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``jarvis tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `jarvis auth` to sign in with "
