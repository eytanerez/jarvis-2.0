# nix/desktop.nix — Jarvis Desktop (Electron) app build + wrapper
#
# `jarvisAgent` is the fully-built `.#default` package — it ships the
# `jarvis` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `JARVIS_DESKTOP_JARVIS` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Jarvis CLI").
...
let
  npm = jarvisNpmLib.mkNpmPassthru {
    folder = "apps/desktop";
    attr = "desktop";
    pname = "jarvis-desktop";
  };
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/jarvis-desktop $out/bin
    cp -r ${renderer}/* $out/share/jarvis-desktop/
    ...
    substituteInPlace $out/share/jarvis-desktop/electron/main.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/jarvis-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # JARVIS_DESKTOP_JARVIS to the absolute path of the nix-built `jarvis`
    # binary so the desktop's resolver step 4 ("existing Jarvis CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/jarvis-desktop \
      --add-flags "$out/share/jarvis-desktop" \
      --set JARVIS_DESKTOP_JARVIS "${lib.getExe jarvisAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Jarvis";
    homepage = "https://github.com/NousResearch/jarvis-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "jarvis-desktop";
  };
}
