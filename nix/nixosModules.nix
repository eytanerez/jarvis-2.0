# nix/nixosModules.nix — NixOS module for jarvis-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: jarvis runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $JARVIS_HOME/.env
# and read by jarvis at startup — no container recreation needed for env changes.
#
# Tool resolution: the jarvis wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.jarvis-agent;
    effectivePackage =
    ...
    jarvis-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-jarvis-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "jarvis-config-attrs";
      description = "Jarvis YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "jarvis-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # JARVIS_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "jarvis-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'JARVIS_DOC_EOF'\n${value}\nJARVIS_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "jarvis-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/jarvis";
    ...
    # jarvis user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "jarvis-container-entrypoint" ''
      set -eu

      JARVIS_UID="''${JARVIS_UID:?JARVIS_UID must be set}"
      JARVIS_GID="''${JARVIS_GID:?JARVIS_GID must be set}"

      # ── Group: ensure a group with GID=$JARVIS_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$JARVIS_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="jarvis"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$JARVIS_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$JARVIS_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$JARVIS_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$JARVIS_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="jarvis"
        TARGET_HOME="/home/jarvis"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$JARVIS_UID" -g "$JARVIS_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$JARVIS_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$JARVIS_UID:$JARVIS_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure JARVIS_HOME is owned by the target user.
      ...
      if [ -n "''${JARVIS_HOME:-}" ] && [ -d "$JARVIS_HOME" ]; then
        find "$JARVIS_HOME" \! -user "$JARVIS_UID" -exec chown "$JARVIS_UID:$JARVIS_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/jarvis-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/jarvis-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/jarvis ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/jarvis
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$JARVIS_UID" --regid="$JARVIS_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $JARVIS_HOME/.env.
    ...
    # Default: /var/lib/jarvis/workspace → /data/workspace.
  ...
  in {
    options.services.jarvis-agent = with lib; {
      enable = mkEnableOption "Jarvis gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = jarvis-agent;
        description = "The jarvis-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/jarvis";
        description = "State directory. Contains .jarvis/ subdir (JARVIS_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Jarvis config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $JARVIS_HOME/.env at activation time.
          Jarvis reads this file on every startup via load_jarvis_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $JARVIS_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $JARVIS_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `jarvis gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the jarvis user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the jarvis plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Jarvis discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          jarvis_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as jarvis (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in jarvis-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the jarvis CLI to environment.systemPackages and export
          JARVIS_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.jarvis symlink to the service
            stateDir. These users are automatically added to the jarvis group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.jarvis-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the jarvis CLI to system PATH and export JARVIS_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.jarvis/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.JARVIS_HOME = "${cfg.stateDir}/.jarvis";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.jarvis-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the jarvis user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.jarvis-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed jarvis
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure jarvis is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.jarvis        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jarvis/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jarvis/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jarvis/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jarvis/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jarvis/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."jarvis-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.jarvis
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.jarvis ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.jarvis ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.jarvis -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.jarvis/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.jarvis/$_subdir"
            chmod 2770 "${cfg.stateDir}/.jarvis/$_subdir"
            find "${cfg.stateDir}/.jarvis/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # jarvis-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.jarvis/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.jarvis/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jarvis/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.jarvis/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.jarvis/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jarvis/.managed
          chmod 0644 ${cfg.stateDir}/.jarvis/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.jarvis/.container-mode <<'JARVIS_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    jarvis_bin=${containerDataDir}/current-package/bin/jarvis
    JARVIS_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jarvis/.container-mode
            chmod 0644 ${cfg.stateDir}/.jarvis/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.jarvis/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.jarvis";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.jarvis" ]; then
                  rm -f "${symlinkPath}"
                  echo "jarvis-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.jarvis -> stateDir/.jarvis for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.jarvis";
                target = "${cfg.stateDir}/.jarvis";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "jarvis-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.jarvis/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.jarvis/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.jarvis/auth.json
              fi
            ''}
          ''}
          ...
          # Jarvis reads $JARVIS_HOME/.env at startup via load_jarvis_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.jarvis/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'JARVIS_NIX_ENV_EOF'
    ${envFileContent}
    JARVIS_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.jarvis/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.jarvis/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jarvis/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.jarvis-agent = {
          ...
          environment = {
            HOME = cfg.stateDir;
            JARVIS_HOME = "${cfg.stateDir}/.jarvis";
            JARVIS_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $JARVIS_HOME/.env by the activation script. load_jarvis_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/jarvis"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the jarvis group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.jarvis-agent = {
          description = "Jarvis Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              JARVIS_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              JARVIS_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env JARVIS_UID="$JARVIS_UID" \
                --env JARVIS_GID="$JARVIS_GID" \
                --env JARVIS_HOME=${containerDataDir}/.jarvis \
                --env JARVIS_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/jarvis gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
