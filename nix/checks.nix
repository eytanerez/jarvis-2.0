...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      jarvis-agent = self'.packages.default;
      jarvisVenv = jarvis-agent.jarvisVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "jarvis-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${jarvisVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "jarvis-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "jarvis-build-package" { } ''
          echo "PASS: package built at ${jarvis-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "jarvis-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "jarvis-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${jarvis-agent}/bin/jarvis || (echo "FAIL: jarvis binary missing"; exit 1)
          test -x ${jarvis-agent}/bin/jarvis-agent || (echo "FAIL: jarvis-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${jarvis-agent}/bin/jarvis version 2>&1 | grep -qi "jarvis" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "jarvis-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in jarvis jarvis-agent jarvis-acp; do
            test -x ${jarvis-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "jarvis-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking jarvis --help ==="
          ${jarvis-agent}/bin/jarvis --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${jarvis-agent}/bin/jarvis --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "jarvis-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${jarvis-agent}/share/jarvis-brain/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${jarvis-agent}/share/jarvis-brain/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "JARVIS_BUNDLED_SKILLS" ${jarvis-agent}/bin/jarvis || \
            (echo "FAIL: JARVIS_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: JARVIS_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "jarvis-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${jarvis-agent}/share/jarvis-brain/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${jarvis-agent}/share/jarvis-brain/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "JARVIS_BUNDLED_PLUGINS" ${jarvis-agent}/bin/jarvis || \
            (echo "FAIL: JARVIS_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: JARVIS_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "jarvis-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${jarvis-agent}/share/jarvis-brain/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${jarvis-agent}/share/jarvis-brain/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${jarvis-agent}/share/jarvis-brain/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "JARVIS_BUNDLED_LOCALES" ${jarvis-agent}/bin/jarvis || \
            (echo "FAIL: JARVIS_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: JARVIS_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (JARVIS_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && JARVIS_BUNDLED_LOCALES=${jarvis-agent}/share/jarvis-brain/locales \
            ${jarvisVenv}/bin/python3 -c "from brain import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with JARVIS_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${jarvisVenv}/bin/python3 -c "from brain import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${jarvisVenv}/bin/python3 -c "from brain import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without JARVIS_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "jarvis-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${jarvis-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${jarvis-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "JARVIS_TUI_DIR" ${jarvis-agent}/bin/jarvis || \
            (echo "FAIL: JARVIS_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: JARVIS_TUI_DIR set in wrapper"
        ...
        # Verify JARVIS_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        jarvis-node = pkgs.runCommand "jarvis-node-version" { } ''
          set -e
          echo "=== Checking JARVIS_NODE in wrapper ==="
          grep -q "JARVIS_NODE" ${jarvis-agent}/bin/jarvis || \
            (echo "FAIL: JARVIS_NODE not set in wrapper"; exit 1)
          echo "PASS: JARVIS_NODE present in wrapper"

          JARVIS_NODE=$(sed -n "s/^export JARVIS_NODE='\(.*\)'/\1/p" ${jarvis-agent}/bin/jarvis)
          test -x "$JARVIS_NODE" || (echo "FAIL: JARVIS_NODE=$JARVIS_NODE not executable"; exit 1)
          echo "PASS: JARVIS_NODE executable at $JARVIS_NODE"

          NODE_MAJOR=$("$JARVIS_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All JARVIS_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify JARVIS_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "jarvis-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(JARVIS_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking JARVIS_MANAGED guards ==="
          check_blocked "config set" ${jarvis-agent}/bin/jarvis config set model foo
          check_blocked "config edit" ${jarvis-agent}/bin/jarvis config edit
        ...
        extra-python-packages = let
          ...
          jarvisWithExtra = jarvis-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "jarvis-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${jarvisWithExtra}/bin/jarvis || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${jarvisWithExtra}/bin/jarvis || \
          ...
          if grep -q "PYTHONPATH" ${jarvis-agent}/bin/jarvis; then
        ...
        extra-dependency-groups = let
          jarvisWithGroups = jarvis-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "jarvis-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${jarvisWithGroups}"
          echo "venv: ${jarvisWithGroups.jarvisVenv}"
        ...
        messaging-variant = pkgs.runCommand "jarvis-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.jarvisVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "jarvis-config-roundtrip" {
          ...
          merge_and_load() {
            ...
            ${configMergeScript} ${nixSettings} "$jarvis_home/config.yaml"
            ${jarvisVenv}/bin/python3 -c '
import json, sys
