...
Usage:
    ...
    agent = AIBrain(base_url="http://localhost:30000/v1", model="claude-opus-4-20250514")
...
#   (b) `patch("run_brain.OpenAI", ...)` test patterns used by ~28 test files.
#
# NOTE: `fire` is ONLY used in the `__main__` block below (for running
# run_brain.py directly as a CLI) — it is NOT needed for library usage.
# It is imported there, not here, so that importing run_brain from a
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``jarvis -c`` / ``--resume`` (relaunch
...
# OpenAI lazy proxy + safe stdio + proxy URL helpers — see brain/process_bootstrap.py.
# `OpenAI` is re-exported here so `patch("run_brain.OpenAI", ...)` in tests works.
# The other `# noqa: F401` re-exports below cover names accessed via
# `mock.patch("run_brain.<X>")`, `from run_brain import <X>` in production
# siblings, or the `_ra().<X>` indirection in brain/system_prompt.py — none
# of which ruff's in-module usage scan can see.
from brain.process_bootstrap import (
    OpenAI,  # noqa: F401  # re-exported for tests that mock.patch("run_brain.OpenAI")
    _SafeWriter,  # noqa: F401  # re-exported for tests that `from run_brain import _SafeWriter`
...
from jarvis_cli.timeouts import (
...
from model_tools import (
    get_tool_definitions,  # noqa: F401  # re-exported for tests that mock.patch("run_brain.get_tool_definitions")
    get_toolset_for_tool,
    handle_function_call,  # noqa: F401  # re-exported for tests that mock.patch("run_brain.handle_function_call")
    check_toolset_requirements,  # noqa: F401  # re-exported for tests that mock.patch("run_brain.check_toolset_requirements")
)
...
# Agent internals extracted to brain/ package for modularity
from brain.memory_manager import sanitize_context
from brain.error_classifier import FailoverReason
from brain.redact import redact_sensitive_text
from brain.model_metadata import (
    estimate_request_tokens_rough,  # noqa: F401  # re-exported for tests that mock.patch("run_brain.estimate_request_tokens_rough")
    is_local_endpoint,
)
from brain.usage_pricing import normalize_usage
# Re-exported for tests that monkeypatch these symbols on run_brain.
from brain.context_compressor import ContextCompressor  # noqa: F401
from brain.retry_utils import jittered_backoff  # noqa: F401
from brain.prompt_builder import (  # noqa: F401  # re-exported via _ra() / mock.patch("run_brain.<name>") / from run_brain import <name>
...
from brain.process_bootstrap import _get_proxy_from_env  # noqa: F401
from brain.message_sanitization import (  # noqa: F401
    _SURROGATE_RE,
...
from brain.codex_responses_adapter import (
...
from brain.tool_guardrails import (
    ToolGuardrailDecision,
    append_toolguard_guidance,
    toolguard_synthetic_result,
)
from brain.tool_result_classification import (
    FILE_MUTATING_TOOL_NAMES as _FILE_MUTATING_TOOLS,
    file_mutation_result_landed,
)
from brain.trajectory import (
    convert_scratchpad_to_think,
    save_trajectory as _save_trajectory_to_file,
)
from brain.tool_dispatch_helpers import (
    _should_parallelize_tool_batch,
    _is_destructive_command,  # noqa: F401  # re-exported for tests that access `run_brain._is_destructive_command`
    _extract_parallel_scope_path,  # noqa: F401  # re-exported for tests that `from run_brain import _extract_parallel_scope_path`
    _paths_overlap,  # noqa: F401  # re-exported for tests that `from run_brain import _paths_overlap`
    _is_multimodal_tool_result,
    _multimodal_text_summary,
    _append_subdir_hint_to_multimodal,  # noqa: F401  # re-exported for tests that `from run_brain import _append_subdir_hint_to_multimodal`
    _extract_file_mutation_targets,
    _extract_error_preview,
    _trajectory_normalize_msg,  # noqa: F401  # re-exported for tests that `from run_brain import _trajectory_normalize_msg`
)
...
# process, not once per AIBrain instantiation.  Without this, long-running
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"JarvisBrain/{_JARVIS_VERSION}",
    }
...
class AIBrain:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[jarvis-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        from brain.agent_init import init_agent
        init_agent(
            self,
            base_url=base_url,
            api_key=api_key,
    ...
    def _get_session_db_for_recall(self):
        ...
        Most frontends pass ``session_db`` into ``AIBrain`` explicitly, but recall
    ...
    def _ensure_db_session(self) -> None:
        ...
        if self._session_db_created or not self._session_db:
            return
        source = self.platform or os.environ.get("JARVIS_SESSION_SOURCE", "cli")
        try:
        ...
        **extra_context,
        ...
        if should_start and target_session_id and hasattr(engine, "on_session_start"):
            start_context = {
                ...
                "platform": getattr(self, "platform", None) or os.environ.get("JARVIS_SESSION_SOURCE", "cli"),
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Jarvis' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from brain.model_metadata import MINIMUM_CONTEXT_LENGTH
            from jarvis_cli.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        from brain.agent_runtime_helpers import switch_model
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``jarvis chat -q``. In that mode,
    ...
    # (brain.log) are unaffected — every individual emission site still
    ...
    from brain.stream_diag import STREAM_DIAG_HEADERS as _STREAM_DIAG_HEADERS  # noqa: E402

    @staticmethod
    def _stream_diag_init() -> Dict[str, Any]:
        """Forwarder — see ``agent.stream_diag.stream_diag_init``."""
        from brain.stream_diag import stream_diag_init
    ...
    def _stream_diag_capture_response(
        ...
        from brain.stream_diag import stream_diag_capture_response
    ...
    def _flatten_exception_chain(error: BaseException) -> str:
        ...
        from brain.stream_diag import flatten_exception_chain
    ...
    def _log_stream_retry(
        ...
        *,
        ...
        from brain.stream_diag import log_stream_retry
        log_stream_retry(
    ...
    def _emit_stream_drop(
        ...
        *,
        ...
        from brain.stream_diag import emit_stream_drop
        emit_stream_drop(
    ...
    def _check_compression_model_feasibility(self) -> None:
        ...
        from brain.conversation_compression import check_compression_model_feasibility
    ...
    def _replay_compression_warning(self) -> None:
        ...
        from brain.conversation_compression import replay_compression_warning
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``JARVIS_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``JARVIS_API_TIMEOUT`` fallback would always be
        passed as a per-call ``timeout=`` kwarg, overriding the client-level
        timeout the AIBrain.__init__ path configured.
        """
        cfg = get_provider_request_timeout(self.provider, self.model)
        if cfg is not None:
            return cfg
        return float(os.getenv("JARVIS_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``JARVIS_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("JARVIS_API_CALL_STALE_TIMEOUT")
    ...
    def _compute_non_stream_stale_timeout(self, api_payload: Any) -> float:
        ...
        from brain.chat_completion_helpers import estimate_request_context_tokens
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        jarvis-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See jarvis-agent#21444 for symptom history."
        )
    ...
    def _anthropic_prompt_cache_policy(
        ...
        *,
        ...
        from brain.agent_runtime_helpers import anthropic_prompt_cache_policy
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from jarvis_cli.models import _should_use_copilot_responses_api
                return _should_use_copilot_responses_api(model)
            except Exception:
                # Fall back to the generic GPT-5 rule if Copilot-specific
                # logic is unavailable for any reason.
                pass
        return AIBrain._model_requires_responses_api(model)
    ...
    def _strip_think_blocks(self, content: str) -> str:
        ...
        from brain.agent_runtime_helpers import strip_think_blocks
    ...
    def _looks_like_codex_intermediate_ack(
        ...
        from brain.agent_runtime_helpers import looks_like_codex_intermediate_ack
    ...
    def _extract_reasoning(self, assistant_message) -> Optional[str]:
        ...
        from brain.agent_runtime_helpers import extract_reasoning
    ...
    def _summarize_background_review_actions(
        ...
        from brain.background_review import summarize_background_review_actions
    ...
    def _spawn_background_review(
        ...
        here so existing tests that patch ``run_brain.threading.Thread``
        keep working.
        """
        from brain.background_review import spawn_background_review_thread
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        from brain.background_review import build_memory_write_metadata
    ...
    def _repair_message_sequence(self, messages: List[Dict]) -> int:
        ...
        from brain.agent_runtime_helpers import repair_message_sequence
    ...
    def _format_tools_for_system_message(self) -> str:
        ...
        from brain.system_prompt import format_tools_for_system_message
    ...
    def _convert_to_trajectory_format(self, messages: List[Dict[str, Any]], user_query: str, completed: bool) -> List[Dict[str, Any]]:
        ...
        from brain.agent_runtime_helpers import convert_to_trajectory_format
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Jarvis against xai-oauth.  Lots of
    ...
    def _coerce_api_error_detail(value: Any) -> str:
        ...
        if isinstance(value, dict):
            ...
            for key in ("message", "detail", "error", "code", "type"):
                if key in value:
                    nested_detail = AIBrain._coerce_api_error_detail(value[key])
                    if nested_detail:
        ...
        if isinstance(value, (list, tuple)):
            parts = [
                AIBrain._coerce_api_error_detail(item)
                for item in value
            ]
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        if isinstance(body, dict):
            ...
            if msg:
                ...
                msg = AIBrain._coerce_api_error_detail(msg)
                return AIBrain._decorate_xai_entitlement_error(f"{prefix}{msg[:300]}")

        # Fallback: truncate the raw string but give more room than 200 chars
        status_code = getattr(error, "status_code", None)
        prefix = f"HTTP {status_code}: " if status_code else ""
        return AIBrain._decorate_xai_entitlement_error(f"{prefix}{raw[:500]}")
    ...
    def _extract_api_error_context(error: Exception) -> Dict[str, Any]:
        ...
        from brain.agent_runtime_helpers import extract_api_error_context
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("JARVIS_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``monkeypatch.setattr("jarvis_cli.plugins.has_hook", ...)`` still
        ...
        try:
            from jarvis_cli import plugins as _plugins
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        from brain.agent_runtime_helpers import dump_api_request_debug
    ...
    def _redact_message_content(content):
        ...
        Respects ``JARVIS_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.jarvis/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects JARVIS_REDACT_SECRETS via
    ...
    def interrupt(self, message: str = None) -> None:
        ...
        # `getattr` fallback covers test stubs that build AIBrain via
    ...
    def steer(self, text: str) -> bool:
        ...
        if _lock is None:
            # Test stubs that built AIBrain via object.__new__ skip __init__.
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.jarvis/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``JARVIS_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("JARVIS_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _apply_pending_steer_to_tool_results(self, messages: list, num_tool_msgs: int) -> None:
        ...
        from brain.agent_runtime_helpers import apply_pending_steer_to_tool_results
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (JARVIS_KANBAN_TASK set),
    ...
    def _capture_rate_limits(self, http_response: Any) -> None:
        ...
        if not headers:
            return
        try:
            from brain.rate_limit_tracker import parse_rate_limit_headers
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (JARVIS_DEV_CREDITS_FIXTURE): inject a chosen notice state
        # each turn for repeatable testing, bypassing real headers. Throwaway scaffolding.
        try:
            from brain.credits_tracker import dev_fixture_credits_state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset JARVIS_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("JARVIS_DEV_CREDITS"))

        # ── Parse (fail-open → miss; never overwrite good state with None) ──
        try:
            from brain.credits_tracker import parse_credits_headers
        ...
        if _dev:
            # JARVIS_DEV_CREDITS: stream each capture to brain.log — watch live with
            # `jarvis logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _emit_credits_notices(self) -> None:
        ...
        if state is None:
            return
        try:
            from brain.credits_tracker import evaluate_credits_notices, is_free_tier_model
    ...
    def release_clients(self) -> None:
        ...
        resume at any time with a freshly-built AIBrain that reuses the
    ...
    def _hydrate_todo_store(self, history: List[Dict[str, Any]]) -> None:
        ...
        The gateway creates a fresh AIBrain per message, so the in-memory
    ...
    def _sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        from brain.agent_runtime_helpers import sanitize_api_messages
    ...
    def _drop_thinking_only_and_merge_users(
        ...
        *,
        ...
        from brain.agent_runtime_helpers import drop_thinking_only_and_merge_users
    ...
    def _repair_tool_call(self, tool_name: str) -> str | None:
        ...
        from brain.agent_runtime_helpers import repair_tool_call
        return repair_tool_call(self, tool_name)

    def _invalidate_system_prompt(self):
        """Forwarder — see ``agent.system_prompt.invalidate_system_prompt``."""
        from brain.system_prompt import invalidate_system_prompt
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        ...
        from brain.agent_runtime_helpers import create_openai_client
    ...
    def _force_close_tcp_sockets(client: Any) -> int:
        ...
        from brain.agent_runtime_helpers import force_close_tcp_sockets
    ...
    def _cleanup_dead_connections(self) -> bool:
        ...
        from brain.agent_runtime_helpers import cleanup_dead_connections
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from jarvis_cli.copilot_auth import copilot_request_headers
    ...
    def _run_codex_stream(self, api_kwargs: dict, client: Any = None, on_first_delta: callable = None):
        ...
        from brain.codex_runtime import run_codex_stream
    ...
    def _run_codex_create_stream_fallback(self, api_kwargs: dict, client: Any = None):
        ...
        from brain.codex_runtime import run_codex_create_stream_fallback
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``jarvis auth add xai-oauth``) whose tokens belong to
        ...
        try:
            if self.provider == "openai-codex":
                from jarvis_cli.auth import resolve_codex_runtime_credentials

                singleton_now = resolve_codex_runtime_credentials(
                    refresh_if_expiring=False,
                )
            else:
                from jarvis_cli.auth import resolve_xai_oauth_runtime_credentials
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from jarvis_cli.copilot_auth import resolve_copilot_token
    ...
    def _try_refresh_anthropic_client_credentials(self) -> bool:
        ...
        try:
            from brain.anthropic_adapter import resolve_anthropic_token, build_anthropic_client
    ...
    def _apply_client_headers_for_base_url(self, base_url: str) -> None:
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            from jarvis_cli.models import copilot_default_headers
    ...
    def _swap_credential(self, entry) -> None:
        ...
        if self.api_mode == "anthropic_messages":
            from brain.anthropic_adapter import build_anthropic_client, _is_oauth_token
    ...
    def _recover_with_credential_pool(
        ...
        *,
        ...
        from brain.agent_runtime_helpers import recover_with_credential_pool
    ...
    def _anthropic_messages_create(self, api_kwargs: dict):
        ...
        from brain.anthropic_adapter import create_anthropic_message
    ...
    def _rebuild_anthropic_client(self) -> None:
        ...
        if getattr(self, "provider", None) == "bedrock":
            from brain.anthropic_adapter import build_anthropic_bedrock_client
            region = getattr(self, "_bedrock_region", "us-east-1") or "us-east-1"
            self._anthropic_client = build_anthropic_bedrock_client(region)
        else:
            from brain.anthropic_adapter import build_anthropic_client
            self._anthropic_client = build_anthropic_client(
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        from brain.chat_completion_helpers import interruptible_api_call
    ...
    def _interruptible_streaming_api_call(
        ...
        from brain.chat_completion_helpers import interruptible_streaming_api_call
        return interruptible_streaming_api_call(self, api_kwargs, on_first_delta=on_first_delta)

    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        """Forwarder — see ``agent.chat_completion_helpers.try_activate_fallback``."""
        from brain.chat_completion_helpers import try_activate_fallback
    ...
    def _restore_primary_runtime(self) -> bool:
        ...
        from brain.agent_runtime_helpers import restore_primary_runtime
    ...
    def _try_recover_primary_transport(
        ...
        from brain.agent_runtime_helpers import try_recover_primary_transport
    ...
    def _model_supports_vision(self) -> bool:
        ...
        try:
            ...
            from brain.image_routing import _lookup_supports_vision
            cfg = load_config()
    ...
    def _get_transport(self, api_mode: str = None):
        ...
        t = cache.get(mode)
        if t is None:
            from brain.transports import get_transport
    ...
    def _prepare_anthropic_messages_for_api(self, api_messages: list) -> list:
        ...
        # The Anthropic adapter (brain/anthropic_adapter.py:_convert_content_part_to_anthropic)
    ...
    def _try_shrink_image_parts_in_messages(
        ...
        *,
        ...
        from brain.conversation_compression import try_shrink_image_parts_in_messages
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        from brain.chat_completion_helpers import build_api_kwargs
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from jarvis_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from jarvis_cli.models import lmstudio_model_reasoning_options
    ...
    def _resolve_lmstudio_summary_reasoning_effort(self) -> Optional[str]:
        ...
        """
        from brain.lmstudio_reasoning import resolve_lmstudio_effort
    ...
    def _build_assistant_message(self, assistant_message, finish_reason: str) -> dict:
        ...
        from brain.chat_completion_helpers import build_assistant_message
    ...
    def _needs_thinking_reasoning_pad(self) -> bool:
        ...
        Result cached on the AIBrain instance keyed by (provider, model,
    ...
    def _copy_reasoning_content_for_api(self, source_msg: dict, api_msg: dict) -> None:
        ...
        from brain.agent_runtime_helpers import copy_reasoning_content_for_api
    ...
    def _reapply_reasoning_echo_for_provider(self, api_messages: list) -> int:
        ...
        from brain.agent_runtime_helpers import reapply_reasoning_echo_for_provider
    ...
    def _sanitize_tool_calls_for_strict_api(api_msg: dict, model: "str | None" = None) -> dict:
        ...
        if not isinstance(tool_calls, list):
            return api_msg
        from brain.transports.chat_completions import _model_consumes_thought_signature
    ...
    def _sanitize_tool_call_arguments(
        ...
        *,
        ...
        from brain.agent_runtime_helpers import sanitize_tool_call_arguments
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None, force: bool = False) -> tuple:
        ...
        ``force=False``.
        """
        from brain.conversation_compression import compress_context
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        from brain.agent_runtime_helpers import invoke_tool
        return invoke_tool(
            self,
            function_name,
            function_args,
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        from brain.tool_executor import execute_tool_calls_concurrent
    ...
    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        from brain.tool_executor import execute_tool_calls_sequential
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        from brain.chat_completion_helpers import handle_max_iterations
    ...
    def run_conversation(
        ...
        from brain.conversation_loop import run_conversation
    ...
    def _run_codex_app_server_turn(
        ...
        *,
        ...
        from brain.codex_runtime import run_codex_app_server_turn
...
def main(
    ...
    if list_tools:
        ...
        print("  python run_brain.py --enabled_toolsets=research --query='search for Python news'")
        print("  python run_brain.py --enabled_toolsets=development --query='debug this code'")
        print("  python run_brain.py --enabled_toolsets=safe --query='analyze without terminal'")
        print("  ")
        print("  # Combine multiple toolsets")
        print("  python run_brain.py --enabled_toolsets=web,vision --query='analyze website'")
        print("  ")
        print("  # Disable toolsets")
        print("  python run_brain.py --disabled_toolsets=terminal --query='no command execution'")
        print("  ")
        print("  # Run with trajectory saving enabled")
        print("  python run_brain.py --save_trajectories --query='your question here'")
        return
