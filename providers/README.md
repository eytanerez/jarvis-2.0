...
Registry and ABC for every inference provider Jarvis knows about.
...
`$JARVIS_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `jarvis_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `jarvis_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `jarvis_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `jarvis_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `jarvis_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
  when URL detection finds nothing.
- `brain/model_metadata.py` maps hostname → provider via
  `profile.get_hostname()`.
- `brain/auxiliary_client.py` reads `profile.default_aux_model` first
  before falling back to the legacy hardcoded dict.
- `brain/transports/chat_completions.py::_build_kwargs_from_profile()`
  invokes `profile.prepare_messages()`, `profile.build_extra_body()`,
  and `profile.build_api_kwargs_extras()` on every call.
- `run_brain.py` passes `provider_profile=<ProviderProfile>` so the
...
under `$JARVIS_HOME/plugins/model-providers/` for a private plugin).
