...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with jarvis-agent)
2. User plugins: ``$JARVIS_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$JARVIS_HOME/plugins/model-providers/`` can override
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple JARVIS_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_jarvis_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$JARVIS_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with jarvis-agent.
    ...
    # 2. User plugins — under $JARVIS_HOME/plugins/model-providers/<name>/.
