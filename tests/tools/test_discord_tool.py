...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in jarvis-discord
# ---------------------------------------------------------------------------
...
class TestToolsetInclusion:
    ...
    def test_discord_tools_not_in_other_toolsets(self):
        ...
        for name, ts in TOOLSETS.items():
            if name in {"jarvis-discord", "jarvis-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def _reset_tools_logger(self):
        ...
        ``AIBrain(quiet_mode=True)`` globally sets ``tools`` and
        ``tools.*`` children to ``ERROR`` (see run_brain.py quiet_mode
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("jarvis_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["jarvis-discord"], quiet_mode=True)
