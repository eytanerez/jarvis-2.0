...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `jarvis-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.jarvis/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.jarvis/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because jarvis uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("JARVIS_INTERACTIVE", raising=False)
        monkeypatch.delenv("JARVIS_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("JARVIS_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("JARVIS_INTERACTIVE", false_like_value)
        monkeypatch.delenv("JARVIS_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("JARVIS_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["JARVIS_INTERACTIVE", "JARVIS_GATEWAY_SESSION", "JARVIS_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("JARVIS_INTERACTIVE", "JARVIS_GATEWAY_SESSION", "JARVIS_EXEC_ASK"):
...
class TestResolveModelOverride:
    ...
    def test_keeps_bare_custom_when_a_named_entry_exists(self, monkeypatch):
        import jarvis_cli.runtime_provider as rp_mod
