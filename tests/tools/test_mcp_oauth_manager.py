...
def test_jarvis_provider_subclass_exists():
    """JarvisMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _JARVIS_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _JARVIS_PROVIDER_CLS is not None
    assert issubclass(_JARVIS_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_jarvis_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns JarvisMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _JARVIS_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _JARVIS_PROVIDER_CLS is not None
    assert isinstance(provider, _JARVIS_PROVIDER_CLS)
    assert provider._jarvis_server_name == "srv"
