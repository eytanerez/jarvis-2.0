...
TTS provider tools must resolve API keys from ``~/.jarvis/.env`` (via
...
already addressed for ``brain/credential_pool`` and ``jarvis_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.jarvis/.env`` carries the
    ...
    ``jarvis_cli.config.load_env`` integration. Before the fix, ``os.getenv``
...
class TestRegressionGuard:
    ...
    ``jarvis_cli.config.load_env`` to simulate ``~/.jarvis/.env`` carrying the
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self, tmp_path, monkeypatch):
        """Importing TTS while jarvis_cli.config.get_env_value is patched must
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        # Simulate ~/.jarvis/.env carrying the key (load_env returns the dict
        ...
        with patch(
            ...
            # os.environ is empty.
            from jarvis_cli.config import get_env_value as live_get
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.jarvis/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
