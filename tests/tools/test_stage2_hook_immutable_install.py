...
Hosted/container Jarvis keeps user-writable state under JARVIS_HOME
(/opt/data). The installed source, venv, TUI bundle, and node_modules under
/opt/jarvis must remain root-owned/non-writable by the runtime jarvis user so
...
def test_stage2_does_not_chown_install_tree_to_jarvis(stage2_text: str) -> None:
    ...
    for install_tree in (
        ...
        assert install_tree not in stage2_text, (
            f"stage2 must not chown {install_tree} back to jarvis; "
            "the Dockerfile keeps /opt/jarvis immutable and writable state "
            "belongs under JARVIS_HOME"
        )
...
def test_stage2_documents_immutable_install_contract(stage2_text: str) -> None:
    ...
    assert "JARVIS_DISABLE_LAZY_INSTALLS=1" in stage2_text
    assert "/opt/jarvis" in stage2_text
