...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad JARVIS_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: JARVIS_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("JARVIS_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("JARVIS_CRON_SESSION", "1")
    monkeypatch.delenv("JARVIS_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_jarvis_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "JARVIS_HOME": "/h", "JARVIS_PROFILE": "p",
        "JARVIS_CONFIG": "/c.yaml", "JARVIS_ENV": "/e",
        # other JARVIS_* → dropped (broad prefix removed)
        "JARVIS_BASE_URL": "https://x", "JARVIS_INTERACTIVE": "1",
        "JARVIS_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("JARVIS_HOME", "JARVIS_PROFILE", "JARVIS_CONFIG", "JARVIS_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "JARVIS_BASE_URL", "JARVIS_INTERACTIVE", "JARVIS_KANBAN_DB",
...
def test_env_scrub_logs_dropped_jarvis_vars(caplog):
    """Dropping a non-allowlisted, non-secret JARVIS_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset JARVIS_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "JARVIS_HOME": "/h",          # allowlisted → kept, not logged
        "JARVIS_BASE_URL": "https://x",   # dropped → logged
        "JARVIS_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "JARVIS_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "JARVIS_HOME" in out and "PATH" in out
    assert "JARVIS_BASE_URL" not in out and "JARVIS_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "JARVIS_BASE_URL" in msgs and "JARVIS_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "JARVIS_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped JARVIS_* vars."""
    import logging
