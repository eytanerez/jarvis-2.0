...
The classifier is tested in tests/brain/test_file_safety_cross_profile.py.
...
def fake_jarvis(tmp_path, monkeypatch):
    """Build a two-profile Jarvis layout and point JARVIS_HOME at
    the jarvis-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setenv("JARVIS_HOME", str(sec_home))
    ...
    monkeypatch.setattr(fs, "_jarvis_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_jarvis):
        from tools.file_tools import write_file_tool
        target = fake_jarvis["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_jarvis):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_jarvis["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "jarvis-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_jarvis):
    ...
    def test_non_jarvis_path_unaffected(self, fake_jarvis, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_jarvis):
    ...
    def test_cross_profile_patch_bypass(self, fake_jarvis):
        from tools.file_tools import patch_tool
        target = fake_jarvis["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_jarvis):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_jarvis, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_jarvis["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up JARVIS_HOME (set in
        ...
        assert "not found in active profile 'jarvis-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_jarvis, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_jarvis["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_jarvis["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.jarvis/profiles/<name>/."""
        # Don't set JARVIS_HOME — falls back to default.
        import brain.file_safety as fs
        monkeypatch.setattr(fs, "_jarvis_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_jarvis_root_path", lambda: tmp_path / "fake")
        ...
        # See brain/system_prompt.py for the exact wording.

    def test_named_profile_line_in_prompt_text(self, fake_jarvis):
        """When active profile is 'jarvis-security', the prompt warns
        ...
        from pathlib import Path
        src = Path("brain/system_prompt.py").read_text()
        assert "Active Jarvis profile" in src
        assert "cross_profile=True" in src
        assert "~/.jarvis/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Jarvis profile: default" in src
        assert "Active Jarvis profile: {active_profile}" in src
