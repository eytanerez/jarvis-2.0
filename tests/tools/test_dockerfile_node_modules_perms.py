...
Older images made /opt/jarvis/ui-tui and /opt/jarvis/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV JARVIS_TUI_DIR=/opt/jarvis/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R jarvis:jarvis /opt/jarvis/ui-tui" not in text
    assert "chown -R jarvis:jarvis /opt/jarvis/node_modules" not in text
