"""Regression test for #17929: AIBrain.__init__ should try fallback_model
...
def test_init_tries_fallback_when_primary_returns_none():
    ...
    with patch("brain.auxiliary_client.resolve_provider_client", side_effect=fake_resolve), \
         patch("run_brain.get_tool_definitions", return_value=_make_tool_defs()), \
         patch("run_brain.check_toolset_requirements", return_value={}), \
         patch("run_brain.OpenAI", return_value=MagicMock()):
...
def test_init_raises_when_no_fallback_configured():
    ...
    with patch("brain.auxiliary_client.resolve_provider_client", return_value=(None, None)), \
         patch("run_brain.get_tool_definitions", return_value=_make_tool_defs()), \
         patch("run_brain.check_toolset_requirements", return_value={}), \
         patch("run_brain.OpenAI", return_value=MagicMock()):
