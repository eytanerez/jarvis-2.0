"""Integration test for the codex_app_server runtime path through AIBrain.

Verifies that:
  - api_mode='codex_app_server' is accepted on AIBrain construction
...
import run_brain
from brain.transports.codex_app_server_session import CodexAppServerSession, TurnResult


@pytest.fixture
def fake_session(monkeypatch):
    """Replace CodexAppServerSession with a stub that returns a fixed
    TurnResult, so we can drive AIBrain without spawning real codex."""
...
class TestRunConversationCodexPath:
    ...
    def test_background_review_skill_trigger_fires_above_threshold(
        ...
        from brain.transports.codex_app_server_session import (
...
class TestReviewForkApiModeDowngrade:
    ...
    def test_codex_app_server_parent_downgrades_review_fork(self):
        ...
        # reaches the AIBrain constructor.
        ...
        # Capture what AIBrain gets constructed with inside the helper.
        captured = {}
        ...
        with _patch("run_brain.AIBrain.__init__", _capture_init):
...
class TestSessionRetirementOnRunAgent:
    """run_brain.py side: when run_turn returns should_retire=True, the
    AIBrain must close + null _codex_session so the next turn respawns."""
