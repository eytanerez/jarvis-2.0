...
suite (we patch ``run_brain.OpenAI`` and drive ``agent.client``), so they
...
# --------------------------------------------------------------------------
# Fixtures (mirrors tests/run_brain/test_tool_call_guardrail_runtime.py)
# --------------------------------------------------------------------------
...
def _make_agent(max_iterations: int = 10, config: dict | None = None) -> AIBrain:
    with (
...
def test_explanation_quiet_for_normal_text_response():
    ...
    out = AIBrain._format_turn_completion_explanation(
...
def test_explanation_quiet_for_empty_reason():
    assert AIBrain._format_turn_completion_explanation("") == ""
    assert AIBrain._format_turn_completion_explanation("unknown") == ""
    # guardrail_halt surfaces its own message; explainer stays out of the way.
    assert AIBrain._format_turn_completion_explanation("guardrail_halt") == ""


def test_explanation_for_empty_response_exhausted():
    out = AIBrain._format_turn_completion_explanation("empty_response_exhausted")
...
def test_explanation_for_partial_stream_recovery():
    out = AIBrain._format_turn_completion_explanation("partial_stream_recovery")
...
def test_explainer_enabled_by_default():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("JARVIS_TURN_COMPLETION_EXPLAINER", None)
        with patch("jarvis_cli.config.load_config", return_value={}):
...
def test_explainer_disabled_via_env():
    agent = _make_agent()
    with patch.dict(
        os.environ, {"JARVIS_TURN_COMPLETION_EXPLAINER": "0"}, clear=False
    ):
