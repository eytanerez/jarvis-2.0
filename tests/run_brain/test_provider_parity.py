"""Provider parity tests: verify that AIBrain builds correct API kwargs
...
import pytest
from brain.codex_responses_adapter import _chat_content_to_responses_parts, _chat_messages_to_responses_input, _normalize_codex_response, _preflight_codex_input_items
...
def _make_agent(monkeypatch, provider, api_mode="chat_completions", base_url="https://openrouter.ai/api/v1", model=None):
    monkeypatch.setattr("run_brain.get_tool_definitions", lambda **kw: _tool_defs("web_search", "terminal"))
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_openrouter_always_wins(self, monkeypatch):
        ...
        from brain.auxiliary_client import get_text_auxiliary_client
        with patch("brain.auxiliary_client.OpenAI") as mock:
    ...
    def test_nous_when_no_openrouter(self, monkeypatch):
        ...
        with patch("brain.auxiliary_client._read_nous_auth", return_value=nous_auth), \
             patch("brain.auxiliary_client.OpenAI") as mock, \
    ...
    def test_codex_not_in_auto_fallback(self, monkeypatch):
        ...
        from brain.auxiliary_client import get_text_auxiliary_client
        with patch("brain.auxiliary_client._read_nous_auth", return_value=None), \
             patch("brain.auxiliary_client._read_codex_access_token", return_value="codex-tok"), \
