"""Tests for payload/context-length → compression retry logic in AIBrain.
...
from brain.context_compressor import SUMMARY_PREFIX
...
class TestPreflightCompression:
    ...
    def test_compress_context_emits_lifecycle_status_before_work(self, agent):
        ...
        with (
            ...
            patch("run_brain.estimate_request_tokens_rough", return_value=42),
        ):
    ...
    def test_preflight_defers_when_recent_real_usage_fit(self, agent):
        ...
        with (
            patch("brain.turn_context.estimate_request_tokens_rough", return_value=114_000),
            patch("brain.conversation_loop.estimate_request_tokens_rough", return_value=114_000),
            patch.object(agent, "_compress_context") as mock_compress,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
    ...
    def test_preflight_compresses_when_rough_growth_after_fit_is_large(self, agent):
        ...
        with (
            patch("brain.turn_context.estimate_request_tokens_rough", side_effect=_rough_estimate),
            patch("brain.conversation_loop.estimate_request_tokens_rough", side_effect=_rough_estimate),
            patch.object(agent, "_compress_context") as mock_compress,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
    ...
    def test_preflight_seeds_display_tokens_when_compression_aborts(self, agent):
        ...
        with (
            patch("brain.turn_context.estimate_request_tokens_rough", return_value=144_669),
            patch("brain.conversation_loop.estimate_request_tokens_rough", return_value=144_669),
...
class TestToolResultPreflightCompression:
    ...
    def test_large_tool_results_trigger_compression(self, agent):
        ...
        with (
            patch("run_brain.handle_function_call", return_value=large_result),
            patch.object(agent, "_compress_context") as mock_compress,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
