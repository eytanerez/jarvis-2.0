...
AlexKucera reported on Discord (2026-04-16) that after ``jarvis update`` pulled
...
Opt-in — not part of default CI:
    JARVIS_LIVE_TESTS=1 pytest tests/run_brain/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.jarvis/.env).
"""
...
# Load ~/.jarvis/.env so live runs pick up OPENROUTER_API_KEY without
...
pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set JARVIS_LIVE_TESTS=1"),
...
def _looks_like_error_reply(reply: str) -> tuple[bool, str]:
    """AIBrain returns an error-sentinel string (not an exception) when the
