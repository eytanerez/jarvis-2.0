...
2. ``AIBrain._record_file_mutation_result`` — builds the per-turn state
   dict, removing entries when a later success supersedes an earlier
   failure for the same path.
3. ``AIBrain._format_file_mutation_failure_footer`` — renders the dict
...
def _bare_agent() -> AIBrain:
    """Skip __init__ and only attach the per-turn state dict.

    AIBrain.__init__ takes ~60 parameters and touches network, auth, and
...
class TestRecordFileMutationResult:
    ...
    def test_no_state_dict_silent_noop(self):
        ...
        """
        agent = object.__new__(AIBrain)  # no state attached
        # Should not raise
        agent._record_file_mutation_result(
...
class TestFormatFooter:
    def test_empty_returns_empty_string(self):
        assert AIBrain._format_file_mutation_failure_footer({}) == ""

    def test_single_failure(self):
        out = AIBrain._format_file_mutation_failure_footer(
    ...
    def test_truncation_at_10_entries(self):
        ...
        out = AIBrain._format_file_mutation_failure_footer(failed)
    ...
    def test_paths_are_backtick_wrapped(self):
        ...
        out = AIBrain._format_file_mutation_failure_footer(
            {"/home/u/.jarvis/config.yaml": {
                "tool": "patch",
                "error_preview": (
                    "Write denied: '/home/u/.jarvis/config.yaml' is a "
                    "protected system/credential file."
                ),
            }},
        )
        # Path still human-readable.
        assert "/home/u/.jarvis/config.yaml" in out
    ...
    def test_footer_path_not_extracted_by_gateway(self):
        ...
        tmp = tempfile.mkdtemp(prefix="jarvis_footer_")
        try:
            cfg = os.path.join(tmp, "config.yaml")
            with open(cfg, "w") as fh:
                fh.write("openrouter_api_key: sk-LEAK\n")
            footer = AIBrain._format_file_mutation_failure_footer(
                {cfg: {
                    "tool": "patch",
                    "error_preview": (
...
class TestVerifierEnabled:
    def test_default_is_enabled(self, monkeypatch):
        monkeypatch.delenv("JARVIS_FILE_MUTATION_VERIFIER", raising=False)
        agent = _bare_agent()
    ...
    def test_env_disables(self, monkeypatch, value):
        monkeypatch.setenv("JARVIS_FILE_MUTATION_VERIFIER", value)
        agent = _bare_agent()
        assert agent._file_mutation_verifier_enabled() is False

    def test_env_enables_over_config(self, monkeypatch):
        monkeypatch.setenv("JARVIS_FILE_MUTATION_VERIFIER", "1")
        import jarvis_cli.config as _cfg_mod
        monkeypatch.setattr(
