...
non-retryable classifier at run_brain.py treats `ValueError` / `TypeError`
...
This test mirrors the exact predicate shape used in run_brain.py so that
...
def _mirror_agent_predicate(err: BaseException) -> bool:
    """Exact shape of run_brain.py's is_local_validation_error check.
    ...
    return (
        ...
        # the brain/conversation_loop.py inline comment for #33136.
        and not (
...
class TestAgentLoopSourceStillHasCarveOut:
    ...
    def test_run_brain_excludes_jsondecodeerror_from_local_validation(self):
        import inspect
        from brain import conversation_loop
        # The agent loop body lives in brain/conversation_loop.py after
        # the run_brain.py refactor.  Assert the carve-out is present in
        ...
        assert "JSONDecodeError" in src, (
            "brain/conversation_loop.py must carve out json.JSONDecodeError "
...
class TestAgentLoopSourceHasNoneTypeCarveOut:
    ...
    def test_conversation_loop_excludes_nonetype_not_iterable_from_local_validation(self):
        ...
        assert "nonetype" in src.lower() and "not iterable" in src.lower(), (
            "brain/conversation_loop.py must carve out 'NoneType is not iterable' "
