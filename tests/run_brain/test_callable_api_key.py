...
Jarvis preserves the callable end-to-end so the SDK refreshes tokens
...
Covered:
  ...
  * ``run_brain.py`` masked-banner path renders the Entra placeholder
...
class TestCreateOpenAIClientCallable:
    """``AIBrain._create_openai_client`` must pass the callable through
    to ``openai.OpenAI(...)`` without coercion."""

    def test_callable_api_key_passed_to_openai_constructor(self, monkeypatch):
        """Construct the smallest possible AIBrain surface and verify
        ...
        monkeypatch.setattr("run_brain.OpenAI", fake_openai)

        # Build a minimal stand-in for AIBrain so we can call the bound
...
class TestNormalizeMainRuntimePreservesCallable:
    ...
    def test_callable_api_key_survives_normalization(self):
        from brain.auxiliary_client import _normalize_main_runtime
    ...
    def test_unknown_field_dropped(self):
        from brain.auxiliary_client import _normalize_main_runtime, _MAIN_RUNTIME_FIELDS
        normalized = _normalize_main_runtime({
            "provider": "azure-foundry",
            "api_key": "k",
...
class TestTruncateTokenCallable:
    def test_callable_returns_placeholder(self):
        ...
        ``"<function ...>"``."""
        from jarvis_cli.web_server import _truncate_token
...
class TestCliEnsureRuntimeCredentialsCallable:
    ...
    ``JarvisCLI`` instance) — the predicate change is the load-bearing
    ...
    def test_callable_predicate_present_in_cli_runtime_validation(self):
        ...
        src = (Path(__file__).resolve().parent.parent.parent
               / "jarvis_cli" / "cli_agent_setup_mixin.py").read_text()
...
class TestInlinedDisplayMasks:
    ...
    def test_run_brain_banner_uses_is_token_provider_guard(self):
        """The masked-banner sites live in ``brain/agent_init.py``
        ...
        assert src.count('"🔑 Using credentials: Microsoft Entra ID"') >= 2, (
            "brain/agent_init.py banner blocks should print a static "
    ...
    def test_cli_show_config_handles_callable(self):
        """``cli.JarvisCLI.show_config`` previously did
        ...
        assert '"Microsoft Entra ID"' in src, (
            ...
            "'Microsoft Entra ID' label (matching run_brain banners) "
            "instead of attempting to slice the callable."
        )

    def test_mask_api_key_for_logs_handles_callable(self):
        """``run_brain._mask_api_key_for_logs`` is called from the
    ...
    def test_anthropic_401_diagnostic_handles_callable(self):
        ...
        ``brain/conversation_loop.py`` (the ``run_conversation`` body
        ...
        assert "Microsoft Entra ID (httpx event hook)" in src, (
            "brain/conversation_loop.py Anthropic 401 diagnostic must "
