...
1. ``AIBrain.__init__``                              (line ~1022)
2. ``AIBrain.switch_model``                          (line ~1832)
3. ``AIBrain._try_refresh_anthropic_client_credentials`` (line ~5335)
4. ``AIBrain._swap_credential``                      (line ~5378)
5. ``AIBrain._try_activate_fallback``                (line ~6536)
"""
...
@pytest.fixture
def agent():
    """Minimal AIBrain construction, skipping tool discovery."""
    with (
...
class TestOAuthFlagOnCredentialSwap:
    ...
    def test_pool_swap_on_third_party_never_flips_oauth(self, agent):
        ...
        with patch("brain.anthropic_adapter.build_anthropic_client",
                   return_value=MagicMock()):
...
class TestOAuthFlagOnConstruction:
    """Site 1 — AIBrain.__init__ on a third-party anthropic_messages provider."""
