...
def _make_agent(
    *,
    ...
    """Build a minimal AIBrain with a compressor, skipping __init__."""
...
@patch("brain.model_metadata.get_model_context_length", return_value=80_000)
...
@patch("brain.model_metadata.get_model_context_length", return_value=32_768)
@patch("brain.auxiliary_client.get_text_auxiliary_client")
...
@patch("brain.model_metadata.get_model_context_length", return_value=200_000)
@patch("brain.auxiliary_client.get_text_auxiliary_client")
...
def test_feasibility_check_passes_live_main_runtime():
    ...
    with patch("brain.auxiliary_client.get_text_auxiliary_client", return_value=(mock_client, "gpt-5.4")) as mock_get_client, \
         patch("brain.model_metadata.get_model_context_length", return_value=200_000):
        agent._emit_status = lambda msg: None
        agent._check_compression_model_feasibility()
...
@patch("brain.model_metadata.get_model_context_length", return_value=1_000_000)
@patch("brain.auxiliary_client.get_text_auxiliary_client")
...
@patch("brain.model_metadata.get_model_context_length", return_value=128_000)
@patch("brain.auxiliary_client.get_text_auxiliary_client")
...
def test_init_feasibility_check_uses_aux_context_override_from_config():
    ...
    NB: feasibility check is deferred from AIBrain.__init__ to the first
    ...
    with (
        ...
        patch("run_brain.ContextCompressor", new=_StubCompressor),
        patch("brain.auxiliary_client.get_text_auxiliary_client", return_value=(mock_client, "custom/big-model")),
        patch("brain.model_metadata.get_model_context_length", return_value=1_000_000) as mock_ctx_len,
    ):
        agent = AIBrain(
...
@patch("brain.model_metadata.get_model_context_length", return_value=100_000)
@patch("brain.auxiliary_client.get_text_auxiliary_client")
...
@patch("brain.model_metadata.get_model_context_length", return_value=99_999)
@patch("brain.auxiliary_client.get_text_auxiliary_client")
