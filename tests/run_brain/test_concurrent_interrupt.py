...
def _make_agent(monkeypatch):
    """Create a minimal AIBrain-like object with just the methods under test."""
    monkeypatch.setenv("OPENROUTER_API_KEY", "")
    monkeypatch.setenv("JARVIS_INFERENCE_PROVIDER", "")
    # Avoid full AIBrain init — just import the class and build a stub
    ...
    class _Stub:
        ...
        _last_activity = 0
        _print_fn = print
        # Worker-thread tracking state mirrored from AIBrain.__init__ so the
    ...
    stub._execute_tool_calls_concurrent = _ra.AIBrain._execute_tool_calls_concurrent.__get__(stub)
    stub.interrupt = _ra.AIBrain.interrupt.__get__(stub)
    stub.clear_interrupt = _ra.AIBrain.clear_interrupt.__get__(stub)
