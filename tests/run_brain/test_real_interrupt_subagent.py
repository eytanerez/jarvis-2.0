"""Test real interrupt propagation through delegate_task with actual AIBrain.

This uses a real AIBrain with mocked HTTP responses to test the complete
...
class TestRealSubagentInterrupt(unittest.TestCase):
    """Test interrupt with real AIBrain child through delegate_tool."""
    ...
    def test_interrupt_child_during_api_call(self):
        """Real AIBrain child interrupted while making API call."""
        ...
        def run_delegate():
            try:
                # Patch the OpenAI client creation inside AIBrain.__init__
                with patch('run_brain.OpenAI') as MockOpenAI:
                    ...
                    with patch.object(AIBrain, '_build_system_prompt', return_value="You are a test agent"):
                        # Signal when child starts
                        original_run = AIBrain.run_conversation

                        def patched_run(self_agent, *args, **kwargs):
                            child_started.set()
                            return original_run(self_agent, *args, **kwargs)

                        with patch.object(AIBrain, 'run_conversation', patched_run):
                            # Build a real child agent (AIBrain is NOT patched here,
