"""Tests for the vision-aware image preprocessing in run_brain.py.
...
def _make_agent() -> AIBrain:
    """Build a bare-bones AIBrain instance without running __init__.
...
class TestModelSupportsVision:
    ...
    def test_uses_get_model_capabilities(self):
        ...
        with patch("brain.models_dev.get_model_capabilities", return_value=fake_caps):
    ...
    def test_none_caps_returns_false(self):
        agent = _make_agent()
        with patch("brain.models_dev.get_model_capabilities", return_value=None):
            assert agent._model_supports_vision() is False

    def test_exception_returns_false(self):
        agent = _make_agent()
        with patch("brain.models_dev.get_model_capabilities", side_effect=RuntimeError("boom")):
            assert agent._model_supports_vision() is False
    ...
    def test_top_level_model_override_wins(self):
        ...
        with patch("jarvis_cli.config.load_config", return_value={"model": {"supports_vision": True}}), \
             patch("brain.models_dev.get_model_capabilities", return_value=None):
            assert agent._model_supports_vision() is True
    ...
    def test_override_false_disables_vision_for_models_dev_models(self):
        ...
        fake_caps = MagicMock()
        fake_caps.supports_vision = True
        with patch("jarvis_cli.config.load_config", return_value={"model": {"supports_vision": False}}), \
             patch("brain.models_dev.get_model_capabilities", return_value=fake_caps):
            assert agent._model_supports_vision() is False
