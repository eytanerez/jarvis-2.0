...
3. Multi-turn replay of ``codex_reasoning_items`` (with
   ...
   reverted shortly after: xAI confirmed they explicitly want Jarvis to
...
def _make_codex_agent():
    """Build a minimal AIBrain wired for codex_responses streaming tests."""
...
def test_codex_stream_wire_error_event_surfaces_stream_error_event(provider_message):
    ...
    from run_brain import _StreamErrorEvent
...
def test_summarize_api_error_decorates_xai_entitlement_403():
    ...
    summary = AIBrain._summarize_api_error(error)
...
def test_summarize_api_error_decorates_xai_body_message():
    ...
    summary = AIBrain._summarize_api_error(_XaiErr("403"))
...
def test_summarize_api_error_handles_nested_provider_message():
    ...
    summary = AIBrain._summarize_api_error(_NestedProviderErr("400"))
...
def test_summarize_api_error_idempotent_for_entitlement_hint():
    ...
    once = AIBrain._decorate_xai_entitlement_error(raw)
    twice = AIBrain._decorate_xai_entitlement_error(once)
    assert once == twice
...
def test_codex_reasoning_replay_includes_encrypted_content_for_xai():
    ...
    want Jarvis to thread encrypted reasoning back across turns for
...
def test_codex_transport_xai_request_includes_encrypted_content():
    ...
    """
    from brain.transports.codex import ResponsesApiTransport
...
def test_is_entitlement_failure_matches_real_xai_bodies(message):
    ...
    assert AIBrain._is_entitlement_failure(
...
def test_is_entitlement_failure_false_for_status_other_than_401_403():
    ...
    assert not AIBrain._is_entitlement_failure(body, 500)
    assert not AIBrain._is_entitlement_failure(body, 429)
    assert not AIBrain._is_entitlement_failure(body, 200)
...
def test_is_entitlement_failure_false_for_unrelated_auth_errors():
    ...
    assert not AIBrain._is_entitlement_failure(
    ...
    # Empty context
    assert not AIBrain._is_entitlement_failure({}, 401)
    assert not AIBrain._is_entitlement_failure(None, 401)
...
def test_recover_with_credential_pool_skips_refresh_on_bare_403_for_xai_oauth():
    ...
    assert not AIBrain._is_entitlement_failure(error_context, 403), (
...
def test_grok_4_3_context_length_is_1m():
    ...
    Jarvis' substring-match fallback used to return 256k (from the
