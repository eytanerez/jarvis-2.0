"""Tests for richer stream-drop diagnostics in brain.log.

When a subagent's stream drops mid-tool-call, the WARNING in brain.log must
...
def test_stream_diag_init_returns_well_formed_dict():
    diag = AIBrain._stream_diag_init()
...
def test_flatten_exception_chain_walks_cause():
    ...
    chain = AIBrain._flatten_exception_chain(outer)
...
def test_flatten_exception_chain_caps_depth():
    ...
    for i in range(1, 8):
        ...
        nxt.__cause__ = prev
        prev = nxt
    chain = AIBrain._flatten_exception_chain(prev)
...
def test_log_stream_retry_includes_diagnostic_fields(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="run_brain"):
...
def test_quiet_mode_does_not_clobber_runagent_logger_level():
    ...
    _ = _make_agent()
    for name in ("run_brain", "tools", "trajectory_compressor", "cron", "jarvis_cli"):
