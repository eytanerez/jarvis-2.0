...
``_jarvis_now()`` timestamp, fresh ``session_id``, and a different skills
...
class _ReviewAgentRecorder:
    """Stand-in for the review-fork AIBrain that records the prompt assignment."""
...
def test_review_fork_inherits_parent_cached_system_prompt():
    ...
    with patch.object(run_brain, "AIBrain", _ReviewAgentRecorder), \
...
def test_review_fork_pins_session_start_and_session_id():
    ...
    with patch.object(run_brain, "AIBrain", _Recorder), \
         patch("threading.Thread", _SyncThread):
        agent._spawn_background_review(
            messages_snapshot=[],
            review_memory=True,
