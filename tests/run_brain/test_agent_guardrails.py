"""Unit tests for AIBrain pre/post-LLM-call guardrails.

Covers three static methods on AIBrain (inspired by PR #1321 — @alireza78a):
...
class TestSanitizeApiMessages:
    ...
    def test_orphaned_result_removed(self):
        ...
        out = AIBrain._sanitize_api_messages(msgs)
        assert len(out) == 2
    ...
    def test_empty_list_is_safe(self):
        assert AIBrain._sanitize_api_messages([]) == []
...
class TestCapDelegateTaskCalls:
    ...
    def test_excess_delegates_truncated(self):
        ...
        out = AIBrain._cap_delegate_task_calls(tcs)
    ...
    def test_empty_list_safe(self):
        assert AIBrain._cap_delegate_task_calls([]) == []

    def test_original_list_not_mutated(self):
        tcs = [make_tc("delegate_task") for _ in range(MAX_CONCURRENT_CHILDREN + 2)]
        original_len = len(tcs)
        AIBrain._cap_delegate_task_calls(tcs)
...
class TestDeduplicateToolCalls:
    ...
    def test_duplicate_pair_deduplicated(self):
        ...
        out = AIBrain._deduplicate_tool_calls(tcs)
        assert len(out) == 1
    ...
    def test_empty_list_safe(self):
        assert AIBrain._deduplicate_tool_calls([]) == []

    def test_first_occurrence_kept(self):
        tc1 = make_tc("terminal", '{"cmd":"ls"}')
        tc2 = make_tc("terminal", '{"cmd":"ls"}')
        out = AIBrain._deduplicate_tool_calls([tc1, tc2])
        assert len(out) == 1
        assert out[0] is tc1
    ...
    def test_original_list_not_mutated(self):
        ...
        original_len = len(tcs)
        AIBrain._deduplicate_tool_calls(tcs)
        assert len(tcs) == original_len
...
class TestGetToolCallIdStatic:
    ...
    def test_dict_with_valid_id(self):
        assert AIBrain._get_tool_call_id_static({"id": "call_123"}) == "call_123"

    def test_dict_with_none_id(self):
        assert AIBrain._get_tool_call_id_static({"id": None}) == ""

    def test_dict_without_id_key(self):
        assert AIBrain._get_tool_call_id_static({"function": {}}) == ""

    def test_object_with_valid_id(self):
        tc = types.SimpleNamespace(id="call_456")
        assert AIBrain._get_tool_call_id_static(tc) == "call_456"

    def test_object_with_none_id(self):
        tc = types.SimpleNamespace(id=None)
        assert AIBrain._get_tool_call_id_static(tc) == ""
...
class TestGetToolCallNameStatic:
    ...
    def test_dict_with_valid_name(self):
        assert AIBrain._get_tool_call_name_static(
            {"id": "call_1", "function": {"name": "terminal", "arguments": "{}"}}
        ) == "terminal"

    def test_dict_with_missing_function(self):
        assert AIBrain._get_tool_call_name_static({"id": "call_1"}) == ""

    def test_dict_with_none_function(self):
        assert AIBrain._get_tool_call_name_static({"id": "call_1", "function": None}) == ""
    ...
    def test_object_with_valid_name(self):
        ...
        assert AIBrain._get_tool_call_name_static(tc) == "read_file"

    def test_object_without_function_attr(self):
        tc = types.SimpleNamespace(id="call_1")
        assert AIBrain._get_tool_call_name_static(tc) == ""
