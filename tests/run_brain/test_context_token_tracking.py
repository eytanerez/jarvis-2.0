"""Tests for context token tracking in run_brain.py's usage extraction.
...
is tested in tests/brain/test_context_compressor.py).
"""
...
def _patch_bootstrap(monkeypatch):
    monkeypatch.setattr(run_brain, "get_tool_definitions", lambda **kwargs: [{
        "type": "function",
...
def _make_agent(monkeypatch, api_mode, provider, response_fn):
    ...
    if api_mode == "anthropic_messages":
        monkeypatch.setattr("brain.anthropic_adapter.build_anthropic_client", lambda k, b=None, **kwargs: _FakeAnthropicClient())
