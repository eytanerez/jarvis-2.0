...
The fix (4 LOC in ``run_brain.py``) snapshots the caller's context with
...
def test_executor_submit_without_copy_context_does_not_propagate():
    ...
    ContextVars by default — the copy_context() wrapper in run_brain.py
...
def test_run_tool_worker_sees_parent_approval_session_key():
    ...
    ``run_brain.py::_execute_tool_calls_concurrent`` — a
...
def test_run_brain_concurrent_executor_wraps_submit_with_copy_context():
    ...
    pass regardless of whether ``run_brain.py`` actually uses it.
    ...
    wrapper in ``run_brain.py`` fails this test with a clear message.
    """
    import ast
    import inspect

    import run_brain
    from brain import tool_executor as tool_executor_module

    # Source for both modules — the concurrent-executor body lives in
    # ``brain/tool_executor.py`` after the run_brain.py refactor (PR
    # following #16660).  Search both so this guard keeps firing
    # regardless of where the call site lives.
    sources = []
    for mod in (run_brain, tool_executor_module):
    ...
    # call sites in run_brain.py (e.g. auxiliary client warm-up) are
    ...
    assert tool_submits, (
        ...
        "run_brain.py. The call site may have been renamed — update this "
        "guard along with the refactor."
    )
    unfixed = [c for kind, c in tool_submits if kind == "unfixed"]
    assert not unfixed, (
        "run_brain.py contains `executor.submit(_run_tool, ...)` without a "
