"""Unit tests for run_brain.py (AIBrain).
...
import pytest
from brain.codex_responses_adapter import _normalize_codex_response

import run_brain
from run_brain import AIBrain
from brain.error_classifier import FailoverReason
from brain.prompt_builder import DEFAULT_AGENT_IDENTITY
...
def _make_tool_defs(*names: str) -> list:
    """Build minimal tool definition list accepted by AIBrain.__init__."""
    return [
        {
            "type": "function",
            "function": {
...
def test_is_destructive_command_treats_cp_as_mutating():
    assert run_brain._is_destructive_command("cp .env.local .env") is True


def test_is_destructive_command_treats_install_as_mutating():
    assert run_brain._is_destructive_command("install template.env .env") is True


@pytest.fixture()
def agent():
    """Minimal AIBrain with mocked OpenAI client and tool loading."""
    with (
        patch(
            "run_brain.get_tool_definitions", return_value=_make_tool_defs("web_search")
        ),
...
def agent_with_memory_tool():
    ...
    with (
        patch(
            "run_brain.get_tool_definitions",
...
def test_aiagent_reuses_existing_errors_log_handler():
    """Repeated AIBrain init should not accumulate duplicate errors.log handlers."""
    root_logger = logging.getLogger()
    original_handlers = list(root_logger.handlers)
    error_log_path = (run_brain._jarvis_home / "logs" / "errors.log").resolve()
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host JARVIS_REDACT_SECRETS state.
...
class TestInit:
    def test_anthropic_base_url_accepted(self):
        ...
        with (
            ...
            patch("run_brain.check_toolset_requirements", return_value={}),
            patch("brain.anthropic_adapter._anthropic_sdk") as mock_anthropic,
        ):
            agent = AIBrain(
                api_key="test-key-1234567890",
    ...
    def test_prompt_caching_native_anthropic(self):
        ...
        with (
            patch("run_brain.get_tool_definitions", return_value=[]),
            patch("run_brain.check_toolset_requirements", return_value={}),
            patch("brain.anthropic_adapter._anthropic_sdk"),
        ):
            a = AIBrain(
                api_key="test-key-1234567890",
                base_url="https://api.anthropic.com/v1/",
    ...
    def test_prompt_caching_cache_ttl_defaults_without_config(self):
        ...
        with (
            ...
            patch("run_brain.check_toolset_requirements", return_value={}),
            patch("run_brain.OpenAI"),
            patch("jarvis_cli.config.load_config", return_value={}),
        ):
            a = AIBrain(
                api_key="test-k...7890",
                model="anthropic/claude-sonnet-4-20250514",
    ...
    def test_model_max_tokens_from_config(self):
        ...
        with (
            patch("run_brain.get_tool_definitions", return_value=_make_tool_defs("terminal")),
            patch("run_brain.check_toolset_requirements", return_value={}),
            patch("run_brain.OpenAI"),
            patch(
                "jarvis_cli.config.load_config",
    ...
    def test_valid_tool_names_populated(self):
        ...
        with (
            patch("run_brain.get_tool_definitions", return_value=tools),
            patch("run_brain.check_toolset_requirements", return_value={}),
            patch("run_brain.OpenAI"),
        ):
            a = AIBrain(
...
class TestInterrupt:
    def test_interrupt_sets_flag(self, agent):
        with patch("run_brain._set_interrupt"):
            agent.interrupt()
...
class TestBuildSystemPrompt:
    ...
    def test_can_use_soul_identity_even_when_context_files_are_skipped(self):
        with (
            ...
            patch("run_brain.check_toolset_requirements", return_value={}),
            patch("run_brain.OpenAI"),
            patch("run_brain.load_soul_md", return_value="SOUL IDENTITY"),
        ):
            agent = AIBrain(
                api_key="test-k...7890",
                base_url="https://openrouter.ai/api/v1",
    ...
    def test_memory_guidance_when_memory_tool_loaded(self, agent_with_memory_tool):
        from brain.prompt_builder import MEMORY_GUIDANCE
    ...
    def test_includes_nous_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_brain, "build_nous_subscription_prompt", lambda tool_names: "NOUS SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
    ...
    def test_skills_prompt_derives_available_toolsets_from_loaded_tools(self):
        ...
        with (
            patch("run_brain.get_tool_definitions", return_value=tools),
            patch(
                "run_brain.check_toolset_requirements",
                side_effect=AssertionError("should not re-check toolset requirements"),
            ),
            patch("run_brain.get_toolset_for_tool", create=True, side_effect=toolset_map.get),
            patch("run_brain.build_skills_system_prompt", return_value="SKILLS_PROMPT") as mock_skills,
            patch("run_brain.OpenAI"),
        ):
            agent = AIBrain(
                api_key="test-k...7890",
...
class TestToolUseEnforcementConfig:
    ...
    def test_auto_injects_for_gpt(self):
        from brain.prompt_builder import TOOL_USE_ENFORCEMENT_GUIDANCE
    ...
    def test_auto_injects_execution_guidance_for_grok(self):
        ...
        """
        from brain.prompt_builder import OPENAI_MODEL_EXECUTION_GUIDANCE
        agent = self._make_agent(model="x-ai/grok-4.3", tool_use_enforcement="auto")
        prompt = agent._build_system_prompt()
...
class TestTaskCompletionGuidance:
    ...
    def test_default_injects_for_claude(self):
        ...
        from brain.prompt_builder import TASK_COMPLETION_GUIDANCE
...
class TestExecuteToolCalls:
    def test_single_tool_executed(self, agent):
        ...
        messages = []
        with patch(
            "run_brain.handle_function_call", return_value="search result"
        ) as mock_hfc:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("jarvis_cli.plugins.invoke_hook", _capture_hook)
        monkeypatch.setattr("jarvis_cli.plugins.has_hook", lambda name: True)

        with (
            patch("run_brain.handle_function_call", side_effect=KeyboardInterrupt),
            patch("run_brain._set_interrupt"),
    ...
    def test_invalid_json_args_defaults_empty(self, agent):
        ...
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tc])
        messages = []
        with patch("run_brain.handle_function_call", return_value="ok") as mock_hfc:
            agent._execute_tool_calls(mock_msg, messages, "task-1")
    ...
    def test_result_truncation_over_100k(self, agent, tmp_path, monkeypatch):
        ...
        with patch("run_brain.handle_function_call", return_value=big_result):
            agent._execute_tool_calls(mock_msg, messages, "task-1")
    ...
    def test_quiet_tool_output_suppressed_when_progress_callback_present(self, agent):
        ...
        with patch("run_brain.handle_function_call", return_value="search result"), \
    ...
    def test_run_conversation_suppresses_retry_noise_in_parseable_quiet_mode(self, agent):
        ...
        with patch("run_brain.time.sleep", return_value=None):
...
class TestConcurrentToolExecution:
    ...
    def test_concurrent_executes_all_tools(self, agent):
        ...
        with patch("run_brain.handle_function_call", side_effect=fake_handle):
    ...
    def test_invoke_tool_dispatches_to_handle_function_call(self, agent):
        ...
        with patch("run_brain.handle_function_call", return_value="result") as mock_hfc:
    ...
    def test_sequential_tool_callbacks_fire_in_order(self, agent):
        ...
        with patch("run_brain.handle_function_call", return_value='{"success": true}'):
            agent._execute_tool_calls_sequential(mock_msg, messages, "task-1")
    ...
    def test_concurrent_tool_callbacks_fire_for_each_tool(self, agent):
        ...
        with patch("run_brain.handle_function_call", side_effect=['{"id":1}', '{"id":2}']):
            agent._execute_tool_calls_concurrent(mock_msg, messages, "task-1")
    ...
    def test_invoke_tool_agent_level_tool_emits_terminal_post_tool_hook(self, agent, monkeypatch):
        ...
        hook_calls = []
        monkeypatch.setattr(
            "jarvis_cli.plugins.get_pre_tool_call_block_message",
    ...
    def test_invoke_tool_blocked_skips_handle_function_call(self, agent, monkeypatch):
        ...
        with patch("run_brain.handle_function_call", side_effect=AssertionError("should not run")):
            result = agent._invoke_tool("web_search", {"q": "test"}, "task-1")
    ...
    def test_sequential_agent_level_tool_execution_middleware_wraps_inline_dispatch(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "jarvis_cli.plugins.invoke_middleware",
    ...
    def test_agent_runtime_post_hook_ownership_predicate_covers_agent_tools(self, agent):
        ...
        from brain.agent_runtime_helpers import agent_runtime_owns_post_tool_hook
    ...
    def test_concurrent_blocked_terminal_skips_checkpoint(self, agent, monkeypatch):
        ...
        with patch("run_brain.handle_function_call", side_effect=fake_handle):
            with patch.object(agent._checkpoint_mgr, "ensure_checkpoint") as cp_mock:
                with patch("brain.tool_executor._is_destructive_command", return_value=True):
                    agent._execute_tool_calls_concurrent(mock_msg, messages, "task-1")
...
class TestAgentRuntimePostHookOwnershipSync:
    ...
    brain/tool_executor.py:execute_tool_calls_sequential (for agent-runtime
    ...
    def test_frozenset_matches_inline_dispatch_chain(self):
        from brain import tool_executor
        from brain.agent_runtime_helpers import AGENT_RUNTIME_POST_HOOK_TOOL_NAMES
        ...
        assert inline_names == set(AGENT_RUNTIME_POST_HOOK_TOOL_NAMES), (
            ...
            "brain/tool_executor.py:execute_tool_calls_sequential has drifted "
            "from AGENT_RUNTIME_POST_HOOK_TOOL_NAMES in "
            "brain/agent_runtime_helpers.py.\n"
    ...
    def test_invoke_tool_dispatch_matches_inline_dispatch_chain(self):
        ...
        the tool."""
        from brain import agent_runtime_helpers, tool_executor
        ...
        assert invoke_tool_names == inline_names, (
            ...
            "brain/agent_runtime_helpers.py:invoke_tool (concurrent path) "
            "and brain/tool_executor.py:execute_tool_calls_sequential "
            "(sequential path).\n"
...
class TestPathsOverlap:
    ...
    def test_same_path_overlaps(self):
        from run_brain import _paths_overlap
...
class TestParallelScopePathNormalization:
    def test_extract_parallel_scope_path_normalizes_relative_to_cwd(self, tmp_path, monkeypatch):
        from run_brain import _extract_parallel_scope_path
    ...
    def test_extract_parallel_scope_path_treats_relative_and_absolute_same_file_as_same_scope(self, tmp_path, monkeypatch):
        from run_brain import _extract_parallel_scope_path, _paths_overlap
    ...
    def test_should_parallelize_tool_batch_rejects_same_file_with_mixed_path_spellings(self, tmp_path, monkeypatch):
        from run_brain import _should_parallelize_tool_batch
...
class TestRunConversation:
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        with (
            ...
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
            caplog.at_level(logging.WARNING, logger="brain.conversation_loop"),
        ):
        ...
        assert "Ollama runtime context too small for Jarvis tool use" in caplog.text
    ...
    def test_tool_calls_then_stop(self, agent):
        ...
        with (
            patch("run_brain.handle_function_call", return_value="search result") as mock_handle_function_call,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            patch("run_brain.handle_function_call", return_value="search result"),
            patch(
                "jarvis_cli.plugins.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("jarvis_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("jarvis_cli.plugins.has_hook", lambda name: False)
        monkeypatch.setattr("jarvis_cli.plugins.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("jarvis_cli.plugins.has_hook", _has_hook)
        ...
        with (
            patch("jarvis_cli.plugins.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_context_compression_triggered(self, agent):
        ...
        with (
            patch("run_brain.handle_function_call", return_value="result"),
            patch.object(
    ...
    def test_length_with_tool_calls_returns_partial_without_executing_tools(self, agent):
        ...
        with (
            patch("run_brain.handle_function_call") as mock_handle_function_call,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_truncated_tool_call_retries_once_before_refusing(self, agent):
        ...
        with (
            patch("run_brain.handle_function_call", return_value='{"success":true}') as mock_hfc,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_stub_stall_mid_tool_call_recovers_within_3_retries(self, agent):
        ...
        from jarvis_constants import PARTIAL_STREAM_STUB_ID
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("JARVIS_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            patch("run_brain.handle_function_call", return_value="ok"),
            patch("jarvis_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("jarvis_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when JARVIS_KANBAN_TASK
...
class TestHookPayloadSanitizesSimpleNamespace:
    ...
    ``brain/bedrock_adapter.py``, ``brain/codex_responses_adapter.py``, and
    ``brain/auxiliary_client.py``. Those raw objects are handed straight to
    ...
    def test_hook_jsonable_normalizes_simplenamespace(self):
        ...
        result = AIBrain._sanitize_hook_payload(ns)
    ...
    def test_api_response_payload_for_hook_normalizes_simplenamespace_tool_calls(self, agent):
        # Shape mirrors brain/bedrock_adapter.py::normalize_converse_response and
        # brain/codex_responses_adapter.py — raw SDK objects are SimpleNamespace.
...
class TestRetryExhaustion:
    ...
    def test_invalid_response_returns_error_not_crash(self, agent):
        ...
        # The conversation loop was extracted out of run_brain.py and pulls
        # in time/jittered_backoff at module level — patch BOTH so the
        # retry waits don't burn 18+ seconds of real wall-clock time here.
        from brain import conversation_loop as _conv_loop
        with (
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
            patch("run_brain.time", self._make_fast_time_mock()),
...
class TestNousCredentialRefresh:
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        with patch("run_brain.OpenAI", side_effect=_fake_openai):
...
class TestCredentialPoolRecovery:
    ...
    def test_extract_api_error_context_parses_resets_in_hours_and_minutes(self, agent, monkeypatch):
        from brain import agent_runtime_helpers
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Jarvis" in agent._cached_system_prompt
...
class TestSafeWriter:
    ...
    def test_write_delegates_normally(self):
        ...
        from run_brain import _SafeWriter
...
class TestBuildApiKwargsAnthropicMaxTokens:
    ...
    def test_max_tokens_passed_to_anthropic(self, agent):
        ...
        with patch("brain.anthropic_adapter.build_anthropic_kwargs") as mock_build:
...
class TestAnthropicImageFallback:
    def test_build_api_kwargs_converts_multimodal_user_image_to_text(self, agent):
        ...
        with (
            ...
            patch("brain.anthropic_adapter.build_anthropic_kwargs") as mock_build,
        ):
            mock_build.return_value = {"model": "claude-sonnet-4-20250514", "messages": [], "max_tokens": 4096}
...
class TestFallbackAnthropicProvider:
    ...
    def test_fallback_to_anthropic_sets_api_mode(self, agent):
        ...
        with (
            patch("brain.auxiliary_client.resolve_provider_client", return_value=(mock_client, None)),
            patch("brain.anthropic_adapter.build_anthropic_client") as mock_build,
            patch("brain.anthropic_adapter.resolve_anthropic_token", return_value=None),
        ):
    ...
    def test_fallback_to_openrouter_uses_openai_client(self, agent):
        ...
        with patch("brain.auxiliary_client.resolve_provider_client", return_value=(mock_client, None)):
            result = agent._try_activate_fallback()
...
def test_aiagent_uses_copilot_acp_client():
    with (
        patch("run_brain.get_tool_definitions", return_value=_make_tool_defs("web_search")),
        patch("run_brain.check_toolset_requirements", return_value={}),
        patch("run_brain.OpenAI") as mock_openai,
        patch("brain.copilot_acp_client.CopilotACPClient") as mock_acp_client,
    ):
...
def test_quiet_spinner_allowed_with_explicit_print_fn(agent):
    ...
    with patch.object(run_brain.sys.stdout, "isatty", return_value=False):
        assert agent._should_start_quiet_spinner() is True


def test_quiet_spinner_allowed_on_real_tty(agent):
    agent._print_fn = None
    with patch.object(run_brain.sys.stdout, "isatty", return_value=True):
        assert agent._should_start_quiet_spinner() is True
...
def test_is_openai_client_closed_honors_custom_client_flag():
    assert AIBrain._is_openai_client_closed(SimpleNamespace(is_closed=True)) is True
    assert AIBrain._is_openai_client_closed(SimpleNamespace(is_closed=False)) is False
...
def test_is_openai_client_closed_handles_method_form():
    ...
    assert AIBrain._is_openai_client_closed(open_client) is False

    # Method returning True - client is closed
    closed_client = MethodFormClient(closed=True)
    assert AIBrain._is_openai_client_closed(closed_client) is True
...
def test_is_openai_client_closed_falls_back_to_http_client():
    ...
    assert AIBrain._is_openai_client_closed(ClientWithHttpClient(http_closed=False)) is False
    assert AIBrain._is_openai_client_closed(ClientWithHttpClient(http_closed=True)) is True
...
class TestAnthropicCredentialRefresh:
    def test_try_refresh_anthropic_client_credentials_rebuilds_client(self):
        ...
        with (
            patch("brain.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat01-fresh-token"),
            patch("brain.anthropic_adapter.build_anthropic_client", return_value=new_client) as rebuild,
        ):
    ...
    def test_try_refresh_anthropic_client_credentials_returns_false_when_token_unchanged(self):
        ...
        with (
            patch("brain.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat01-same-token"),
            patch("brain.anthropic_adapter.build_anthropic_client") as rebuild,
        ):
...
class TestInterruptVprintForceTrue:
    ...
    def test_all_interrupt_vprint_have_force_true(self):
        ...
        import inspect
        source = inspect.getsource(AIBrain)
        lines = source.split("\n")
        violations = []
...
class TestOAuthFlagAfterCredentialRefresh:
    ...
    def test_oauth_flag_updates_api_key_to_oauth(self, agent):
        ...
        with (
            patch("brain.anthropic_adapter.resolve_anthropic_token",
                  return_value="sk-ant-setup-oauth-token"),
            patch("brain.anthropic_adapter.build_anthropic_client",
...
class TestMemoryNudgeCounterPersistence:
    ...
    def test_counters_initialized_in_init(self):
        ...
        with patch("run_brain.get_tool_definitions", return_value=[]):
            a = AIBrain(
    ...
    def test_counters_not_reset_in_preamble(self):
        ...
        import inspect
        from brain.turn_context import build_turn_context as _btc
...
class TestDeadRetryCode:
    ...
    def test_no_unreachable_max_retries_after_backoff(self):
        import inspect
        from brain.conversation_loop import run_conversation as _rc
