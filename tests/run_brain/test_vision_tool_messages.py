...
def _make_agent(provider="openrouter", model="gpt-4o"):
    """Create a minimal AIBrain mock with provider/model attributes."""
    ...
    agent._model_supports_vision = lambda: AIBrain._model_supports_vision(agent)
    agent._provider_supports_vision_tool_messages = lambda: AIBrain._provider_supports_vision_tool_messages(agent)
    agent._tool_result_content_for_active_model = (
        lambda name, result: AIBrain._tool_result_content_for_active_model(agent, name, result)
    )
    return agent
