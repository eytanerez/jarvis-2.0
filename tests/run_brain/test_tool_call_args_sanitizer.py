"""Tests for AIBrain._sanitize_tool_call_arguments."""
...
def test_valid_arguments_unchanged():
    ...
    repaired = AIBrain._sanitize_tool_call_arguments(messages)
...
def test_truncated_arguments_replaced_with_empty_object(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="run_brain"):
        repaired = AIBrain._sanitize_tool_call_arguments(
            messages,
...
def test_marker_appended_to_existing_tool_message():
    marker = AIBrain._TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER
    messages = [
        _assistant_message(_tool_call(arguments='{"path": "/tmp/foo')),
