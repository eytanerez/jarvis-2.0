...
from brain.credential_pool import PooledCredential, STATUS_EXHAUSTED
...
def test_rotate_immediately_when_credential_already_exhausted():
    ...
    with patch("run_brain.get_tool_definitions", return_value=[]),          patch("run_brain.check_toolset_requirements", return_value={}),          patch("run_brain.OpenAI"):
        agent = MagicMock(spec=AIBrain)
        agent._credential_pool = pool
        agent._swap_credential = MagicMock()
        recovered, retried = AIBrain._recover_with_credential_pool(
            agent,
            status_code=429,
