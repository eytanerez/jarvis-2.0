...
def _bare_agent():
    return AIBrain.__new__(AIBrain)
...
def test_drop_scaffolding_rewinds_orphan_tool_tail():
    ...
    AIBrain._drop_trailing_empty_response_scaffolding(agent, messages)
...
def test_repair_merges_consecutive_user_messages():
    ...
    repairs = AIBrain._repair_message_sequence(agent, messages)
...
def test_repair_preserves_user_content_when_one_side_empty():
    ...
    AIBrain._repair_message_sequence(agent, messages)
...
def test_flush_guard_clamps_overshooting_cursor():
    ...
    AIBrain._flush_messages_to_session_db(agent, messages, conversation_history=[])
