...
Reported: "switched from openrouter provider to anthropic api key via jarvis
...
def _switch_to_anthropic(agent):
    with (
        ...
        patch("brain.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-xyz"),
        patch("brain.anthropic_adapter._is_oauth_token", return_value=False),
        patch("jarvis_cli.timeouts.get_provider_request_timeout", return_value=None),
    ):
        agent.switch_model(
...
def test_switch_drops_old_primary_from_fallback_chain():
    ...
    assert agent._fallback_model == {"provider": "nous", "model": "jarvis-4"}
...
def test_switch_within_same_provider_preserves_chain():
    ...
    with patch("jarvis_cli.timeouts.get_provider_request_timeout", return_value=None):
        agent.switch_model(
