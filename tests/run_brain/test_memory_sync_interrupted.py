...
The fix is ``AIBrain._sync_external_memory_for_turn`` — a small helper
...
These tests exercise the helper directly on a bare ``AIBrain`` built
...
def _bare_agent():
    """Build an ``AIBrain`` with only the attributes
    ``_sync_external_memory_for_turn`` touches — matches the bare-agent
    pattern used across ``tests/run_brain/test_interrupt_propagation.py``.
    """
    ...
    def test_completed_turn_syncs_messages_when_present(self):
        ...
        messages = [
            ...
            },
            {
                ...
                "name": "terminal",
                "tool_call_id": "call-1",
                "content": "final Jarvis-processed output",
            }
        ]
