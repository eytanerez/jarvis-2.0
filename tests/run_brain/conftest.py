"""Fast-path fixtures shared across tests/run_brain/.
...
monkeypatch ``run_brain.time.sleep`` locally (see
...
def _fast_retry_backoff(monkeypatch):
    ...
    # The conversation loop was extracted out of run_brain.py into
    # ``agent.conversation_loop``, which imports ``jittered_backoff``
    # directly (``from brain.retry_utils import jittered_backoff``).
    # Patching ``run_brain.jittered_backoff`` alone misses every retry
