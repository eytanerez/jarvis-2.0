...
import run_brain as run_brain_module
...
def test_background_review_shuts_down_memory_provider_before_close(monkeypatch):
    ...
    monkeypatch.setattr(run_brain_module, "AIBrain", FakeReviewAgent)
    monkeypatch.setattr(run_brain_module.threading, "Thread", ImmediateThread)

    agent = _bare_agent()

    AIBrain._spawn_background_review(
        agent,
...
def test_background_review_summarizer_receives_captured_messages_after_close(monkeypatch):
    ...
    """
    import json
    import brain.background_review as bg_review
...
def test_background_review_fork_skips_external_memory_plugins(monkeypatch):
    ...
    Without skip_memory=True on the fork constructor, AIBrain.__init__
    ...
    assert captured_kwargs.get("skip_memory") is True, (
        ...
        "so AIBrain.__init__ does not rebuild a _memory_manager wired to "
