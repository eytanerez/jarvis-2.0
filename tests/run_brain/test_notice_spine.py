...
Covers:
  A. _emit_notice / _emit_notice_clear emitter behaviour (bare AIBrain via
...
def _bare_agent() -> AIBrain:
    """Build an AIBrain without running __init__ (no heavy init required).
...
class TestSignatureThreading:
    def test_agent_init_exposes_notice_callback(self):
        sig = inspect.signature(AIBrain.__init__)
