"""Tests for the decomposer module + `jarvis kanban decompose` CLI surface.
...
def _patch_extra_body():
    return patch(
        "brain.auxiliary_client.get_auxiliary_extra_body",
        return_value={},
    )
...
def _patch_list_profiles(names: list[str]):
    ...
    return [
        patch("jarvis_cli.profiles.list_profiles", return_value=fake_profiles),
        patch("jarvis_cli.profiles.profile_exists", side_effect=lambda x: x in names),
        patch("jarvis_cli.profiles.get_active_profile_name", return_value=names[0] if names else "default"),
    ]
...
def test_decompose_fanout_false_assigns_default_when_unassigned(kanban_home):
    ...
    try:
        with _patch_aux_client(llm_payload), _patch_extra_body(), patch(
            "jarvis_cli.kanban_decompose._load_config",
