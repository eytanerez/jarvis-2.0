...
subparser, placing the flag BEFORE the subcommand (e.g. 'jarvis --yolo chat')
...
def _build_parser():
    """Build the jarvis argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("jarvis_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "jarvis_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets JARVIS_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `jarvis <argv>` must exit 0 (help) rather than
