...
Covers:
 - Normal path (tokens already in Jarvis auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Jarvis
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `jarvis auth openai-codex`. The old
...
@pytest.fixture()
def jarvis_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Jarvis auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(jarvis_auth_only_env):
    """openai-codex appears when tokens are already in Jarvis auth store."""
...
def test_codex_picker_uses_live_codex_catalog(jarvis_auth_only_env, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "jarvis_cli.codex_models._fetch_models_from_api",
        lambda access_token: [],
    )
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Jarvis
    auth store."""
    jarvis_home = tmp_path / ".jarvis"
    jarvis_home.mkdir()
