...
base deps) during ``jarvis update``, leaving the venv in a broken state
...
def temp_pyproject(tmp_path, monkeypatch):
    """Point jarvis_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    def test_no_action_when_all_deps_present(self, temp_pyproject, fake_venv_python):
        ...
        with patch("jarvis_cli.main._resolve_install_target_python", return_value=py), \
             patch("jarvis_cli.main.subprocess.run") as mock_run, \
             patch("jarvis_cli.main._run_install_with_heartbeat") as mock_install:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")

            from jarvis_cli.main import _verify_core_dependencies_installed
    ...
    def test_triggers_reinstall_when_dep_missing(self, temp_pyproject, fake_venv_python):
        ...
        with patch("jarvis_cli.main._resolve_install_target_python", return_value=py), \
             patch("jarvis_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("jarvis_cli.main._run_install_with_heartbeat") as mock_install:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("jarvis_cli.main._resolve_install_target_python", return_value=py), \
             patch("jarvis_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("jarvis_cli.main._run_install_with_heartbeat"), \
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("jarvis_cli.main._resolve_install_target_python") as mock_resolve, \
             patch("jarvis_cli.main._run_install_with_heartbeat") as mock_install:
            from jarvis_cli.main import _verify_core_dependencies_installed
    ...
    def test_repair_reinstall_quarantines_running_shim_on_windows(
        ...
        quarantine the running ``jarvis.exe`` on Windows before installing.
        ...
        is left missing and ``jarvis`` drops off PATH. Previously this path
        ...
        with patch("jarvis_cli.main._resolve_install_target_python", return_value=py), \
             patch("jarvis_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("jarvis_cli.main._is_windows", return_value=True), \
             patch("jarvis_cli.main._venv_scripts_dir", return_value=fake_scripts), \
             patch("jarvis_cli.main._run_install_with_heartbeat"), \
             patch("jarvis_cli.main._quarantine_running_jarvis_exe", return_value=[]) as mock_quar:
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `jarvis update`.
        ...
        with patch("jarvis_cli.main._is_windows", return_value=True):
            from jarvis_cli.main import _resolve_install_target_python
