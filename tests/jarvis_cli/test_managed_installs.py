...
def test_get_managed_system_homebrew(monkeypatch):
    ...
    assert recommended_update_command() == "brew upgrade jarvis-agent"


def test_format_managed_message_homebrew(monkeypatch):
    monkeypatch.setenv("JARVIS_MANAGED", "homebrew")

    message = format_managed_message("update Jarvis")

    assert "managed by Homebrew" in message
    assert "brew upgrade jarvis-agent" in message
...
def test_cmd_update_blocks_managed_homebrew(monkeypatch, capsys):
    ...
    assert "brew upgrade jarvis-agent" in captured.err
