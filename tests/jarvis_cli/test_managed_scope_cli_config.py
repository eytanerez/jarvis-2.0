...
jarvis_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `jarvis config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_jarvis_home = get_jarvis_home()`` at import time (module
    singleton), so monkeypatching JARVIS_HOME after import doesn't move it.
    ...
    this only matters for tests that swap JARVIS_HOME.)
    """
    import cli

    cli._jarvis_home = home
