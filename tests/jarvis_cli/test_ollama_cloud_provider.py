...
from jarvis_cli.auth import PROVIDER_REGISTRY, resolve_provider, resolve_api_key_provider_credentials
from jarvis_cli.models import _PROVIDER_MODELS, _PROVIDER_LABELS, _PROVIDER_ALIASES, normalize_provider
from jarvis_cli.model_normalize import normalize_model_for_provider
from brain.model_metadata import _URL_TO_PROVIDER, _PROVIDER_PREFIXES
from brain.models_dev import PROVIDER_TO_MODELS_DEV, list_agentic_models
...
class TestOllamaCloudModelCatalog:
    ...
    def test_provider_model_ids_returns_dynamic_models(self, tmp_path, monkeypatch):
        ...
        with patch("jarvis_cli.models.fetch_api_models", return_value=["qwen3.5:397b"]), \
             patch("brain.models_dev.fetch_models_dev", return_value=mock_mdev):
...
class TestOllamaCloudMergedDiscovery:
    def test_merges_live_and_models_dev(self, tmp_path, monkeypatch):
        ...
        with patch("jarvis_cli.models.fetch_api_models", return_value=["qwen3.5:397b", "glm-5"]), \
             patch("brain.models_dev.fetch_models_dev", return_value=mock_mdev):
    ...
    def test_falls_back_to_models_dev_without_api_key(self, tmp_path, monkeypatch):
        ...
        with patch("brain.models_dev.fetch_models_dev", return_value=mock_mdev):
            result = fetch_ollama_cloud_models(force_refresh=True)
    ...
    def test_uses_disk_cache(self, tmp_path, monkeypatch):
        ...
        with patch("jarvis_cli.models.fetch_api_models", return_value=["model-a"]) as mock_api, \
             patch("brain.models_dev.fetch_models_dev", return_value={}):
    ...
    def test_stale_cache_used_on_total_failure(self, tmp_path, monkeypatch):
        ...
        from jarvis_cli.models import fetch_ollama_cloud_models, _save_ollama_cloud_cache
        ...
        with patch("jarvis_cli.models.fetch_api_models", return_value=None), \
             patch("brain.models_dev.fetch_models_dev", return_value={}):
            result = fetch_ollama_cloud_models(force_refresh=True)
    ...
    def test_empty_on_total_failure_no_cache(self, tmp_path, monkeypatch):
        ...
        with patch("brain.models_dev.fetch_models_dev", return_value={}):
            result = fetch_ollama_cloud_models(force_refresh=True)
...
class TestOllamaCloudModelsDev:
    ...
    def test_list_agentic_models_with_mock_data(self):
        ...
        with patch("brain.models_dev.fetch_models_dev", return_value=mock_data):
...
class TestOllamaCloudAgentInit:
    def test_agent_imports_without_error(self):
        """Verify run_brain.py has no SyntaxError."""
        import importlib
        import run_brain
        importlib.reload(run_brain)
    ...
    def test_ollama_cloud_agent_uses_chat_completions(self, monkeypatch):
        ...
        monkeypatch.setenv("OLLAMA_API_KEY", "test-key")
        with patch("run_brain.OpenAI") as mock_openai:
...
class TestOllamaCloudProvidersNew:
    def test_overlay_exists(self):
        ...
        assert "ollama-cloud" in JARVIS_OVERLAYS
        overlay = JARVIS_OVERLAYS["ollama-cloud"]
        assert overlay.transport == "openai_chat"
        assert overlay.base_url_env_var == "OLLAMA_BASE_URL"

    def test_alias_resolves(self):
        from jarvis_cli.providers import normalize_provider as np
        assert np("ollama") == "custom"  # bare "ollama" = local
        assert np("ollama-cloud") == "ollama-cloud"

    def test_label_override(self):
        from jarvis_cli.providers import _LABEL_OVERRIDES
    ...
    def test_get_provider(self):
        from jarvis_cli.providers import get_provider
...
class TestOllamaCloudSuffixStripping:
    ...
    def test_no_duplicate_when_live_clean_and_mdev_suffixed(self, tmp_path, monkeypatch):
        ...
        with patch("jarvis_cli.models.fetch_api_models", return_value=["kimi-k2.6", "glm-5.1"]), \
             patch("brain.models_dev.fetch_models_dev", return_value=mock_mdev):
            result = fetch_ollama_cloud_models(force_refresh=True)
    ...
    def test_strip_suffix_helper(self):
        ...
        from jarvis_cli.models import _strip_ollama_cloud_suffix
