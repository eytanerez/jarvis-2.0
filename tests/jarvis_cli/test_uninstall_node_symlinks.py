"""Tests for jarvis_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $JARVIS_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Jarvis-managed node dir.
"""
...
def _make_jarvis_node(jarvis_home: Path) -> Path:
    """Create a fake $JARVIS_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = jarvis_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_jarvis_node(fake_home):
    jarvis_home = fake_home / ".jarvis"
    node_bin = _make_jarvis_node(jarvis_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(jarvis_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(jarvis_home) == []


def test_removes_dangling_symlink_into_jarvis_node(fake_home):
    """A link into the Jarvis node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Jarvis links that exist; ignores the ones that don't."""
    jarvis_home = fake_home / ".jarvis"
    node_bin = _make_jarvis_node(jarvis_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Jarvis-managed; node is a real user binary.
