...
from brain.models_dev import ModelInfo
from brain.usage_pricing import PricingEntry
...
def test_warns_when_pricing_entry_output_price_exceeds_threshold(monkeypatch):
    monkeypatch.setattr("brain.models_dev.get_model_info", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        "brain.usage_pricing.get_pricing_entry",
...
def test_openai_gpt55_pro_warns_for_nous_portal_pricing(monkeypatch):
    monkeypatch.setattr("brain.models_dev.get_model_info", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        "brain.usage_pricing.fetch_endpoint_model_metadata",
