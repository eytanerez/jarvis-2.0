"""Tests for the top-level `./jarvis` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./jarvis` should use `jarvis_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "jarvis"
    called = []

    fake_main_module = types.ModuleType("jarvis_cli.main")

    def fake_main():
        called.append("jarvis_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "jarvis_cli.main", fake_main_module)
