"""Tests for jarvis backup and import commands."""
...
def _make_jarvis_tree(root: Path) -> None:
    """Create a realistic ~/.jarvis directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "jarvis_state.db").write_bytes(b"fake-state")
    ...
    (root / "jarvis-agent" / "run_brain.py").write_text("# big file\n")
    ...
    (root / "logs" / "brain.log").write_text("log line\n")
...
class TestShouldExclude:
    def test_excludes_jarvis_agent(self):
        ...
        assert _should_exclude(Path("jarvis-brain/run_brain.py"))
        assert _should_exclude(Path("jarvis-brain/.git/HEAD"))
    ...
    def test_includes_logs(self):
        from jarvis_cli.backup import _should_exclude
        assert not _should_exclude(Path("logs/brain.log"))

    def test_includes_nested_jarvis_agent_in_skills(self):
        """skills/autonomous-ai-agents/jarvis-brain/ must NOT be excluded —
        only the root-level jarvis-brain/ repo is skipped."""
        from jarvis_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/jarvis-brain/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/jarvis-brain/sub/item.txt"))
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under JARVIS_HOME must be skipped —
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        # get_default_jarvis_root needs this
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            ...
            # Logs
            assert "logs/brain.log" in names
            # Skins
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import jarvis_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = jarvis_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, jarvis_home)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_jarvis_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include jarvis-brain/ directory."""
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        _make_jarvis_tree(jarvis_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "jarvis-agent" in n]
            assert agent_files == [], f"jarvis-agent files leaked into backup: {agent_files}"

    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under JARVIS_HOME must be
        ...
        venv_pkg = jarvis_home / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = jarvis_home / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_jarvis_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../jarvis-brain/ but NOT root jarvis-brain/."""
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        _make_jarvis_tree(jarvis_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root jarvis-agent must be excluded
            root_agent = [n for n in names if n.startswith("jarvis-brain/")]
            assert root_agent == [], f"root jarvis-agent leaked: {root_agent}"
            # Nested skill jarvis-agent must be included
            assert "skills/autonomous-ai-agents/jarvis-brain/SKILL.md" in names
            assert "skills/autonomous-ai-agents/jarvis-brain/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/jarvis-backup-*.zip."""
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        (jarvis_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("jarvis-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside JARVIS_HOME."""
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        _make_jarvis_tree(jarvis_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(jarvis_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Jarvis backup."""
        from jarvis_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only jarvis_state.db (old wrong name) is rejected."""
        from jarvis_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["jarvis_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into jarvis home."""
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (jarvis_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (jarvis_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (jarvis_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (jarvis_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_jarvis_prefix(self, tmp_path, monkeypatch):
        """Import strips .jarvis/ prefix if all entries share it."""
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".jarvis/config.yaml": "model: test\n",
            ".jarvis/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (jarvis_home / "config.yaml").read_text() == "model: test\n"
        assert (jarvis_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_jarvis_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a jarvis backup."""
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (jarvis_home / "config.yaml").exists()
        # traversal file should NOT exist outside jarvis home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        (jarvis_home / "gateway_state.json").write_text(live_state)
        ...
        assert (jarvis_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (jarvis_home / "config.yaml").exists()
        assert not (jarvis_home / "gateway_state.json").exists()
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        jarvis_home = tmp_path / ".jarvis"
        (jarvis_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (jarvis_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (jarvis_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            jarvis_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (jarvis_home / "gateway.pid").write_text("4242")
        (jarvis_home / "processes.json").write_text('{"live": true}')
        ...
        assert (jarvis_home / "gateway.pid").read_text() == "4242"
        assert (jarvis_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (jarvis_home / "cron.pid").exists()
        assert not (jarvis_home / "gateway.lock").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (jarvis_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (jarvis_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (jarvis_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (jarvis_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".jarvis"
        src_home.mkdir(parents=True)
        _make_jarvis_tree(src_home)

        monkeypatch.setenv("JARVIS_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from jarvis_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".jarvis"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("JARVIS_HOME", str(dst_home))
        ...
        assert (dst_home / "logs" / "brain.log").exists()

        # jarvis-agent should NOT be present
        assert not (dst_home / "jarvis-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from jarvis_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without jarvis markers fails validation."""
        import io
        from jarvis_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_jarvis(self):
        """Detects .jarvis/ prefix wrapping all entries."""
        import io
        from jarvis_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".jarvis/config.yaml", "test")
            zf.writestr(".jarvis/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".jarvis/"
...
class TestBackupEdgeCases:
    def test_nonexistent_jarvis_home(self, tmp_path, monkeypatch):
        """Backup exits when jarvis home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".jarvis"
        monkeypatch.setenv("JARVIS_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("jarvis-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_jarvis_home(self, tmp_path, monkeypatch):
        """Backup handles empty jarvis home (no files to back up)."""
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        # Only excluded dirs, no actual files
        (jarvis_home / "__pycache__").mkdir()
        (jarvis_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = jarvis_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = jarvis_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_jarvis(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside jarvis root."""
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        (jarvis_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside jarvis home
        out_zip = jarvis_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        (jarvis_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        (jarvis_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (jarvis_home / "config.yaml").exists()
        assert (jarvis_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (jarvis_home / "profiles" / "coder" / "config.yaml").exists()
        assert (jarvis_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "jarvis -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from jarvis_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def jarvis_home(self, tmp_path):
        """Create a fake JARVIS_HOME with critical state files."""
    ...
    def test_creates_snapshot(self, jarvis_home):
        from jarvis_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(jarvis_home=jarvis_home)
        assert snap_id is not None
        snap_dir = jarvis_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, jarvis_home):
        from jarvis_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", jarvis_home=jarvis_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, jarvis_home):
        from jarvis_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(jarvis_home=jarvis_home)
        db_copy = jarvis_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, jarvis_home):
        from jarvis_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(jarvis_home=jarvis_home)
        assert (jarvis_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_copies_channel_aliases(self, jarvis_home):
        from jarvis_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(jarvis_home=jarvis_home)
        copied = jarvis_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, jarvis_home):
        from jarvis_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(jarvis_home=jarvis_home)
        with open(jarvis_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(jarvis_home=empty) is None

    def test_list_snapshots(self, jarvis_home):
        from jarvis_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", jarvis_home=jarvis_home)
        id2 = create_quick_snapshot(label="second", jarvis_home=jarvis_home)

        snaps = list_quick_snapshots(jarvis_home=jarvis_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, jarvis_home):
        from jarvis_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", jarvis_home=jarvis_home)
        snaps = list_quick_snapshots(limit=3, jarvis_home=jarvis_home)
        assert len(snaps) == 3

    def test_restore_config(self, jarvis_home):
        from jarvis_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(jarvis_home=jarvis_home)

        (jarvis_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (jarvis_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, jarvis_home=jarvis_home)
        assert result is True
        assert "openrouter" in (jarvis_home / "config.yaml").read_text()

    def test_restore_state_db(self, jarvis_home):
        from jarvis_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(jarvis_home=jarvis_home)

        conn = sqlite3.connect(str(jarvis_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, jarvis_home=jarvis_home)
    ...
    def test_restore_nonexistent(self, jarvis_home):
        from jarvis_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", jarvis_home=jarvis_home) is False

    def test_auto_prune(self, jarvis_home):
        from jarvis_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", jarvis_home=jarvis_home)
        snaps = list_quick_snapshots(limit=100, jarvis_home=jarvis_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, jarvis_home):
        from jarvis_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", jarvis_home=jarvis_home)
        deleted = prune_quick_snapshots(keep=3, jarvis_home=jarvis_home)
        assert deleted == 7
        assert len(list_quick_snapshots(jarvis_home=jarvis_home)) == 3

    def test_snapshot_includes_pairing_directories(self, jarvis_home):
        ...
        (jarvis_home / "platforms" / "pairing").mkdir(parents=True)
        (jarvis_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (jarvis_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (jarvis_home / "pairing").mkdir()
        (jarvis_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (jarvis_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, jarvis_home):
        ...
        assert restore_quick_snapshot(snap_id, jarvis_home=jarvis_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, jarvis_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (jarvis update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``jarvis update``
    ...
    def jarvis_home(self, tmp_path):
        ...
        root.mkdir()
        _make_jarvis_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, jarvis_home):
        from jarvis_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(jarvis_home=jarvis_home)
        assert out is not None
        assert out.exists()
        assert out.parent == jarvis_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, jarvis_home):
        """Pre-update backup should include the same user data that
        ``jarvis backup`` would, and should exclude the same directories."""
        from jarvis_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(jarvis_home=jarvis_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # jarvis-agent repo excluded
        assert not any(n.startswith("jarvis-brain/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, jarvis_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from jarvis_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(jarvis_home=jarvis_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(jarvis_home=jarvis_home)
    ...
    def test_rotation_keeps_only_n(self, jarvis_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(jarvis_home=jarvis_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (jarvis_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, jarvis_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from jarvis_cli.backup import create_pre_update_backup

        (jarvis_home / "backups").mkdir(exist_ok=True)
        manual = jarvis_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(jarvis_home=jarvis_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from jarvis_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(jarvis_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, jarvis_home):
        ...
        """
        from jarvis_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(jarvis_home=jarvis_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, jarvis_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, jarvis_home):
        ...
        first = create_pre_update_backup(jarvis_home=jarvis_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(jarvis_home=jarvis_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(jarvis_home=jarvis_home, keep=0)
    ...
    def test_skips_symlinked_files(self, jarvis_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside JARVIS_HOME."""
        from jarvis_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def jarvis_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".jarvis"
        root.mkdir()
        _make_jarvis_tree(root)
        # Point JARVIS_HOME at the temp dir so config + backup paths resolve here
    ...
    def test_backup_flag_creates_backup(self, jarvis_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from jarvis_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "jarvis import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((jarvis_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_enabled_creates_backup(self, jarvis_home, capsys):
        """With the new safe default (``pre_update_backup: true``), every
        ``jarvis update`` creates a backup before any destructive step
        runs — the cost is a few minutes of zip time vs. the alternative
        of silent total data loss of ``~/.jarvis/`` observed in #48200
    ...
    def test_no_backup_flag_skips(self, jarvis_home, capsys):
        from jarvis_cli.main import _run_pre_update_backup
        ...
        # No backup written
        assert not (jarvis_home / "backups").exists() or not list(
            (jarvis_home / "backups").glob("pre-update-*.zip")
        )

    def test_config_enabled_creates_backup(self, jarvis_home, capsys):
        ...
        import yaml
        (jarvis_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("jarvis_cli.config"):
    ...
    def test_config_disabled_is_silent(self, jarvis_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((jarvis_home / "backups").glob("pre-update-*.zip")) \
            if (jarvis_home / "backups").exists() else True

    def test_cli_flag_overrides_enabled_config(self, jarvis_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (jarvis claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``jarvis claw migrate`` runs before mutating ~/.jarvis/."""
    ...
    def test_creates_backup_under_backups_dir(self, jarvis_home):
        from jarvis_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(jarvis_home=jarvis_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `jarvis
        # import` and the update-backup listing both pick them up.
        assert out.parent == jarvis_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, jarvis_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``jarvis backup`` / ``create_pre_update_backup`` — no drift."""
        from jarvis_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(jarvis_home=jarvis_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_jarvis_import(self, jarvis_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Jarvis
        backup — `jarvis import` should accept it."""
        from jarvis_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(jarvis_home=jarvis_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, jarvis_home):
        from jarvis_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(jarvis_home=jarvis_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(jarvis_home=jarvis_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, jarvis_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(jarvis_home=jarvis_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((jarvis_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_jarvis_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.jarvis yet — nothing to back up."""
        from jarvis_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(jarvis_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, jarvis_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from jarvis_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(jarvis_home=jarvis_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(jarvis_home=jarvis_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`jarvis update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, jarvis_home: Path, label="pre-update"):
        from jarvis_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, jarvis_home=jarvis_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from jarvis_cli.backup import restore_cron_jobs_if_emptied
        jarvis_home = tmp_path / ".jarvis"
        jobs_path = jarvis_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(jarvis_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, jarvis_home=jarvis_home)
        assert result is not None
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = jarvis_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, jarvis_home=jarvis_home) is None
        assert restore_cron_jobs_if_emptied("", jarvis_home=jarvis_home) is None
