"""Tests for jarvis_cli.gui_uninstall — GUI-only uninstall + install discovery.

Covers the cross-platform artifact discovery, the brain/GUI detection the
...
import jarvis_cli.gui_uninstall as gu


def _make_agent(jarvis_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = jarvis_home / "jarvis-agent"
    (agent_root / "jarvis_cli").mkdir(parents=True)
    (agent_root / "jarvis_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(jarvis_home: Path) -> None:
    """Create the source-built GUI artifacts a `jarvis desktop` run produces."""
    desktop = jarvis_home / "jarvis-agent" / "apps" / "desktop"
    ...
    (jarvis_home / "jarvis-agent" / "node_modules").mkdir(parents=True)
    (jarvis_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(jarvis_home: Path) -> None:
    (jarvis_home / "config.yaml").write_text("x: 1\n")
    (jarvis_home / ".env").write_text("KEY=secret\n")
    (jarvis_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    jarvis_home = tmp_path / ".jarvis"
    jarvis_home.mkdir()
    assert gu.agent_is_installed(jarvis_home) is False
    _make_agent(jarvis_home)
    assert gu.agent_is_installed(jarvis_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    jarvis_home = tmp_path / ".jarvis"
    (jarvis_home / "jarvis-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(jarvis_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    jarvis_home = tmp_path / ".jarvis"
    _make_gui_build(jarvis_home)
    artifacts = gu.source_built_gui_artifacts(jarvis_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(jarvis_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(jarvis_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    jarvis_home = tmp_path / ".jarvis"
    agent_root = _make_agent(jarvis_home)
    _make_gui_build(jarvis_home)
    _make_user_data(jarvis_home)
    ...
    removed = gu.uninstall_gui(jarvis_home)
    ...
    assert not (jarvis_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "jarvis_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (jarvis_home / "config.yaml").exists()
    assert (jarvis_home / ".env").exists()
    assert (jarvis_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    jarvis_home = tmp_path / ".jarvis"
    _make_agent(jarvis_home)
    userdata = tmp_path / "Jarvis-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(jarvis_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(jarvis_home)
    ...
    assert summary["jarvis_home"] == str(jarvis_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Jarvis"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Jarvis"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Jarvis"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Jarvis"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    JARVIS_HOME with all the destructive externals stubbed out.
    """
    import jarvis_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_jarvis_home", lambda: jarvis_home)
    ...
    from jarvis_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (jarvis_home / "config.yaml").exists()
    assert jarvis_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole JARVIS_HOME non-interactively."""
    import jarvis_cli.uninstall as uninstall

    jarvis_home = tmp_path / ".jarvis"
    (jarvis_home / "jarvis-agent" / "jarvis_cli").mkdir(parents=True)
    (jarvis_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
...
def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m jarvis_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_jarvis_home", lambda: jarvis_home)
    ...
    assert not (desktop / "release").exists()
    assert not (jarvis_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "jarvis_cli").exists()
    assert (jarvis_home / "config.yaml").exists()
