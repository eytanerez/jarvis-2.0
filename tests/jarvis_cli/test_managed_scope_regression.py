...
def jarvis_home(tmp_path, monkeypatch):
    ...
    # /etc/jarvis on the dev/CI box can't influence the test.
    monkeypatch.setenv("JARVIS_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(jarvis_home, monkeypatch):
    from jarvis_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(jarvis_home, monkeypatch):
    from jarvis_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(jarvis_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_jarvis_dotenv(jarvis_home=str(home))
