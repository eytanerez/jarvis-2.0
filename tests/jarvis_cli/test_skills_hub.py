...
from jarvis_cli.skills_hub import do_check, do_install, do_list, do_update, handle_skills_slash
...
def _capture_update(monkeypatch, results) -> tuple[str, list[tuple[str, str, bool]]]:
    import tools.skills_hub as hub
    import jarvis_cli.skills_hub as cli_hub
...
def test_do_list_platform_env_is_ignored(three_source_env, monkeypatch):
    """`jarvis skills list` reads the active profile's config via
    JARVIS_HOME (swapped by -p), so it must NOT pass a platform arg to
    ``get_disabled_skill_names`` — otherwise per-platform overrides
    would silently leak in from JARVIS_PLATFORM env."""
    from brain import skill_utils
...
def test_do_install_scans_official_bundles_with_source_provenance(
    ...
    class _OfficialSource:
        def inspect(self, identifier):
            return type("Meta", (), {
                "extra": {},
                "identifier": "official/brain/prunus-gaia",
            })()
    ...
    do_install("official/brain/prunus-gaia", console=console, skip_confirm=True)
...
def test_existing_categories_skips_top_level_skills(monkeypatch, tmp_path, hub_env):
    import tools.skills_hub as hub
    from jarvis_cli.skills_hub import _existing_categories
...
def test_browse_skills_dedup_uses_identifier_not_name(monkeypatch):
    ...
    # so the patch must target the source module, not jarvis_cli.skills_hub.
...
def test_do_browse_reports_live_per_source_progress():
    ...
    from jarvis_cli.skills_hub import do_browse
    from tools.skills_hub import SkillMeta
...
# ---------------------------------------------------------------------------
# Regression: full identifier must be recoverable from `jarvis skills search`
...
def test_do_search_identifier_column_does_not_truncate_long_slug():
    ...
    """
    from jarvis_cli.skills_hub import do_search
