...
def test_ensure_dependency_skips_when_present():
    ...
    with patch("jarvis_cli.dep_ensure.shutil") as mock_shutil:
...
def test_ensure_dependency_returns_false_when_missing_noninteractive():
    ...
    with patch("jarvis_cli.dep_ensure.shutil") as mock_shutil:
        ...
        with patch("jarvis_cli.dep_ensure._find_install_script", return_value=(None, None)):
            result = ensure_dependency("node", interactive=False)
...
def test_find_install_script_from_checkout(tmp_path):
    ...
    with patch("jarvis_cli.dep_ensure._IS_WINDOWS", False):
        path, shell = _find_install_script(package_dir=tmp_path / "jarvis_cli", repo_root=tmp_path)
...
def test_find_install_script_from_wheel(tmp_path):
    ...
    from jarvis_cli.dep_ensure import _find_install_script
    bundled = tmp_path / "jarvis_cli" / "scripts"
...
def test_find_install_script_prefers_ps1_on_windows(tmp_path):
    ...
    scripts_dir = tmp_path / "jarvis_cli" / "scripts"
    ...
    with patch("jarvis_cli.dep_ensure._IS_WINDOWS", True):
        path, shell = _find_install_script(package_dir=tmp_path / "jarvis_cli")
...
def test_has_system_browser_checks_windows_names():
    from jarvis_cli.dep_ensure import _has_system_browser
    with patch("jarvis_cli.dep_ensure._IS_WINDOWS", True), \
...
def test_has_jarvis_agent_browser_windows_path(tmp_path):
...
def test_has_jarvis_agent_browser_posix_path(tmp_path):
...
def test_has_jarvis_agent_browser_legacy_node_modules_path(tmp_path):
    """Legacy git-clone installs put agent-browser in $JARVIS_HOME/node_modules/.bin/."""
