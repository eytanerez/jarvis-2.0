"""Tests for IRC gateway configuration via `jarvis setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Jarvis install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Jarvis install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#jarvis")
            monkeypatch.setenv("IRC_NICKNAME", "jarvis-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `jarvis setup gateway` experience with IRC present."""
