...
``jarvis_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``jarvis --help``, ``jarvis version``,
``jarvis logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``jarvis --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from jarvis_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["jarvis", "-m", "gpt5", "chat"], "chat"),
        (["jarvis", "--model", "gpt5", "chat", "hi"], "chat"),
        (["jarvis", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["jarvis", "-z", "hello world"], None),
        (["jarvis", "-z", "hello", "chat"], "chat"),
        (["jarvis", "--model=gpt5", "chat"], "chat"),     # inline form
        (["jarvis", "--", "chat"], "chat"),               # -- terminator
        (["jarvis", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["jarvis", "some-plugin-cmd"], "some-plugin-cmd"),
        (["jarvis", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["jarvis", "meet", "join"],          # potential google_meet plugin
        ["jarvis", "honcho", "status"],      # potential memory plugin
        ["jarvis", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``jarvis help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"jarvis_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
