"""Tests for jarvis_cli.profile_distribution — git-based profile installs.
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_jarvis_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Jarvis"):
                check_jarvis_requires(spec, cur)
...
class TestEnvTemplate:
    ...
    def test_empty_env_requires_is_header_only(self):
        m = DistributionManifest(name="x")
        out = _env_template_from_manifest(m)
        assert "Jarvis distribution" in out
...
class TestInstall:
    ...
    def test_install_enforces_jarvis_requires(self, profile_env, monkeypatch):
        # Pin current Jarvis version to something well below the requirement
        import jarvis_cli
        monkeypatch.setattr(jarvis_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            jarvis_requires=">=99.0.0",
        )
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "jarvis_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from jarvis_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_plain_profile_has_no_distribution_fields(self, profile_env):
        from jarvis_cli.profiles import create_profile, list_profiles
        create_profile(name="plain", no_alias=True)
        rows = {p.name: p for p in list_profiles()}
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from jarvis_cli.profiles import create_profile, list_profiles, get_profile_dir
