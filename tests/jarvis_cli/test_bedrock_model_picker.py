...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (JARVIS_OVERLAYS) — bedrock
...
class TestProviderModelIdsBedrock:
    ...
    def test_returns_live_discovered_model_ids(self, monkeypatch):
        ...
        with patch("brain.bedrock_adapter.discover_bedrock_models", side_effect=_mock_discover), \
             patch("brain.bedrock_adapter.resolve_bedrock_region", return_value="eu-central-1"):
    ...
    def test_region_determines_model_ids(self, monkeypatch):
        ...
        with patch("brain.bedrock_adapter.discover_bedrock_models", side_effect=_mock_discover):
            with patch("brain.bedrock_adapter.resolve_bedrock_region", return_value="eu-central-1"):
                eu_result = provider_model_ids("bedrock")
            with patch("brain.bedrock_adapter.resolve_bedrock_region", return_value="us-east-1"):
    ...
    def test_falls_back_to_static_list_when_discovery_empty(self, monkeypatch):
        ...
        with patch("brain.bedrock_adapter.discover_bedrock_models", return_value=[]), \
             patch("brain.bedrock_adapter.resolve_bedrock_region", return_value="eu-central-1"):
            result = provider_model_ids("bedrock")
    ...
    def test_falls_back_to_static_list_on_exception(self, monkeypatch):
        ...
        with patch("brain.bedrock_adapter.discover_bedrock_models",
    ...
    def test_accepts_bedrock_aliases(self, monkeypatch):
        ...
        with patch("brain.bedrock_adapter.discover_bedrock_models", side_effect=_mock_discover), \
             patch("brain.bedrock_adapter.resolve_bedrock_region", return_value="us-east-1"):
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via JARVIS_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestListAuthenticatedProvidersBedrock:
    ...
    def test_bedrock_appears_with_aws_profile(self, monkeypatch):
        ...
        with patch("brain.bedrock_adapter.has_aws_credentials", return_value=True), \
             patch("brain.bedrock_adapter.discover_bedrock_models", side_effect=_mock_discover), \
             patch("brain.bedrock_adapter.resolve_bedrock_region", return_value="eu-central-1"):
    ...
    def test_bedrock_total_models_matches_discovery(self, monkeypatch):
        ...
        with patch("brain.bedrock_adapter.has_aws_credentials", return_value=True), \
             patch("brain.bedrock_adapter.discover_bedrock_models", return_value=_EU_MODELS), \
             patch("brain.bedrock_adapter.resolve_bedrock_region", return_value="eu-central-1"):
    ...
    def test_non_bedrock_picker_does_not_probe_full_aws_chain(self, monkeypatch):
        ...
        with patch("brain.bedrock_adapter.has_aws_credentials", side_effect=_has_aws_credentials):
    ...
    def test_bedrock_falls_back_to_curated_when_discovery_fails(self, monkeypatch):
        ...
        with patch("brain.bedrock_adapter.has_aws_credentials", return_value=True), \
             patch("brain.bedrock_adapter.discover_bedrock_models",
...
class TestBedrockOverlayRegistration:
    """bedrock entry in JARVIS_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from jarvis_cli.providers import JARVIS_OVERLAYS
        assert "bedrock" in JARVIS_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from jarvis_cli.providers import JARVIS_OVERLAYS
        assert JARVIS_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from jarvis_cli.providers import JARVIS_OVERLAYS
        assert JARVIS_OVERLAYS["bedrock"].auth_type == "aws_sdk"
    ...
    def test_bedrock_aliases_resolve(self):
        from jarvis_cli.providers import normalize_provider
