...
``jarvis_cli.runtime_provider.resolve_runtime_provider`` already strips
``/v1`` at fresh agent init (PR #4918), but the ``/model`` mid-session
switch path in ``jarvis_cli.model_switch.switch_model`` was missing the
...
def _run_opencode_switch(
    ...
    with (
        patch("jarvis_cli.model_switch.resolve_alias", return_value=None),
...
class TestAgentSwitchModelDefenseInDepth:
    """run_brain.AIBrain.switch_model() also strips /v1 as defense-in-depth."""
    ...
    def test_agent_switch_model_strips_v1_for_anthropic_messages(self):
        ...
        ), patch("brain.anthropic_adapter.resolve_anthropic_token", return_value=""), patch(
            "brain.anthropic_adapter._is_oauth_token", return_value=False
        ):
        ...
        assert captured.get("base_url") == "https://opencode.ai/zen/go", (
            f"brain.switch_model did not strip /v1; passed {captured.get('base_url')} "
...
class TestStaleConfigDefaultDoesNotWedgeResolver:
    ...
    def test_kimi_switch_keeps_v1_despite_claude_config_default(self, tmp_path, monkeypatch):
        ...
        # Re-import with the new JARVIS_HOME so config cache is fresh.
        import jarvis_cli.config as _cfg_mod
        importlib.reload(_cfg_mod)
        import jarvis_cli.runtime_provider as _rp_mod
        importlib.reload(_rp_mod)
        import jarvis_cli.model_switch as _ms_mod
