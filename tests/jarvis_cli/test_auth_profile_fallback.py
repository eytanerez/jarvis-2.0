...
When ``JARVIS_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.jarvis/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.jarvis            (has its own auth.json fixture)
    * Profile     -> tmp_path/.jarvis/profiles/coder   (active, JARVIS_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Jarvis is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from jarvis_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    jarvis_home = tmp_path / "classic"
    jarvis_home.mkdir()
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))

    _write(jarvis_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (JARVIS_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.jarvis as the "real home" — which is NOT equal
    # to the JARVIS_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    jarvis_home = tmp_path / "classic"
    ...
    _write(jarvis_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from jarvis_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: JARVIS_HOME is set to a custom path that is NOT under
    # ~/.jarvis/profiles/ — get_default_jarvis_root() returns JARVIS_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from jarvis_cli.auth import read_credential_pool, write_credential_pool
