...
JARVIS_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot") while
_PROVIDER_MODELS and config.yaml use Jarvis IDs ("copilot").  The slug
...
def test_copilot_uses_jarvis_slug():
...
def test_kimi_for_coding_overlay_uses_jarvis_slug():
...
def test_kilo_overlay_uses_jarvis_slug():
...
def test_mapped_provider_credential_pool_visibility(monkeypatch):
    ...
    monkeypatch.setattr("brain.models_dev.fetch_models_dev", lambda: {"google-ai-studio": {"env": ["GEMINI_API_KEY"]}})
    monkeypatch.setattr("brain.models_dev.PROVIDER_TO_MODELS_DEV", {"gemini": "google-ai-studio"})
    monkeypatch.setattr(
        "jarvis_cli.auth._load_auth_store",
