...
Bug 1 — OpenAI picker dumped the raw ``/v1/models`` catalog
    ...
    moderation, gpt-3.5, …). The ``jarvis model`` CLI shows only the curated
...
Bug 2 — OpenRouter appeared authenticated whenever OPENAI_API_KEY was set
    OpenRouter's JarvisOverlay carried ``extra_env_vars=("OPENAI_API_KEY",)``.
...
from jarvis_cli import models as M
...
def test_openrouter_overlay_does_not_list_openai_api_key():
    overlay = JARVIS_OVERLAYS["openrouter"]
