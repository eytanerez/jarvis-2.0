"""Regression tests for TOCTOU-safe credential file writers in ``jarvis_cli.auth``.
...
shipped for ``brain/google_oauth.py`` (#19673) and ``tools/mcp_oauth.py``
(#21148).
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.jarvis/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
def test_shared_nous_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    ...
    # home" reference — which currently tracks JARVIS_HOME via
    # get_default_jarvis_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("JARVIS_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from jarvis_cli import auth as auth_mod
