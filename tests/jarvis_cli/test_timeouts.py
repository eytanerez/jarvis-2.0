...
def test_resolved_api_call_timeout_priority(monkeypatch, tmp_path):
    """AIBrain._resolved_api_call_timeout() honors config > env > default priority."""
    # Isolate JARVIS_HOME
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
          ...
          openrouter:
            ...
            models:
              openai/gpt-4o-mini:
                timeout_seconds: 42
        """)
    monkeypatch.setenv("JARVIS_API_TIMEOUT", "999")
    ...
    from jarvis_cli import timeouts as to_mod
    importlib.reload(to_mod)
    import run_brain as ra_mod
    importlib.reload(ra_mod)

    agent2 = ra_mod.AIBrain(
        model="some/model",
        provider="openrouter",
        api_key="sk-dummy",
        base_url="https://openrouter.ai/api/v1",
...
def test_resolved_api_call_stale_timeout_priority(monkeypatch, tmp_path):
          ...
          openai-codex:
            ...
            models:
              gpt-5.4:
                stale_timeout_seconds: 1800
        """)
    monkeypatch.setenv("JARVIS_API_CALL_STALE_TIMEOUT", "999")
    ...
    monkeypatch.delenv("JARVIS_API_CALL_STALE_TIMEOUT", raising=False)
...
def test_explicit_non_stream_stale_timeout_is_honored_for_local_endpoints(monkeypatch, tmp_path):
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path))
    (tmp_path / ".env").write_text("", encoding="utf-8")
    monkeypatch.setenv("JARVIS_API_CALL_STALE_TIMEOUT", "300")
