...
Bug: `brain/redact.py` snapshots `_REDACT_ENABLED` from the env var
...
in .env), the toggle was silently ignored in both `jarvis chat` and
`jarvis gateway run`.
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    env.pop("JARVIS_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env JARVIS_REDACT_SECRETS takes precedence over config.yaml."""
    jarvis_home = tmp_path / ".jarvis"
    jarvis_home.mkdir()
    (jarvis_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (jarvis_home / ".env").write_text("JARVIS_REDACT_SECRETS=true\n")
