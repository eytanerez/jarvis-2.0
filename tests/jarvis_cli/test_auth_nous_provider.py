...
from jarvis_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from jarvis_cli.auth import _resolve_verify
    ...
    def test_missing_jarvis_ca_bundle_env_falls_back(self, monkeypatch):
        from jarvis_cli.auth import _resolve_verify

        monkeypatch.setenv("JARVIS_CA_BUNDLE", "/nonexistent/jarvis-ca.pem")
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    monkeypatch.setenv("JARVIS_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="jarvis_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    from brain.credential_pool import PooledCredential, load_pool
    pool = load_pool("nous")
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
...
def test_get_nous_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from jarvis_cli.auth import get_nous_auth_status, invalidate_nous_auth_status_cache
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    jarvis_home = tmp_path / "jarvis"
    _setup_nous_auth(jarvis_home, access_token="at-123")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    jarvis_home = tmp_path / "jarvis"
    _setup_nous_auth(jarvis_home, access_token="at-fresh")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("jarvis_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_refreshed_jwt_lacks_invoke_scope(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("jarvis_cli.auth._refresh_access_token", _fake_refresh_access_token)
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    from jarvis_cli import auth as auth_mod
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    jarvis_home = tmp_path / "jarvis"
    _setup_nous_auth(jarvis_home, refresh_token="refresh-old")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    from brain.credential_pool import load_pool
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `jarvis model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return jarvis_home, config_path, auth_path
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from jarvis_cli.auth import PROVIDER_REGISTRY, _login_nous

        jarvis_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `jarvis auth add nous`
    wrote only the pool. After the Nous agent_key's 24h TTL expired, the
    401-recovery path in run_brain.py called resolve_nous_runtime_credentials
    ...
    """
    from jarvis_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Jarvis is not logged into Nous Portal").

    This is the exact path that run_brain.py's `_try_refresh_nous_client_credentials`
    ...
    # doesn't raise "Jarvis is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `jarvis auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Jarvis must surface an
    ...
    re-authenticate" string led users to report this as a Jarvis persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from jarvis_cli.auth import _refresh_access_token
    ...
    assert "jarvis auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect JARVIS_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("JARVIS_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without JARVIS_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.jarvis/shared/`` across CI runs.
    """
    from jarvis_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("JARVIS_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """JARVIS_SHARED_AUTH_DIR must redirect the path."""
    from jarvis_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("JARVIS_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from jarvis_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from jarvis_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from jarvis_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `jarvis auth add nous
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from jarvis_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "jarvis-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different JARVIS_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("JARVIS_HOME", str(profile_a))
    ...
    # Profile B: fresh JARVIS_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("JARVIS_HOME", str(profile_b))
