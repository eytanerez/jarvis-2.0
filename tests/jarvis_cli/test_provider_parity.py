...
the SAME provider universe as ``jarvis model`` (the CLI/TUI picker).
...
def test_every_jarvis_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `jarvis model` universe."""
    ...
    assert not missing, (
        "providers shown in `jarvis model` but not configurable in the desktop "
