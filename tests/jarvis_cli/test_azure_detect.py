"""Tests for jarvis_cli.azure_detect — transport & model auto-detection."""
...
def test_http_get_json_on_urlerror_returns_zero_none():
    ...
    import urllib.error
    with patch("jarvis_cli.azure_detect.urllib_request.urlopen",
...
def test_http_get_json_on_http_error_returns_code_none():
    ...
    with patch("jarvis_cli.azure_detect.urllib_request.urlopen", side_effect=err):
...
def test_lookup_context_length_returns_known():
    ...
    with patch("brain.model_metadata.get_model_context_length", fake), \
         patch("brain.model_metadata.DEFAULT_FALLBACK_CONTEXT", 128000):
...
def test_lookup_context_length_returns_none_on_fallback():
    ...
    with patch("brain.model_metadata.get_model_context_length", return_value=128000), \
         patch("brain.model_metadata.DEFAULT_FALLBACK_CONTEXT", 128000):
        n = azure_detect.lookup_context_length(
...
def test_lookup_context_length_swallows_exceptions():
    ...
    with patch("brain.model_metadata.get_model_context_length",
