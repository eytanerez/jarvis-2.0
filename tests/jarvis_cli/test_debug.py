"""Tests for ``jarvis debug`` CLI command and debug utilities."""
...
def jarvis_home(tmp_path, monkeypatch):
    """Set up an isolated JARVIS_HOME with minimal logs."""
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO jarvis_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from jarvis_cli.debug import _upload_paste_rs
        ...
        with patch("jarvis_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "jarvis_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from jarvis_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from jarvis_cli.debug import upload_to_pastebin

        with patch("jarvis_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("jarvis_cli.debug._upload_paste_rs",
             ...
             patch("jarvis_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, jarvis_home):
        from jarvis_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, jarvis_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (jarvis_home / "logs" / "brain.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, jarvis_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = jarvis_home / "logs" / "brain.log"
        from jarvis_cli import debug
    ...
    def test_truncates_large_file(self, jarvis_home):
        """Files larger than max_bytes get tail-truncated."""
        from jarvis_cli.debug import _capture_log_snapshot

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (jarvis_home / "logs" / "brain.log").write_text(big_content * 200)

        snap = _capture_log_snapshot("agent", tail_lines=10, max_bytes=1024)
        assert snap.full_text is not None
        assert "truncated" in snap.full_text

    def test_keeps_first_line_when_truncation_on_boundary(self, jarvis_home):
        ...
        (jarvis_home / "logs" / "brain.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, jarvis_home):
    ...
    def test_unknown_log_returns_none(self, jarvis_home):
        from jarvis_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, jarvis_home):
    ...
    def test_prefers_primary_over_rotated(self, jarvis_home):
    ...
    def test_falls_back_when_primary_empty(self, jarvis_home):
        """Empty primary log falls back to .1 rotation."""
        from jarvis_cli.debug import _capture_log_snapshot

        logs_dir = jarvis_home / "logs"
        (logs_dir / "brain.log").write_text("")
        (logs_dir / "brain.log.1").write_text("rotated agent data\n")
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of JARVIS_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def jarvis_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated JARVIS_HOME whose brain.log contains a vendor-prefixed token."""
        home = tmp_path / ".jarvis"
        home.mkdir()
        monkeypatch.setenv("JARVIS_HOME", str(home))
    ...
    def test_default_redacts_tail_and_full_text(self, jarvis_home_with_secret):
        from jarvis_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, jarvis_home_with_secret):
        from jarvis_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, jarvis_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when JARVIS_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("JARVIS_REDACT_SECRETS", "false")

        from jarvis_cli.debug import _capture_log_snapshot

        assert os.environ.get("JARVIS_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, jarvis_home_with_secret
    ):
        from jarvis_cli.debug import _capture_log_snapshot

        log_path = jarvis_home_with_secret / "logs" / "brain.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, jarvis_home_with_secret):
        from jarvis_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from jarvis_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, jarvis_home):
        from jarvis_cli.debug import collect_debug_report

        with patch("jarvis_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- jarvis dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- jarvis dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, jarvis_home):
        from jarvis_cli.debug import collect_debug_report

        with patch("jarvis_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- brain.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, jarvis_home):
        from jarvis_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, jarvis_home):
        from jarvis_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, jarvis_home):
        from jarvis_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, jarvis_home):
        from jarvis_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, jarvis_home, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from jarvis_cli.debug import run_debug_share
        ...
        with patch("jarvis_cli.dump.run_dump"), \
             patch("jarvis_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("jarvis_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, jarvis_home, capsys):
        ...
        with patch("jarvis_cli.dump.run_dump"), \
             patch(
                 "jarvis_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, jarvis_home, capsys):
        ...
        assert "--- brain.log" in out
        assert "FULL brain.log" in out
        assert "FULL gateway.log" in out

    def test_share_uploads_five_pastes(self, jarvis_home, capsys):
        """Successful share uploads report + brain.log + gateway.log + gui.log + desktop.log."""
        from jarvis_cli.debug import run_debug_share
        ...
        with patch("jarvis_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- jarvis dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)

        out = capsys.readouterr().out
        # Should have 5 uploads: report, brain.log, gateway.log, gui.log, desktop.log
        assert call_count[0] == 5
        assert "paste.rs/paste1" in out  # Report
        assert "paste.rs/paste2" in out  # brain.log
        ...
        assert "--- jarvis dump ---" in agent_paste
        assert "--- full brain.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- jarvis dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste
        gui_paste = uploaded_content[3]
        assert "--- jarvis dump ---" in gui_paste
        assert "--- full gui.log ---" in gui_paste
        desktop_paste = uploaded_content[4]
        assert "--- jarvis dump ---" in desktop_paste
        assert "--- full desktop.log ---" in desktop_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, jarvis_home, capsys):
        """A mid-run rotation must not make full brain.log older than the report."""
        from jarvis_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report

        logs_dir = jarvis_home / "logs"
        (logs_dir / "brain.log").write_text(
            "2026-04-22 12:00:00 INFO agent: newest line\n"
        )
        (logs_dir / "brain.log.1").write_text(
        ...
        def _wrapped_collect_debug_report(*, log_lines=200, dump_text="", log_snapshots=None):
            ...
            # before the old implementation would have re-read brain.log for
            # standalone upload.
            (logs_dir / "brain.log").write_text("")
            (logs_dir / "brain.log.1").write_text(
                "2026-04-10 12:00:00 INFO agent: old rotated line\n"
        ...
        with patch("jarvis_cli.dump.run_dump"), \
             patch("jarvis_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("jarvis_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, jarvis_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, jarvis_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, jarvis_home_with_secret, capsys
    ):
        ...
        with patch("jarvis_cli.dump.run_dump"), \
             patch("jarvis_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("jarvis_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
    ...
    def test_no_redact_flag_disables_redaction_and_banner(
        ...
        # The brain.log paste should now contain the raw token.
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "jarvis debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, jarvis_home):
        from jarvis_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from jarvis_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from jarvis_cli.debug import delete_paste
        ...
        with patch("jarvis_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.jarvis/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``jarvis debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, jarvis_home):
        ...
        import ast
        import inspect
        from jarvis_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, jarvis_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from jarvis_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, jarvis_home):
    ...
    def test_merges_with_existing_pending(self, jarvis_home):
        """Subsequent calls merge into existing pending.json."""
        from jarvis_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, jarvis_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, jarvis_home):
    ...
    def test_sweep_deletes_expired_entries(self, jarvis_home):
        from jarvis_cli.debug import (
        ...
        with patch("jarvis_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, jarvis_home):
        from jarvis_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("jarvis_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, jarvis_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, jarvis_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, jarvis_home):
        from jarvis_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("jarvis_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, jarvis_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: jarvis debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from jarvis_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("jarvis_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("jarvis_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, jarvis_home, capsys):
        from jarvis_cli.debug import run_debug_share
        ...
        with patch("jarvis_cli.dump.run_dump"), \
             ...
             patch("jarvis_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, jarvis_home, capsys):
        from jarvis_cli.debug import run_debug_share
        ...
        with patch("jarvis_cli.dump.run_dump"), \
             patch("jarvis_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("jarvis_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, jarvis_home, capsys):
        from jarvis_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``jarvis debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, jarvis_home):
        from jarvis_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("jarvis_cli.dump.run_dump"), patch(
            "jarvis_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("jarvis_cli.debug._schedule_auto_delete"):
            result = build_debug_share(log_lines=50, redact=True)

        assert isinstance(result, DebugShareResult)
        # All four seeded logs (brain/gateway/desktop) + the summary report.
        assert "Report" in result.urls
        assert "brain.log" in result.urls
    ...
    def test_skips_missing_logs_without_failure(self, jarvis_home):
        from jarvis_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (jarvis_home / "logs" / "desktop.log").unlink()

        with patch("jarvis_cli.dump.run_dump"), patch(
            "jarvis_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, jarvis_home):
        from jarvis_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, jarvis_home):
        from jarvis_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, jarvis_home):
        from jarvis_cli.debug import build_debug_share
