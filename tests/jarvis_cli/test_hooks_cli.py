"""Tests for the ``jarvis hooks`` CLI subcommand."""
...
from jarvis_cli import hooks as hooks_cli


@pytest.fixture(autouse=True)
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path / "home"))
    monkeypatch.delenv("JARVIS_ACCEPT_HOOKS", raising=False)
...
class TestHooksTest:
    def test_synthetic_payload_matches_production_shape(self, tmp_path):
        """`jarvis hooks test` must feed the script stdin in the same
        shape invoke_hook() would at runtime.  Prior to this fix,
        run_once bypassed _serialize_payload and the two paths diverged —
        scripts tested with `jarvis hooks test` saw different top-level
...
class TestHooksDoctor:
    ...
    def test_flags_mtime_drift(self, tmp_path, monkeypatch):
        ...
        from brain.shell_hooks import allowlist_path
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `jarvis hooks doctor` used to run every
