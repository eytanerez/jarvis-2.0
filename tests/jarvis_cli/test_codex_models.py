...
from jarvis_cli.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
...
def test_get_codex_model_ids_prioritizes_default_and_cache(tmp_path, monkeypatch):
    ...
    # backend still accepts it via the OAuth-backed CLI/Jarvis route.
...
def test_model_command_uses_runtime_access_token_for_codex_list(monkeypatch):
    from jarvis_cli.main import _model_flow_openai_codex
    ...
    monkeypatch.setattr(
        "jarvis_cli.auth.get_codex_auth_status",
...
def test_model_command_prompts_to_reuse_or_reauthenticate_codex_session(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "jarvis_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True, "source": "jarvis-auth-store"},
    )
    monkeypatch.setattr(
        "jarvis_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("jarvis_cli.auth._login_openai_codex", _fake_login)
    monkeypatch.setattr(
        "jarvis_cli.codex_models.get_codex_model_ids",
...
def test_model_command_uses_existing_codex_session_without_relogin(monkeypatch):
    ...
    monkeypatch.setattr(
        "jarvis_cli.auth._login_openai_codex",
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    ...
    with (
        ...
        cli = JarvisCLI(model=model, **kwargs)
    return cli
