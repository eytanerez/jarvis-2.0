...
- Bare `jarvis setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `jarvis setup --quick` runs the narrower "fill in missing items" flow.
- `jarvis setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("jarvis_cli.setup.ensure_jarvis_home", {}),
        ("jarvis_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("jarvis_cli.config.is_managed", {"return_value": False}),
        ("jarvis_cli.setup.load_config", {"return_value": {}}),
        ("jarvis_cli.setup.save_config", {}),
        ("jarvis_cli.setup.get_env_value", {"return_value": None}),
        ("jarvis_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("jarvis_cli.setup._print_setup_summary", {}),
        ("jarvis_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("jarvis_cli.setup.load_config", {"return_value": {}}),
        ("jarvis_cli.setup.save_config", {}),
        ("jarvis_cli.auth.get_active_provider", {"return_value": None}),
        ("jarvis_cli.setup.get_env_value", {"return_value": None}),
        ("jarvis_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `jarvis setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="jarvis_cli.setup.prompt_choice",
                quick="jarvis_cli.setup._run_quick_setup",
                model="jarvis_cli.setup.setup_model_provider",
                terminal="jarvis_cli.setup.setup_terminal_backend",
                agent="jarvis_cli.setup.setup_agent_settings",
                gateway="jarvis_cli.setup.setup_gateway",
                tools="jarvis_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`jarvis setup --reconfigure` behaves the same as bare `jarvis setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("jarvis_cli.setup.prompt_choice", {"return_value": 0}),
                first="jarvis_cli.setup._run_first_time_quick_setup",
            )
            from jarvis_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "jarvis_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["jarvis", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["jarvis", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["jarvis", "setup"])
        try:
            main()
        except SystemExit:
            pass
