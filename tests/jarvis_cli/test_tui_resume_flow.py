...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["jarvis", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "jarvis_cli.plugins",
...
def test_termux_fast_tui_launch_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["jarvis", "--tui", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
        main_mod,
        "cmd_chat",
...
def test_termux_fast_tui_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(sys, "argv", ["jarvis", "--tui", "--help"])
...
def test_fast_tui_launch_is_termux_only(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["jarvis", "--tui"])
...
def test_termux_fast_cli_launch_chat_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("JARVIS_TUI", raising=False)
    monkeypatch.setattr(
        sys, "argv", ["jarvis", "chat", "-q", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
...
def test_termux_fast_cli_launch_bare_defers_agent_startup(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("JARVIS_TUI", raising=False)
    monkeypatch.delenv("JARVIS_DEFER_AGENT_STARTUP", raising=False)
    monkeypatch.delenv("JARVIS_FAST_STARTUP_BANNER", raising=False)
    ...
    assert os.environ["JARVIS_DEFER_AGENT_STARTUP"] == "1"
    assert os.environ["JARVIS_FAST_STARTUP_BANNER"] == "1"
...
def test_termux_fast_cli_launch_oneshot_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["jarvis", "-z", "hello", "--model", "gpt-test", "--provider", "openai"],
    )
    monkeypatch.setattr(
        main_mod, "_prepare_agent_startup", lambda args: prepared.append(args.command)
    )
...
def test_termux_ultrafast_version_runs_before_heavy_startup(
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("JARVIS_TERMUX_DISABLE_FAST_CLI", raising=False)
    monkeypatch.setattr(sys, "argv", ["jarvis", "--version"])

    assert main_mod._try_termux_ultrafast_version() is True

    out = capsys.readouterr().out
    assert "Jarvis v" in out
...
def test_termux_fast_cli_launch_can_be_disabled(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setenv("JARVIS_TERMUX_DISABLE_FAST_CLI", "1")
    monkeypatch.delenv("JARVIS_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["jarvis", "version"])
...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(main_mod, "get_jarvis_home", lambda: tmp_path)
    ...
    monkeypatch.setenv("JARVIS_TERMUX_FORCE_SKILLS_SYNC", "1")
    assert main_mod._termux_bundled_skills_sync_needed() is True
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["jarvis", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "jarvis_cli.plugins",
...
def test_oneshot_fails_closed_on_empty_final_response(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    import jarvis_cli.oneshot as oneshot_mod

    monkeypatch.setattr(oneshot_mod, "_run_brain", lambda *_args, **_kwargs: "")
...
def test_oneshot_prints_nonempty_final_response(monkeypatch, capsys):
    ...
    monkeypatch.setattr(oneshot_mod, "_run_brain", lambda *_args, **_kwargs: "done")
...
def test_oneshot_fails_closed_on_agent_exception(monkeypatch, capsys):
    ...
    monkeypatch.setattr(oneshot_mod, "_run_brain", _boom)
...
def test_oneshot_reraises_keyboard_interrupt(monkeypatch):
    ...
    monkeypatch.setattr(oneshot_mod, "_run_brain", _interrupt)
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from jarvis_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "jarvis -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """jarvis -z bypasses JarvisCLI, but recall still needs SessionDB."""
    from jarvis_cli.oneshot import _run_brain
    ...
    monkeypatch.setitem(sys.modules, "run_brain", mod("run_brain", AIBrain=FakeAgent))
    monkeypatch.setitem(sys.modules, "jarvis_state", mod("jarvis_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "jarvis_cli.config",
        mod("jarvis_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "jarvis_cli.models",
        mod("jarvis_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "jarvis_cli.runtime_provider",
        mod(
            "jarvis_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "jarvis_cli.tools_config",
        mod("jarvis_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )

    assert _run_brain("recall this") == "ok"
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["JARVIS_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/jarvis-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["JARVIS_MODEL"] == "nous/jarvis-test"
    assert env["JARVIS_INFERENCE_MODEL"] == "nous/jarvis-test"
    assert env["JARVIS_TUI_PROVIDER"] == "nous"
    assert env["JARVIS_INFERENCE_PROVIDER"] == "nous"
    assert env["JARVIS_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["JARVIS_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("jarvis-tui-active-session-")
...
def test_launch_tui_applies_terminal_backend_config(
    monkeypatch, main_mod, _isolate_jarvis_home
):
    captured = {}
    ...
    assert captured["env"]["TERMINAL_DOCKER_IMAGE"] == "example/jarvis-tools:latest"
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("jarvis_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("JARVIS_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "JARVIS_TUI_RESUME" not in captured["env"]
...
def test_launch_tui_sets_resume_env_from_resume_arg(monkeypatch, main_mod):
    ...
    assert captured["env"]["JARVIS_TUI_RESUME"] == "20260518_000000_goodid"


def test_make_tui_argv_dev_prebuilds_jarvis_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("JARVIS_TUI_DIR", raising=False)
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "jarvis_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "jarvis --tui --resume 20260409_000001_abc123" in out
    assert 'jarvis --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "jarvis --tui --resume actual_session" in out
