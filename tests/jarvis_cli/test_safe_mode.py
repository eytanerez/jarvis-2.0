"""Tests for `jarvis chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("JARVIS_SAFE_MODE", "JARVIS_IGNORE_USER_CONFIG", "JARVIS_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in jarvis_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("JARVIS_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under JARVIS_SAFE_MODE=1."""
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under JARVIS_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `jarvis chat`."""
