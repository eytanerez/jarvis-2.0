"""Unit tests for jarvis_cli.win_pty_bridge — ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/jarvis_cli/test_pty_bridge.py.  Drives
...
``jarvis_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
from jarvis_cli.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from jarvis_cli.win_pty_bridge import WinPtyBridge, PtyUnavailableError
...
class TestWinPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = WinPtyBridge.spawn(["cmd.exe", "/c", "echo jarvis-ok"])
        try:
            output = _read_until(bridge, b"jarvis-ok")
            assert b"jarvis-ok" in output
        finally:
            bridge.close()
...
class TestClampDimension:
    ...
    def test_clamps_above_max(self):
        from jarvis_cli.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp

        assert _clamp(131072, _MAX_COLS) == _MAX_COLS
        assert _clamp(131072, _MAX_ROWS) == _MAX_ROWS

    def test_floors_at_one(self):
        from jarvis_cli.win_pty_bridge import _MAX_COLS, _clamp
    ...
    def test_env_is_forwarded(self):
        bridge = WinPtyBridge.spawn(
            [
                sys.executable,
                "-c",
                "import os; print('JARVIS_PTY_TEST=' + os.environ.get('JARVIS_PTY_TEST',''))",
            ],
            env={**os.environ, "JARVIS_PTY_TEST": "pty-env-works"},
        )
        try:
