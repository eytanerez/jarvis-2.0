"""Tests for the Jarvis plugin system (jarvis_cli.plugins)."""
...
from jarvis_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<jarvis_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<jarvis_home>/plugins/``; we derive
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <JARVIS_HOME>/config.yaml.
        # Config is always read from JARVIS_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        jarvis_home_str = os.environ.get("JARVIS_HOME")
        if jarvis_home_str:
            jarvis_home = Path(jarvis_home_str)
        else:
            jarvis_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("JARVIS_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".jarvis" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under jarvis_plugins.<name>."""
        plugins_dir = tmp_path / "jarvis_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("JARVIS_HOME", str(tmp_path / "jarvis_test"))

        # Clean up any prior namespace module
        sys.modules.pop("jarvis_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "jarvis_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="jarvis_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `jarvis plugins list`. Attribution now counts what each plugin's own
...
class TestPreLlmCallTargetRouting:
    ...
    The routing logic lives in run_brain.py, but the return format is collected
    ...
    def test_routing_logic_all_to_user_message(self, tmp_path, monkeypatch):
        """Simulate the routing logic from run_brain.py.
        ...
        # Replicate run_brain.py routing logic — everything goes to user msg
        _ctx_parts = []
        for r in results:
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("jarvis_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "jarvis_test" / "plugins"
        # Set JARVIS_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("jarvis_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("jarvis_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("jarvis_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("jarvis_cli.plugins.PluginContext.dispatch_tool.__module__", "jarvis_cli.plugins"):
...
class TestPluginDebugLogging:
    """JARVIS_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("JARVIS_PLUGINS_DEBUG", raising=False)
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With JARVIS_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("JARVIS_PLUGINS_DEBUG", "1")
        from jarvis_cli import plugins as plugins_mod
