...
class TestSlackNativeSlashes:
    ...
    def test_jarvis_catchall_is_first(self):
        """``/jarvis`` must be reserved as the first slot so the legacy
        ``/jarvis <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "jarvis"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /jarvis <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        aliases stay reachable via ``/jarvis <alias>``.
        """
        slashes = slack_native_slashes()
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /jarvis <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_jarvis_norm = {_norm(n) for n in _SLACK_VIA_JARVIS_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_jarvis_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `jarvis slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_completes_toolset_names(self, monkeypatch):
        ...
        from jarvis_cli import commands as commands_mod
        ...
        monkeypatch.setattr(
            "jarvis_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
class TestDiscordSkillCmdKeyDispatch:
    ...
    def test_long_skill_name_retains_cmd_key(self, tmp_path, monkeypatch):
        ...
        with patch("brain.skill_commands.get_skill_commands", return_value=fake_cmds), \
             patch("tools.skills_tool.SKILLS_DIR", fake_skills_dir), \
             patch("brain.skill_utils.get_external_skills_dirs", return_value=[]):
...
class TestTelegramMenuCommands:
    ...
    def test_excludes_telegram_disabled_skills(self, tmp_path, monkeypatch):
        ...
        with (
            patch("brain.skill_commands.get_skill_commands", return_value=fake_cmds),
...
from jarvis_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch jarvis_cli.plugins.get_plugin_commands() to a fixed dict."""
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/jarvis metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
