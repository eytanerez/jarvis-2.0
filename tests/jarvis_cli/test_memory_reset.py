"""Tests for the `jarvis memory reset` CLI command.
...
- Profile-scoped reset (uses JARVIS_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake JARVIS_HOME with memory files."""
    jarvis_home = tmp_path / ".jarvis"
    memories = jarvis_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nJarvis repo is at ~/.jarvis/jarvis-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        jarvis_home = tmp_path / ".jarvis"
        (jarvis_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's JARVIS_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_jarvis_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
