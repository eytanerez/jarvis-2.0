...
import jarvis_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "jarvis gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`jarvis gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `jarvis gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `jarvis gateway restart`)."""
        unit_path = tmp_path / "jarvis-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``JARVIS_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../jarvis_test`` JARVIS_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir JARVIS_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="JARVIS_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/jarvis_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E JARVIS_HOME like
        ``/tmp/jarvis-e2e-41264`` carries none of the pytest markers but
        poisons the unit identically (seen live 2026-06-11 — an E2E probe ran
        ``jarvis gateway restart`` with a /tmp JARVIS_HOME exported; the
        ...
        must refuse ANY temp-dir JARVIS_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "jarvis-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="JARVIS_HOME=/tmp/jarvis-e2e-41264"\n'
            "WorkingDirectory=/tmp/jarvis-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        ...
        assert (
            ...
            == "/tmp/jarvis-e2e-41264"
        )

    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="JARVIS_HOME=/var/tmp/jarvis-x"\n'
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="JARVIS_HOME=/tmpfs-data/.jarvis"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "jarvis gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_jarvis_home_for_target_user", lambda home: "/home/alice/.jarvis")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        ...
        monkeypatch.setenv("JARVIS_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("JARVIS_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.jarvis.gateway.plist"
        ...
        # JARVIS_HOME resolves under the (pytest tmp) test JARVIS_HOME.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist>--replace\n<key>JARVIS_HOME</key>"
                "<string>/Users/alice/.jarvis</string></plist>"
            ),
        )
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test JARVIS_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>JARVIS_HOME</key>"
                "<string>/Users/alice/.jarvis</string></plist>"
            ),
        )
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup jarvis gateway run" in out
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_jarvis_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: jarvis gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup jarvis gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.jarvis.gateway"])
            ),
        )
...
class TestSystemUnitJarvisHome:
    """JARVIS_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'JARVIS_HOME=/home/alice/.jarvis' in unit
        assert '/root/.jarvis' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: JARVIS_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("JARVIS_HOME", "/root/.jarvis/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'JARVIS_HOME=/home/alice/.jarvis/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_jarvis_home(self, monkeypatch):
        # Custom JARVIS_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("JARVIS_HOME", "/opt/jarvis-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'JARVIS_HOME=/opt/jarvis-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's JARVIS_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        jarvis_home = str(gateway_cli.get_jarvis_home().resolve())
        assert f'JARVIS_HOME={jarvis_home}' in unit


class TestJarvisHomeForTargetUser:
    """Unit tests for _jarvis_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("JARVIS_HOME", raising=False)

        result = gateway_cli._jarvis_home_for_target_user("/home/alice")
        assert result == "/home/alice/.jarvis"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._jarvis_home_for_target_user("/home/alice")
        assert result == "/home/alice/.jarvis/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("JARVIS_HOME", "/opt/jarvis")

        result = gateway_cli._jarvis_home_for_target_user("/home/alice")
        assert result == "/opt/jarvis"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "jarvis gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_jarvis_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.jarvis should not produce a --profile flag."""
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
        result = gateway_cli._profile_arg(str(jarvis_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.jarvis/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".jarvis" / "profiles" / "mybot"
    ...
    def test_named_profile_under_target_user_root_returns_flag(self, tmp_path):
        ...
        target_root = tmp_path / "home" / "alice" / ".jarvis"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile JARVIS_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "jarvis"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.jarvis/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".jarvis" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".jarvis" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_jarvis_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_profile = root_home / ".jarvis" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("JARVIS_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_jarvis_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
        ...
        assert f'JARVIS_HOME={target_home / ".jarvis" / "profiles" / "mybot"}' in unit
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".jarvis" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.jarvis.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".jarvis" / "jarvis-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".jarvis" / "jarvis-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/jarvis", str(tmp_path / "alice"))
        assert result == "/opt/jarvis"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".jarvis" / "jarvis-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".jarvis" / "jarvis-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("JARVIS_HOME", str(root_home / ".jarvis"))
        monkeypatch.setattr(gateway_cli, "get_jarvis_home", lambda: root_home / ".jarvis")
        ...
        # WorkingDirectory is anchored at the target user's JARVIS_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.jarvis" in unit
        assert "WorkingDirectory=/home/alice/.jarvis/jarvis-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "jarvis-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "jarvis-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'jarvis gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'jarvis gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'jarvis gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "jarvis gateway run" in out


class TestLegacyJarvisUnitDetection:
    """Tests for _find_legacy_jarvis_units / has_legacy_jarvis_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``jarvis.service`` unit behind when the service was
    renamed to ``jarvis-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``jarvis.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``jarvis-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Jarvis Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m jarvis_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_jarvis_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "jarvis.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_jarvis_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "jarvis.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_jarvis_units() is True

    def test_detects_legacy_jarvis_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "jarvis.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_jarvis_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``jarvis-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "jarvis-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "jarvis-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "jarvis-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_jarvis_units() is False

    def test_ignores_unrelated_jarvis_service(self, tmp_path, monkeypatch):
        """Third-party ``jarvis.service`` that isn't ours stays untouched.

        If a user has some other package named ``jarvis`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "jarvis.service").write_text(
            "[Unit]\nDescription=Some Other Jarvis\n[Service]\n"
            "ExecStart=/opt/other-jarvis/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_jarvis_units() == []
        assert gateway_cli.has_legacy_jarvis_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "jarvis.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "jarvis.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "jarvis.service" in out
        assert "jarvis gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "jarvis.service"
...
class TestRemoveLegacyJarvisUnits:
    """Tests for remove_legacy_jarvis_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_jarvis_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_jarvis_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop jarvis.service" in c for c in cmds_joined)
        assert any("--user disable jarvis.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo jarvis gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop jarvis.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable jarvis.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "jarvis.service"
        system_legacy = system_dir / "jarvis.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (jarvis-gateway-coder.service)
        ...
        profile_unit = user_dir / "jarvis-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "jarvis-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_jarvis_units(interactive=True)
...
class TestMigrateLegacyCommand:
    """Tests for the `jarvis gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import jarvis_cli.main as cli_main
        ...
        # public, just confirm that `jarvis gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import jarvis_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "jarvis_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_jarvis_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "jarvis_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_jarvis_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_jarvis_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_jarvis_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "jarvis-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "jarvis-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "jarvis-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``jarvis gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "jarvis-gateway")
        ...
        assert "sudo systemctl start jarvis-gateway" in out
        assert "sudo jarvis gateway uninstall --system" in out
        assert "jarvis gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart jarvis-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop jarvis-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``jarvis gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (JARVIS_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """

    def test_stable_working_dir_uses_jarvis_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".jarvis"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_jarvis_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # JARVIS_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".jarvis"
        monkeypatch.setattr(gateway_cli, "get_jarvis_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_jarvis_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".jarvis"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_jarvis_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_jarvis_home(self, tmp_path, monkeypatch):
        import re
