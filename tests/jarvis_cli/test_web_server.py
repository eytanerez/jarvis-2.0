...
@pytest.fixture
def _install_example_plugin(_isolate_jarvis_home):
    """Drop the example-dashboard fixture into the per-test JARVIS_HOME
    ...
    isolated ``JARVIS_HOME``.

    The user-plugin source is preferred over a transient
    ``JARVIS_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, jarvis-achievements, model providers
    ...
    #      and start serving requests against a torn-down JARVIS_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Jarvis vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Jarvis env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints JARVIS_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("JARVIS_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("JARVIS_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_jarvis_home):
        """Create a TestClient and isolate the state DB under the test JARVIS_HOME."""
        try:
    ...
    def test_get_status(self):
        ...
        assert "jarvis_home" in data
        assert "active_sessions" in data
        assert data["can_update_jarvis"] is True
    ...
    def test_get_status_hides_update_capability_in_managed_runtime(self, monkeypatch):
        ...
        resp = self.client.get("/api/status")
        assert resp.status_code == 200
        assert resp.json()["can_update_jarvis"] is False

    def test_dashboard_update_capability_detects_generic_container(self, monkeypatch):
        import jarvis_constants
        import jarvis_cli.web_server as web_server

        monkeypatch.setattr(jarvis_constants, "is_container", lambda: True)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "jarvis"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from jarvis_constants import get_jarvis_home
        from jarvis_cli.config import load_config, load_env
        ...
        config_path = get_jarvis_home() / "hindsight" / "config.json"
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_jarvis_home() / "images" / "nope.png"
        resp = self.client.get("/api/media", params={"path": str(missing)})
        assert resp.status_code == 404

    def test_get_media_requires_auth(self):
        from jarvis_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("jarvis_state.SessionDB", _FakeDB)
    ...
    def test_get_sessions_order_recent_surfaces_compression_tip(self):
        ...
        try:
            ...
            # recognised as a compression tip (not a sub-brain/branch).
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from jarvis_cli.web_server import app
    ...
    def test_update_jarvis_returns_docker_guidance_without_spawning(self, monkeypatch):
        import jarvis_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn jarvis update")

        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_jarvis_action", fail_spawn)
        web_server._ACTION_PROCS.pop("jarvis-update", None)
        web_server._ACTION_RESULTS.pop("jarvis-update", None)

        resp = self.client.post("/api/jarvis/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "jarvis-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/jarvis-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/jarvis-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/jarvis-agent:latest" in line for line in status_data["lines"])

    def test_update_jarvis_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        import jarvis_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn jarvis update")
    ...
    def test_update_jarvis_spawns_on_non_docker_install(self, monkeypatch):
        import jarvis_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_jarvis_action", fake_spawn)
        web_server._ACTION_PROCS.pop("jarvis-update", None)
        web_server._ACTION_RESULTS.pop("jarvis-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "jarvis-update"}
        assert calls == [(["update"], "jarvis-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("jarvis-update", None)
        web_server._ACTION_RESULTS.pop("jarvis-update", None)
        web_server._ACTION_PROCS["jarvis-update"] = proc

        resp = self.client.get("/api/actions/jarvis-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "jarvis-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["jarvis-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from jarvis_cli.web_server import _channel_managed_env_keys
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `jarvis model` but
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from jarvis_cli.web_server import (
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "jarvis_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Jarvis provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `jarvis import` doesn't auto-abort at the overwrite prompt."""
        import jarvis_cli.web_server as ws
        ...
        monkeypatch.setattr(ws, "_spawn_jarvis_action", fake_spawn)
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from jarvis_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from jarvis_cli.config import save_env_value
        from jarvis_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://jarvis-agent.nousresearch.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Jarvis"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("JarvisDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "jarvis_pair123_bot",
                "deep_link": "https://t.me/newbot/JarvisSetupBot/jarvis_pair123_bot",
                "qr_payload": "https://t.me/newbot/JarvisSetupBot/jarvis_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "jarvis_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/JarvisSetupBot/jarvis_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/JarvisSetupBot/jarvis_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "jarvis_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_jarvis_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "jarvis_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/JarvisSetupBot/jarvis_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/JarvisSetupBot/jarvis_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "jarvis_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_jarvis_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``jarvis gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "jarvis_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/JarvisSetupBot/jarvis_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/JarvisSetupBot/jarvis_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "jarvis_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "jarvis_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/JarvisSetupBot/jarvis_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/JarvisSetupBot/jarvis_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "jarvis_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/JarvisSetupBot/jarvis_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/JarvisSetupBot/jarvis_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/jarvis_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get("/assets/app.css", headers={"x-forwarded-prefix": "/jarvis"})
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import jarvis_cli.nous_subscription as ns
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "jarvis-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from jarvis_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from jarvis_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``jarvis model`` custom flow. Regression for the desktop loop where a
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "jarvis-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `jarvis model`), not the first curated paid entry."""
        import jarvis_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from jarvis_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from jarvis_cli.web_server import _build_schema_from_config
    ...
    def test_nested_keys_get_parent_category(self):
        ...
        from jarvis_cli.web_server import CONFIG_SCHEMA
        if "brain.max_turns" in CONFIG_SCHEMA:
            assert CONFIG_SCHEMA["brain.max_turns"]["category"] == "agent"
...
class TestNewEndpoints:
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("jarvis://")
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        ...
        named = jarvis_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_jarvis_for_default_profile(self):
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "jarvis setup"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/jarvis/bin/jarvis")
        ...
        assert wrapper_path.read_text() == '#!/bin/sh\nexec /opt/jarvis/bin/jarvis -p writer "$@"\n'
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_jarvis_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_jarvis_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_jarvis_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_jarvis_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_jarvis_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_jarvis_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_jarvis_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_jarvis_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_jarvis_home() / "profiles" / "builder"
        token = set_jarvis_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_jarvis_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import jarvis_cli.skills_config as skills_config
        import jarvis_cli.web_server as web_server
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from jarvis_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from jarvis_cli.web_server import _denormalize_config_from_web
...
class TestModelInfoEndpoint:
    ...
    def test_model_info_with_dict_config(self, monkeypatch):
        ...
        with patch("brain.model_metadata.get_model_context_length", return_value=200000):
            resp = self.client.get("/api/model/info")
    ...
    def test_model_info_capabilities(self, monkeypatch):
        ...
        with patch("brain.model_metadata.get_model_context_length", return_value=200000), \
             patch("brain.models_dev.get_model_capabilities", return_value=mock_caps):
            resp = self.client.get("/api/model/info")
    ...
    def test_model_info_graceful_on_metadata_error(self, monkeypatch):
        ...
        with patch("brain.model_metadata.get_model_context_length", side_effect=Exception("boom")):
            resp = self.client.get("/api/model/info")
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_jarvis_home):
        ...
        monkeypatch.setattr(
            jarvis_state, "DEFAULT_DB_PATH", get_jarvis_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``jarvis_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in jarvis_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_jarvis_home):
        ...
        # Pin the SessionDB to the isolated JARVIS_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_jarvis_home, _install_example_plugin):
        ...
        installs it into the per-test ``JARVIS_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into JARVIS_HOME by the class-level
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["JARVIS_TUI_DASHBOARD"] == "1"
    ...
    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_jarvis_home
    ):
        import jarvis_cli.main as main_mod

        config_path = Path(os.environ["JARVIS_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/jarvis-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/jarvis-tools:latest"
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("JARVIS_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_jarvis_home, _install_example_plugin):
        ...
        the per-test ``JARVIS_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_jarvis_home):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under JARVIS_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_jarvis_home):
        import threading
        import cron.scheduler as sched
