"""Tests for the ``jarvis send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``jarvis_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from jarvis_cli import send_cmd
...
def test_load_jarvis_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    jarvis_home.mkdir()
    (jarvis_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_jarvis_home() to re-resolve under the patched env.
    from importlib import reload

    import jarvis_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_jarvis_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_jarvis_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    jarvis_home = tmp_path / ".jarvis"
    jarvis_home.mkdir()
    (jarvis_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_jarvis_env_handles_missing_files(tmp_path, monkeypatch):
