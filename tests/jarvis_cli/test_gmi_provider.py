...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from jarvis_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # jarvis model, not via upgrade prompts.
...
class TestGmiModelCatalog:
    ...
    def test_provider_model_ids_prefers_live_api(self, monkeypatch):
        monkeypatch.setattr(
            "jarvis_cli.auth.resolve_api_key_provider_credentials",
            lambda provider_id: {
        ...
        monkeypatch.setattr(
            "jarvis_cli.models.fetch_api_models",
    ...
    def test_provider_model_ids_falls_back_to_static_models(self, monkeypatch):
        ...
        monkeypatch.setattr("jarvis_cli.models.fetch_api_models", lambda api_key, base_url: None)
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in JARVIS_OVERLAYS
        overlay = JARVIS_OVERLAYS["gmi"]
...
class TestGmiDoctor:
    ...
    def test_run_doctor_checks_gmi_models_endpoint(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(doctor_mod, "JARVIS_HOME", home)
        ...
        try:
            from jarvis_cli import auth as _auth_mod
...
class TestGmiModelMetadata:
    ...
    def test_known_gmi_endpoint_still_uses_endpoint_metadata(self):
        with patch(
            "brain.model_metadata.get_cached_context_length",
            return_value=None,
        ), patch(
            "brain.model_metadata.fetch_endpoint_model_metadata",
            return_value={"anthropic/claude-opus-4.6": {"context_length": 409600}},
        ), patch(
            "brain.models_dev.lookup_models_dev_context",
            return_value=None,
        ), patch(
            "brain.model_metadata.fetch_model_metadata",
            return_value={},
        ):
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a JarvisBrain User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("JarvisBrain/")

    def test_gmi_profile_declares_jarvis_user_agent(self):
        """The GMI plugin sets a JarvisBrain/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("JarvisBrain/"), (
            f"expected GMI profile User-Agent to start with 'JarvisBrain/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("jarvis_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "jarvis_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["jarvis", "chat", "--provider", "gmi"])
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("jarvis_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("jarvis_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("jarvis_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            "jarvis_cli.auth._prompt_model_selection",
            return_value="openai/gpt-5.4-mini",
        ), patch(
            "jarvis_cli.auth.deactivate_provider",
        ), patch(
            "builtins.input",
            return_value="",
        ):
            from jarvis_cli.main import _model_flow_api_key_provider
