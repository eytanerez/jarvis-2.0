"""Tests for xAI Grok OAuth — tokens stored in Jarvis auth store (~/.jarvis/auth.json)."""
...
def _setup_jarvis_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Jarvis auth store at the given root."""
    ...
    auth_file = jarvis_home / "auth.json"
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("jarvis_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_authorize_url_includes_referrer_jarvis_agent():
    """Attribution: xAI's OAuth server can identify Jarvis-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "jarvis-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "jarvis auth add xai-oauth" in body
...
def test_read_xai_oauth_tokens_missing_access_token(tmp_path, monkeypatch):
    jarvis_home = tmp_path / "jarvis"
    _setup_jarvis_auth(jarvis_home, access_token="")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    jarvis_home = tmp_path / "jarvis"
    _setup_jarvis_auth(jarvis_home, refresh_token="")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    jarvis_home = tmp_path / "jarvis"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_jarvis_auth(jarvis_home, access_token=fresh)
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    ...
    assert creds["source"] == "jarvis-auth-store"
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("jarvis_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_jarvis_auth(jarvis_home, access_token=fresh)
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    monkeypatch.setenv("JARVIS_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
# ``XAI_BASE_URL`` / ``JARVIS_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    jarvis_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    jarvis_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (jarvis_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    jarvis_home = tmp_path / "jarvis"
    _seed_xai_oauth_state(jarvis_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    ...
    monkeypatch.setattr("jarvis_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((jarvis_home / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    jarvis_home = tmp_path / "jarvis"
    _seed_xai_oauth_state(jarvis_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    ...
    monkeypatch.setattr("jarvis_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``jarvis model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``jarvis model``
    ...
    assert "jarvis model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from jarvis_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Jarvis that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("jarvis_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `jarvis model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `jarvis auth list` reflects truth and
    refreshes route through the pool consistently with codex."""
    from brain.credential_pool import load_pool

    jarvis_home = tmp_path / "jarvis"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_jarvis_auth(jarvis_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`jarvis auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `jarvis auth remove`.
    from jarvis_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``jarvis auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `jarvis auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((jarvis_home / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Jarvis invocation."
    )
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_jarvis_auth(jarvis_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    ...
    monkeypatch.setattr("jarvis_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
    ...
    auth_path = jarvis_home / "auth.json"
...
def test_runtime_provider_default_base_url_when_pool_entry_missing_url(tmp_path, monkeypatch):
    ...
    from brain.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    import uuid
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from brain.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from jarvis_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("jarvis_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    jarvis_home = tmp_path / "jarvis"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_jarvis_auth(jarvis_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Jarvis process refreshed the singleton
    ...
    profiles + Jarvis processes."""
    from brain.credential_pool import load_pool

    jarvis_home = tmp_path / "jarvis"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_jarvis_auth(jarvis_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    ...
    (jarvis_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_jarvis_auth(jarvis_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``jarvis model`` -> xAI Grok OAuth (or another process
    ...
    from brain.credential_pool import load_pool, STATUS_EXHAUSTED
    from dataclasses import replace

    jarvis_home = tmp_path / "jarvis"
    stale_at = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_jarvis_auth(jarvis_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    ...
    # Simulate the user re-running `jarvis model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``jarvis auth add xai-oauth``) own their own refresh-token lifecycle
    ...
    _setup_jarvis_auth(jarvis_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`jarvis auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``jarvis auth status``, ``jarvis setup``, and the
    ``jarvis`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from brain.credential_pool import load_pool

    jarvis_home = tmp_path / "jarvis"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_jarvis_auth(jarvis_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
