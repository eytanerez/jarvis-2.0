...
# ── deprecated `jarvis login` fails gracefully, not with argparse error ────
#
# `jarvis login` is a removed command; its handler (`login_command` in
# `jarvis_cli/auth.py`) prints a deprecation notice pointing at `jarvis auth` /
# `jarvis model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `jarvis --help`.
