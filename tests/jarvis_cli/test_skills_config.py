...
class TestSaveDisabledSkills:
    @patch("jarvis_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from jarvis_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("jarvis_cli.config.load_config")
    ...
    @patch("jarvis_cli.config.load_config")
    @patch.dict("os.environ", {"JARVIS_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """JARVIS_SESSION_PLATFORM should be used when JARVIS_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_jarvis_platform_takes_precedence(self, tmp_path, monkeypatch):
        """JARVIS_PLATFORM should win over JARVIS_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestFindAllSkillsFiltering:
    ...
    def test_disabled_skill_excluded(self, mock_platform, mock_disabled, tmp_path, monkeypatch):
        ...
        import brain.skill_utils as _su
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from jarvis_cli.skills_config import _get_categories
        skills = [
