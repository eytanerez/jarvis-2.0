...
def container_env(tmp_path, monkeypatch):
    """Set up a fake JARVIS_HOME with .container-mode file."""
    jarvis_home = tmp_path / ".jarvis"
    jarvis_home.mkdir()
    monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
    monkeypatch.delenv("JARVIS_DEV", raising=False)

    container_mode = jarvis_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=jarvis-agent\n"
        "exec_user=jarvis\n"
        "jarvis_bin=/data/current-package/bin/jarvis\n"
    )
    return jarvis_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("jarvis_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "jarvis-agent"
    assert info["exec_user"] == "jarvis"
    assert info["jarvis_bin"] == "/data/current-package/bin/jarvis"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("jarvis_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_jarvis_dev(container_env, monkeypatch):
    """Returns None when JARVIS_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("JARVIS_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_jarvis_dev_zero(container_env, monkeypatch):
    """JARVIS_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("JARVIS_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        jarvis_home = Path(tmpdir) / ".jarvis"
        jarvis_home.mkdir()
        (jarvis_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("jarvis_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_jarvis_home": lambda: jarvis_home}), \
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "jarvis-custom"
    assert info["exec_user"] == "myuser"
    assert info["jarvis_bin"] == "/opt/jarvis/bin/jarvis"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from jarvis_cli.main import _exec_in_container
