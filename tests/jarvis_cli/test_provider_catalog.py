"""Tests for the unified provider catalog (jarvis_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``jarvis model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from jarvis_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
...
def test_catalog_covers_every_jarvis_model_provider():
    """PARITY CONTRACT: the catalog == the `jarvis model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `jarvis model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_jarvis_model():
    """The catalog must not invent providers `jarvis model` doesn't show."""
