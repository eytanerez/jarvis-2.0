...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh JARVIS_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_cli_passthrough_")
    os.makedirs(os.path.join(test_home, "profiles", "default"), exist_ok=True)
    monkeypatch.setenv("JARVIS_HOME", test_home)
    for mod in list(sys.modules.keys()):
        if mod.startswith("jarvis_cli") or mod.startswith("jarvis_state") or mod == "jarvis_constants":
            del sys.modules[mod]
    yield test_home


def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    """#33488: jarvis kanban dispatch must pass kanban.max_in_progress from
    ...
    monkeypatch.setattr(
        "jarvis_cli.config.load_config", lambda: fake_config
    )
...
def test_cli_max_flag_overrides_config_max_spawn(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr("jarvis_cli.config.load_config", lambda: fake_config)
