"""Tests for --ignore-user-config and --ignore-rules flags on `jarvis chat`.
...
files. In Jarvis the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.jarvis/config.yaml`` in ``load_cli_config()``
  (credentials in ``.env`` are still loaded).
* ``--ignore-rules`` → skip AGENTS.md / SOUL.md / .cursorrules auto-injection
  and persistent memory (maps to ``AIBrain(skip_context_files=True,
  skip_memory=True)``).

Both flags are wired via env vars so they work cleanly across the
argparse → cmd_chat → cli.main() → JarvisCLI → AIBrain call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("JARVIS_IGNORE_USER_CONFIG", "JARVIS_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``JARVIS_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<jarvis_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._jarvis_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("JARVIS_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``JarvisCLI.ignore_rules``
    so ``AIBrain`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        ...
        # Import JarvisCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of JarvisCLI to reach the ignore_rules assignment.
        ...
        # Replicate the exact logic from cli.py JarvisCLI.__init__:
    ...
    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("JARVIS_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.JarvisCLI)
...
class TestCmdChatWiring:
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("JARVIS_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("JARVIS_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("JARVIS_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("JARVIS_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("JARVIS_IGNORE_USER_CONFIG") == "1"
        assert "JARVIS_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "JARVIS_IGNORE_USER_CONFIG" not in os.environ
        assert "JARVIS_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real jarvis parser accepts both flags."""
        ...
        import inspect
        import jarvis_cli.main as hm
        src = inspect.getsource(hm)
        assert "JARVIS_IGNORE_USER_CONFIG" in src
        assert "JARVIS_IGNORE_RULES" in src
