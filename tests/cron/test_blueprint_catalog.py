...
the shared /blueprint command handler (jarvis_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated JARVIS_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("jarvis://blueprint/morning-brief?")
    ...
    def test_catalog_entry_has_all_surfaces(self):
        ...
        assert entry["appUrl"].startswith("jarvis://")
...
class TestCommandHandler:
    ...
    def test_name_match_is_forgiving(self, isolated_home):
        from jarvis_cli.blueprint_cmd import handle_blueprint_command, match_blueprint
