...
def test_create_job_no_agent_stores_field(jarvis_env):
    from cron.jobs import create_job

    script_path = jarvis_env / "scripts" / "watchdog.sh"
...
def test_create_job_default_is_not_no_agent(jarvis_env):
    from cron.jobs import create_job
...
def test_cronjob_tool_create_no_agent_with_script_succeeds(jarvis_env):
    from tools.cronjob_tools import cronjob
...
def test_cronjob_tool_update_toggles_no_agent(jarvis_env):
    from tools.cronjob_tools import cronjob
...
def test_cronjob_tool_update_no_agent_without_script_errors(jarvis_env):
...
def test_cronjob_tool_create_does_not_require_prompt_when_no_agent(jarvis_env):
...
def test_run_job_no_agent_wake_gate_is_silent(jarvis_env):
    """wakeAgent=false gate in stdout triggers a silent run."""
    from cron.jobs import create_job
    from cron.scheduler import run_job, SILENT_MARKER

    script_path = jarvis_env / "scripts" / "gated.sh"
...
def test_run_job_no_agent_script_failure_delivers_error(jarvis_env):
    """Non-zero exit → success=False, error alert is the delivered message."""
    from cron.jobs import create_job
    from cron.scheduler import run_job

    script_path = jarvis_env / "scripts" / "broken.sh"
    script_path.write_text("#!/bin/bash\necho oops >&2\nexit 3\n")
...
def test_run_job_no_agent_never_invokes_aiagent(jarvis_env):
    """no_agent jobs must NOT import/construct the AIBrain."""
    from cron.jobs import create_job
    ...
    with patch("run_brain.AIBrain") as ai_mock:
        from cron.scheduler import run_job
...
def test_run_job_script_shell_script_runs_via_bash(jarvis_env):
    """.sh files should execute under /bin/bash even without a shebang line."""
    from cron.scheduler import _run_job_script

    script_path = jarvis_env / "scripts" / "shelly.sh"
...
def test_run_job_script_bash_extension_also_runs_via_bash(jarvis_env):
    from cron.scheduler import _run_job_script

    script_path = jarvis_env / "scripts" / "thing.bash"
...
def test_run_job_script_python_still_runs_via_python(jarvis_env):
    """Regression: .py files must keep running via sys.executable."""
    from cron.scheduler import _run_job_script

    script_path = jarvis_env / "scripts" / "py.py"
