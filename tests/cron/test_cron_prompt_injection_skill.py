...
def cron_env(tmp_path, monkeypatch):
    """Isolated JARVIS_HOME with an empty skills tree.

    `tools.skills_tool` snapshots `SKILLS_DIR` at module-import time, so
    setting `JARVIS_HOME` alone doesn't reach it. We also patch the
    ...
    monkeypatch.setenv("JARVIS_BUNDLES_DIR", str(jarvis_home / "skill-bundles"))
    ...
    import brain.skill_bundles as _skill_bundles
...
def _plant_bundle(jarvis_home: Path, name: str, skills: list[str], instruction: str = "") -> None:
    """Drop a bundle YAML into ~/.jarvis/skill-bundles/ and refresh cache."""
    bundles_dir = jarvis_home / "skill-bundles"
...
class TestBuildJobPromptScansSkillContent:
    ...
    def test_skill_with_env_exfil_command_in_prose_is_allowed(self, cron_env):
        ...
        ``cat ~/.jarvis/.env``") must NOT be blocked. This was a real
        false positive in the bundled `jarvis-agent-dev` skill that
        ...
        _plant_skill(
            jarvis_home,
            "security-postmortem",
            "Lessons learned: the attacker could just `cat ~/.jarvis/.env`\n"
        ...
        prompt = scheduler._build_job_prompt(job)
        assert prompt is not None
        assert "cat ~/.jarvis/.env" in prompt
    ...
    def test_skill_bundle_in_job_skills_loads_referenced_skills(self, cron_env):
        jarvis_home, scheduler = cron_env
        _plant_skill(jarvis_home, "alpha-skill", "Alpha guidance for the cron task.")
        _plant_skill(jarvis_home, "beta-skill", "Beta guidance for the cron task.")
        _plant_bundle(
            jarvis_home,
            "article-pipeline",
    ...
    def test_bundle_name_shadows_skill_name_for_cron_jobs(self, cron_env):
        jarvis_home, scheduler = cron_env
        _plant_skill(jarvis_home, "article-pipeline", "Standalone skill should not win.")
        _plant_skill(jarvis_home, "bundle-member", "Bundle member should win.")
        _plant_bundle(jarvis_home, "article-pipeline", ["bundle-member"])
...
class TestScriptOutputNotStrictScanned:
    ...
    Live incident: the ``jarvis-triage`` cron was blocked every 5 minutes
