...
The hook fires inside ``AIBrain.run_conversation`` once the tool-calling
...
invoke_hook dispatch semantics that the wiring in ``run_brain.py``
depends on:
...
from jarvis_cli.plugins import PluginManager, VALID_HOOKS
...
def test_first_non_empty_string_wins_semantics():
    """Simulate the run_brain.py loop: first non-empty string replaces text."""
...
def test_hook_exception_does_not_replace_response(tmp_path, monkeypatch):
    ...
    to the results list, and the walk in run_brain.py finds nothing to
    replace with.
    """
    jarvis_home = tmp_path / "jarvis_test"
    jarvis_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        jarvis_home, "raising_hook",
        register_body=(
            'def _boom(**kw):\n'
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path / "jarvis_empty"))
