...
def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:jarvis-agent"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:jarvis-agent"]
...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Jarvis and the User"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        assert cfg["banks"]["jarvis"]["bankId"] == "env-bank"
        assert cfg["banks"]["jarvis"]["budget"] == "high"
...
class TestPostSetup:
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(jarvis_home))
        provider.post_setup(str(jarvis_home), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        jarvis_env = user_home / ".hindsight" / "profiles" / "jarvis.env"
        assert coder_env.exists()
        assert not jarvis_env.exists()
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config(existing_config, str(jarvis_home))
        ...
        monkeypatch.setattr("jarvis_cli.memory_setup._curses_select", lambda *args, **kwargs: kwargs.get("default", 0))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "")
        ...
        saved = json.loads((jarvis_home / "hindsight" / "config.json").read_text())
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between Jarvis and the User"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Jarvis and the User"
...
class TestBankIdTemplate:
    ...
    def test_resolve_with_profile(self):
        result = _resolve_bank_id_template(
            "jarvis-{profile}", fallback="jarvis",
            profile="coder", workspace="", platform="", user="", session="",
        )
        assert result == "jarvis-coder"
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "jarvis-{user}" becomes "jarvis-" -> trimmed to "jarvis"
        result = _resolve_bank_id_template(
            "jarvis-{user}", fallback="default",
    ...
    def test_resolve_invalid_template_returns_fallback(self):
        ...
        result = _resolve_bank_id_template(
            "jarvis-{unknown}", fallback="jarvis",
            profile="", workspace="", platform="", user="", session="",
        )
        assert result == "jarvis"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "jarvis-" which collapses to "jarvis"
        p.initialize(session_id="s1", jarvis_home=str(tmp_path), platform="cli")
        assert p._bank_id == "jarvis"
...
class TestSharedEventLoopLifecycle:
    ...
    loader builds one provider per AIBrain, and the gateway builds one AIBrain
