...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "jarvis_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_board_query_param_default_overrides_current_board_pointer(client):
    ...
    through to whichever board ``jarvis kanban boards switch`` last pinned.
    """
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "jarvis_cli.web_server", stub)
    monkeypatch.setattr(jarvis_cli, "web_server", stub, raising=False)
...
# tests/jarvis_cli/test_kanban_db.py. The OUTER fallback here is not, which
...
def test_board_endpoint_survives_task_age_exception(client, monkeypatch):
    ...
    monkeypatch.setattr("jarvis_cli.kanban_db.task_age", _boom)
...
def test_create_task_probe_error_does_not_break_create(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "jarvis_cli.kanban._check_dispatcher_presence", _raise,
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
