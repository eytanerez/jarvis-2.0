...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure JARVIS_HOME and RETAINDB vars are isolated."""
...
# We need the repo root on sys.path so the plugin can import brain.memory_provider
import sys
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", jarvis_home=str(tmp_path / ".jarvis"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", jarvis_home=profile_home)
        assert p._client.project == "jarvis-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".jarvis" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", jarvis_home=str(jarvis_home))
        return p
