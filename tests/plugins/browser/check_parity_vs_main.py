...
Run from the PR worktree:
    ...
    cd ~/.jarvis/jarvis-brain/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.jarvis/jarvis-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.jarvis/jarvis-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a jarvis-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a jarvis-agent checkout"
)
...
# Isolated JARVIS_HOME for the config write.
...
for name in list(sys.modules):
    if name.startswith("tools.") or name.startswith("brain.") or name.startswith("plugins."):
