...
def _fake_warning():
    return SimpleNamespace(
        message=(
            ...
            "openai/gpt-5.5-pro has known pricing above Jarvis' safety threshold.\n"
...
def _setup_isolated_home(tmp_path, monkeypatch, *, warn):
    ...
    monkeypatch.setattr(
        "jarvis_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    monkeypatch.setattr("jarvis_constants.get_jarvis_home", lambda: jarvis_home)
    monkeypatch.setattr("jarvis_cli.config.get_jarvis_home", lambda: jarvis_home)
    monkeypatch.setattr(
