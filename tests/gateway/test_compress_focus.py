...
async def test_compress_focus_topic_passed_to_agent():
    ...
    with (
        ...
        patch("run_brain.AIBrain", return_value=agent_instance),
        patch("brain.model_metadata.estimate_messages_tokens_rough", side_effect=_estimate),
    ):
...
async def test_compress_no_focus_passes_none():
    ...
    with (
        ...
        patch("gateway.run._resolve_gateway_model", return_value="test-model"),
        patch("run_brain.AIBrain", return_value=agent_instance),
        patch("brain.model_metadata.estimate_messages_tokens_rough", return_value=100),
    ):
