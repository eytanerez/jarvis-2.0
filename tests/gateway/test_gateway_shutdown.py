...
def test_cleanup_agent_resources_reaps_stale_aux_clients():
    ...
    with patch("brain.auxiliary_client.cleanup_stale_async_clients") as cleanup_mock:
...
async def test_gateway_stop_interrupts_running_agents_and_cancels_adapter_tasks():
    ...
    with (
        ...
        patch("brain.auxiliary_client.shutdown_cached_clients") as shutdown_cached_clients,
    ):
        await runner.stop()
...
async def test_operator_initiated_stop_persists_stopped(tmp_path, monkeypatch):
    ...
    gateway_state=stopped so an explicit `jarvis gateway stop` stays down."""
    monkeypatch.setattr(gateway_run, "_jarvis_home", tmp_path)
    runner, adapter = make_restart_runner()
    adapter.disconnect = AsyncMock()
