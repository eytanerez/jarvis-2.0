...
class TestRuntimeProviderUsesScope:
    """jarvis_cli.runtime_provider._getenv resolves through the secret scope."""

    def test_getenv_reads_scope_under_multiplex(self, monkeypatch):
        from jarvis_cli.runtime_provider import _getenv
    ...
    def test_getenv_global_var_still_reads_environ(self, monkeypatch):
        from jarvis_cli.runtime_provider import _getenv
        monkeypatch.setenv("JARVIS_MAX_ITERATIONS", "42")
        ss.set_multiplex_active(True)
        # global var: no scope needed, no raise
        assert _getenv("JARVIS_MAX_ITERATIONS") == "42"
