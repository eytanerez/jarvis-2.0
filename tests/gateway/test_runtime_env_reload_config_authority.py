...
JARVIS_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env JARVIS_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    jarvis_home = tmp_path / ".jarvis"
    jarvis_home.mkdir()
    (jarvis_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (jarvis_home / ".env").write_text("JARVIS_MAX_ITERATIONS=123\n", encoding="utf-8")
    ...
    assert os.environ["JARVIS_MAX_ITERATIONS"] == "123"


def test_current_max_iterations_reloads_before_reading(monkeypatch) -> None:
    monkeypatch.setenv("JARVIS_MAX_ITERATIONS", "90")

    def _fake_reload() -> None:
        os.environ["JARVIS_MAX_ITERATIONS"] = "200"
