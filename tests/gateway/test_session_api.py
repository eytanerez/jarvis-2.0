...
@pytest.mark.asyncio
async def test_run_brain_binds_api_session_context_for_tool_env(adapter, monkeypatch):
...
async def test_session_crud_and_message_history(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        ...
        assert created["object"] == "jarvis.session"
        assert created["session"]["title"] == "Mobile chat"

        session_db.append_message(session_id, "user", "hello from phone")
        session_db.append_message(session_id, "assistant", "hello from jarvis")
        ...
        assert deleted == {"object": "jarvis.session.deleted", "id": session_id, "deleted": True}
...
async def test_session_fork_uses_current_sessiondb_branch_primitives(adapter, session_db):
    ...
    assert payload["object"] == "jarvis.session"
...
async def test_session_chat_loads_history_and_preserves_session_headers(auth_adapter, session_db):
    ...
    with patch.object(auth_adapter, "_run_brain", mock_run):
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                f"/api/sessions/{session_id}/chat",
                json={"message": "next", "system_message": "stay focused"},
                headers={"Authorization": "Bearer sk-test", "X-Jarvis-Session-Key": "client-42"},
            )
            assert resp.status == 200
            payload = await resp.json()

    assert resp.headers["X-Jarvis-Session-Id"] == session_id
    assert resp.headers["X-Jarvis-Session-Key"] == "client-42"
    assert payload["object"] == "jarvis.session.chat.completion"
...
async def test_session_chat_stream_accepts_multimodal_message(adapter, session_db):
    ...
    app = _create_session_app(adapter)
    with patch.object(adapter, "_run_brain", side_effect=fake_run):
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
...
async def test_session_header_rejected_without_api_key(adapter, session_db):
    ...
    async with TestClient(TestServer(app)) as cli:
        resp = await cli.post(
            f"/api/sessions/{session_id}/chat",
            json={"message": "hello"},
            headers={"X-Jarvis-Session-Key": "client-42"},
        )
        assert resp.status == 403
        data = await resp.json()
        assert "X-Jarvis-Session-Key requires API key" in data["error"]["message"]
