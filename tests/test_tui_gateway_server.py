...
from jarvis_cli.active_sessions import active_session_registry_snapshot
...
def test_session_context_uses_session_cwd(monkeypatch, tmp_path):
    ...
    """
    from brain.runtime_cwd import resolve_agent_cwd
...
def test_tui_verbose_tool_details_fail_closed_when_redaction_fails(monkeypatch):
    redact_module = types.ModuleType("brain.redact")

    def fail_redaction(*_args, **_kwargs):
        raise RuntimeError("redaction unavailable")

    setattr(redact_module, "redact_sensitive_text", fail_redaction)
    monkeypatch.setitem(sys.modules, "brain.redact", redact_module)
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["JARVIS_VOICE"]``
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("JARVIS_VOICE", "1")
    monkeypatch.delenv("JARVIS_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("JARVIS_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("JARVIS_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "jarvis_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("JARVIS_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("JARVIS_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "jarvis_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("JARVIS_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "jarvis_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("JARVIS_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("JARVIS_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("JARVIS_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("JARVIS_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "jarvis_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown JARVIS_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("jarvis_state.SessionDB", lambda db_path=None: profile_db)
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    ...
    monkeypatch.setenv("JARVIS_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set JARVIS_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("JARVIS_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("JARVIS_MODEL", raising=False)
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("JARVIS_MODEL", "sonnet")
    monkeypatch.delenv("JARVIS_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("JARVIS_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "jarvis_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with JarvisCLI / gateway: fallback_providers stays first and keeps
...
def test_make_agent_passes_configured_fallback_chain(monkeypatch):
    ...
    monkeypatch.setattr("run_brain.AIBrain", fake_agent)
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "jarvis_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "jarvis_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "jarvis_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("JARVIS_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("JARVIS_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("JARVIS_EXEC_ASK", raising=False)
    monkeypatch.delenv("JARVIS_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("JARVIS_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["JARVIS_GATEWAY_SESSION"] == "1"
    assert server.os.environ["JARVIS_EXEC_ASK"] == "1"
    assert server.os.environ["JARVIS_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("jarvis_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("jarvis_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "jarvis_cli.runtime_provider.resolve_runtime_provider",
...
def test_config_set_model_requires_confirmation_for_expensive_model(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setattr(
        "jarvis_cli.model_switch.switch_model", lambda **_kwargs: result
    )
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("jarvis_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("jarvis_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("jarvis_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing JARVIS_INFERENCE_PROVIDER on a switch leaked the new
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("JARVIS_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "jarvis_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    ...
    try:
        ...
        assert os.environ["JARVIS_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "JARVIS_TUI_PROVIDER" not in os.environ
        assert "JARVIS_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write JARVIS_MODEL / JARVIS_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("JARVIS_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("JARVIS_MODEL", raising=False)
    monkeypatch.delenv("JARVIS_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("jarvis_cli.model_switch.switch_model", fake_switch_model)
    ...
    try:
        ...
        assert os.environ["JARVIS_TUI_PROVIDER"] == "openai-codex"
        assert "JARVIS_MODEL" not in os.environ
        assert "JARVIS_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    """When AIBrain._compress_context rotates session_id (compression split),
...
def test_prompt_submit_expands_context_refs(monkeypatch):
    ...
    fake_ctx = types.ModuleType("brain.context_references")
    ...
    fake_meta = types.ModuleType("brain.model_metadata")
    ...
    monkeypatch.setattr(server, "make_stream_renderer", lambda cols: None)
    monkeypatch.setattr(server, "render_message", lambda raw, cols: None)
    monkeypatch.setitem(sys.modules, "brain.context_references", fake_ctx)
    monkeypatch.setitem(sys.modules, "brain.model_metadata", fake_meta)
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".jarvis" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.jarvis/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".jarvis" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.jarvis/desktop-attachments/outside.txt"
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert not (workspace / ".jarvis" / "desktop-attachments").exists()
    finally:
        server._sessions.pop("sid", None)
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.jarvis/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Jarvis TUI Status" in out
...
def test_skills_reload_runs_in_gateway_process(monkeypatch):
    import brain.skill_commands as skill_commands
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("jarvis_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "jarvis_cli.commands.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("jarvis_state")
    ...
    monkeypatch.setitem(sys.modules, "jarvis_state", fake_mod)
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `jarvis model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `jarvis model` and the classic CLI /model picker.
    ...
    with patch(
        "jarvis_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("jarvis_cli.models.provider_model_ids") as live_fetch:
...
def test_prompt_submit_auto_titles_session_on_complete(monkeypatch):
    ...
    with patch("brain.title_generator.maybe_auto_title") as mock_title:
        server.handle_request(
            {
                "id": "1",
                "method": "prompt.submit",
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``jarvis --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"jarvis_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "jarvis_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "jarvis_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_jarvis_cli_reload_env(monkeypatch):
    ...
    with patch.dict(sys.modules, {"jarvis_cli.config": fake}):
...
def test_make_agent_reads_nested_max_turns(monkeypatch):
    ...
    with patch("run_brain.AIBrain") as mock_agent:
...
def test_make_agent_waits_for_shared_mcp_discovery(monkeypatch):
    ...
    from jarvis_cli import mcp_startup
...
def test_session_save_writes_under_jarvis_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Jarvis profile
    ...
    Regression: the gateway handler wrote ``jarvis_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Jarvis.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("jarvis_conversation_*.json"))
    ...
    assert payload["model"] == "jarvis-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Jarvis."
