"""Tests for the secret-source tracking in ``jarvis_cli.env_loader``.

These cover the small public surface that lets `jarvis model` / `jarvis setup`
...
from jarvis_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_records_bitwarden_origin(tmp_path, monkeypatch):
    ...
    from brain.secret_sources.bitwarden import FetchResult
    ...
    import brain.secret_sources.bitwarden as bw_module
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_jarvis_dotenv()`` is called at module-import time from several
    hot modules (cli.py, jarvis_cli/main.py, run_brain.py, ...).  The
    ...
    runs exactly once per JARVIS_HOME per process.
    """
    ...
    # cli.py, jarvis_cli/main.py, run_brain.py, trajectory_compressor.py,
