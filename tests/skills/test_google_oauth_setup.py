...
class TestJarvisConstantsFallback:
    ...
    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_jarvis_home.py"
    )
    ...
    def test_fallback_ignores_empty_jarvis_home(self, monkeypatch):
        """Empty/whitespace JARVIS_HOME is treated as unset."""
        monkeypatch.setenv("JARVIS_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_jarvis_home() == Path.home() / ".jarvis"

    def test_fallback_display_jarvis_home_shortens_path(self, monkeypatch):
        """Fallback display_jarvis_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("JARVIS_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_jarvis_home() == "~/.jarvis"

    def test_fallback_display_jarvis_home_profile_path(self, monkeypatch):
        """Fallback display_jarvis_home() handles profile paths under ~/."""
        monkeypatch.setenv("JARVIS_HOME", str(Path.home() / ".jarvis/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_jarvis_home() == "~/.jarvis/profiles/coder"

    def test_fallback_display_jarvis_home_custom_path(self, monkeypatch):
        """Fallback display_jarvis_home() returns full path for non-home locations."""
        monkeypatch.setenv("JARVIS_HOME", "/opt/jarvis-custom")
        module = self._load_helper(monkeypatch)
...
class TestInstallDeps:
    ...
    Regression coverage for the Jarvis Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "jarvis-agent[google]" in out
