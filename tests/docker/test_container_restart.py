...
$JARVIS_HOME/profiles/<name>/gateway_state.json on every boot and
...
def restart_container(request, built_image: str):
    ...
    preserves $JARVIS_HOME/profiles/."""
    safe = request.node.name.replace("[", "_").replace("]", "_")
    name = f"jarvis-restart-{safe}"
    volume = f"jarvis-restart-vol-{safe}"
    _docker("rm", "-f", name)
...
def test_running_gateway_survives_container_restart(restart_container: str) -> None:
    ...
    r = _exec(container, "jarvis", "profile", "create", "coder")
    assert r.returncode == 0, f"profile create failed: {r.stderr}"

    r = _exec(container, "jarvis", "-p", "coder", "gateway", "start", timeout=60)
...
def test_stopped_gateway_stays_stopped_after_restart(restart_container: str) -> None:
    ...
    _exec(container, "jarvis", "profile", "create", "writer").check_returncode()
...
def test_stale_gateway_pid_cleaned_up_on_restart(restart_container: str) -> None:
    ...
    _exec(container, "jarvis", "profile", "create", "ghost").check_returncode()
...
def test_live_gateway_autostarts_after_real_restart_without_manual_state_stamp(
    ...
    _exec(container, "jarvis", "profile", "create", "live").check_returncode()
    r = _exec(container, "jarvis", "-p", "live", "gateway", "start", timeout=60)
    assert r.returncode == 0, f"gateway start failed: {r.stderr}"
    ...
    # operator `jarvis gateway stop`), so the shutdown is signal-initiated.
    _docker("restart", container, timeout=60).check_returncode()
