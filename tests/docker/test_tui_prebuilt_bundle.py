...
package-lock.json. Without JARVIS_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV JARVIS_TUI_DIR=/opt/jarvis/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the jarvis user, return stdout."""
    inner = (
        "source /opt/jarvis/.venv/bin/activate && "
        "cd /opt/jarvis && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the jarvis user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "jarvis", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_jarvis_tui_dir_env_is_set(built_image: str) -> None:
    """JARVIS_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$JARVIS_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/jarvis/ui-tui", (
        f"JARVIS_TUI_DIR={r.stdout.strip()!r} (expected /opt/jarvis/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from jarvis_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
        "ui = Path('/opt/jarvis/ui-tui')\n"
    ...
    # With JARVIS_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
