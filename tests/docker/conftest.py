...
Override the image with ``JARVIS_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("JARVIS_TEST_IMAGE", "jarvis-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``JARVIS_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("JARVIS_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged jarvis user
# ---------------------------------------------------------------------------
#
# Background: every Jarvis runtime path inside the container drops to UID
# 10000 (the ``jarvis`` user) via ``s6-setuidgid jarvis``. ``docker exec``
...
# fails (PermissionError swallowed) for jarvis, so a test that ran as root
...
# helpers below run every probe under ``-u jarvis`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: jarvis).
