...
class FakeAgent:
    def __init__(self):
        ...
        self.enabled_toolsets = ["jarvis-acp"]
...
def test_acp_real_agent_gets_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "run_brain", mod("run_brain", AIBrain=CapturingAgent))
    monkeypatch.setitem(
        sys.modules,
        "jarvis_cli.config",
        mod("jarvis_cli.config", load_config=lambda: {"model": {"default": "m", "provider": "p"}}),
    )
    monkeypatch.setitem(
        sys.modules,
