"""Shared fixtures for the jarvis-agent test suite.
...
2. **Isolated JARVIS_HOME.** JARVIS_HOME points to a per-test tempdir so
   code reading ``~/.jarvis/*`` via ``get_jarvis_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".jarvis"`` instead of the canonical
   ``get_jarvis_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No JARVIS_SESSION_* inheritance** — the agent's current gateway
...
_JARVIS_BEHAVIORAL_VARS = frozenset({
    ...
    "JARVIS_INTERACTIVE",
    "JARVIS_QUIET",
    "JARVIS_TOOL_PROGRESS",
    "JARVIS_TOOL_PROGRESS_MODE",
    "JARVIS_MAX_ITERATIONS",
    "JARVIS_SESSION_PLATFORM",
    "JARVIS_SESSION_CHAT_ID",
    "JARVIS_SESSION_CHAT_NAME",
    "JARVIS_SESSION_THREAD_ID",
    "JARVIS_SESSION_SOURCE",
    "JARVIS_SESSION_KEY",
    "JARVIS_GATEWAY_SESSION",
    "JARVIS_CRON_SESSION",
    "_JARVIS_GATEWAY",
    "JARVIS_PLATFORM",
    "JARVIS_MODEL",
    ...
    "JARVIS_TIMEZONE",
    "JARVIS_REDACT_SECRETS",
    "JARVIS_BACKGROUND_NOTIFICATIONS",
    "JARVIS_EXEC_ASK",
    "JARVIS_HOME_MODE",
    "JARVIS_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.jarvis/kanban.db instead of the per-test JARVIS_HOME.
    "JARVIS_KANBAN_DB",
    "JARVIS_KANBAN_BOARD",
    "JARVIS_KANBAN_HOME",
    "JARVIS_KANBAN_WORKSPACES_ROOT",
    "JARVIS_KANBAN_LOGS_ROOT",
    "JARVIS_KANBAN_TASK",
    "JARVIS_KANBAN_WORKSPACE",
    "JARVIS_KANBAN_RUN_ID",
    "JARVIS_KANBAN_CLAIM_LOCK",
    "JARVIS_KANBAN_DISPATCH_IN_GATEWAY",
    "JARVIS_TENANT",
    ...
    "JARVIS_DASHBOARD_OAUTH_CLIENT_ID",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and JARVIS_HOME to per-test tempdirs so code that
    reads ``~/.jarvis/*`` can't touch the real one, and pins TZ/LANG so
    ...
    #    ``~/.jarvis/*`` via ``get_jarvis_home()`` now gets the tempdir.
    #
...
@pytest.fixture()
def mock_config():
    """Return a minimal jarvis config dict suitable for unit tests."""
    return {
...
#    ``check_output`` reject any ``systemctl ... <verb> jarvis-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart jarvis-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting jarvis/python patterns are also blocked.
    """
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch jarvis-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m jarvis_cli.main".
                if (
                    "jarvis" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live jarvis-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting jarvis/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `jarvis update` (or the
        # equivalent `python -m jarvis_cli.main update`).  These commands
        ...
        if "update" in low and (
            # jarvis update / jarvis update --gateway / setsid bash -c ... jarvis update
            ("jarvis" in low and "update" in low.split())
            or
            # python -m jarvis_cli.main update --gateway
            ("jarvis_cli" in low and "update" in low.split())
            or
            # venv/bin/jarvis update  (absolute path variant used in tests)
            (".venv/bin/jarvis" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`jarvis update` against the real checkout, fetching "
