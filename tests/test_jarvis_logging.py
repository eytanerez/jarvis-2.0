"""Tests for jarvis_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
# Use whatever RotatingFileHandler class jarvis_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. jarvis_logging
...
from jarvis_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    jarvis_logging._logging_initialized = False
    ...
    jarvis_logging._install_session_record_factory()
    yield
    ...
    jarvis_logging._logging_initialized = False
    jarvis_logging.clear_session_context()


@pytest.fixture
def jarvis_home(tmp_path, monkeypatch):
    """Provide an isolated JARVIS_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_jarvis_home from conftest,
...
class TestSetupLogging:
    """setup_logging() creates brain.log + errors.log with RotatingFileHandler."""

    def test_creates_log_directory(self, jarvis_home):
        log_dir = jarvis_logging.setup_logging(jarvis_home=jarvis_home)
        assert log_dir == jarvis_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in root.handlers
            if isinstance(h, RotatingFileHandler)
            and "brain.log" in getattr(h, "baseFilename", "")
        ]
        assert len(agent_handlers) == 1
        assert agent_handlers[0].level == logging.INFO

    def test_creates_errors_log_handler(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        jarvis_logging.setup_logging(jarvis_home=jarvis_home, force=True)
    ...
    def test_custom_log_level(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, jarvis_home):
        jarvis_logging.setup_logging(
            jarvis_home=jarvis_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)
    ...
    def test_writes_to_agent_log(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)

        test_logger = logging.getLogger("test_jarvis_logging.write_test")
        test_logger.info("test message for brain.log")

        # Flush handlers
        for h in logging.getLogger().handlers:
            h.flush()

        agent_log = jarvis_home / "logs" / "brain.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for brain.log" in content

    def test_warnings_appear_in_both_logs(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)

        test_logger = logging.getLogger("test_jarvis_logging.warning_test")
        ...
        agent_log = jarvis_home / "logs" / "brain.log"
        errors_log = jarvis_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)

        test_logger = logging.getLogger("test_jarvis_logging.info_test")
    ...
    def test_explicit_params_override_config(self, jarvis_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (jarvis_home / "config.yaml").write_text(yaml.dump(config))

        jarvis_logging.setup_logging(jarvis_home=jarvis_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, jarvis_home):
        """The custom record factory injects session_tag on all records."""
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_jarvis_session_injector", False), (
            "Record factory should have _jarvis_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, jarvis_home):
        ...
        gw_log = jarvis_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, jarvis_home):
    ...
    def test_gateway_log_receives_gateway_records(self, jarvis_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, jarvis_home):
        ...
        agent_logger = logging.getLogger("brain.context_compressor")
    ...
    def test_agent_log_still_receives_all(self, jarvis_home):
        """brain.log (catch-all) still receives gateway AND tool records."""
        jarvis_logging.setup_logging(jarvis_home=jarvis_home, mode="gateway")
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_created_after_cli_init(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home, mode="cli")
        jarvis_logging.setup_logging(jarvis_home=jarvis_home, mode="gui")
    ...
    def test_gui_log_receives_only_gui_components(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home, mode="gui")

        logging.getLogger("jarvis_cli.web_server").info("dashboard online")
        ...
        gui_log = jarvis_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, jarvis_home):
        """When session context is set, log lines include [session_id]."""
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)
        jarvis_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, jarvis_home):
    ...
    def test_clear_session_context(self, jarvis_home):
        """After clearing, session tag disappears."""
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)
        jarvis_logging.set_session_context("xyz789")
        jarvis_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, jarvis_home):
        ...
        def thread_a():
            jarvis_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            jarvis_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        jarvis_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        jarvis_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = jarvis_logging._ComponentFilter(("gateway",))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in jarvis_logging.COMPONENT_PREFIXES
        # The gateway component captures both core gateway logs and the
        # jarvis_plugins facility (plugin-installed gateway adapters log
        # under that prefix).
        assert ("gateway", "jarvis_plugins") == jarvis_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = jarvis_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_brain" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == jarvis_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = jarvis_logging.COMPONENT_PREFIXES["cli"]
        assert "jarvis_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == jarvis_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = jarvis_logging.COMPONENT_PREFIXES["gui"]
        assert "jarvis_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)
        jarvis_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_jarvis_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, jarvis_home):
        jarvis_logging.setup_logging(jarvis_home=jarvis_home)
        jarvis_logging.setup_verbose_logging()
        jarvis_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        jarvis_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = jarvis_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        jarvis_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("jarvis_cli.config.is_managed", return_value=True):
                jarvis_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, jarvis_home):
        level, max_size, backup = jarvis_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, jarvis_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, jarvis_home):
        import yaml
        config = {"model": "test"}
        (jarvis_home / "config.yaml").write_text(yaml.dump(config))
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> jarvis_logging._ManagedRotatingFileHandler:
        handler = jarvis_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that jarvis_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        handler.flush()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        records leaking to brain.log) when something external rotates the
        file between setup_logging() calls.
        """
        jarvis_logging.setup_logging(jarvis_home=jarvis_home, mode="gateway")
        gw_path = jarvis_home / "logs" / "gateway.log"
        rotated = jarvis_home / "logs" / "gateway.log.1"
        ...
        # going into brain.log only, never gateway.log.
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = jarvis_logging._safe_stderr()
