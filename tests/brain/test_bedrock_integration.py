...
Note: Tests that import ``jarvis_cli.auth`` or ``jarvis_cli.runtime_provider``
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        with patch("jarvis_cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_resolution(self, monkeypatch):
        ...
        with patch("jarvis_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("jarvis_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}):
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("jarvis_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("jarvis_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("jarvis_cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
...
class TestProvidersModule:
    ...
    def test_bedrock_transport_mapping(self):
        from jarvis_cli.providers import TRANSPORT_TO_API_MODE
...
class TestErrorClassifierBedrock:
    ...
    def test_throttling_in_rate_limit_patterns(self):
        from brain.error_classifier import _RATE_LIMIT_PATTERNS
        assert "throttlingexception" in _RATE_LIMIT_PATTERNS

    def test_context_overflow_patterns(self):
        from brain.error_classifier import _CONTEXT_OVERFLOW_PATTERNS
...
class TestPackaging:
    ...
    def test_bedrock_is_not_eager_installed_by_all_extra(self):
        extras = self._optional_dependencies()
        assert "jarvis-agent[bedrock]" not in extras["all"]
...
# ``AIBrain._anthropic_preserve_dots`` returned False for the ``bedrock``
...
class TestBedrockPreserveDotsFlag:
    """``AIBrain._anthropic_preserve_dots`` must return True on Bedrock so
    ...
    def test_bedrock_provider_preserves_dots(self):
        ...
        assert AIBrain._anthropic_preserve_dots(agent) is True
    ...
    def test_non_bedrock_aws_url_does_not_preserve_dots(self):
        ...
        from run_brain import AIBrain
        assert AIBrain._anthropic_preserve_dots(agent) is False
...
class TestBedrockModelNameNormalization:
    ...
    def test_global_anthropic_inference_profile_preserved(self):
        ...
        from brain.anthropic_adapter import normalize_model_name
...
class TestBedrockBuildAnthropicKwargsEndToEnd:
    ...
    def test_bedrock_inference_profile_survives_build_kwargs(self):
        from brain.anthropic_adapter import build_anthropic_kwargs
...
class TestBedrockModelIdDetection:
    ...
    def test_bare_bedrock_id_detected(self):
        from brain.anthropic_adapter import _is_bedrock_model_id
...
class TestAuxiliaryClientBedrockResolution:
    ...
    def test_bedrock_returns_client_with_credentials(self, monkeypatch):
        ...
        with patch("brain.anthropic_adapter.build_anthropic_bedrock_client",
                   return_value=mock_anthropic_bedrock):
            from brain.auxiliary_client import resolve_provider_client, AnthropicAuxiliaryClient
    ...
    def test_bedrock_returns_none_without_credentials(self, monkeypatch):
        ...
        with patch("brain.bedrock_adapter.has_aws_credentials", return_value=False):
    ...
    def test_bedrock_async_mode(self, monkeypatch):
        ...
        with patch("brain.anthropic_adapter.build_anthropic_bedrock_client",
                   return_value=MagicMock()):
            from brain.auxiliary_client import resolve_provider_client, AsyncAnthropicAuxiliaryClient
