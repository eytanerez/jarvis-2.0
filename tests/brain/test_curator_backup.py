"""Tests for brain/curator_backup.py — snapshot + rollback of the skills tree."""
...
def backup_env(monkeypatch, tmp_path):
    """Isolate JARVIS_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_jarvis_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd jarvis_constants
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level JARVIS_DIR picks up the tmp HOME."""
    import jarvis_constants
    importlib.reload(jarvis_constants)
