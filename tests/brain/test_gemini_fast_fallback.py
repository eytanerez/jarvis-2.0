...
from run_brain import _pool_may_recover_from_rate_limit
...
def test_conversation_loop_resolves_pool_helper_through_run_brain_module():
    """Extracted conversation loop must honor tests/patches on run_brain.

    conversation_loop intentionally lazy-loads run_brain via _ra().  If this
    call site uses a bare imported helper, monkeypatching run_brain in tests (and
    production wrappers that patch run_brain) will not propagate into the
