...
class TestCallbackSubprocess:
    ...
    def test_block_translation_end_to_end(self, tmp_path):
        ...
        must translate it to the canonical Jarvis block shape so that
    ...
    def test_block_aggregation_through_plugin_manager(self, tmp_path, monkeypatch):
        ...
        end-to-end control flow used by run_brain._invoke_tool."""
        from jarvis_cli import plugins
        ...
        monkeypatch.setenv("JARVIS_ACCEPT_HOOKS", "1")
...
class TestAllowlistConcurrency:
    ...
    def test_save_allowlist_failure_logs_actionable_warning(
        ...
        re-prompt consequence so "jarvis keeps asking" is debuggable."""
        import logging
        monkeypatch.setenv("JARVIS_HOME", str(tmp_path / "home"))
        monkeypatch.setattr(
    ...
    def test_command_script_path_resolution(self):
        ...
        ``jarvis hooks doctor``'s executability check and silently
