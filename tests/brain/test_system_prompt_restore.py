...
byte-stable across turns (fresh AIBrain → must restore from session DB
...
from brain.conversation_loop import _restore_or_build_system_prompt
...
class TestStoredPromptReuse:
    def test_present_row_is_reused_verbatim(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="brain.conversation_loop"):
    ...
    def test_present_row_with_stale_runtime_identity_rebuilds(self, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="brain.conversation_loop"):
            _restore_or_build_system_prompt(agent, None, [{"role": "user", "content": "hi"}])
