...
from brain.credits_tracker import (
    CREDITS_NOTICE_KIND,
...
class TestIsFreeTierModel:
    def test_free_suffix_is_free(self):
        from brain.credits_tracker import is_free_tier_model

        assert is_free_tier_model("nvidia/nemotron-3-ultra:free") is True
        assert is_free_tier_model("Jarvis-4-70B:free", "https://inference-api.nousresearch.com") is True

    def test_empty_or_paid_model_is_not_free(self):
        from brain.credits_tracker import is_free_tier_model

        assert is_free_tier_model("") is False
        assert is_free_tier_model("Jarvis-4-405B") is False
