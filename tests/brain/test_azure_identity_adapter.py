"""Tests for the Microsoft Entra ID adapter (brain/azure_identity_adapter.py).
...
def _reset_adapter_cache():
    from brain.azure_identity_adapter import reset_credential_cache
...
class TestEntraScopeConstant:
    ...
    def test_default_scope_matches_microsoft_documentation(self):
        from brain.azure_identity_adapter import SCOPE_AI_AZURE_DEFAULT
...
class TestMaterializeBearerForHttp:
    ...
    def test_callable_is_invoked_and_returns_token(self):
        from brain.azure_identity_adapter import materialize_bearer_for_http
...
class TestBuildBearerHttpClient:
    ...
    def test_hook_strips_auth_headers_and_warns_when_token_provider_fails(self, caplog):
        ...
        try:
            ...
            with caplog.at_level(logging.WARNING, logger="brain.azure_identity_adapter"):
...
class TestEntraIdentityConfig:
    ...
    def test_to_dict_round_trip(self):
        from brain.azure_identity_adapter import EntraIdentityConfig
...
def fake_azure_identity(monkeypatch):
    ...
    # singleton state.
    from brain import azure_identity_adapter as _adapter
...
class TestBuildCredential:
    def test_default_kwargs_are_minimal(self, fake_azure_identity):
        ...
        azure-identity directly), not Jarvis config kwargs."""
        from brain.azure_identity_adapter import EntraIdentityConfig, build_credential
...
class TestBuildTokenProvider:
    def test_returns_callable_for_scope(self, fake_azure_identity):
        from brain.azure_identity_adapter import build_token_provider
...
class TestHasAzureIdentityCredentials:
    ...
    def test_returns_true_on_successful_token_mint(self, fake_azure_identity):
        from brain.azure_identity_adapter import has_azure_identity_credentials
...
# ---------------------------------------------------------------------------
# describe_active_credential — used by jarvis doctor + jarvis auth
# ---------------------------------------------------------------------------
...
class TestDescribeActiveCredential:
    ...
    def test_reports_env_sources_for_managed_identity(self, fake_azure_identity, monkeypatch):
        from brain.azure_identity_adapter import describe_active_credential
