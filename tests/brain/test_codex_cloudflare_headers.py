...
header set so the primary chat client (``run_brain.AIBrain.__init__`` +
...
- graceful handling of malformed tokens (drop the account-ID header, don't
  raise)
- primary-client wiring at both entry points in ``run_brain.py``
- aux-client wiring at both entry points in ``brain/auxiliary_client.py``
"""
...
# ---------------------------------------------------------------------------
# Primary chat client wiring (run_brain.AIBrain)
# ---------------------------------------------------------------------------
...
class TestAuxiliaryClientWiring:
    def test_build_codex_client_passes_codex_headers(self, monkeypatch):
        ...
        headers."""
        from brain import auxiliary_client
