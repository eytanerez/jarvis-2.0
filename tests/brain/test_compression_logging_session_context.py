...
which moves ``JARVIS_SESSION_ID`` env + ContextVar). The ``[session_id]`` tag on
log lines comes from a SEPARATE mechanism — ``jarvis_logging._session_context``
...
def _build_agent_with_db(db: SessionDB, session_id: str):
    """Mirror tests/brain/test_compression_concurrent_fork.py's harness."""
...
def test_logging_session_context_follows_compression_rotation(tmp_path: Path) -> None:
    ...
    jarvis_logging.set_session_context(parent_sid)
    try:
        ...
        current = getattr(jarvis_logging._session_context, "session_id", None)
