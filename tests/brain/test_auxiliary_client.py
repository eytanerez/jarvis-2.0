...
def _clean_env(monkeypatch):
    ...
    import brain.auxiliary_client as _aux_mod
...
def codex_auth_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "brain.auxiliary_client._read_codex_access_token",
...
class TestAuxiliaryMaxTokensParam:
    def test_uses_max_completion_tokens_for_github_copilot_custom_base(self):
        with patch("brain.auxiliary_client._resolve_custom_runtime", return_value=("https://api.githubcopilot.com", "key", None)), \
             patch("brain.auxiliary_client._read_nous_auth", return_value=None):
    ...
    def test_uses_max_completion_tokens_for_github_copilot_custom_base_path(self):
        with patch("brain.auxiliary_client._resolve_custom_runtime", return_value=("https://api.githubcopilot.com/chat/completions", "key", None)), \
             patch("brain.auxiliary_client._read_nous_auth", return_value=None):
            assert auxiliary_max_tokens_param(2048) == {"max_completion_tokens": 2048}
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("nous", "jarvis-4", "https://inference-api.nousresearch.com/v1"),
    ...
    def test_omits_max_tokens_for_openai_compatible(self, provider, model, base_url):
        from brain.auxiliary_client import _build_call_kwargs
...
class TestNousTagsScoping:
    def test_tags_injected_when_provider_is_nous(self, monkeypatch):
        import brain.auxiliary_client as aux
...
class TestReadCodexAccessToken:
    ...
    def test_pool_without_selected_entry_falls_back_to_auth_store(self, tmp_path, monkeypatch):
        ...
        with patch("brain.auxiliary_client._select_pool_entry", return_value=(True, None)), \
             patch("jarvis_cli.auth._read_codex_tokens", return_value={
    ...
    def test_missing_returns_none(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("JARVIS_HOME", str(jarvis_home))
        with patch("brain.auxiliary_client._select_pool_entry", return_value=(False, None)):
            result = _read_codex_access_token()
    ...
    def test_malformed_json_returns_none(self, tmp_path):
        ...
        with patch("brain.auxiliary_client.Path.home", return_value=tmp_path):
            result = _read_codex_access_token()
        assert result is None
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``jarvis auth status`` already reports these as logged in; compression
        should not fall through to "no auxiliary provider configured" just
        because the singleton auth-store entry is absent.
        """
        from brain.credential_pool import AUTH_TYPE_OAUTH, PooledCredential, load_pool
...
class TestAnthropicOAuthFlag:
    ...
    def test_oauth_token_sets_flag(self, monkeypatch):
        ...
        with patch("brain.anthropic_adapter.build_anthropic_client") as mock_build:
            mock_build.return_value = MagicMock()
            from brain.auxiliary_client import _try_anthropic, AnthropicAuxiliaryClient
    ...
    def test_api_key_no_oauth_flag(self, monkeypatch):
        ...
        with patch("brain.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-api03-testkey1234"), \
             patch("brain.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("brain.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from brain.auxiliary_client import _try_anthropic, AnthropicAuxiliaryClient
            client, model = _try_anthropic()
            assert client is not None
    ...
    def test_pool_entry_takes_priority_over_legacy_resolution(self):
        ...
        with (
            patch("brain.auxiliary_client.load_pool", return_value=_Pool()),
            patch("brain.anthropic_adapter.resolve_anthropic_token", side_effect=AssertionError("legacy path should not run")),
            patch("brain.anthropic_adapter.build_anthropic_client", return_value=MagicMock()) as mock_build,
        ):
            from brain.auxiliary_client import _try_anthropic
...
class TestBuildCodexClient:
    def test_pool_without_selected_entry_falls_back_to_auth_store(self):
        with (
            patch("brain.auxiliary_client._select_pool_entry", return_value=(True, None)),
            patch("brain.auxiliary_client._read_codex_access_token", return_value="codex-auth-token"),
            patch("brain.auxiliary_client.OpenAI") as mock_openai,
        ):
            mock_openai.return_value = MagicMock()
            from brain.auxiliary_client import _build_codex_client
    ...
    def test_cached_codex_client_rebuilds_when_pool_entry_changes(self):
        ...
        with (
            patch("brain.auxiliary_client.load_pool", return_value=pool),
            patch("brain.auxiliary_client.OpenAI", side_effect=[client_a, client_b]) as mock_openai,
        ):
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``jarvis model``, no per-task override is
    ...
    def test_empty_model_for_oauth_provider_falls_back_to_main_model(self):
        ...
        with (
            patch(
                "brain.auxiliary_client._read_main_model",
                return_value="grok-4.3",
            ),
            patch(
                "brain.auxiliary_client._get_aux_model_for_provider",
                return_value="",  # xai-oauth has no catalog default
            ),
            patch(
                "brain.auxiliary_client._build_xai_oauth_aux_client",
    ...
    def test_empty_model_for_codex_also_uses_main_model(self):
        ...
        with (
            ...
            patch(
                "brain.auxiliary_client._build_codex_client",
                return_value=(MagicMock(), "gpt-5.4"),
            ) as mock_build,
            patch(
                "brain.auxiliary_client._select_pool_entry",
    ...
    def test_empty_model_for_catalog_provider_uses_catalog_default(self):
        ...
        with (
            ...
            patch(
                "brain.anthropic_adapter.build_anthropic_client",
                return_value=MagicMock(),
            ),
            patch(
                "brain.anthropic_adapter.resolve_anthropic_token",
                return_value="sk-ant-***",
            ),
            patch(
                "brain.auxiliary_client._read_nous_auth", return_value=None
            ),
        ):
    ...
    def test_explicit_model_takes_precedence_over_fallbacks(self):
        ...
        with (
            patch("brain.auxiliary_client._read_main_model") as mock_read_main,
            patch(
                "brain.auxiliary_client._get_aux_model_for_provider",
...
class TestExpiredCodexFallback:
    ...
    def test_expired_codex_falls_through_to_next(self, tmp_path, monkeypatch):
        ...
        with patch("brain.anthropic_adapter.build_anthropic_client") as mock_build:
            mock_build.return_value = MagicMock()
            from brain.auxiliary_client import _resolve_auto
    ...
    def test_expired_codex_openrouter_wins(self, tmp_path, monkeypatch):
        ...
        with patch("brain.auxiliary_client.OpenAI") as mock_openai:
            mock_openai.return_value = MagicMock()
            from brain.auxiliary_client import _resolve_auto
            client, model = _resolve_auto()
            assert client is not None
    ...
    def test_expired_codex_custom_endpoint_wins(self, tmp_path, monkeypatch):
        ...
        with patch("brain.auxiliary_client._resolve_custom_runtime",
    ...
    def test_jarvis_oauth_file_sets_oauth_flag(self, monkeypatch):
        ...
        with patch("brain.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-oat-jarvis-token"), \
             patch("brain.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("brain.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from brain.auxiliary_client import _try_anthropic
...
class TestExplicitProviderRouting:
    ...
    def test_explicit_anthropic_api_key(self, monkeypatch):
        ...
        with patch("brain.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-api-regular-key"), \
             patch("brain.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("brain.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            client, model = resolve_provider_client("anthropic")
    ...
    def test_explicit_openrouter_pool_exhausted_logs_precise_warning(self, monkeypatch, caplog):
        ...
        with patch("brain.auxiliary_client._select_pool_entry", return_value=(True, None)):
            with caplog.at_level(logging.WARNING, logger="brain.auxiliary_client"):
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            patch("brain.auxiliary_client.load_pool", return_value=_Pool()),
            patch("brain.auxiliary_client.OpenAI"),
            patch("jarvis_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from brain.auxiliary_client import _build_codex_client

            client, model = _build_codex_client("gpt-5.4")

        from brain.auxiliary_client import CodexAuxiliaryClient
    ...
    def test_returns_none_when_nothing_available(self, monkeypatch):
        ...
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        with patch("brain.auxiliary_client._read_nous_auth", return_value=None), \
             patch("brain.auxiliary_client._read_codex_access_token", return_value=None), \
             patch("brain.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(self):
        with patch("brain.auxiliary_client._resolve_custom_runtime",
             ...
             patch("brain.auxiliary_client._read_nous_auth", return_value=None), \
             patch("brain.auxiliary_client._resolve_nous_runtime_api", return_value=None), \
             patch("brain.auxiliary_client._read_main_model", return_value="gpt-5.3-codex"), \
             patch("brain.auxiliary_client.OpenAI") as mock_openai:
            client, model = get_text_auxiliary_client()
...
class TestVisionClientFallback:
    ...
    def test_vision_auto_includes_active_provider_when_configured(self, monkeypatch):
        ...
        with (
            patch("brain.auxiliary_client._read_nous_auth", return_value=None),
            patch("brain.auxiliary_client._read_main_provider", return_value="anthropic"),
            patch("brain.auxiliary_client._read_main_model", return_value="claude-sonnet-4"),
            patch("brain.anthropic_adapter.build_anthropic_client", return_value=MagicMock()),
            patch("brain.anthropic_adapter.resolve_anthropic_token", return_value="***"),
        ):
    ...
    def test_anthropic_auxiliary_client_aggregates_stream_response(self):
        from brain.auxiliary_client import AnthropicAuxiliaryClient
...
class TestAuxiliaryPoolAwareness:
    def test_try_nous_uses_pool_entry(self):
        ...
        with (
            ...
            from brain.auxiliary_client import _try_nous
    ...
    def test_try_nous_uses_portal_recommendation_for_text(self):
        ...
        with (
            patch("brain.auxiliary_client._read_nous_auth", return_value={"access_token": "***"}),
            patch("brain.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", fresh_base)),
    ...
    def test_call_llm_retries_nous_after_401(self):
        ...
        with (
            patch("brain.auxiliary_client._resolve_task_provider_model", return_value=("nous", "nous-model", None, None, None)),
            patch("brain.auxiliary_client._get_cached_client", return_value=(stale_client, "nous-model")),
            patch("brain.auxiliary_client.OpenAI", return_value=fresh_client),
            patch("brain.auxiliary_client._validate_llm_response", side_effect=lambda resp, _task: resp),
            patch("brain.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", "https://inference-api.nousresearch.com/v1")),
        ):
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hi"}],
    ...
    async def test_async_call_llm_retries_nous_after_401(self):
        ...
        with (
            patch("brain.auxiliary_client._resolve_task_provider_model", return_value=("nous", "nous-model", None, None, None)),
            patch("brain.auxiliary_client._get_cached_client", return_value=(stale_client, "nous-model")),
            patch("brain.auxiliary_client._to_async_client", return_value=(fresh_async_client, "nous-model")),
            patch("brain.auxiliary_client._validate_llm_response", side_effect=lambda resp, _task: resp),
            patch("brain.auxiliary_client._resolve_nous_runtime_api", return_value=("fresh-agent-key", "https://inference-api.nousresearch.com/v1")),
        ):
    ...
    def test_cached_gmi_client_keeps_explicit_slash_model_override(self):
        ...
        with patch(
            "brain.auxiliary_client.resolve_provider_client",
...
class TestRefreshNousRecommendedModel:
    ...
    def test_returns_fresh_portal_recommendation(self, monkeypatch):
        monkeypatch.setattr(
            "jarvis_cli.models.get_nous_recommended_aux_model",
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "jarvis_cli.models.get_nous_recommended_aux_model", _boom)
        out = _refresh_nous_recommended_model(
...
class TestGetProviderChain:
    ...
    def test_picks_up_patched_functions(self):
        ...
        with patch("brain.auxiliary_client._try_openrouter", sentinel):
            chain = _get_provider_chain()
...
class TestTryPaymentFallback:
    ...
    def _clear_unhealthy_cache(self):
        ...
        """
        from brain.auxiliary_client import _aux_unhealthy_until, _aux_unhealthy_logged_at
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("brain.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("brain.auxiliary_client._try_nous", return_value=(mock_client, "nous-model")), \
             patch("brain.auxiliary_client._read_main_provider", return_value="openrouter"):
    ...
    def test_returns_none_when_no_fallback(self):
        with patch("brain.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("brain.auxiliary_client._try_nous", return_value=(None, None)), \
             patch("brain.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("brain.auxiliary_client._resolve_api_key_provider", return_value=(None, None)), \
             patch("brain.auxiliary_client._read_main_provider", return_value="openrouter"):
    ...
    def test_codex_alias_maps_to_chain_label(self):
        ...
        mock_client = MagicMock()
        with patch("brain.auxiliary_client._try_openrouter", return_value=(mock_client, "or-model")), \
             patch("brain.auxiliary_client._read_main_provider", return_value="openai-codex"):
...
class TestCallLlmPaymentFallback:
    ...
    def test_non_payment_error_not_caught(self, monkeypatch):
        ...
        with patch("brain.auxiliary_client._get_cached_client",
                    return_value=(primary_client, "google/gemini-3-flash-preview")), \
             patch("brain.auxiliary_client._resolve_task_provider_model",
    ...
    def test_429_rate_limit_triggers_fallback(self, monkeypatch):
        ...
        with patch("brain.auxiliary_client._get_cached_client",
             ...
             patch("brain.auxiliary_client._try_payment_fallback",
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_auto_provider_uses_task_then_main_chain_before_builtin_chain(self, monkeypatch):
        ...
        with patch("brain.auxiliary_client._get_cached_client",
             ...
             patch("brain.auxiliary_client._try_configured_fallback_chain",
                   return_value=(None, None, "")) as mock_task_chain, \
             patch("brain.auxiliary_client._try_main_fallback_chain",
                   return_value=(main_chain_client, "inclusionai/ring-2.6-1t:free", "openrouter")) as mock_main_chain, \
             patch("brain.auxiliary_client._try_payment_fallback") as mock_builtin_chain:
            result = call_llm(
    ...
    def test_explicit_provider_uses_configured_chain_first(self, monkeypatch, caplog):
        ...
        with patch("brain.auxiliary_client._get_cached_client",
             ...
             patch("brain.auxiliary_client._try_main_agent_model_fallback",
    ...
    def test_warning_emitted_when_all_fallbacks_exhausted(self, monkeypatch, caplog):
        ...
        with patch("brain.auxiliary_client._get_cached_client",
             ...
             patch("brain.auxiliary_client._try_main_agent_model_fallback",
                   return_value=(None, None, "")), \
             caplog.at_level("WARNING", logger="brain.auxiliary_client"):
...
class TestTryMainAgentModelFallback:
    ...
    def test_returns_none_when_main_provider_is_auto(self):
        from brain.auxiliary_client import _try_main_agent_model_fallback
        with patch("brain.auxiliary_client._read_main_provider", return_value="auto"), \
             patch("brain.auxiliary_client._read_main_model", return_value="some-model"):
    ...
    def test_returns_none_when_failed_provider_equals_main(self):
        ...
        from brain.auxiliary_client import _try_main_agent_model_fallback
        with patch("brain.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("brain.auxiliary_client._read_main_model", return_value="anthropic/claude-sonnet-4"):
    ...
    def test_resolves_main_provider_client(self):
        ...
        with patch("brain.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("brain.auxiliary_client._read_main_model", return_value="anthropic/claude-sonnet-4"), \
             patch("brain.auxiliary_client._is_provider_unhealthy", return_value=False), \
             patch("brain.auxiliary_client.resolve_provider_client",
    ...
    def test_skips_when_main_provider_is_unhealthy(self):
        ...
        with patch("brain.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("brain.auxiliary_client._read_main_model", return_value="anthropic/claude-sonnet-4"), \
             patch("brain.auxiliary_client._is_provider_unhealthy", return_value=True):
            client, model, label = _try_main_agent_model_fallback("glm", task="vision")
        assert client is None
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from jarvis_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("brain.auxiliary_client._try_anthropic", mock_try_anthropic)
    monkeypatch.setattr("jarvis_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
        "jarvis_cli.auth.is_provider_explicitly_configured",
        lambda pid: False,
    )

    from brain.auxiliary_client import _resolve_api_key_provider
...
class TestTransientTransportRetry:
    ...
    def _patches(self, client):
        return (
            patch(
                "brain.auxiliary_client._resolve_task_provider_model",
                return_value=("openrouter", "some-model", None, None, None),
            ),
            patch(
                "brain.auxiliary_client._get_cached_client",
                return_value=(client, "some-model"),
            ),
            patch(
                "brain.auxiliary_client._validate_llm_response",
    ...
    def test_second_transient_failure_escalates_to_fallback(self):
        ...
        with (
            p1, p2, p3,
            patch(
                "brain.auxiliary_client._try_configured_fallback_chain",
                return_value=(None, None, ""),
            ),
            patch(
                "brain.auxiliary_client._try_main_agent_model_fallback",
...
class TestIsConnectionError:
    ...
    def test_connection_refused(self):
        from brain.auxiliary_client import _is_connection_error
...
class TestStaleBaseUrlWarning:
    ...
    def test_warns_when_openai_base_url_set_with_named_provider(self, monkeypatch, caplog):
        ...
        import brain.auxiliary_client as mod
        ...
        with patch("brain.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("brain.auxiliary_client._read_main_model", return_value="google/gemini-flash"), \
             caplog.at_level(logging.WARNING, logger="brain.auxiliary_client"):
            _resolve_auto()
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("jarvis_cli.config.load_config", return_value=config), patch(
            "brain.auxiliary_client._get_cached_client",
    ...
    def test_no_warning_when_provider_is_custom(self, monkeypatch, caplog):
        ...
        with patch("brain.auxiliary_client._read_main_provider", return_value="custom"), \
             patch("brain.auxiliary_client._read_main_model", return_value="llama3"), \
             patch("brain.auxiliary_client._resolve_custom_runtime",
    ...
    def test_no_warning_when_provider_is_named_custom(self, monkeypatch, caplog):
        ...
        with patch("brain.auxiliary_client._read_main_provider", return_value="custom:ollama-local"), \
             patch("brain.auxiliary_client._read_main_model", return_value="llama3"), \
             patch("brain.auxiliary_client.resolve_provider_client",
...
class TestAnthropicCompatImageConversion:
    ...
    def test_known_providers_detected(self):
        from brain.auxiliary_client import _is_anthropic_compat_endpoint
    ...
    def test_base64_image_converted(self):
        from brain.auxiliary_client import _convert_openai_images_to_anthropic
        messages = [{
            "role": "user",
            "content": [
...
class TestAuxiliaryAuthRefreshRetry:
    def test_call_llm_refreshes_codex_on_401_for_vision(self):
        ...
        with (
            patch(
                "brain.auxiliary_client.resolve_vision_provider_client",
                side_effect=[("openai-codex", failing_client, "gpt-5.4"), ("openai-codex", fresh_client, "gpt-5.4")],
            ),
            patch("brain.auxiliary_client._refresh_provider_credentials", return_value=True) as mock_refresh,
        ):
            resp = call_llm(
                task="vision",
    ...
    def test_call_llm_refreshes_codex_on_401_for_non_vision(self):
        ...
        with (
            patch("brain.auxiliary_client._resolve_task_provider_model", return_value=("openai-codex", "gpt-5.4", None, None, None)),
            patch("brain.auxiliary_client._get_cached_client", side_effect=[(stale_client, "gpt-5.4"), (fresh_client, "gpt-5.4")]),
            patch("brain.auxiliary_client._refresh_provider_credentials", return_value=True) as mock_refresh,
        ):
            resp = call_llm(
                task="compression",
    ...
    def test_call_llm_refreshes_anthropic_on_401_for_non_vision(self):
        ...
        with (
            patch("brain.auxiliary_client._resolve_task_provider_model", return_value=("anthropic", "claude-haiku-4-5-20251001", None, None, None)),
            patch("brain.auxiliary_client._get_cached_client", side_effect=[(stale_client, "claude-haiku-4-5-20251001"), (fresh_client, "claude-haiku-4-5-20251001")]),
            patch("brain.auxiliary_client._refresh_provider_credentials", return_value=True) as mock_refresh,
        ):
            resp = call_llm(
                task="compression",
    ...
    def test_refresh_provider_credentials_force_refreshes_anthropic_oauth_and_evicts_cache(self, monkeypatch):
        ...
        with (
            patch("brain.auxiliary_client._client_cache", {cache_key: (stale_client, "claude-haiku-4-5-20251001", None)}),
            patch("brain.anthropic_adapter.read_claude_code_credentials", return_value={
                "accessToken": "expired-token",
                "refreshToken": "refresh-token",
                "expiresAt": 0,
            }),
            patch("brain.anthropic_adapter.refresh_anthropic_oauth_pure", return_value={
                "access_token": "fresh-token",
                "refresh_token": "refresh-token-2",
                "expires_at_ms": 9999999999999,
            }) as mock_refresh_oauth,
            patch("brain.anthropic_adapter._write_claude_code_credentials") as mock_write,
        ):
            from brain.auxiliary_client import _refresh_provider_credentials
...
class TestAuxiliaryPoolRotationRetry:
    def test_call_llm_rotates_explicit_codex_pool_on_429(self):
        ...
        with (
            patch("brain.auxiliary_client._resolve_task_provider_model", return_value=("openai-codex", "gpt-5.4", None, None, None)),
            patch("brain.auxiliary_client._get_cached_client", side_effect=[(stale_client, "gpt-5.4"), (fresh_client, "gpt-5.4")]),
            patch("brain.auxiliary_client._refresh_provider_credentials", return_value=False),
            patch("brain.auxiliary_client.load_pool", return_value=pool),
            patch("brain.auxiliary_client._try_payment_fallback") as mock_fallback,
        ):
            resp = call_llm(
                task="compression",
                provider="openai-codex",
...
class TestCodexAdapterReasoningTranslation:
    ...
    brain/transports/codex.py::build_kwargs() behavior.
    ...
    def _build_adapter():
        ...
        from brain.auxiliary_client import _CodexCompletionsAdapter
        from types import SimpleNamespace
    ...
    def test_reasoning_effort_null_falls_back_to_medium(self):
        """Parity with brain/transports/codex.py::build_kwargs() — falsy
...
class TestVisionAutoSkipsKimiCoding:
    ...
    def test_kimi_coding_skipped_falls_through_to_openrouter(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "brain.auxiliary_client._read_main_provider", lambda: "kimi-coding",
        )
        monkeypatch.setattr(
            "brain.auxiliary_client._read_main_model", lambda: "kimi-code",
        )
        ...
        monkeypatch.setattr(
            "brain.auxiliary_client.resolve_provider_client", rpc_mock,
        )
        ...
        monkeypatch.setattr(
            "brain.auxiliary_client._resolve_strict_vision_backend",
            fake_strict,
        )
    ...
    def test_kimi_coding_cn_skipped_too(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "brain.auxiliary_client._read_main_provider", lambda: "kimi-coding-cn",
        )
        monkeypatch.setattr(
            "brain.auxiliary_client._read_main_model", lambda: "kimi-code",
        )
    ...
    def test_explicit_override_to_kimi_coding_still_honored(self, monkeypatch):
        ...
        """
        monkeypatch.setattr(
            "brain.auxiliary_client._read_main_provider", lambda: "openrouter",
        )
        fake_kimi_client = MagicMock(name="kimi_client")
        gcc_mock = MagicMock(return_value=(fake_kimi_client, "kimi-code"))
        monkeypatch.setattr(
            "brain.auxiliary_client._get_cached_client", gcc_mock,
        )
    ...
    def test_skip_set_covers_exactly_known_entries(self):
        ...
        from brain.auxiliary_client import _PROVIDERS_WITHOUT_VISION
...
class TestCodexAuxiliaryAdapterNullOutputRecovery:
    ...
    def test_handles_final_output_is_none_after_consumer(self):
        ...
        from brain import codex_runtime
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/NousResearch/jarvis-brain/issues/23432.
    """
    ...
    def test_evict_cached_client_instance_handles_none_and_misses(self):
        from brain.auxiliary_client import _evict_cached_client_instance
    ...
    def test_call_llm_evicts_on_connection_error_with_explicit_provider(self):
        ...
        """
        from brain.auxiliary_client import _client_cache, _client_cache_lock
        ...
        try:
            with patch(
                ...
                "brain.auxiliary_client._try_payment_fallback",
                return_value=(None, None, ""),
            ):
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/NousResearch/jarvis-brain/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        with patch("brain.auxiliary_client.OpenAI", mock_openai):
            client, model = resolve_provider_client(
                provider="nvidia",
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "JarvisBrain"
...
class TestAnthropicExplicitApiKey:
    ...
    def test_try_anthropic_uses_explicit_api_key_over_env(self):
        ...
        with patch("brain.anthropic_adapter.resolve_anthropic_token", return_value="env-fallback-key"), \
             patch("brain.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("brain.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from brain.auxiliary_client import _try_anthropic
    ...
    def test_resolve_provider_client_passes_explicit_api_key_to_anthropic(self):
        ...
        with patch("brain.anthropic_adapter.resolve_anthropic_token", return_value="env-key"), \
             patch("brain.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("brain.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            client, model = resolve_provider_client(
...
class TestAuxUnhealthyCache:
    ...
    def setup_method(self):
        from brain.auxiliary_client import _reset_aux_unhealthy_cache
    ...
    def test_resolve_auto_skips_unhealthy_step2(self):
        ...
        _mark_provider_unhealthy("openrouter")
        with patch("brain.auxiliary_client._read_main_provider", return_value=""), \
             patch("brain.auxiliary_client._read_main_model", return_value=""), \
             patch("brain.auxiliary_client._try_openrouter") as or_try, \
             patch("brain.auxiliary_client._try_nous", return_value=(nous_client, "nous-model")), \
             patch("brain.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("brain.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
            client, model = _resolve_auto()
    ...
    def test_resolve_auto_skips_unhealthy_main_in_step1(self):
        ...
        _mark_provider_unhealthy("openrouter")
        with patch("brain.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("brain.auxiliary_client._read_main_model", return_value="anthropic/claude-sonnet-4.6"), \
             patch("brain.auxiliary_client.resolve_provider_client") as step1, \
             patch("brain.auxiliary_client._try_openrouter") as or_try, \
             patch("brain.auxiliary_client._try_nous", return_value=(nous_client, "n-model")), \
             patch("brain.auxiliary_client._try_custom_endpoint", return_value=(None, None)), \
             patch("brain.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
            client, model = _resolve_auto()
    ...
    def test_payment_fallback_skips_unhealthy(self):
        ...
        with patch("brain.auxiliary_client._read_main_provider", return_value="openrouter"), \
             patch("brain.auxiliary_client._try_openrouter") as or_try, \
             patch("brain.auxiliary_client._try_nous", return_value=(nous_client, "n-model")), \
             patch("brain.auxiliary_client._try_custom_endpoint") as custom_try, \
             patch("brain.auxiliary_client._resolve_api_key_provider", return_value=(None, None)):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
        assert client is nous_client
        assert label == "nous"
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        with patch("brain.auxiliary_client._get_cached_client",
             ...
             patch("brain.auxiliary_client._build_call_kwargs",
...
class TestAuxiliaryMaxTokensParam:
    ...
    def test_direct_openai_returns_max_completion_tokens(self):
        with (
            patch("brain.auxiliary_client._current_custom_base_url",
