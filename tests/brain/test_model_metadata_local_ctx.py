...
class TestQueryLocalContextLengthOllama:
    ...
    def test_ollama_model_info_context_length(self):
        ...
        from brain.model_metadata import _query_local_context_length
        ...
        with patch("brain.model_metadata.detect_local_server_type", return_value="ollama"), \
    ...
    def test_ollama_num_ctx_wins_over_model_info(self):
        ...
        training max — using it would let Jarvis grow conversations past
        the runtime limit and Ollama would silently truncate.

        Concrete example: jarvis-brain:qwen3-14b-ctx32k is a Modelfile
        derived from qwen3:14b with `num_ctx 32768`, but the underlying
        GGUF reports `qwen3.context_length: 40960` (training max). If
        Jarvis used 40960 it would let the conversation grow past 32768
        ...
        with patch("brain.model_metadata.detect_local_server_type", return_value="ollama"), \
            ...
            result = _query_local_context_length(
                "jarvis-brain:qwen3-14b-ctx32k", "http://100.77.243.5:11434/v1"
            )

        assert result == 32768, (
            f"Expected num_ctx (32768) to win over model_info (40960), got {result}. "
            "If Jarvis uses the GGUF training max, conversations will silently truncate."
        )
...
class TestQueryLocalContextLengthVllm:
    ...
    def test_vllm_max_model_len(self):
        ...
        with patch("brain.model_metadata.detect_local_server_type", return_value="vllm"), \
             patch("httpx.Client", return_value=client_mock):
...
class TestQueryLocalContextLengthModelsList:
    ...
    def test_models_list_max_model_len(self):
        ...
        with patch("brain.model_metadata.detect_local_server_type", return_value=None), \
             patch("httpx.Client", return_value=client_mock):
...
class TestQueryLocalContextLengthLmStudio:
    ...
    def test_lmstudio_exact_key_match(self):
        ...
        with patch("brain.model_metadata.detect_local_server_type", return_value="lm-studio"), \
             patch("httpx.Client", return_value=client_mock):
            result = _query_local_context_length(
...
class TestDetectLocalServerTypeAuth:
    def test_passes_bearer_token_to_probe_requests(self):
        from brain.model_metadata import detect_local_server_type
...
class TestFetchEndpointModelMetadataLmStudio:
    ...
    def test_uses_native_models_endpoint_only(self):
        from brain.model_metadata import fetch_endpoint_model_metadata
        ...
        with patch("brain.model_metadata.detect_local_server_type", return_value="lm-studio"), \
             patch("brain.model_metadata.requests.get", return_value=native_resp) as mock_get:
...
class TestGetModelContextLengthLocalFallback:
    ...
    def test_local_endpoint_unknown_model_queries_server(self):
        ...
        with patch("brain.model_metadata.get_cached_context_length", return_value=None), \
             ...
             patch("brain.model_metadata.fetch_model_metadata", return_value={}), \
             patch("brain.model_metadata.is_local_endpoint", return_value=True), \
             patch("brain.model_metadata._query_local_context_length", return_value=131072), \
    ...
    def test_local_endpoint_server_returns_none_falls_back_to_2m(self):
        ...
        from brain.model_metadata import get_model_context_length, CONTEXT_PROBE_TIERS

        with patch("brain.model_metadata.get_cached_context_length", return_value=None), \
             patch("brain.model_metadata.fetch_endpoint_model_metadata", return_value={}), \
             patch("brain.model_metadata.fetch_model_metadata", return_value={}), \
             patch("brain.model_metadata.is_local_endpoint", return_value=True), \
             patch("brain.model_metadata._query_local_context_length", return_value=None):
            result = get_model_context_length("omnicoder-9b", "http://localhost:11434/v1")
    ...
    def test_non_local_endpoint_does_not_query_local_server(self):
        ...
        with patch("brain.model_metadata.get_cached_context_length", return_value=None), \
             patch("brain.model_metadata.fetch_endpoint_model_metadata", return_value={}), \
             patch("brain.model_metadata.fetch_model_metadata", return_value={}), \
             patch("brain.model_metadata.is_local_endpoint", return_value=False), \
             patch("brain.model_metadata._query_local_context_length") as mock_query:
    ...
    def test_cached_result_skips_local_query(self):
        ...
        with patch("brain.model_metadata.get_cached_context_length", return_value=65536), \
             patch("brain.model_metadata._query_local_context_length") as mock_query:
            result = get_model_context_length("omnicoder-9b", "http://localhost:11434/v1")
