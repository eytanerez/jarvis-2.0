"""Tests for brain/anthropic_adapter.py — Anthropic Messages API adapter."""
...
class TestReadClaudeCodeCredentials:
    ...
    def no_keychain(self, monkeypatch):
        monkeypatch.setattr(
            "brain.anthropic_adapter._read_claude_code_credentials_from_keychain",
            lambda: None,
        )
    ...
    def test_reads_valid_credentials(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("brain.anthropic_adapter.Path.home", lambda: tmp_path)
...
class TestResolveAnthropicToken:
    @pytest.fixture(autouse=True)
    def no_keychain(self, monkeypatch):
        # The resolver consults the real macOS Keychain first; a dev machine
        # running Claude Code has live credentials there that would shadow
        # every tmp_path fixture below.
        monkeypatch.setattr(
            "brain.anthropic_adapter._read_claude_code_credentials_from_keychain",
            lambda: None,
        )
...
class TestResolveWithRefresh:
    def test_auto_refresh_on_expired_creds(self, monkeypatch, tmp_path):
        ...
        with patch("brain.anthropic_adapter._refresh_oauth_token", return_value="refreshed-token"):
...
class TestBuildAnthropicKwargs:
    ...
    def test_supports_fast_mode_predicate(self):
        ...
        """
        from brain.anthropic_adapter import _supports_fast_mode
...
class TestConvertToolsToAnthropicDedup:
    ...
    https://github.com/NousResearch/jarvis-brain/issues/18478
    """
