...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate JARVIS_HOME under tmp_path."""
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path / ".jarvis"))
    (tmp_path / ".jarvis").mkdir()

    # Force the constants/image cache helpers to re-read JARVIS_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("jarvis_constants") or mod.startswith("brain.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_jarvis_home_cache(self, http_server):
        base, _ = http_server
        from brain.image_gen_provider import save_url_image

        path = save_url_image(f"{base}/image.png", prefix="xai_test")

        assert path.exists()
        assert path.read_bytes() == PNG_1PX
        # The cache directory must be under JARVIS_HOME — gateway cleanup
    ...
    def test_oversize_raises_and_cleans_up(self, http_server, tmp_path):
        ...
        base, _ = http_server
        from brain.image_gen_provider import save_url_image, _images_cache_dir
