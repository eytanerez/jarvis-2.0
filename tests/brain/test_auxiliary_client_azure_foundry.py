...
that delegates to :func:`jarvis_cli.runtime_provider._resolve_azure_foundry_runtime`
...
class TestAuxAzureFoundryApiKey:
    def test_chat_completions_returns_plain_openai_client(self, monkeypatch, patch_load_config):
        from brain.auxiliary_client import _try_azure_foundry
    ...
    def test_codex_responses_wraps_in_codex_aux_client(self, monkeypatch, patch_load_config):
        from brain.auxiliary_client import _try_azure_foundry, CodexAuxiliaryClient
...
class TestAuxAzureFoundryEntra:
    def test_callable_api_key_reaches_openai_constructor(
        ...
        """
        from brain import auxiliary_client as _aux
    ...
    def test_entra_anthropic_messages_uses_bearer_hook(
        ...
        from brain import auxiliary_client as _aux
        from brain import anthropic_adapter as _anthropic
...
class TestResolveProviderClientAzureFoundry:
    ...
    def test_warns_and_returns_none_on_failure(
        ...
        clear warning pointing at ``jarvis doctor``."""
        import logging
        ...
        assert resolved is None
        assert any(
            "azure-foundry" in rec.message and "jarvis doctor" in rec.message
