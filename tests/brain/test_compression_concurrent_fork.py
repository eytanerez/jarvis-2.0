...
Damien's incident (Discord, 2026-05-28): a long Jarvis session in a Discord
...
``_spawn_background_review(...)`` — which builds a forked ``AIBrain`` that
inherits ``agent.session_id`` (see ``brain/background_review.py``::
...
def _build_agent_with_db(db: SessionDB, session_id: str):
    """Build an AIBrain that's wired to ``db`` and pinned to ``session_id``."""
...
def test_concurrent_compression_does_not_fork_session(tmp_path: Path) -> None:
    """Two AIBrains that share a session_id MUST NOT both rotate it.
...
class _NoLockSubsystemDB:
    ...
    A long-lived process can hold ``jarvis_state.SessionDB`` bound to the
...
def test_review_fork_disables_compression_to_prevent_stale_parent_fork() -> None:
    ...
    compression trigger sites in ``brain/conversation_loop.py`` gate on
    ...
    ``AIBrain.run_conversation`` patched (so no LLM call happens) and
    captures the constructed review agent to assert the flag.
    """
    import tempfile

    import brain.background_review as br
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # The worker does a local ``from run_brain import AIBrain``; patching
        # the class method covers that import path.
        from run_brain import AIBrain

        with patch.object(AIBrain, "run_conversation", _fake_run_conversation):
    ...
    assert captured, (
        ...
        "changed; update this test to capture the review AIBrain."
    )
