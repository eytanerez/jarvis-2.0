...
``AIBrain.run_conversation`` against an in-process mock provider — not a snapshot
...
# Repo root = three levels up from tests/brain/<file>.
...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated JARVIS_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="jarvis_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".jarvis"))
    prev_home = os.environ.get("JARVIS_HOME")
    os.environ["JARVIS_HOME"] = os.path.join(test_home, ".jarvis")
    ...
    for mod in list(sys.modules):
        if mod == "run_brain" or mod.startswith("brain.") or mod.startswith("tools.") or mod.startswith("jarvis_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["JARVIS_HOME"] = prev_home
