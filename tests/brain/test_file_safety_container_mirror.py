...
``…/sandboxes/<backend>/<task>/home/.jarvis/…`` prefix. This covers the
complementary inner-container case: when file tools execute inside Docker,
the bind-mount strips that prefix and the guard sees plain ``/root/.jarvis/…``.
...
class TestClassifyContainerMirrorTarget:
    def test_returns_none_without_context(self):
        """No Docker context — /root/.jarvis/… must not be flagged."""
        from brain.file_safety import classify_container_mirror_target

        assert classify_container_mirror_target("/root/.jarvis/profiles/group1/SOUL.md") is None
    ...
    def test_catches_soul_md_with_context(self):
        ...
        result = classify_container_mirror_target(
            "/root/.jarvis/profiles/group1/SOUL.md",
            mirror_prefix="/root/.jarvis",
        )
        assert result is not None
        assert result["mirror_root"].replace("\\", "/").endswith("root/.jarvis")
    ...
    def test_non_jarvis_path_not_flagged(self):
        """/root/workspace/… is not .jarvis state and must not be blocked."""
        from brain.file_safety import classify_container_mirror_target
...
class TestGetContainerMirrorWarning:
    def test_warning_names_inner_path_and_bypass(self):
        from brain.file_safety import get_container_mirror_warning
...
class TestOrthogonality:
    ...
    def test_inner_container_path_caught_by_context_guard(self):
        ...
        path = "/root/.jarvis/profiles/group1/SOUL.md"

        assert classify_container_mirror_target(path) is None  # no context
        assert classify_container_mirror_target(path, mirror_prefix="/root/.jarvis") is not None
