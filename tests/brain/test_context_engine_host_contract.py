...
engine plugins (e.g. jarvis-lcm) rely on:
...
def test_update_from_response_forwards_canonical_cache_buckets():
    ...
    from brain.usage_pricing import CanonicalUsage
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``jarvis plugins`` picker."""
    from jarvis_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "jarvis_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "jarvis_cli.plugins.get_plugin_context_engine",
