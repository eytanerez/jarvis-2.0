...
def isolated_home(monkeypatch):
    """Temp JARVIS_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="jarvis_test_31179_")
    jarvis_home = os.path.join(test_home, ".jarvis")
...
def _fresh_modules():
    """Drop cached jarvis modules so each test reloads against current env."""
    for mod in list(sys.modules.keys()):
        if mod.startswith(("brain.auxiliary_client", "brain.image_routing",
...
""")
        ...
        from brain.auxiliary_client import _resolve_task_provider_model
...
""")
    ...
    def test_unknown_capability_does_not_block(self, isolated_home, monkeypatch):
        ...
        """
        _fresh_modules()
        from brain.auxiliary_client import _main_model_supports_vision
