...
class TestEagerFallbackWithPool:
    """Test the eager fallback guard in run_brain.py's error handling loop."""

    def _make_agent(self, has_pool=True, pool_has_creds=True, has_fallback=True):
        """Create a minimal AIBrain mock with the fields needed."""
        from run_brain import AIBrain

        with patch.object(AIBrain, "__init__", lambda self, **kw: None):
            agent = AIBrain()
    ...
    def test_eager_fallback_deferred_when_pool_has_credentials(self):
        ...
        # Simulate the check from run_brain.py lines 7180-7191
