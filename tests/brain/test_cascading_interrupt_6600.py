...
``_interruptible_streaming_api_call`` methods in run_brain.py. Those methods
have since been extracted into ``brain/chat_completion_helpers.py``, so the
...
gateway caches AIBrain instances per session, the stale worker outlived the
...
from brain import chat_completion_helpers as cch
