...
Verifies that gateway user_id flows from AIBrain -> MemoryManager -> plugins,
...
class TestMemoryManagerUserIdThreading:
    ...
    def test_user_id_none_not_forwarded(self):
        ...
        # Simulates what happens when AIBrain passes user_id=None
...
class TestMem0UserIdScoping:
    ...
    def test_no_user_id_no_config_uses_jarvis_user(self):
        """Without user_id or config override, should default to 'jarvis-user'."""
        from plugins.memory.mem0 import Mem0MemoryProvider
        ...
        assert provider._user_id == "jarvis-user"
...
# ---------------------------------------------------------------------------
# AIBrain user_id propagation test
# ---------------------------------------------------------------------------


class TestAIBrainUserIdPropagation:
    """Verify AIBrain stores user_id and passes it to memory init kwargs."""

    def test_user_id_stored_on_agent(self):
        """AIBrain should store user_id as instance attribute."""
        with patch.dict(os.environ, {"JARVIS_HOME": "/tmp/test_jarvis"}):
    ...
    def test_user_id_none_by_default(self):
        """AIBrain should have None user_id when not provided (CLI mode)."""
        with patch.dict(os.environ, {"JARVIS_HOME": "/tmp/test_jarvis"}):
            from run_brain import AIBrain
            agent = object.__new__(AIBrain)
            agent._user_id = None
