...
class TestCodexAppServerModule:
    ...
    def test_module_imports(self) -> None:
        from brain.transports import codex_app_server
    ...
    def test_parse_codex_version_valid(self) -> None:
        from brain.transports.codex_app_server import parse_codex_version
    ...
    def test_codex_error_class_is_runtimeerror(self) -> None:
        from brain.transports.codex_app_server import CodexAppServerError
...
class TestSpawnEnvIsolation:
    ...
    def test_spawn_env_preserves_HOME(self, monkeypatch):
        ...
        import subprocess
        from brain.transports import codex_app_server as cas
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Jarvis passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("JARVIS_HOME", "/users/alice/.jarvis/profiles/backend-worker")
        monkeypatch.setenv("JARVIS_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "JARVIS_KANBAN_DB",
            "/users/alice/.jarvis/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.jarvis/kanban/boards/smoke"]'
            in cmd
        )
