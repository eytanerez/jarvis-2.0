"""Tests for brain/context_compressor.py — compression logic, thresholds, truncation fallback."""
...
def compressor():
    ...
    with patch("brain.context_compressor.get_model_context_length", return_value=100000):
        c = ContextCompressor(
            model="test/model",
...
class TestCompress:
    ...
    def test_truncation_fallback_no_client(self, compressor):
        ...
        with patch("brain.context_compressor.call_llm", side_effect=RuntimeError("no provider")):
            result = compressor.compress(msgs)
    ...
    def test_summary_failure_uses_deterministic_fallback_with_recovered_context(self):
        ...
        msgs = [
            ...
            {"role": "user", "content": "Please fix the compression summary failure"},
            {
                ...
                "tool_calls": [{
                    ...
                    "function": {
                        "name": "read_file",
                        "arguments": '{"path":"brain/context_compressor.py","offset":1}',
                    },
                }],
            },
            {
                "role": "tool",
                "tool_call_id": "call_1",
                "content": "read brain/context_compressor.py and found static fallback marker",
            },
        ...
        with (
            ...
            patch(
                "brain.context_compressor.call_llm",
        ...
        assert "brain/context_compressor.py" in combined
...
class TestGenerateSummaryNoneContent:
    ...
    def test_none_content_does_not_crash(self):
        ...
        with patch("brain.context_compressor.call_llm", return_value=mock_response):
...
class TestNonStringContent:
    ...
    def test_summary_call_does_not_force_temperature(self):
        ...
        with patch("brain.context_compressor.call_llm", return_value=mock_response) as mock_call:
...
class TestSummaryFailureCooldown:
    def test_summary_failure_enters_cooldown_and_skips_retry(self):
        ...
        with patch("brain.context_compressor.call_llm", side_effect=Exception("boom")) as mock_call:
...
class TestSummaryFallbackToMainModel:
    ...
    def test_json_decode_error_on_main_uses_short_cooldown(self):
        ...
        ), patch("brain.context_compressor.time.monotonic", return_value=1000.0):
            result = c._generate_summary(self._msgs())
...
class TestStreamingClosedFallback:
    ...
    def test_incomplete_chunked_read_falls_back_to_main(self):
        ...
        with patch(
            ...
            "brain.context_compressor._is_connection_error",
            return_value=True,
        ):
            result = c._generate_summary(self._msgs())
...
class TestSummaryFailureTrackingForGatewayWarning:
    ...
    def test_compress_records_fallback_and_dropped_count_on_summary_failure(self):
        ...
        with patch("brain.context_compressor.call_llm", side_effect=Exception("404 model not found")):
            result = c.compress(msgs)
    ...
    def test_summary_failure_fallback_preserves_tool_paths_and_redacts_secret_context(self):
        ...
        with patch("brain.context_compressor.call_llm", side_effect=Exception("timeout")):
            result = c.compress(msgs)
    ...
    def test_compress_clears_fallback_flag_on_subsequent_success(self):
        ...
        with patch("brain.context_compressor.call_llm", side_effect=Exception("boom")):
            c.compress(msgs)
        assert c._last_summary_fallback_used is True
...
class TestSummaryTargetRatio:
    ...
    def test_tail_budget_scales_with_context(self):
        ...
        with patch("brain.context_compressor.get_model_context_length", return_value=200_000):
            c = ContextCompressor(model="test", quiet_mode=True, summary_target_ratio=0.40)
        # 200K * 0.50 threshold * 0.40 ratio = 40K
        assert c.tail_token_budget == 40_000

        with patch("brain.context_compressor.get_model_context_length", return_value=1_000_000):
            c = ContextCompressor(model="test", quiet_mode=True, summary_target_ratio=0.40)
    ...
    def test_ratio_clamped(self):
        ...
        with patch("brain.context_compressor.get_model_context_length", return_value=100_000):
...
class TestTruncateToolCallArgsJson:
    ...
    def _helper(self):
        from brain.context_compressor import _truncate_tool_call_args_json
    ...
    def test_shrunken_args_remain_valid_json(self):
        ...
        original = _json.dumps({
            "path": "~/.jarvis/skills/shopping/browser-setup-notes.md",
        ...
        assert parsed["path"] == "~/.jarvis/skills/shopping/browser-setup-notes.md"
...
class TestPreflightSentinelGuard:
    ...
    def _seed(self, last_prompt_tokens, preflight_tokens):
        # Mirror the exact guard in brain/conversation_loop.py run_conversation.
