...
def test_random_strategy_uses_random_choice(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("brain.credential_pool.random.choice", lambda entries: entries[-1])
...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "jarvis_cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_token_invalidated_marks_credential_dead(tmp_path, monkeypatch):
    ...
    from brain.credential_pool import load_pool, STATUS_DEAD
...
def test_load_pool_does_not_persist_env_seeded_secret_value(tmp_path, monkeypatch):
    ...
    auth_text = (tmp_path / "jarvis" / "auth.json").read_text()
...
def test_load_pool_persists_bitwarden_origin_metadata_without_secret(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENROUTER_API_KEY", sentinel)
    monkeypatch.setattr(
        "jarvis_cli.env_loader.get_secret_source",
...
def test_pooled_credential_to_dict_strips_borrowed_secret_fields():
    from brain.credential_pool import PooledCredential
...
def test_write_credential_pool_sanitizes_borrowed_payload_at_disk_boundary(tmp_path, monkeypatch):
    ...
    from jarvis_cli.auth import write_credential_pool
    ...
    write_credential_pool("openrouter", [
        {
            ...
            "auth_type": "api_key",
            "priority": 0,
            "source": "systemd://jarvis/openrouter",
            "access_token": sentinel,
            "refresh_token": f"refresh-{sentinel}",
            "agent_key": f"agent-{sentinel}",
            "api_key": f"extra-{sentinel}",
    ...
    assert borrowed["source"] == "systemd://jarvis/openrouter"
...
def test_write_credential_pool_preserves_known_provider_owned_oauth_state(tmp_path, monkeypatch):
    ...
    persisted = json.loads((tmp_path / "jarvis" / "auth.json").read_text())["credential_pool"]["nous"][0]
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.jarvis/.env when
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.jarvis/.env does not define OPENROUTER_API_KEY (typical Docker /
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "brain.anthropic_adapter.read_jarvis_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("jarvis_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `jarvis setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / Jarvis PKCE
    ...
    monkeypatch.setattr("brain.anthropic_adapter.read_jarvis_oauth_credentials", _fake_pkce)
    monkeypatch.setattr("brain.anthropic_adapter.read_claude_code_credentials", _fake_cc)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `jarvis_pkce` into auth.json) and later switches to the API key at
    `jarvis setup` would still have those OAuth entries dormant on disk.
    ...
    monkeypatch.setattr("jarvis_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("brain.anthropic_adapter.read_jarvis_oauth_credentials", lambda: None)
    monkeypatch.setattr("brain.anthropic_adapter.read_claude_code_credentials", lambda: None)
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `jarvis setup`.  When ANTHROPIC_TOKEN is set (and
...
def test_least_used_strategy_selects_lowest_count(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path / "jarvis"))
    monkeypatch.setattr(
        "brain.credential_pool.get_pool_strategy",
    ...
    monkeypatch.setattr(
        "brain.credential_pool._seed_from_env",
        lambda provider, entries: (False, set()),
    )
    _write_auth_store(
        tmp_path,
...
def test_custom_endpoint_pool_keyed_by_name(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "brain.credential_pool._seed_custom_pool",
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    from brain.credential_pool import get_custom_provider_pool_key
...
def test_list_custom_pool_providers(tmp_path, monkeypatch):
    ...
    from brain.credential_pool import list_custom_pool_providers
...
class TestLeastUsedStrategy:
    ...
    def test_request_count_increments(self):
        ...
        from brain.credential_pool import CredentialPool, PooledCredential, STRATEGY_LEAST_USED
        ...
        with _patch("brain.credential_pool.get_pool_strategy", return_value=STRATEGY_LEAST_USED):
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `jarvis auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `jarvis model` to reauth, but the pool
    ...
    # Simulate `jarvis model` / `jarvis auth` refreshing the tokens.
...
def test_is_terminal_xai_oauth_refresh_error():
    from jarvis_cli.auth import AuthError, _is_terminal_xai_oauth_refresh_error
...
def test_is_terminal_codex_oauth_refresh_error():
    from jarvis_cli.auth import AuthError, _is_terminal_codex_oauth_refresh_error
