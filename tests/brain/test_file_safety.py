"""Tests for brain/file_safety.py read guards — env file blocking.

Run with:  python -m pytest tests/brain/test_file_safety.py -v
"""
...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_jarvis_env(self):
        """Jarvis' own .env inside JARVIS_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: jarvis internal .env is in ~/.jarvis/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.jarvis/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.jarvis/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Jarvis cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        jarvis_home = tmp_path / ".jarvis"
        cache = jarvis_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("brain.file_safety._jarvis_home_path", return_value=jarvis_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Jarvis cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        jarvis_home = tmp_path / ".jarvis"
        hub = jarvis_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_jarvis_home(self, tmp_path):
        """The env basename guard does not depend on JARVIS_HOME resolution."""
        jarvis_home = tmp_path / ".jarvis"
        jarvis_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        jarvis_home = tmp_path / ".jarvis"
        cache = jarvis_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
