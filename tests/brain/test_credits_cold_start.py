...
JARVIS_DEV_CREDITS_FIXTURE) so depletion AND the 90% grant warning fire immediately
...
def test_dev_fixtures_drive_cold_start():
    """Every JARVIS_DEV_CREDITS_FIXTURE state produces a valid seed CreditsState."""
    import os
    ...
    for name, want in expected.items():
        os.environ["JARVIS_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
        os.environ["JARVIS_DEV_CREDITS_FIXTURE"] = name
        try:
            fx = dev_fixture_credits_state()
            assert fx is not None, name
            assert _cold_start_notices(fx) == want, (name, _cold_start_notices(fx))
        finally:
            os.environ.pop("JARVIS_DEV_CREDITS_FIXTURE", None)
            os.environ.pop("JARVIS_DEV_CREDITS", None)
...
class _FakeAgent:
    ...
    the real policy against the latch (mirroring run_brain._emit_credits_notices,
...
def _seed(agent, fixture):
    ...
    os.environ["JARVIS_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
    os.environ["JARVIS_DEV_CREDITS_FIXTURE"] = fixture
    try:
