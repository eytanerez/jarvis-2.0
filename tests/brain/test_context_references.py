...
def test_parse_typed_references_ignores_emails_and_handles():
    from brain.context_references import parse_context_references
...
def test_folder_listing_falls_back_when_rg_is_blocked(sample_repo: Path):
    ...
    with patch("brain.context_references.subprocess.run", side_effect=blocked_rg):
        result = preprocess_context_references(
            "Review @folder:src/",
            cwd=sample_repo,
            context_length=100_000,
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_jarvis_paths(tmp_path: Path, monkeypatch):
    from brain.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("JARVIS_HOME", str(tmp_path / ".jarvis"))

    jarvis_env = tmp_path / ".jarvis" / ".env"
    jarvis_env.parent.mkdir(parents=True)
    jarvis_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.jarvis/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
