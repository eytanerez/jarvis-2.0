"""Tests for brain/curator.py — orchestrator, idle gating, state transitions.
...
def curator_env(tmp_path, monkeypatch):
    ...
    import brain.curator as curator
...
def test_dry_run_does_not_advance_state(curator_env, monkeypatch):
    ...
    `jarvis curator status`. Fixes #18373.
    """
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
...
def test_run_review_consolidate_override_runs_llm(curator_env, monkeypatch):
    ...
    the LLM consolidation pass — mirrors `jarvis curator run --consolidate`."""
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "a")
...
def test_curator_review_prompt_has_invariants():
    ...
    from brain.curator import CURATOR_REVIEW_PROMPT
...
def test_cli_unpin_refuses_bundled_skill(curator_env, capsys):
    """jarvis curator unpin must refuse bundled/hub skills too (matches pin)."""
...
def test_review_runtime_legacy_auxiliary_carry_credentials(curator_env, caplog):
    ...
    import logging
    with caplog.at_level(logging.INFO, logger="brain.curator"):
        binding = curator._resolve_review_runtime(cfg)
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from jarvis_cli.main import _AUX_TASKS
    from jarvis_cli.web_server import _AUX_TASK_SLOTS
