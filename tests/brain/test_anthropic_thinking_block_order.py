...
``AnthropicTransport.normalize_response`` (brain/transports/anthropic.py)
...
ordering.  ``run_brain`` stores those as separate fields on the assistant
message.  On replay, ``_convert_assistant_message`` (brain/anthropic_adapter.py)
...
def _stored_assistant_message(normalized) -> dict:
    """Reconstruct the OpenAI-style assistant message the way run_brain stores it.

    run_brain.py persists assistant turns as separate fields: content,
    reasoning_details (from provider_data), and tool_calls.  See
    run_brain.py L1513-1516 and jarvis_state.py.
    """
