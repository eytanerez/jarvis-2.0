...
def test_build_native_request_preserves_thought_signature_on_tool_replay():
    from brain.gemini_native_adapter import build_gemini_request
...
def test_translate_native_response_surfaces_reasoning_and_tool_calls():
    from brain.gemini_native_adapter import translate_gemini_response
        ...
        "candidates": [
            {
                "content": {
                    "parts": [
                        ...
                        {"functionCall": {"name": "search", "args": {"q": "jarvis"}}},
                    ]
                },
                "finishReason": "STOP",
            }
    ...
    assert json.loads(choice.message.tool_calls[0].function.arguments) == {"q": "jarvis"}
...
def test_native_client_uses_x_goog_api_key_and_native_models_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("brain.gemini_native_adapter.httpx.Client", lambda *a, **k: DummyHTTP())
...
def test_bare_gemini_model_id_strips_only_self_prefix(model, expected):
    from brain.gemini_native_adapter import bare_gemini_model_id
...
def test_native_http_error_keeps_status_and_retry_after():
    from brain.gemini_native_adapter import gemini_http_error
...
async def test_async_native_client_streams_without_requiring_async_iterator_from_sync_client():
    from brain.gemini_native_adapter import AsyncGeminiNativeClient
...
def test_stream_event_translation_emits_tool_call_delta_with_stable_index():
    from brain.gemini_native_adapter import translate_stream_event
...
def test_max_tokens_none_defaults_to_gemini_output_ceiling():
    ...
    maxOutputTokens is absent, truncating tool calls mid-stream. Jarvis passes
    ...
    """
    from brain.gemini_native_adapter import (
