"""Tests for brain/prompt_builder.py — context scanning, truncation, skills index."""
...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("jarvis_cli.config.load_config", default_load_config)
    ...
    def test_configured_context_file_max_chars_controls_truncation(self, monkeypatch):
        ...
        monkeypatch.setattr("jarvis_cli.config.load_config", fake_load_config)
...
class TestParseSkillFile:
    ...
    def test_logs_parse_failures_and_returns_defaults(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.DEBUG, logger="brain.prompt_builder"):
            is_compat, frontmatter, desc = _parse_skill_file(skill_file)
...
class TestPromptBuilderImports:
    def test_module_import_does_not_eagerly_import_skills_tool(self, monkeypatch):
        ...
        monkeypatch.delitem(sys.modules, "brain.prompt_builder", raising=False)
        monkeypatch.setattr(builtins, "__import__", guarded_import)

        module = importlib.import_module("brain.prompt_builder")
...
class TestBuildSkillsSystemPrompt:
    ...
    def test_excludes_disabled_skills(self, monkeypatch, tmp_path):
        ...
        with patch(
            "brain.prompt_builder.get_disabled_skill_names",
...
class TestBuildContextFilesPrompt:
    ...
    def test_jarvis_md_parent_dir_discovery(self, tmp_path):
        ...
        (tmp_path / ".git").mkdir()
        (tmp_path / ".jarvis.md").write_text("Root project rules.")
    ...
    def test_jarvis_md_beats_agents_md(self, tmp_path):
        ...
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert "Jarvis project rules" in result
...
class TestFindJarvisMd:
    ...
    def test_stops_at_git_root(self, tmp_path):
        ...
        (tmp_path / ".jarvis.md").write_text("outside")
        repo = tmp_path / "repo"
        repo.mkdir()
        (repo / ".git").mkdir()
...
class TestEnvironmentHints:
    ...
    def test_build_environment_hints_on_wsl(self, monkeypatch):
        import brain.prompt_builder as _pb
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """JARVIS_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import brain.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("JARVIS_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()
    ...
    def test_environment_hint_env_var_overrides_config(self, monkeypatch):
        ...
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("JARVIS_ENVIRONMENT_HINT", "ENV-WINS")
        monkeypatch.setattr(
            "jarvis_cli.config.load_config",
    ...
    def test_environment_hint_falls_back_to_config(self, monkeypatch):
        ...
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.delenv("JARVIS_ENVIRONMENT_HINT", raising=False)
        monkeypatch.setattr(
            "jarvis_cli.config.load_config",
            lambda: {"agent": {"environment_hint": "CONFIG-VALUE"}},
        )
    ...
    def test_environment_hint_empty_by_default(self, monkeypatch):
        ...
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.delenv("JARVIS_ENVIRONMENT_HINT", raising=False)
        monkeypatch.setattr("jarvis_cli.config.load_config", lambda: {"agent": {}})
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()
