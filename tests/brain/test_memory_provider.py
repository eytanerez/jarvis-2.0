...
from brain.memory_manager import MemoryManager, inject_memory_provider_tools
...
class TestUserInstalledProviderDiscovery:
    ...
    def test_load_user_plugin_with_relative_import(self, tmp_path, monkeypatch):
        ...
        synthetic ``_jarvis_user_memory.<name>`` package but never registered
        that parent namespace in sys.modules, so any relative import inside
        the plugin raised
        ``ModuleNotFoundError: No module named '_jarvis_user_memory'``.
        """
        from plugins.memory import load_memory_provider
    ...
    def test_load_user_plugin_with_nested_subpackage(self, tmp_path, monkeypatch):
        ...
        impl_dir = plugin_dir / "adapters" / "jarvis"  # adapters/ has no __init__.py
        ...
        (plugin_dir / "__init__.py").write_text(
            "from .adapters.jarvis import MyProvider\n"
            "def register(ctx):\n"
...
class TestUserInstalledProviderCli:
    ...
    ``_jarvis_user_memory.<name>.cli`` without registering the parent
...
class TestSequentialDispatchRouting:
    ...
    in run_brain.py had its own inline dispatch chain that skipped
...
# ---------------------------------------------------------------------------
# AIBrain.commit_memory_session — routes to MemoryManager.on_session_end
# ---------------------------------------------------------------------------
...
class TestOnMemoryWriteBridge:
    ...
    def test_on_memory_write_remove_not_bridged(self):
        ...
        # This tests the contract that run_brain.py checks:
        #   function_args.get("action") in ("add", "replace")
        mgr = MemoryManager()
        p = FakeMemoryProvider("ext")
        mgr.add_provider(p)

        # Manager itself doesn't filter — run_brain.py does.
    ...
    def test_memory_manager_tool_injection_deduplicates(self):
        ...
        # Apply the same dedup logic from run_brain.py __init__
        _existing_names = {
...
class TestMemoryToolToolsetGate:
    ...
    to AIBrain.tools unconditionally in agent_init.py — bypassing the
    ...
    def test_composite_toolset_with_memory_injects(self):
        ...
        tools, names = self._run_memory_injection(["jarvis-acp"], mgr)
...
class TestContextEngineToolsetGate:
    ...
    lcm_expand) was appended to AIBrain.tools unconditionally. Same blind
