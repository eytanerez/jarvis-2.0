...
import brain.account_usage as account_usage
from brain.account_usage import CreditsView, build_credits_view
from jarvis_cli.nous_account import NousPortalAccountInfo, NousPaidServiceAccessInfo
...
def test_view_fetch_failure_is_logged_out(monkeypatch):
    ...
    monkeypatch.setattr("jarvis_cli.nous_account.get_nous_portal_account_info", _boom)
...
def test_credits_command_registered():
    from jarvis_cli.commands import resolve_command, COMMAND_REGISTRY
...
def test_cli_show_credits_non_interactive_renders_text_not_modal(monkeypatch, capsys):
    ...
    monkeypatch.setattr(JarvisCLI, "_prompt_text_input_modal", _boom_modal, raising=False)
