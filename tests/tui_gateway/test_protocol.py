...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `JARVIS_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("JARVIS_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("JARVIS_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_make_agent_accepts_list_system_prompt(server, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "run_brain", types.SimpleNamespace(AIBrain=_Agent))
    monkeypatch.setitem(
        sys.modules,
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/jarvis-agent-dev": {"name": "jarvis-agent-dev", "description": "Dev workflow"}}

    with patch("brain.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "jarvis-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "jarvis_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("brain.skill_commands.scan_skill_commands", return_value=fake_skills), \
         patch("brain.skill_commands.build_skill_invocation_message", return_value=fake_msg):
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "jarvis-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "jarvis-agent-dev"
