...
class TestGetDefaultJarvisRoot:
    """Tests for get_default_jarvis_root() — Docker/custom deployment awareness."""

    def test_no_jarvis_home_returns_native(self, tmp_path, monkeypatch):
        """When JARVIS_HOME is not set, returns ~/.jarvis."""
        monkeypatch.delenv("JARVIS_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_jarvis_root() == tmp_path / ".jarvis"

    def test_jarvis_home_is_native(self, tmp_path, monkeypatch):
        """When JARVIS_HOME = ~/.jarvis, returns ~/.jarvis."""
        native = tmp_path / ".jarvis"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JARVIS_HOME", str(native))
        assert get_default_jarvis_root() == native

    def test_jarvis_home_is_profile(self, tmp_path, monkeypatch):
        """When JARVIS_HOME is a profile under ~/.jarvis, returns ~/.jarvis."""
        native = tmp_path / ".jarvis"
    ...
    def test_jarvis_home_is_docker(self, tmp_path, monkeypatch):
        """When JARVIS_HOME points outside ~/.jarvis (Docker), returns JARVIS_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JARVIS_HOME", str(docker_home))
        assert get_default_jarvis_root() == docker_home

    def test_jarvis_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any JARVIS_HOME outside ~/.jarvis is treated as the root."""
        custom = tmp_path / "my-jarvis-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JARVIS_HOME", str(custom))
        assert get_default_jarvis_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (JARVIS_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JARVIS_HOME", str(profile))
        assert get_default_jarvis_root() == docker_root

    def test_no_jarvis_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\jarvis, not ~/.jarvis."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("JARVIS_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(jarvis_constants.sys, "platform", "win32")

        assert get_default_jarvis_root() == local_appdata / "jarvis"

    def test_no_jarvis_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/jarvis without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("JARVIS_HOME", raising=False)
        ...
        assert get_default_jarvis_root() == home / "AppData" / "Local" / "jarvis"


class TestGetJarvisHome:
    """Tests for get_jarvis_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When JARVIS_HOME is unset on Windows, use %LOCALAPPDATA%\\jarvis."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("JARVIS_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        ...
        assert get_jarvis_home() == local_appdata / "jarvis"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(jarvis_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(jarvis_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".jarvis"
