...
($INSTALL_DIR), not into the shared $JARVIS_HOME.

Background (shared-$JARVIS_HOME bug)
------------------------------------
$JARVIS_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.jarvis:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$JARVIS_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``jarvis update``).
...
``~/.jarvis/jarvis-agent``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$JARVIS_HOME/\.install_method"', text), (
        "install.sh must not stamp $JARVIS_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `jarvis update`"
    )
