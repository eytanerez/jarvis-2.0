...
binary download are exercised manually by `jarvis secrets bitwarden
...
from brain.secret_sources import bitwarden as bw  # noqa: E402
...
def test_install_bws_rejects_malicious_member(jarvis_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from jarvis_cli.env_loader import _apply_external_secret_sources
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "brain.secret_sources.bitwarden.apply_bitwarden_secrets",
        fake_apply,
    )
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under jarvis_home/cache/."""
    home = tmp_path / ".jarvis"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
