...
Regression: the old implementation wrote ``jarvis_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``jarvis sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.jarvis/sessions/saved/`` and prints
...
def test_save_conversation_writes_under_jarvis_home(jarvis_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.jarvis/sessions/saved/, not CWD."""
    ...
    cli.JarvisCLI.save_conversation(stub)

    # File must NOT be in CWD
    cwd_leak = list(work.glob("jarvis_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.jarvis/sessions/saved/
    saved_dir = jarvis_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("jarvis_conversation_*.json"))
    ...
    assert "jarvis --resume 20260101_120000_abc123" in out, out
