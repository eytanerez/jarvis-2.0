...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("jarvis/jarvis-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned jarvis/* and pr-* branches."""

    def test_prunes_orphaned_jarvis_branch(self, git_repo):
        """jarvis/jarvis-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "jarvis/jarvis-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "jarvis/jarvis-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "jarvis/jarvis-deadbeef" in result.stdout
        ...
        assert "jarvis/jarvis-deadbeef" in orphaned
        ...
        assert "jarvis/jarvis-deadbeef" not in result.stdout
