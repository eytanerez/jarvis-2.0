...
def test_gquota_uses_chat_console_when_tui_is_live():
    from brain.google_oauth import GoogleOAuthError
    ...
    with patch("cli.ChatConsole", return_value=live_console), \
         patch("brain.google_oauth.get_valid_access_token", side_effect=GoogleOAuthError("No Google OAuth credentials found")), \
         patch("brain.google_oauth.load_credentials", return_value=None), \
         patch("brain.google_code_assist.retrieve_user_quota"):
