...
@pytest.fixture
def bare_cli():
    """A JarvisCLI with no __init__ — we only exercise the redraw helper."""
    cli = object.__new__(JarvisCLI)
    return cli
...
class TestForceFullRedraw:
    ...
    def test_missing_app_attr_is_safe(self, bare_cli):
        # Simulate JarvisCLI before the TUI has ever been constructed.
        bare_cli._force_full_redraw()  # must not raise
