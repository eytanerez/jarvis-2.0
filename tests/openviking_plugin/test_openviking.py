...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/jarvis/.overview.md") == "viking://user/jarvis"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/jarvis/memories/profile.md") == "viking://user/jarvis/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_jarvis_scaffolding(self, tmp_path, monkeypatch):
        import brain.skill_bundles as skill_bundles
        ...
        monkeypatch.setenv("JARVIS_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingTurnConversion:
    def test_extract_current_turn_anchors_on_latest_matching_user_and_assistant(self):
        messages = [
            ...
            },
            {
                ...
                "name": "shell_command",
                "content": "brain/context_engine.py: no preassemble hook",
            },
        ...
        assert batch[2]["parts"] == [
            {
                ...
                "tool_output": "brain/context_engine.py: no preassemble hook",
            ...
            turn = [
                ...
                },
                {
                    ...
                    "content": json.dumps({
                        "results": [
                            {
                                "uri": "viking://user/jarvis/memories/context",
    ...
    def test_messages_to_openviking_batch_adds_assistant_peer_id_when_requested(self):
        ...
        assert batch == [
            {"role": "user", "parts": [{"type": "text", "text": "hello"}]},
            {"role": "assistant", "parts": [{"type": "text", "text": "answer"}], "peer_id": "jarvis"},
        ]
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    (("uri", "viking://user/jarvis"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/jarvis/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/jarvis/.overview.md"
        assert result["resolved_uri"] == "viking://user/jarvis"
        ...
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/content/read",
                    (("uri", "viking://user/jarvis/memories/profile.md"),),
                ): {"result": "full text"},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/jarvis/memories/profile.md", "level": "full"}))

        assert result["uri"] == "viking://user/jarvis/memories/profile.md"
        assert result["resolved_uri"] == "viking://user/jarvis/memories/profile.md"
    ...
    def test_overview_file_uri_routes_straight_to_content_read_via_stat_probe(self):
        ...
        provider = OpenVikingMemoryProvider()
        file_uri = "viking://user/jarvis/memories/entities/mem_abc.md"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_overview_dir_uri_skips_stat_when_pseudo_summary(self):
        ...
        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/jarvis"}),
        ]
    ...
    def test_overview_directory_uri_uses_stat_probe_then_overview(self):
        ...
        provider = OpenVikingMemoryProvider()
        dir_uri = "viking://user/jarvis/memories"
        provider._client = FakeVikingClient(
            {
                (
                    "/api/v1/fs/stat",
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/jarvis/.overview.md", "level": "overview"})
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/jarvis/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/jarvis/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/jarvis"}))

        assert result["path"] == "viking://user/jarvis"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/jarvis/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/jarvis/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/jarvis"},
