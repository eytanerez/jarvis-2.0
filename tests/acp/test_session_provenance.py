...
def test_root_session_no_compression(db):
    ...
    assert prov["currentJarvisSessionId"] == "root1"
    assert prov["rootJarvisSessionId"] == "root1"
    assert prov["parentJarvisSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_jarvis_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentJarvisSessionId"] == "old"
    assert prov["rootJarvisSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousJarvisSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootJarvisSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootJarvisSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_jarvis_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousJarvisSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"jarvis"}
    assert "sessionProvenance" in meta["jarvis"]
    assert meta["jarvis"]["sessionProvenance"]["currentJarvisSessionId"] == "root1"
