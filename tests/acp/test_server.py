...
def mock_manager():
    ...
    return SessionManager(agent_factory=lambda: MagicMock(name="MockAIBrain"))
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.version == JARVIS_VERSION
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Jarvis' interactive model/provider setup in a terminal. "
                    "Use this when Jarvis has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Jarvis provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        with patch(
            "jarvis_cli.models.curated_models_for_provider",
    ...
    def test_build_usage_update_for_zed_context_indicator(self, agent, mock_manager):
        ...
        with patch(
            "brain.model_metadata.estimate_request_tokens_rough",
            return_value=25_000,
        ):
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "JarvisACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("JarvisACPAgent")
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "jarvis_cli.models.parse_model_input",
...
class TestPrompt:
    ...
    @pytest.mark.asyncio
    async def test_prompt_propagates_jarvis_session_id_env(self, agent, monkeypatch):
        """ACP must propagate the originating session id to the agent loop
        via ``JARVIS_SESSION_ID`` so tools that want to stamp side-effects
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("JARVIS_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "JARVIS_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("JARVIS_SESSION_ID") is None, (
            "JARVIS_SESSION_ID must be restored after the agent call so "
            "a re-used executor thread doesn't leak the id into the next "
            "session's tools"
        )

    @pytest.mark.asyncio
    async def test_prompt_restores_prior_jarvis_session_id(self, agent, monkeypatch):
        """If the env already had JARVIS_SESSION_ID set (e.g. nested
        agent loops), the prior value must be restored after the inner
        prompt completes — not popped, not left at the inner id."""
        monkeypatch.setenv("JARVIS_SESSION_ID", "outer-sess")
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("JARVIS_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("JARVIS_SESSION_ID") == "outer-sess"
    ...
    async def test_prompt_sends_session_info_update_after_auto_title(self, agent):
        ...
        with patch("brain.title_generator.maybe_auto_title", side_effect=fake_auto_title):
            mock_conn.session_update.reset_mock()
            await agent.prompt(
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert JARVIS_VERSION in result
