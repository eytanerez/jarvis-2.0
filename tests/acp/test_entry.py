...
def test_main_enables_unstable_protocol(monkeypatch):
    ...
    async def fake_run_brain(agent, **kwargs):
        calls["kwargs"] = kwargs
...
def test_main_version_prints_without_starting_server(monkeypatch, capsys):
    ...
    assert output
    assert "Starting jarvis-agent ACP adapter" not in output
...
def test_main_check_prints_ok_without_starting_server(monkeypatch, capsys):
    ...
    assert capsys.readouterr().out.strip() == "Jarvis ACP check OK"


def test_main_setup_runs_model_configuration(monkeypatch):
    calls = {}

    def fake_jarvis_main():
        calls["argv"] = sys.argv[:]

    monkeypatch.setattr("jarvis_cli.main.main", fake_jarvis_main)
...
def test_main_setup_offers_browser_install_when_tty(monkeypatch):
    ...
    monkeypatch.setattr("jarvis_cli.main.main", lambda: None)
...
def test_main_setup_browser_calls_ensure_dependency(monkeypatch):
    """`jarvis-acp --setup-browser` routes through dep_ensure.ensure_dependency."""
    calls = []
    ...
    monkeypatch.setattr("jarvis_cli.dep_ensure.ensure_dependency", fake_ensure)
