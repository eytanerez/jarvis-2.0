...
class TestToolsetConsistency:
    ...
    def test_jarvis_platforms_share_core_tools(self):
        """All jarvis-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on jarvis-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["jarvis-cli", "jarvis-telegram", "jarvis-discord", "jarvis-whatsapp", "jarvis-slack", "jarvis-signal", "jarvis-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_jarvis_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("jarvis-whatsapp")

    def test_jarvis_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("jarvis-api-server")
