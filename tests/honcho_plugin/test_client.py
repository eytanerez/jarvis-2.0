...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "jarvis"
        assert config.workspace_id == "jarvis"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.jarvis/honcho.json
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"jarvis": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"jarvis": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"jarvis": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="jarvis-agent"
        ):
            result = config.resolve_session_name("/home/user/jarvis-brain/subdir")
        assert result == "jarvis-agent"
...
class TestResolveConfigPath:
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".jarvis".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".jarvis"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "jarvis_coder"
        assert profile_host_key("default") == "jarvis"
    ...
    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"JARVIS_HONCHO_HOST": "jarvis.coder"}):
    ...
    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            with patch("jarvis_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "jarvis_coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("JARVIS_HONCHO_HOST", None)
            # Temporarily remove jarvis_cli.profiles to simulate import failure
            saved = sys.modules.get("jarvis_cli.profiles")
            sys.modules["jarvis_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "jarvis"
            finally:
                if saved is not None:
                    sys.modules["jarvis_cli.profiles"] = saved
                else:
                    sys.modules.pop("jarvis_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="jarvis_coder")
        assert config.host == "jarvis_coder"
        assert config.workspace_id == "jarvis"  # shared workspace
        assert config.ai_peer == "jarvis_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="jarvis")
        assert config.host == "jarvis"
        assert config.workspace_id == "jarvis"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "jarvis": {"aiPeer": "jarvis", "peerName": "alice"},
                "jarvis_coder": {
                    "aiPeer": "jarvis_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="jarvis_coder", config_path=config_file,
        )
        assert config.host == "jarvis_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "jarvis_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="jarvis_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "jarvis_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "jarvis.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="jarvis_dreamer",
        ...
        assert config.host == "jarvis_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "jarvis_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"jarvis": {"enabled": True, "aiPeer": "jarvis"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"jarvis": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_jarvis_config_timeout_override_used_when_config_timeout_missing(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("jarvis_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_jarvis_request_timeout_alias_used(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("jarvis_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
