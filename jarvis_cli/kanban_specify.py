...
Used by ``jarvis kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``jarvis_cli/goals.py`` — same aux
...
JARVIS_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("JARVIS_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Jarvis board.
...
def _profile_author() -> str:
    """Mirror of ``jarvis_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("JARVIS_PROFILE")
...
def specify_task(
    ...
    *,
    ...
    try:
        from brain.auxiliary_client import get_auxiliary_extra_body, get_text_auxiliary_client
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=JARVIS_KANBAN_SPECIFY_MAX_TOKENS,
