"""
Unified self-relaunch for Jarvis CLI.
...
from jarvis_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched jarvis."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_jarvis_bin() -> Optional[str]:
    """Find the jarvis entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("jarvis")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m jarvis_cli.main``.
    ...
    ``jarvis.exe`` on PATH or the ``sys.executable -m jarvis_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with jarvis.
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh jarvis invocation.
    ...
    relaunch contract wants: "run jarvis again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``jarvis.exe`` (a Python console-script shim that wraps
    ``python -m jarvis_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "jarvis exited, then
    new jarvis started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``jarvis`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nJarvis relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run jarvis.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
