"""
Jarvis Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``jarvis desktop`` / ``jarvis gui``)
     Built inside the agent checkout under ``$JARVIS_HOME/jarvis-brain/``:
       ...
       - ``$JARVIS_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Jarvis.app`` or ``~/Applications/Jarvis.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Jarvis`` (NSIS per-user)
...
the app name ("Jarvis"), separate from ``$JARVIS_HOME``:
  - macOS:   ``~/Library/Application Support/Jarvis``
  - Windows: ``%APPDATA%\\Jarvis``
  - Linux:   ``$XDG_CONFIG_HOME/Jarvis`` (default ``~/.config/Jarvis``)
...
``jarvis uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(jarvis_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return jarvis_home / "jarvis-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Jarvis"
...
def source_built_gui_artifacts(jarvis_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``jarvis desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``jarvis-brain/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(jarvis_home)
    ...
    return [
        ...
        jarvis_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Jarvis" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            ...
            home / "Applications" / "Jarvis.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Jarvis).
            local_base / "Programs" / "Jarvis",
            # Older / alternate layout some builds used.
            local_base / "jarvis-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Jarvis")
    else:
        ...
        paths += [
            data_base / "applications" / "jarvis.desktop",
            data_base / "applications" / "Jarvis.desktop",
        ]
    return paths


def agent_is_installed(jarvis_home: Path) -> bool:
    """Return True when a usable Python agent install exists under JARVIS_HOME.
    ...
    if (agent_root / "jarvis_cli").is_dir():
        return True
...
def gui_is_installed(jarvis_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(jarvis_home):
        if p.exists():
            return True
...
def gui_install_summary(jarvis_home: "Path | None" = None) -> dict:
...
def uninstall_gui(jarvis_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``jarvis-brain/jarvis_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$JARVIS_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove jarvis' or "
            "'sudo dnf remove jarvis'). AppImage builds are a single file you "
