...
Add, remove, or reorder entries here — both `jarvis setup` and
`jarvis` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `jarvis model`
    flow without maintaining a separate static list.
    """
    from jarvis_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $JARVIS_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Jarvis-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $JARVIS_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Jarvis refreshes the cache from
    ...
    """
    try:
        from brain.models_dev import _load_disk_cache
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Jarvis that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Jarvis that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_jarvis_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$JARVIS_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  jarvis model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `jarvis model` interactive picker
# ---------------------------------------------------------------------------
...
#
# Some vendors expose several Jarvis provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `jarvis model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``jarvis model``, the
...
# path. The *interactive* default (GUI onboarding / ``jarvis model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in jarvis_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``jarvis auth add openai-codex`` without
    ``jarvis model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``jarvis model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    jarvis-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from jarvis_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — jarvis-agent
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``jarvis model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from jarvis_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from jarvis_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Jarvis' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``jarvis_cli.auth.resolve_provider()`` to resolve it to a concrete
...
# _is_third_party_anthropic_endpoint in brain/anthropic_adapter.py), so
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Jarvis should expose the /fast toggle for this model."""
...
def resolve_fast_mode_overrides(model_id: Optional[str]) -> dict[str, Any] | None:
    ...
    ``_build_api_kwargs`` in run_brain.py — each API path handles its own
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``jarvis auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.jarvis/.env``.
    ...
    """
    try:
        from jarvis_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from jarvis_cli.copilot_auth import (
...
def _merge_with_models_dev(provider: str, curated: list[str]) -> list[str]:
    ...
    """
    try:
        from brain.models_dev import list_agentic_models
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Jarvis release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Jarvis release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Jarvis release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``jarvis model`` shows.
    ...
    if normalized == "bedrock":
        try:
            from brain.bedrock_adapter import bedrock_model_ids_or_none
...
#
# Cache strategy:
#   - One JSON file at $JARVIS_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from jarvis_constants import get_jarvis_home
    return get_jarvis_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$JARVIS_HOME/auth.json`` and external
...
def _fetch_anthropic_models(
    ...
    *,
    ...
    """
    try:
        from brain.anthropic_adapter import resolve_anthropic_token, _is_oauth_token
    except ImportError:
        return None
    ...
    if is_oauth:
        ...
        from brain.anthropic_adapter import _COMMON_BETAS, _OAUTH_ONLY_BETAS, _CONTEXT_1M_BETA
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Jarvis' default Claude IDs elsewhere use
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from jarvis_constants import get_jarvis_home
    return get_jarvis_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Jarvis will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Jarvis cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Jarvis actually ships.  (#46850)
    ...
    if normalized == "bedrock":
        try:
            from brain.bedrock_adapter import discover_bedrock_models, resolve_bedrock_region
