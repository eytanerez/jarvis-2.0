...
- Keep data: Remove code but keep ~/.jarvis/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Jarvis PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing jarvis-agent or jarvis PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Jarvis" comment and following line
                if '# Jarvis' in line or '# jarvis-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('jarvis' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing jarvis
                if 'jarvis' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the jarvis wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "jarvis",
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains jarvis_cli reference)
                content = wrapper.read_text()
                if 'jarvis_cli' in content or 'jarvis-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(jarvis_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``jarvis`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Jarvis home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (jarvis_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``jarvis gateway run`` processes
    ...
    try:
        from jarvis_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from jarvis_cli.gateway import get_launchd_plist_path
...
#      like ``%LOCALAPPDATA%\jarvis\git\cmd``, ``%LOCALAPPDATA%\jarvis\git\bin``,
#      ``%LOCALAPPDATA%\jarvis\git\usr\bin``, ``%LOCALAPPDATA%\jarvis\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\jarvis\git\`` and Node to
#      ``%LOCALAPPDATA%\jarvis\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``jarvis dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\jarvis\gateway-service\`` and sometimes
...
def remove_jarvis_env_vars_windows() -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("JARVIS_HOME", "JARVIS_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its JARVIS_HOME directory.

    We shell out to ``jarvis -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current JARVIS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    # 3. Wipe the profile's JARVIS_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``jarvis uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$JARVIS_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from jarvis_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(jarvis_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Jarvis Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Jarvis Chat GUI installation was found.")
        print(f"  Checked: {jarvis_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Jarvis agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(jarvis_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Jarvis agent at {jarvis_home / 'jarvis-agent'}")
        print(f"  • Your config, sessions, and secrets under {jarvis_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Jarvis agent is still installed. Run 'jarvis' to use the CLI,")
    print("or 'jarvis uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.jarvis/ (configs, data, logs)
    ...
    # full wipe (code + ~/.jarvis data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Jarvis Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their JARVIS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Jarvis data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Jarvis code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``jarvis`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$JARVIS_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        if removed_env:
            ...
            log_info("No Jarvis-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing jarvis command...")
    ...
    #     (only when they still point into this Jarvis home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Jarvis-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(jarvis_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Jarvis-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(jarvis_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE JARVIS_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from jarvis_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(jarvis_home)
        if not gui_removed:
    ...
    # 5. Optionally remove ~/.jarvis/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile JARVIS_HOME dirs live under
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://jarvis-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://jarvis-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m jarvis_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import jarvis_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``jarvis_constants`` + ``jarvis_cli.colors``
    (and lazily ``jarvis_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m jarvis_cli.uninstall")
    parser.add_argument(
        "--mode",
