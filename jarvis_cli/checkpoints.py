"""`jarvis checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.jarvis/checkpoints/``.  Actions:

    jarvis checkpoints               # same as `status`
    jarvis checkpoints status        # total size, project count, breakdown
    jarvis checkpoints list          # per-project checkpoint counts + workdir
    jarvis checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    jarvis checkpoints clear [-f]    # nuke the entire base (asks first)
    jarvis checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    jarvis checkpoints
    jarvis checkpoints prune --retention-days 3 --max-size-mb 200
    jarvis checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: jarvis checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``jarvis checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `jarvis checkpoints` → status
