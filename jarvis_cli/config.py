...
Config files are stored in ~/.jarvis/ for easy access:
- ~/.jarvis/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.jarvis/.env         - API keys and secrets

This module provides:
- jarvis config          - Show current configuration
- jarvis config edit     - Open config in editor
- jarvis config set      - Set a specific value
- jarvis config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``jarvis config set`` (which rewrites
    ...
    ``config.yaml`` in place: jarvis never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.jarvis/config.yaml`` causes ``load_config()``
    ...
    ``brain.log`` / ``errors.log`` at WARNING level so ``jarvis logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  jarvis config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Jarvis makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Jarvis itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``jarvis update``, the TUI build.
...
#   on every plugin install / ``jarvis update``.
...
# * ``JARVIS_HOME`` / ``JARVIS_PROFILE`` / ``JARVIS_CONFIG`` /
#   ``JARVIS_ENV`` — Jarvis runtime location flags. Writing these into
...
#
# IMPORTANT: ``JARVIS_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (JARVIS_GEMINI_CLIENT_ID,
# JARVIS_LANGFUSE_PUBLIC_KEY, JARVIS_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Jarvis runtime location — never via dashboard env writer.
    # NOT a JARVIS_* blanket: integration credentials (JARVIS_GEMINI_*,
    # JARVIS_LANGFUSE_*, JARVIS_SPOTIFY_*, ...) ARE allowed.
    "JARVIS_HOME", "JARVIS_PROFILE", "JARVIS_CONFIG", "JARVIS_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Jarvis runtime location "
            "(JARVIS_HOME, JARVIS_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "JARVIS_TOOL_PROGRESS", "JARVIS_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `jarvis plugins enable
    # observability/langfuse` or `jarvis tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "JARVIS_LANGFUSE_ENV",
    "JARVIS_LANGFUSE_RELEASE",
    "JARVIS_LANGFUSE_SAMPLE_RATE",
    "JARVIS_LANGFUSE_MAX_CHARS",
    "JARVIS_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from jarvis_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("JARVIS_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_jarvis_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Jarvis is running in package-manager-managed mode.

    Two signals: the JARVIS_MANAGED env var (set by the systemd service),
    or a .managed marker file in JARVIS_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``jarvis_cli/`` — i.e. the git checkout for source
    installs, ``/opt/jarvis`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$JARVIS_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Jarvis was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$JARVIS_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. JARVIS_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.jarvis``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$JARVIS_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.jarvis:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$JARVIS_HOME``, a
    ...
    ``docker`` and ``jarvis update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/jarvis-agent`` image bakes a ``docker``
        stamp into ``/opt/jarvis`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $JARVIS_HOME.
    try:
    ...
    #    host install whose shared $JARVIS_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``jarvis update``.
    try:
        method = (
            (get_jarvis_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``jarvis_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$JARVIS_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/jarvis`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Jarvis lives in a ``uv tool`` layout.

    ``uv tool install jarvis-agent`` places the install at
    ``.../uv/tools/jarvis-brain/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``jarvis-agent`` happens to be uv-tool-installed on the machine while
    the *active* Jarvis is a regular pip/venv install, causing
    ``jarvis update`` to upgrade the wrong copy. It would also block on a
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade jarvis-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade jarvis-agent"
        return "pip install --upgrade jarvis-agent"
    return "jarvis update"
...
# Long-form text for ``jarvis update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# jarvis_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Jarvis, it doesn't update
...
✗ ``jarvis update`` doesn't apply inside the Docker container.

Jarvis runs as a published image (nousresearch/jarvis-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/jarvis-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate jarvis-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/jarvis-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/jarvis-brain/tags
  • Your config and session history live under ``$JARVIS_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``jarvis update`` inside Docker.
...
def format_managed_message(action: str = "modify this Jarvis installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("JARVIS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Jarvis installation is managed by NixOS "
            f"(JARVIS_MANAGED={env_hint}).\n"
            "Edit services.jarvis-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Jarvis installation is managed by Homebrew "
            f"(JARVIS_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade jarvis-agent"
        )

    return (
        f"Cannot {action}: this Jarvis installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Jarvis."
    )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    container_mode_file = get_jarvis_home() / ".container-mode"
...
def get_env_path() -> Path:
    ...
    return get_jarvis_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_jarvis_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the JARVIS_UID / JARVIS_GID env vars set by Docker deployments.

    Docker containers running Jarvis commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level JARVIS_HOME once, but
    subdirectories created at runtime by ``ensure_jarvis_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("JARVIS_UID", "").strip()
    gid_str = os.environ.get("JARVIS_GID", "").strip()
    try:
...
def _chown_to_jarvis_uid(path) -> None:
    """Chown ``path`` to ``JARVIS_UID:JARVIS_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_jarvis_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_jarvis_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the jarvis group can
    share state with the gateway service.

    The mode can be overridden via the JARVIS_HOME_MODE environment variable
    (e.g. JARVIS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse JARVIS_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``JARVIS_UID``/``JARVIS_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("JARVIS_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_jarvis_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Jarvis runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("JARVIS_CONTAINER") or os.environ.get("JARVIS_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set JARVIS_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into JARVIS_HOME if the user doesn't have one yet."""
...
def _ensure_jarvis_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"JARVIS_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Jarvis-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Jarvis
        ...
        # identity slot (SOUL.md). Empty by default. The JARVIS_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Jarvis adds a coding operating brief
        # + a live git/workspace snapshot to the system prompt. See
        # brain/coding_context.py.
        ...
        #              (see run_brain._prepare_messages_for_api).
    ...
    "terminal": {
        ...
        #             JARVIS_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force JARVIS_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Jarvis auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Jarvis sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Jarvis
        ...
        # bundled Jarvis image, which drops to the `jarvis` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Jarvis sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Jarvis should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``jarvis chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.jarvis/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: jarvis sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .jarvis.md, or .cursorrules before Jarvis applies
    ...
    # (see brain/turn_context.py), so correctness never depends on it.  Keep it
    # small so a slow/dead server adds little to first-response latency.
    "mcp_discovery_timeout": 1.5,

    # Tool-output truncation thresholds. When terminal output or a
    # single read_file page exceeds these limits, Jarvis truncates the
    ...
    # See tools/kanban_tools.py and jarvis_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``jarvis dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``jarvis kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``jarvis kanban decompose`` and the kanban
        ...
        # ``jarvis profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `jarvis model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `jarvis` (and `jarvis chat`) launches by default:
        ...
        # REPL and `--tui` (or JARVIS_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        ...
        # When true (default), `jarvis --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/jarvis`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``JARVIS_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``JARVIS_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``JARVIS_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``JARVIS_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/jarvis``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Jarvis appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.jarvis/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.jarvis/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Jarvis feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Jarvis auto-pauses the goal and
    ...
    # always goes to ~/.jarvis/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${JARVIS_SKILL_DIR} and ${JARVIS_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `jarvis curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `jarvis curator run --consolidate` overrides
        ...
        # normally restored on every `jarvis update`, so pruning them only
        ...
        # skipped), snapshot ~/.jarvis/skills/ into
        # ~/.jarvis/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `jarvis curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for jarvis-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Jarvis*" header.
    ...
    # replace Jarvis' built-in platform hint for a single messaging platform
    ...
    # toggleable per-invocation via --accept-hooks or JARVIS_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `jarvis doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``jarvis_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Jarvis to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $JARVIS_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via JARVIS_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `jarvis -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `jarvis` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `jarvis kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     jarvis-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Jarvis tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.jarvis/logs/.
    # brain.log captures INFO+ (all agent activity); errors.log captures WARNING+.
    "logging": {
        "level": "INFO",       # Minimum level for brain.log: DEBUG, INFO, WARNING
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://jarvis-agent.nousresearch.com/docs/api/model-catalog.json",
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.jarvis/.env,
        ...
        # behavior: files must live under the Jarvis cache, under
        ...
        # user. Bridged to JARVIS_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Jarvis
        # cache (~/.jarvis/cache/{documents,images,audio,video,screenshots})
        ...
        # to JARVIS_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # JARVIS_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to JARVIS_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.jarvis/state.db.
    ...
    "sessions": {
        ...
        # default of ``jarvis sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.jarvis/sessions/session_{sid}.json`` on every turn
    ...
    # Contextual first-touch onboarding hints (see brain/onboarding.py).
    ...
    # ``jarvis update`` behaviour.
    "updates": {
        # Run a full ``jarvis backup``-style zip of JARVIS_HOME before every
        # ``jarvis update``.  Backups land in ``<JARVIS_HOME>/backups/`` and
        # can be restored with ``jarvis import <path>``.  Defaults to true
        # after the #48200 incident: a ``jarvis update --yes`` run that
        ...
        "backup_keep": 5,
        # What `jarvis update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<JARVIS_HOME>/lsp/bin/`` on first use.
    ...
    # `jarvis tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.jarvis/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.jarvis/.env (or your shell) and never in config.yaml.
            ...
            # ~/.jarvis/bin/ on first use.  When False you must install
            ...
            # `jarvis secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'jarvis model' for endpoint-specific config)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Jarvis, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @jarvis:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. JARVIS_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #jarvis)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: jarvis-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'jarvis-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Jarvis API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Jarvis API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Jarvis API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # JARVIS_TOOL_PROGRESS and JARVIS_TOOL_PROGRESS_MODE are deprecated —
    ...
    "JARVIS_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.jarvis.config`` entries, then checks
    which ones are absent or empty under ``skills.config.<key>`` in the user's
    config.yaml.  Returns a list of dicts suitable for prompting.
    """
    try:
        from brain.skill_utils import discover_all_skill_config_vars, SKILL_CONFIG_PREFIX
    except Exception:
        return []
    ...
    try:
        ...
        # should never break `jarvis update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Jarvis versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      * ``AIBrain.__init__`` (startup resolution)
      * ``AIBrain.switch_model`` (mid-session ``/model`` switch)
      * ``jarvis_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
    ...
    Before this helper existed, the lookup was duplicated in ``run_brain.py``'s
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'jarvis setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Jarvis won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'jarvis doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("JARVIS_HOME", "~/.jarvis")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("JARVIS_TOOL_PROGRESS")
            old_mode = get_env_value("JARVIS_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from JARVIS_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from JARVIS_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("JARVIS_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from JARVIS_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$JARVIS_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_jarvis_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `jarvis plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `jarvis curator status` has no
    ...
    if current_ver < 23:
        try:
            curator_dir = get_jarvis_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from jarvis_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: jarvis config set <key> <value>")
    ...
    # metadata.jarvis.config in their SKILL.md frontmatter.
    ...
    if missing_skill_config and interactive and not quiet:
        ...
        if answer in {"y", "yes"}:
            ...
            config = load_config()
            try:
                from brain.skill_utils import SKILL_CONFIG_PREFIX
            except Exception:
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.jarvis/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.jarvis/config.yaml.
    ...
    (which change ``JARVIS_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/jarvis/config.yaml invalidates the
...
#   nous         (OAuth — jarvis auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.jarvis/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`jarvis tools`, `jarvis setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `jarvis tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Jarvis env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.jarvis/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.jarvis/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.jarvis/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.jarvis/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Jarvis — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Jarvis vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.jarvis/.env or environment."""
...
def redact_key(key: str) -> str:
    ...
    """
    from brain.redact import mask_secret
...
def show_config():
    ...
    print(color("│              ⚕ Jarvis Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from jarvis_cli.auth import get_anthropic_key
    ...
    # Warn on stale JARVIS_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("JARVIS_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale JARVIS_MAX_ITERATIONS={_env_ghost} "
                f"(run 'jarvis doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    # Skill config
    try:
        from brain.skill_utils import discover_all_skill_config_vars, resolve_skill_config_values
    ...
    print(color("  jarvis config edit     # Edit config file", Colors.DIM))
    print(color("  jarvis config set <key> <value>", Colors.DIM))
    print(color("  jarvis setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: jarvis config set <key> <value>")
            print()
            print("Examples:")
            print("  jarvis config set model anthropic/claude-sonnet-4")
            print("  jarvis config set terminal.backend docker")
            print("  jarvis config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'jarvis config migrate' to add them")
        ...
        print("  jarvis config           Show current configuration")
        print("  jarvis config edit      Open config in editor")
        print("  jarvis config set <key> <value>   Set a config value")
        print("  jarvis config check     Check for missing/outdated config")
        print("  jarvis config migrate   Update config with new options")
        print("  jarvis config path      Show config file path")
        print("  jarvis config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``jarvis config`` UI so users
