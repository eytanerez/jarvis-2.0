"""MCP picker — interactive `jarvis mcp picker` (also the default `jarvis mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``jarvis mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `jarvis plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from jarvis_cli.cli_output import prompt_yes_no
from jarvis_cli.curses_ui import curses_single_select
from jarvis_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Jarvis session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from jarvis_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `jarvis mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: jarvis mcp install <name>    Picker: jarvis mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Jarvis is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Jarvis — run `jarvis update` "
...
def show_catalog() -> None:
    """`jarvis mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`jarvis mcp picker` (and default `jarvis mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`jarvis mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `jarvis mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
