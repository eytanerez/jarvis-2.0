"""Slash command definitions and autocomplete for the Jarvis CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Jarvis config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Jarvis works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Jarvis is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.jarvis/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Jarvis to the latest version", "Info"),
    CommandDef("version", "Show Jarvis version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`jarvis_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/jarvis`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Jarvis
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        from brain.skill_utils import get_external_skills_dirs
        ...
        # ``jarvis skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``jarvis skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``jarvis skills list`` and
...
# unrelated command landed. These claim their slots right after /jarvis,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /jarvis <command>).
...
# native slot, the alias spelling stays reachable via /jarvis reset).
...
# ``/jarvis <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/jarvis" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /jarvis credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /jarvis billing.
#   - debug: the log/report upload surface; reached via /jarvis debug on Slack.
_SLACK_VIA_JARVIS_ONLY = frozenset({"credits", "billing", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/jarvis <verb>`` subcommand.
    ...
    can still reach them via ``/jarvis <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/jarvis`` is always reserved as the first entry so the
    legacy ``/jarvis <subcommand>`` form keeps working for anything that
    ...
    # Reserve /jarvis as the catch-all top-level command.
    entries.append(("jarvis", "Talk to Jarvis or run a subcommand", "[subcommand] [args]"))
    seen.add("jarvis")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_JARVIS_ONLY:
            # Intentionally Slack-via-/jarvis only (see _SLACK_VIA_JARVIS_ONLY).
            return
    ...
    # everything except /jarvis, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://jarvis-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /jarvis handler.

    Maps both canonical names and aliases so /jarvis bg do stuff works
    the same as /jarvis background do stuff.

    Plugin-registered slash commands are included so ``/jarvis <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from jarvis_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
