"""Profile distributions — shareable, packaged Jarvis profiles via git.

A distribution is a Jarvis profile published as a git repository (or
...
* ``jarvis profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``jarvis skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``jarvis profile``, not a parallel tree):

    jarvis profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "jarvis-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            jarvis_requires=str(data.get("jarvis_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.jarvis_requires:
            out["jarvis_requires"] = self.jarvis_requires
        if self.author:
...
def check_jarvis_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Jarvis {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``jarvis profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Jarvis profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Jarvis distribution."
        )

    # Version check up-front so we fail fast
    check_jarvis_requires(manifest.jarvis_requires, jarvis_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.jarvis).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="jarvis_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `jarvis profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`jarvis profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="jarvis_dist_update_") as tmp:
        plan = plan_install(
