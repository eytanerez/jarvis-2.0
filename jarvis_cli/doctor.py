"""
Doctor command for jarvis CLI.

Diagnoses issues with Jarvis setup.
"""
...
_DHH = display_jarvis_home()  # user-facing display path (e.g. ~/.jarvis or ~/.jarvis/profiles/coder)
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.jarvis/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("JARVIS_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from jarvis_cli.auth import get_gemini_oauth_auth_status
            return bool((get_gemini_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "minimax":
        try:
            from jarvis_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches jarvis_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``jarvis --version`` reporting a stale version while
    ...
    """
    try:
        from jarvis_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != jarvis_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'jarvis update', or set "
            "jarvis_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-jarvis and dashboard static services are up
    ...
    for static in ("main-jarvis", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `jarvis profile create <name>`")
        return
...
def check_certificates() -> None:
    ...
    """
    try:
        from brain.ssl_guard import verify_ca_bundle_with_fallback
        from brain.errors import SSLConfigurationError
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from jarvis_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the JARVIS_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("JARVIS_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via JARVIS_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `jarvis`.
    os.environ.setdefault("JARVIS_INTERACTIVE", "1")

    # Handle `jarvis doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.jarvis/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Jarvis Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`jarvis doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and jarvis_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.jarvis/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'jarvis setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'jarvis setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'jarvis setup' to create one")
                issues.append("Run 'jarvis setup' to create .env")
    
    # Check ~/.jarvis/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from jarvis_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from jarvis_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'jarvis config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from jarvis_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.jarvis/.env or run 'jarvis setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'jarvis setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'jarvis config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
    ...
    if config_path.exists():
        try:
            from jarvis_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'jarvis setup' to migrate config")
                else:
                    issues.append("Run 'jarvis doctor --fix' or 'jarvis setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'jarvis doctor --fix'")
        except Exception:
            pass

        # Detect stale JARVIS_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives JARVIS_MAX_ITERATIONS from brain.max_turns
        ...
        try:
            import yaml
            from jarvis_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("JARVIS_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"JARVIS_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"brain.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `jarvis setup` run)",
                )
                if should_fix:
                    if remove_env_value("JARVIS_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale JARVIS_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove JARVIS_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the JARVIS_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale JARVIS_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'jarvis doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Jarvis' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Jarvis a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Jarvis Persona\n\n"
                "<!-- Edit this file to customize how Jarvis communicates. -->\n\n"
    ...
    if state_db_path.exists():
        try:
            ...
            from jarvis_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'jarvis doctor --fix' "
                        "(or 'jarvis sessions repair') to recover hidden sessions"
                    )
            else:
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'jarvis doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "jarvis"
        ...
        _cmd_link = _cmd_link_dir / "jarvis"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(jarvis not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/jarvis → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/jarvis points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/jarvis → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/jarvis — run 'jarvis doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/jarvis exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/jarvis not found",
                    "(jarvis command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/jarvis → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/jarvis symlink — run 'jarvis doctor --fix'")
    ...
    try:
        from jarvis_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `jarvis doctor` invocation.
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Jarvis one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'jarvis config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            if base and base.rstrip("/").endswith("/anthropic"):
                from brain.auxiliary_client import _to_openai_base_url
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``jarvis doctor``.
    ...
    def _probe_bedrock() -> _ConnectivityResult:
        try:
            from brain.bedrock_adapter import (
                has_aws_credentials,
    ...
    # never the legitimate source for `jarvis doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'jarvis setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = JARVIS_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: jarvis skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = JARVIS_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: jarvis memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: jarvis memory setup",
                    "No Honcho API key — run 'jarvis memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run jarvis memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: jarvis memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: jarvis memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from jarvis_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "jarvis -p" in content:
                            _m = _re.search(r"jarvis -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'jarvis doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
