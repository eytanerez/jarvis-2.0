...
Pure logic: walks a Jarvis config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `jarvis doctor` and a future `jarvis migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Jarvis config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Jarvis config and return retirement issues.
