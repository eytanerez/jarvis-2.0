...
- ``jarvis_cli.providers``        -- canonical provider identity + overlays
- ``jarvis_cli.model_normalize``  -- per-provider name formatting
...
#   NousResearch/Jarvis-3-Llama-3.1-70B, jarvis-4-405b, openrouter/jarvis3:70b
# Negative examples it must NOT match:
#   jarvis-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_JARVIS_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])jarvis[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_jarvis_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_JARVIS_NON_AGENTIC_RE.search(model_name))
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``jarvis config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from jarvis_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'jarvis model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'jarvis doctor' — config issues detected:"
        ...
        from jarvis_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from jarvis_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from jarvis_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in jarvis_cli.runtime_provider.resolve_runtime_provider;
...
# process — mirrors run_brain's _openrouter_prewarm_done. Without a guard a
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from jarvis_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from jarvis_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from jarvis_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for jarvis_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(jarvis_id)
        if (
            _alias_target
            and _alias_target != jarvis_id
        ...
        # /model picker sees the SAME list `jarvis model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(jarvis_id)
        if not model_ids:
    ...
    for pid, overlay in JARVIS_OVERLAYS.items():
        ...
        if jarvis_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(jarvis_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(jarvis_slug)
                model_ids = _ids if _ids else (curated.get(jarvis_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(jarvis_slug, []) or curated.get(pid, [])
        elif jarvis_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `jarvis model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from jarvis_cli.auth import get_provider_auth_state as _nous_state
    ...
    # in PROVIDER_TO_MODELS_DEV or JARVIS_OVERLAYS (keeps /model in sync
    # with `jarvis model`).
    try:
        from jarvis_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Jarvis's canonical write key (matches
            ...
            # Jarvis writes ``models:`` as a dict keyed by model id
            # (see jarvis_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Jarvis's own writer uses; a hyphen variant is accepted
            ...
            # downstream readers (brain/models_dev.py, gateway/run.py,
            # run_brain.py, jarvis_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`jarvis_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from jarvis_cli.models import fetch_openrouter_models
