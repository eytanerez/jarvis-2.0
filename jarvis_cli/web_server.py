...
Usage:
    python -m jarvis_cli.main web          # Start on http://127.0.0.1:9119
    python -m jarvis_cli.main web --port 8080
"""
...
from jarvis_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
WEB_DIST = Path(os.environ["JARVIS_WEB_DIST"]) if "JARVIS_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``jarvis gateway run`` — but the
    desktop app spawns a ``jarvis dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same JARVIS_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (JARVIS_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `jarvis
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("JARVIS_DESKTOP") == "1":
...
app = FastAPI(title="Jarvis", version=__version__, lifespan=_lifespan)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# JARVIS_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("JARVIS_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
#
# This list is defined in ``jarvis_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from jarvis_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Jarvis-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
async def _dashboard_auth_gate(request: Request, call_next):
    from jarvis_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "tts.provider": {
        ...
        "options": ["edge", "elevenlabs", "openai", "neutts", "kokoro"],
    },
    "stt.provider": {
        "type": "select",
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Jarvis (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``jarvis model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Jarvis provider slug — fine.
    ...
    1. Vendor-name → Jarvis-provider mapping: when the provider string is
       not a known Jarvis provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from jarvis_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from jarvis_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from jarvis_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "JARVIS_DASHBOARD_FILES_ROOT"
...
def _default_jarvis_root_is_opt_data() -> bool:
    ...
    try:
        ...
        root = get_default_jarvis_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``jarvis update``.
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Jarvis root is actually /opt/data
    # (the container/hosted layout) or when JARVIS_DASHBOARD_FILES_ROOT is set.
    if _default_jarvis_root_is_opt_data():
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_jarvis_home() at call time
    ...
    try:
        ...
        # and which providers are registered so ``jarvis status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from jarvis_cli.dashboard_auth import list_providers as _list_providers
        ...
        status = {
            ...
            "can_update_jarvis": not _dashboard_local_update_managed_externally(),
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the jarvis home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_jarvis_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_jarvis_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def run_prompt_size():
    try:
        proc = _spawn_jarvis_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_jarvis_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_jarvis_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from jarvis_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.jarvis/logs/<action>.log`` so
...
def _spawn_jarvis_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    cmd = [sys.executable, "-m", "jarvis_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "JARVIS_NONINTERACTIVE": "1"},
    }
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["jarvis", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``jarvis gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``jarvis gateway restart`` children race each other on the
    ...
    return _spawn_jarvis_action(subcommand, "gateway-restart"), False
...
async def update_jarvis():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "Jarvis updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("jarvis-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "jarvis-update",
...
@app.get("/api/jarvis/update/check")
async def check_jarvis_update(force: bool = False):
    """Report whether a Jarvis update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/jarvis/update`` actually runs ``jarvis update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Jarvis version string
    ...
    try:
        ...
        if force:
            try:
                (get_jarvis_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.jarvis/config.yaml`` under ``tts.``.
    """
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Jarvis surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Jarvis supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_jarvis_home() / provider.name / "config.json"
...
def get_model_info(profile: Optional[str] = None):
    ...
    try:
        ...
        caps = {}
        try:
            from brain.models_dev import get_model_capabilities
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `jarvis model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `jarvis model` does so GUI onboarding lands on a
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (jarvis_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from jarvis_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``jarvis model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from jarvis_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``jarvis model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from jarvis_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from jarvis_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `jarvis model` provider — would be
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict) -> dict:
        ...
        return {
            ...
            # CLI `jarvis model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Jarvis from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Jarvis to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Jarvis from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Jarvis to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Jarvis in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Jarvis through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Jarvis via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Jarvis through an IMAP/SMTP mailbox.",
        "docs_url": "https://jarvis-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Jarvis to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Jarvis inside Feishu / Lark.",
    ...
    "weixin": {
        ...
        "docs_url": "https://jarvis-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Jarvis through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Jarvis to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Jarvis to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Jarvis as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://jarvis-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://jarvis-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in jarvis gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in jarvis gateway setup",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.jarvis-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"JarvisDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Jarvis").strip() or "Jarvis"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Jarvis appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's JARVIS_HOME.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.jarvis/.anthropic_oauth.json`` — Jarvis-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_jarvis_dotenv()``, so the same check covers them)
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Jarvis even
    when they also have a separate Jarvis-managed PKCE login.
    """
    try:
        from brain.anthropic_adapter import read_claude_code_credentials
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Jarvis never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "jarvis auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "jarvis auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "jarvis auth add xai-oauth",
        "docs_url": "https://jarvis-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "jarvis auth add google-gemini-cli",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "jarvis auth add anthropic",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Jarvis, so the disconnect
    ...
    user sees exactly what executes, and Jarvis then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Jarvis — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``jarvis model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `jarvis model` universe (zero-edit for new plugins).
    try:
        from jarvis_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"jarvis auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `jarvis model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Jarvis-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from brain.anthropic_adapter import _JARVIS_OAUTH_FILE
            ...
            try:
                from jarvis_cli.auth import clear_provider_auth
        ...
        try:
            from jarvis_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
# Guarded so jarvis web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Jarvis file AND credential pool.
    ...
    tmp_path = _JARVIS_OAUTH_FILE.with_name(
        f"{_JARVIS_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _JARVIS_OAUTH_FILE)
        try:
            _JARVIS_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
    ...
    try:
        from brain.credential_pool import (
            PooledCredential,
            load_pool,
            AUTH_TYPE_OAUTH,
            SOURCE_MANUAL,
...
# the tokens exactly like `jarvis auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("JARVIS_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `jarvis auth add xai-oauth`'s credential-pool insert.
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from jarvis_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `jarvis sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `jarvis sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_jarvis_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from jarvis_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_jarvis_home() / "logs" / log_name
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, JARVIS_HOME)."""
    from jarvis_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process JARVIS_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (jarvis_cli.mcp_config), so
# servers managed here show up under `jarvis mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from jarvis_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from jarvis_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from jarvis_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # JARVIS_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds JARVIS_HOME-derived paths in the child.
    if entry.install is not None:
        try:
            proc = _spawn_jarvis_action(
...
#
# Wraps the same JSON store the CLI uses (jarvis_cli.webhook); the webhook
...
async def list_webhooks():
    import jarvis_cli.webhook as wh
...
# spawn the real `jarvis gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_jarvis_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_jarvis_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from jarvis_cli.auth import read_credential_pool
...
# `jarvis memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `jarvis memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_jarvis_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_jarvis_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_jarvis_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `jarvis import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_jarvis_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from jarvis_cli.config import load_config as _load_config
    from brain import shell_hooks

    try:
        from jarvis_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <jarvis_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_jarvis_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_jarvis_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``jarvis`` subprocess, and
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from jarvis_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`jarvis -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the JARVIS_HOME
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local JARVIS_HOME override so the write lands in the target
    ...
    token = set_jarvis_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local JARVIS_HOME override (same mechanism as
    ...
    """
    from jarvis_constants import set_jarvis_home_override, reset_jarvis_home_override
    from jarvis_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the JARVIS_HOME override. Returns the
    number of skills newly disabled.
    """
    from jarvis_constants import set_jarvis_home_override, reset_jarvis_home_override
    from jarvis_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's JARVIS_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``jarvis profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from JARVIS_HOME).
    """
    from jarvis_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``jarvis profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the JARVIS_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``jarvis profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from jarvis_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_jarvis_home()`` at call
       time — the context-local override from ``set_jarvis_home_override``
    ...
    retargeted to the *current* ``get_jarvis_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a JARVIS_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_jarvis_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_jarvis_home()`` at call time
...
def _clear_skills_prompt_cache() -> None:
    ...
    """
    try:
        from brain.prompt_builder import clear_skills_system_prompt_cache
...
async def get_toolsets(profile: Optional[str] = None):
    from jarvis_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``jarvis tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``jarvis tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``jarvis tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.jarvis/.env`` via ``save_env_value`` —
    the same store ``jarvis tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``jarvis tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``jarvis -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same JARVIS_HOME the rest of the
...
if sys.platform.startswith("win"):
    try:
        from jarvis_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
...
def _ws_client_reason(ws: "WebSocket") -> Optional[str]:
    ...
    so a "WS keeps closing" report can be diagnosed from brain.log without a
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from jarvis_cli.dashboard_auth.audit import AuditEvent, audit_log
        from jarvis_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    ``JARVIS_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    ...
    ``JARVIS_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_jarvis_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``jarvis -p <name>`` performs. The in-process
    ``JARVIS_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    env["JARVIS_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["JARVIS_HOME"] = str(profile_dir)
    ...
    # inherits the profile JARVIS_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["JARVIS_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from jarvis_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`jarvis_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from jarvis_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/jarvis``)
        ...
        ``__JARVIS_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                ...
                f'window.__JARVIS_BASE_PATH__="{prefix}";'
                f"window.__JARVIS_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
    ...
    # under ``/jarvis`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Jarvis backend. Intercept CSS asset
...
def _discover_user_themes() -> list:
    """Scan ~/.jarvis/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_jarvis_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.jarvis/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as jarvis_cli.plugins):
    1. User plugins:    ~/.jarvis/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.jarvis/plugins/  (only if JARVIS_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from jarvis_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_jarvis_home() / "plugins", "user"),
    ...
    # ``jarvis_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("JARVIS_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".jarvis" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_jarvis_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"jarvis auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from jarvis_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from jarvis_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from jarvis_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from jarvis_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.jarvis/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.jarvis/plugins/ if you trust it)",
...
from jarvis_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from jarvis_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing JARVIS_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Jarvis exposes a
    ...
    # JARVIS_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"JARVIS_DASHBOARD_READY port={actual_port}", flush=True)
            print(f"  Jarvis Web UI → http://{host}:{actual_port}")
