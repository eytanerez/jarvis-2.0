"""Windows ConPTY bridge for the `jarvis dashboard` chat tab.

Drop-in counterpart to ``jarvis_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``jarvis_cli.web_server`` consumes: ``spawn``, ``read``,
