"""Prompt-size diagnostic: ``jarvis prompt-size``.
...
credentials so ``AIBrain.__init__`` takes the direct-construction path, then
...
def _build_inspection_agent(platform: str) -> Any:
    """Construct an offline AIBrain for prompt inspection.

    Dummy ``api_key`` + ``base_url`` force the direct-construction path in
    ``run_brain.py`` (no provider auto-detection, no network). Toolsets and
...
def compute_prompt_breakdown(platform: str = "cli") -> Dict[str, Any]:
    ...
    """
    from brain.system_prompt import build_system_prompt, build_system_prompt_parts
...
def cmd_prompt_size(args: Any) -> None:
    """Entry point for ``jarvis prompt-size``."""
