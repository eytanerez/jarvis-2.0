...
from jarvis_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
_UPSTREAM_REPO_URL = "https://github.com/NousResearch/jarvis-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/jarvis-agent"
...
def _fetch_pypi_latest(package: str = "jarvis-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    # JARVIS_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `jarvis update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/jarvis/update/check`
    ...
    if embedded_rev:
        ...
        # $JARVIS_HOME/jarvis-brain/ may be a stale copy from --clone-all;
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Jarvis git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$JARVIS_HOME/jarvis-brain/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``jarvis_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from jarvis_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/jarvis-brain/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical NousResearch/jarvis-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Jarvis v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else JARVIS_CADUCEUS
    except Exception:
        _bskin = None
        _hero = JARVIS_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from jarvis_cli.codex_runtime_switch import get_current_runtime
        from jarvis_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from jarvis_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
    ...
    # Pip-install warning — `pip install jarvis-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else JARVIS_AGENT_LOGO
        console.print(_logo)
        console.print()
