"""jarvis hooks — inspect and manage shell-script hooks.

Usage::

    jarvis hooks list
    jarvis hooks test <event> [--for-tool X] [--payload-file F]
    jarvis hooks revoke <command>
    jarvis hooks doctor

Consent records live under ``~/.jarvis/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.jarvis/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``jarvis hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: jarvis hooks {list|test|revoke|doctor}")
        print("Run 'jarvis hooks --help' for details.")
        return
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `jarvis hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `jarvis hooks test` and `jarvis hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Jarvis wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `jarvis hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `jarvis hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `jarvis hooks doctor`.")
