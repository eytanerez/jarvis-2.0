"""``jarvis debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    jarvis debug share              Upload debug report and print URL
    jarvis debug share --lines 500  Include more log lines
    jarvis debug share --expire 30  Keep paste for 30 days
    jarvis debug share --local      Print report locally (no upload)
    jarvis debug share --no-redact  Disable upload-time secret redaction
    jarvis debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'jarvis debug share'",
    )
