"""``jarvis skills`` subcommand parser.
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `jarvis update` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `jarvis update` reports as user-modified and skips. "
            "Use `jarvis skills diff <name>` to see changes and `jarvis skills reset "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`jarvis update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `jarvis update`. Pass --sync to re-seed now."
        ),
    )
