"""``jarvis gateway`` and ``jarvis proxy`` subcommand parsers.
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if JARVIS_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy jarvis.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Jarvis gateway unit files "
            "(e.g. jarvis.service) left over from older installs. Profile "
            "units (jarvis-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.jarvis/.env. "
            "Requires being logged in (jarvis setup). Not available in managed installs."
        ),
    )
