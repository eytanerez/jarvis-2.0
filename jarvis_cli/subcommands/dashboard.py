"""``jarvis dashboard`` subcommand parser.
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Jarvis web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `jarvis dashboard` cmdlines and SIGTERM them directly — the same
    # path `jarvis update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running jarvis dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running jarvis dashboard processes and exit",
    )
    # Backward-compat shim: older Jarvis desktop app shells (<= 0.15.x) spawn the
    # backend as `jarvis dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Jarvis couldn't start" with no actionable cause. Accept and silently
    ...
    # `jarvis dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.jarvis/.env.
    # Nested subparser so bare `jarvis dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "JARVIS_DASHBOARD_OAUTH_CLIENT_ID into ~/.jarvis/.env, and prints "
            "how to engage the login gate. Requires being logged in (jarvis setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://jarvis.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via JARVIS_DASHBOARD_PORTAL_URL. Mainly for "
