"""
Gateway subcommand for jarvis CLI.

Handles: jarvis gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "jarvis CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``jarvis gateway status`` from falsely counting the ``jarvis`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``jarvis gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_jarvis_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"jarvis_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. JARVIS_HOME
        ...
        # JARVIS_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "jarvis_home=" in command_lc
            and f"jarvis_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``jarvis update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Jarvis profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Jarvis profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "jarvis_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``jarvis update`` users
    # with no running gateway until they re-invoke ``jarvis gateway``
...
def _sync_jarvis_home_from_systemd_unit(system: bool) -> None:
    ...
    :func:`get_jarvis_home` falls back to ``/root/.jarvis`` — the wrong
    profile. The unit file pins ``JARVIS_HOME`` for the actual gateway
    ...
    unit_home = env.get("JARVIS_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("JARVIS_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["JARVIS_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}jarvis gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}jarvis gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from jarvis_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `jarvis gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``jarvis gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from jarvis_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (JARVIS_HOME-scoped).
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Jarvis Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``jarvis gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``JARVIS_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("JARVIS_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "jarvis-gateway"
SERVICE_DESCRIPTION = "Jarvis Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current JARVIS_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (JARVIS_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary JARVIS_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(jarvis_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        ...
        default_root: Optional Jarvis root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_jarvis_root()``
    ...
    default = Path(default_root).resolve() if default_root else get_default_jarvis_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(jarvis_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".jarvis"
    try:
        Path(jarvis_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(jarvis_home, default_root=target_root)
    except ValueError:
        return _profile_arg(jarvis_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this JARVIS_HOME.

    Default ``~/.jarvis`` returns ``jarvis-gateway`` (backward compatible).
    Profile ``~/.jarvis/profiles/coder`` returns ``jarvis-gateway-coder``.
    Any other JARVIS_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Jarvis installs that predate the
# jarvis-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (jarvis-gateway-*.service) and unrelated third-party
# "jarvis" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("jarvis.service",)
...
def _find_legacy_jarvis_units() -> list[tuple[str, Path, bool]]:
    ...
    different service name (e.g. ``jarvis.service`` before the rename to
    ``jarvis-gateway.service``). When both a legacy unit and the current
    ``jarvis-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``jarvis-gateway-coder.service`` and unrelated third-party
      ``jarvis-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``jarvis.service`` running an unrelated
...
def print_legacy_unit_warning() -> None:
    """Warn about legacy Jarvis gateway unit files if any are installed.
    ...
    print_info("  These run alongside the current jarvis-gateway service and")
...
def remove_legacy_jarvis_units(
    ...
    print()
    print("Legacy Jarvis gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: jarvis gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo jarvis gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    jarvis gateway uninstall")
    print_info("    sudo jarvis gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Jarvis can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo jarvis gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo jarvis gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo jarvis gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Jarvis often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.jarvis`` → ``ai.jarvis.gateway.plist`` (backward compatible).
    Profile ``~/.jarvis/profiles/coder`` → ``ai.jarvis.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.jarvis.gateway-{suffix}" if suffix else "ai.jarvis.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.jarvis/jarvis-agent  -> /home/alice/.jarvis/jarvis-agent
      /opt/jarvis                 -> /opt/jarvis  (kept as-is)
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m jarvis_cli.main``, so module
    ...
    dir, or a clone that ``jarvis update`` later relocates/removes — the path
    ...
    ``JARVIS_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if JARVIS_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        profile_arg = _profile_arg_for_target_user(jarvis_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's JARVIS_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(jarvis_home) if jarvis_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m jarvis_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir JARVIS_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved JARVIS_HOME
    ...
    exported a throwaway ``JARVIS_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``jarvis gateway restart``
    with ``JARVIS_HOME=/tmp/jarvis-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="JARVIS_HOME=..."`` lines and launchd
    ``<key>JARVIS_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'JARVIS_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>JARVIS_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp JARVIS_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: JARVIS_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported JARVIS_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # JARVIS_HOME into the unit (manual E2E homes like /tmp/jarvis-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Jarvis install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo jarvis gateway uninstall --system")
    print_info("         jarvis gateway install")
    print_info("         jarvis gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (jarvis.service from pre-rename installs)
    # before installing the new jarvis-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}jarvis gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}jarvis gateway status{scope_flag}             # Check status"
    )
    print(
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `jarvis gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `jarvis gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}jarvis gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}jarvis gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}jarvis gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}jarvis gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.jarvis.gateway-{suffix}" if suffix else "ai.jarvis.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``jarvis gateway start/stop/status`` call).
...
# (the documented `nohup jarvis gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m jarvis_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active JARVIS_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "jarvis_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup jarvis gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: jarvis gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup jarvis gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Jarvis install"
    )
    return True
...
def launchd_stop():
    ...
    # `jarvis gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Jarvis install")
    else:
        print("⚠ Service definition is stale relative to the current Jarvis install")
        print("  Run: jarvis gateway start")
    ...
    if deep:
        log_file = get_jarvis_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``JARVIS_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("JARVIS_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``jarvis gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``jarvis gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``jarvis gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other JARVIS_HOME roots.
    """
    ...
    print("  Use 'jarvis gateway restart' to replace it,")
    print("  or 'jarvis gateway stop' first.")
    print("  Or use 'jarvis gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("JARVIS_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Jarvis gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'jarvis' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/jarvis/docker/entrypoint.sh before the Jarvis command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$JARVIS_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set JARVIS_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `jarvis gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set JARVIS_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `jarvis gateway restart` which already
    ...
    # `jarvis gateway start/restart` — leaving the gateway vulnerable to
    ...
    # JARVIS_GATEWAY_EXIT_DIAG env var (default: on while we're still
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Jarvis will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @jarvis:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. jarvis) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Jarvis delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Jarvis agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Jarvis will use (e.g., jarvis@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Jarvis connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: jarvis pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Jarvis will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``jarvis setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run jarvis under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.jarvis/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_jarvis_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from jarvis_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'jarvis pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: jarvis pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'jarvis gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from jarvis_cli.main import cmd_whatsapp
    import argparse
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: jarvis pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'jarvis gateway setup'")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from jarvis_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Jarvis will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Jarvis will store the returned account_id/token in ~/.jarvis/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `jarvis gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_jarvis_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `jarvis pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `jarvis pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "JarvisBrain"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``jarvis_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from jarvis_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.jarvis/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.jarvis/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: jarvis gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: jarvis gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: jarvis gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo jarvis gateway install --system"
                        )
                    print_info("  Or run in foreground:  jarvis gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: jarvis gateway run")
                print_info(
                    "  For persistence:   tmux new -s jarvis 'jarvis gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: jarvis gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup jarvis gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'jarvis gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`jarvis_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from jarvis_cli.service_manager import (
    ...
    if profile is None:
        ...
        # JARVIS_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``jarvis gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``jarvis gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``jarvis gateway run`` are unaffected.
      2. ``JARVIS_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``jarvis gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``JARVIS_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("JARVIS_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("JARVIS_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${JARVIS_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if JARVIS_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or JARVIS_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-jarvis service's
    # pattern in docker/s6-rc.d/main-jarvis/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: jarvis gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: jarvis gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s jarvis 'jarvis gateway run'"
                )
                print()
            ...
            print()
            print(
                "  jarvis gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s jarvis 'jarvis gateway run'         # persistent via tmux"
            )
            print(
                "  nohup jarvis gateway run > ~/.jarvis/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from jarvis_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  jarvis profile create <name>     # creates the s6 service slot")
                print("  jarvis -p <name> gateway start   # bring it up via s6")
                print("  jarvis status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: jarvis gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: jarvis gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: jarvis gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  jarvis profile delete <name>     # tears down the s6 service slot")
                print("  jarvis -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `jarvis -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: jarvis gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_JARVIS_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `jarvis gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_JARVIS_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `jarvis gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if restart_all:
            ...
            if supports_systemd_services() and (
                ...
                # when invoked from a gateway-hosted brain/tool call, foreground
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: jarvis gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  jarvis gateway install")
                else:
                    print("To install as a service:")
                    print("  jarvis gateway install")
                    print("  sudo jarvis gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  jarvis gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup jarvis gateway run > ~/.jarvis/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s jarvis 'jarvis gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  jarvis gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  jarvis gateway install  # Install as user service")
                    print(
                        "  sudo jarvis gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Jarvis gateway unit files from
        # pre-rename installs (e.g. jarvis.service). Profile units and
        # unrelated third-party services are never touched.
