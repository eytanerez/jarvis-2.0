"""
Dump command for jarvis CLI.

Outputs a compact, plain-text summary of the user's Jarvis setup
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.jarvis_build_sha`` by the Dockerfile's
    ``JARVIS_GIT_SHA`` build-arg (see ``jarvis_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``jarvis dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from jarvis_cli.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `jarvis auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `jarvis auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["jarvis-cli"])
