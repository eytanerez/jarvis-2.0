"""``jarvis slack ...`` CLI subcommands.

Today only ``jarvis slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ jarvis slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Jarvis. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Jarvis deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from jarvis_cli.commands import slack_app_manifest
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``jarvis_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$JARVIS_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Jarvis")
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("JARVIS_HOME") or str(Path.home() / ".jarvis")) / "slack-manifest.json"
        else:
