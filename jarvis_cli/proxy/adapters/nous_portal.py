...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `jarvis auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `jarvis auth add nous` to re-authenticate."
                )
    ...
    # to jarvis_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
