...
from brain.credential_pool import CredentialPool, PooledCredential, load_pool
...
# xAI's public API is OpenAI-compatible for the endpoints Jarvis commonly
# uses. The Responses endpoint is included because Jarvis' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Jarvis-managed OAuth credentials."""

    auth_hint = "jarvis auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`jarvis auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`jarvis auth reset xai-oauth` or re-authenticate with "
                    "`jarvis auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `jarvis auth add xai-oauth --type oauth`."
            )
