"""CLI handlers for the ``jarvis proxy`` subcommand."""
...
from jarvis_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "jarvis proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"jarvis auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Jarvis proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Jarvis proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: jarvis proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``jarvis proxy <subcommand>``."""
    ...
    print(
        "jarvis proxy — local OpenAI-compatible proxy that attaches your\n"
