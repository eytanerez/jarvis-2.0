"""Per-provider model-selection wizard flows for ``jarvis setup`` / ``jarvis model``.

Extracted from ``jarvis_cli/main.py`` as part of the god-file decomposition
campaign (``~/.jarvis/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from jarvis_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``jarvis_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from jarvis_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``jarvis_cli.main`` at import time -> no import cycle.
"""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from jarvis_cli.main import _prompt_api_key
    ...
    # in-flow (K/R/C) instead of having to edit ~/.jarvis/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `jarvis setup` from scratch. OpenRouter
    ...
    from jarvis_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `jarvis auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "reauth":
            ...
            print()
            try:
                # Forward CLI flags from ``jarvis model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``jarvis model``.
                mock_args = argparse.Namespace(
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from jarvis_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from jarvis_cli.auth import (
    ...
    from jarvis_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    if not status.get("logged_in"):
        try:
            from brain.google_oauth import resolve_project_id_from_env, start_oauth_flow
...
def _model_flow_custom(config):
    ...
    """
    from jarvis_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from jarvis_cli.auth import _save_model_choice, deactivate_provider
    from jarvis_cli.config import get_env_value, load_config, save_config
    ...
    from jarvis_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `jarvis model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from jarvis_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from jarvis_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from jarvis_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Jarvis will probe your")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from jarvis_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from jarvis_cli.auth import _save_model_choice, deactivate_provider
    from jarvis_cli.config import load_config, save_config
    from jarvis_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from jarvis_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from jarvis_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from jarvis_cli.config import save_env_value, load_config, save_config
    from jarvis_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from jarvis_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from jarvis_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Jarvis turns to `copilot --acp`.")
    print("  Jarvis currently starts its own ACP subprocess for each request.")
    print("  Jarvis uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set JARVIS_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from jarvis_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from jarvis_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from jarvis_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so jarvis knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        try:
            from brain.gemini_native_adapter import probe_gemini_tier
        except Exception:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Jarvis typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Jarvis, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        from jarvis_cli.models import fetch_lmstudio_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from jarvis_cli.main import _run_anthropic_oauth_flow
    from jarvis_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
