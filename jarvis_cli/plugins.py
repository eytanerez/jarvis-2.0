...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with jarvis-agent;
...
from jarvis_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``JARVIS_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("JARVIS_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``JARVIS_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.jarvis/logs/brain.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("JARVIS_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When JARVIS_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Jarvis loggers.
    """
    ...
    # config also writes to stderr. brain.log still captures everything.
    logger.propagate = True
    _DEBUG_HANDLER_INSTALLED = True
    logger.debug(
        "JARVIS_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
class PluginManifest:
    ...
    #              in ~/.jarvis/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``jarvis plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def llm(self) -> Any:
        ...
        if self._llm is None:
            from brain.plugin_llm import PluginLlm
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``jarvis honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``jarvis <subcommand>``
    ...
    def register_image_gen_provider(self, provider) -> None:
        ...
        from brain.image_gen_registry import register_provider
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`jarvis_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from jarvis_cli.dashboard_auth import (
    ...
    def register_video_gen_provider(self, provider) -> None:
        ...
        from brain.video_gen_registry import register_provider as _register_video_provider
    ...
    def register_web_search_provider(self, provider) -> None:
        ...
        from brain.web_search_registry import register_provider as _register_web_provider
    ...
    def register_browser_provider(self, provider) -> None:
        ...
        from brain.browser_registry import register_provider as _register_browser_provider
    ...
    def register_tts_provider(self, provider) -> None:
        ...
        from brain.tts_registry import register_provider as _register_tts_provider
    ...
    def register_transcription_provider(self, provider) -> None:
        ...
        **but only when**:
        ...
        from brain.transcription_registry import register_provider as _register_stt_provider
    ...
    def register_slack_action_handler(
        ...
        Jarvis' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``jarvis model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: jarvis_cli.main imports plugins indirectly
        from jarvis_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.jarvis/skills/`` tree and is **not** listed in the system
        ...
        """
        from brain.skill_utils import _NAMESPACE_RE
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        # Safe mode (--safe-mode / JARVIS_SAFE_MODE=1): troubleshooting run
        ...
        if env_var_enabled("JARVIS_SAFE_MODE"):
            logger.info("JARVIS_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to jarvis_cli/. Two layouts are
        ...
        # 2. User plugins (~/.jarvis/plugins/)
        user_dir = get_jarvis_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.jarvis/plugins/)
        if _env_enabled("JARVIS_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".jarvis" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set JARVIS_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with jarvis and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Jarvis ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `jarvis plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `jarvis plugins list`.
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="jarvis-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``jarvis tools`` TUI so plugin-provided toolsets appear
