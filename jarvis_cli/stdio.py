...
on Japanese locales, etc.).  Jarvis's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``jarvis_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Jarvis is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``JARVIS_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("JARVIS_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Jarvis-managed Git install directories so
    ...
    # already-running shells don't see the change — which means jarvis
    ...
    # install without Jarvis-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Jarvis's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Jarvis's
    ``jarvis config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Jarvis (User env var in Windows Settings, or
    export in a PowerShell profile) and Jarvis picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Jarvis-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\jarvis\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``jarvis`` from right after install) retain their old PATH.

    Any subprocess Jarvis spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Jarvis-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "jarvis", "git", "cmd"),
        os.path.join(local_appdata, "jarvis", "git", "bin"),
        os.path.join(local_appdata, "jarvis", "git", "usr", "bin"),
        # Jarvis venv Scripts directory — host of the jarvis.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes jarvis, but harmless to include.
        os.path.join(local_appdata, "jarvis", "jarvis-agent", "venv", "Scripts"),
