...
def _provider_base_url(provider: str) -> str:
    ...
    if provider.startswith(CUSTOM_POOL_PREFIX):
        from brain.credential_pool import _get_custom_provider_config
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        from brain import anthropic_adapter as anthropic_mod
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``jarvis auth add openai-codex`` overwrote
    ...
    if provider == "google-gemini-cli":
        from brain.google_oauth import run_gemini_oauth_login_pure
    ...
    raise SystemExit(f"`jarvis auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Jarvis reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `jarvis auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `jarvis auth` is called bare."""
    ...
    try:
        from brain.bedrock_adapter import has_aws_credentials, resolve_aws_auth_env_var, resolve_bedrock_region
