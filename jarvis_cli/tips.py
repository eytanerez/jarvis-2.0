...
TIPS = [
    ...
    "jarvis chat -q \"query\" runs a single non-interactive query and exits.",
    "jarvis chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "jarvis chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "jarvis --yolo bypasses all dangerous command approval prompts for the entire session.",
    "jarvis chat --source telegram tags the session for filtering in jarvis sessions list.",
    "jarvis -p work chat runs under a specific profile without changing your default.",
    ...
    "jarvis sessions browse opens an interactive session picker with search.",
    "jarvis sessions stats shows session counts by platform and database size.",
    "jarvis sessions prune --older-than 30 cleans up old sessions.",
    "jarvis skills search react --source skills-sh searches the skills.sh public directory.",
    "jarvis skills check scans installed hub skills for upstream updates.",
    "jarvis skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "jarvis skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "jarvis mcp add github --command npx adds MCP servers from the command line.",
    "jarvis mcp serve runs Jarvis itself as an MCP server for other agents.",
    "jarvis auth add lets you add multiple API keys for credential pool rotation.",
    "jarvis completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "jarvis logs -f follows brain.log in real time. --level WARNING --since 1h filters output.",
    "jarvis backup creates a zip backup of your entire Jarvis home directory.",
    "jarvis profile create coder creates an isolated profile that becomes its own command.",
    "jarvis profile create work --clone copies your current config and keys to a new profile.",
    "jarvis update syncs new bundled skills to ALL profiles automatically.",
    "jarvis gateway install sets up Jarvis as a system service (systemd/launchd).",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as jarvis -w).",
    ...
    "JARVIS_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- MCP ---
    "jarvis mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "jarvis mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "jarvis mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "jarvis plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "jarvis acp runs Jarvis as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "jarvis auth add nous or jarvis auth add openai-codex sets up OAuth-based providers.",
    ...
    "brain.tool_use_enforcement steers models that describe actions instead of calling tools — auto for GPT/Codex.",
    "brain.restart_drain_timeout (default 60s) lets running agents finish before a gateway restart takes effect.",
    "brain.api_max_retries (default 3) controls how many times the agent retries a failed API call before surfacing the error — lower it for fast fallback.",
    "The gateway caches AIBrain instances per session — destroying this cache breaks Anthropic prompt caching.",
    ...
    "Profiles scope Jarvis state via JARVIS_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    # --- TUI & Dashboard ---
    'JARVIS_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "JARVIS_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'jarvis dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'jarvis dashboard embeds the full Jarvis TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.jarvis/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.jarvis/dashboard-plugins/ — no npm build required.',
    ...
    'JARVIS_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'JARVIS_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'JARVIS_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'JARVIS_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    'jarvis chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
