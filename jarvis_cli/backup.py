"""
Backup and import commands for jarvis CLI.

`jarvis backup` creates a zip archive of the entire ~/.jarvis/ directory
(excluding the jarvis-agent repo and transient files).

`jarvis import` restores from a backup zip, overlaying onto the current
JARVIS_HOME root.
"""
...
# ``jarvis-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/jarvis-brain/``
...
# JARVIS_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under JARVIS_HOME) —
...
# File names that ``jarvis import`` must never overwrite, matched by basename so
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside JARVIS_HOME.
...
def run_backup(args) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"jarvis-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"jarvis-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_jarvis_home()} ...")
    ...
    print(f"\nRestore with: jarvis import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Jarvis backup.
    ...
    # Look for telltale files that a jarvis home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Jarvis backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.jarvis/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a jarvis dir name
        if prefix in {".jarvis", "jarvis"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_jarvis_home()}")
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Jarvis configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # jarvis_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: jarvis profile list  (after installing jarvis)")
        ...
        if restored_profiles:
            ...
            for pname in gw_profiles:
                print(f"  jarvis -p {pname} gateway install")

        print("Done. Your Jarvis configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and jarvis backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to JARVIS_HOME).
...
# — `jarvis update` snapshots this set before pulling so approved-user lists
...
# Relative path of the cron job database inside JARVIS_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``jarvis update``.
    ...
    Args:
        ...
        jarvis_home: Override for the Jarvis home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for jarvis backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_jarvis_home()}/state-snapshots/")
...
def create_pre_update_backup(
    ...
    but writes to ``<JARVIS_HOME>/backups/pre-update-<timestamp>.zip`` and
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `jarvis claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    ``jarvis claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``jarvis import <archive>``.  Writes to
    ``<JARVIS_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
