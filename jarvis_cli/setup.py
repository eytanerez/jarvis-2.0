...
_DOCS_BASE = "https://jarvis-agent.nousresearch.com/docs"
...
from jarvis_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from jarvis_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Jarvis Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Jarvis using environment variables or config commands:")
    print_info("  jarvis config set model.provider custom")
    print_info("  jarvis config set model.base_url http://localhost:8080/v1")
    print_info("  jarvis config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'jarvis setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from jarvis_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, jarvis_home):
    ...
    try:
        from brain.auxiliary_client import get_available_vision_backends
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'jarvis setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        _img_backend = None
        try:
            from brain.image_gen_registry import list_providers
    ...
    # Video generation — opt-in via `jarvis tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            from brain.video_gen_registry import list_providers as _list_video_providers
            from jarvis_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'jarvis setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'jarvis setup tts'"))
    else:
    ...
    # Spotify (OAuth via jarvis auth spotify — check auth.json, not env vars)
    try:
    ...
    print()
    print(f"   {color('jarvis setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('jarvis setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('jarvis setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('jarvis setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('jarvis setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('jarvis config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('jarvis config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('jarvis config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('jarvis', Colors.GREEN)}              Start chatting")
    print(f"   {color('jarvis gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('jarvis doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `jarvis tools` and `jarvis setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``jarvis model``)
    ...
    provider added to ``jarvis model`` is automatically available here.
    ...
    # Delegate to the shared jarvis model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from jarvis_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: jarvis model")
    ...
    # on demand via `jarvis auth add`, `jarvis setup` vision, and
    # `jarvis setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``jarvis model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.jarvis/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Jarvis auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'jarvis setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Jarvis runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `jarvis setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `jarvis setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into JARVIS_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("JARVIS_MAX_ITERATIONS")
    ...
    print_info("  Run `jarvis setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # JARVIS_MAX_ITERATIONS from brain.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from jarvis_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_jarvis_home(Path(get_jarvis_home()))
    except Exception:
        pass
...
def _profile_name_from_jarvis_home(jarvis_home) -> str | None:
    """Return the active profile name when JARVIS_HOME is a profile dir."""
    if jarvis_home.parent.name == "profiles":
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Jarvis delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `jarvis update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://jarvis-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under JARVIS_HOME, and print
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="Jarvis",
            bot_description="Your Jarvis agent on Slack",
        )
        ...
        print_info(
            "   Re-run `jarvis slack manifest --write` anytime to refresh after "
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Jarvis delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Jarvis to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from jarvis_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://jarvis-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://jarvis-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  jarvis config edit")
    print_info("   Open config in your editor:  jarvis config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from jarvis_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Jarvis from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'jarvis setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     jarvis config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   jarvis gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `jarvis setup tools` and `jarvis tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``jarvis_cli.auth`` — lists every supported
    ...
    """
    try:
        from jarvis_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from jarvis_cli.config
    ...
    if section_key == "model":
        ...
        from jarvis_cli.gateway import _all_platforms, _platform_status
...
# Config values may have different semantics between OpenClaw and Jarvis.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Jarvis to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Jarvis at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Jarvis at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Jarvis at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Jarvis at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Jarvis equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Jarvis config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Jarvis.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Jarvis's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(jarvis_home: Path) -> bool:
    ...
    print_info("Jarvis can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: jarvis claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: jarvis claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Jarvis configs are
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Jarvis (use jarvis claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``jarvis setup --portal`` and ``jarvis portal``. This is the
    ...
    Jarvis session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``jarvis setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``jarvis model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``jarvis portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Jarvis Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `jarvis portal` == quick setup's Nous step.
    try:
        from jarvis_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `jarvis portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `jarvis portal`: %s", exc)
        print()
    ...
    print_info("  Run `jarvis portal info` to inspect routing.")
    print_info("  Run `jarvis` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      jarvis setup           — full or quick (auto-detected)
      jarvis setup model     — just model/provider
      jarvis setup tts       — just text-to-speech
      jarvis setup terminal  — just terminal backend
      jarvis setup gateway   — just messaging platforms
      jarvis setup tools     — just tool configuration
      jarvis setup agent     — just agent settings
    """
    from jarvis_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Jarvis Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Jarvis Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Jarvis installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Jarvis configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'jarvis setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Jarvis?",
            [
    ...
    # Tune later with `jarvis setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, jarvis_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``jarvis setup <section>``
    or switch providers with ``jarvis model``.
    """
    from jarvis_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'jarvis setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    jarvis setup")
    if gateway_choice != 0:
...
def _run_quick_setup(config: dict, jarvis_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'jarvis setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Jarvis to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'jarvis setup gateway'.")
