...
billing screens drive. Companion to ``jarvis_cli/nous_account.py`` (which owns
...
- **This client raises typed exceptions; it does NOT fail open.** Fail-open is the
  *caller's* job (the ``brain/billing_view.py`` builders) so each surface can
...
- **Auth** = the OAuth bearer JWT Jarvis already holds for inference
...
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``JARVIS_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → the
...
# jarvis_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``JARVIS_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("JARVIS_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Nous Portal — run `jarvis portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
