...
configured for "cli" in `jarvis tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (JARVIS_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `jarvis chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "jarvis -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"jarvis -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "jarvis -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "jarvis -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to JARVIS_INFERENCE_MODEL
    ...
    """
    # Silence every stdlib logger for the duration.  AIBrain, tools, and
    ...
    env_model_early = os.getenv("JARVIS_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "jarvis -z: --provider requires --model (or JARVIS_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        if isinstance(failure, (KeyboardInterrupt, SystemExit)):
            raise failure
        real_stderr.write(f"jarvis -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1

    if not (response or "").strip():
        real_stderr.write("jarvis -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``jarvis -z`` / oneshot mode.

    Oneshot bypasses ``JarvisCLI._init_agent()``, so it must wire the SQLite
...
def _run_brain(
    ...
    """Build an AIBrain exactly like a normal CLI chat turn would, then
    run a single conversation.  Returns the final response string."""
    # Imports are local so they don't run when jarvis is invoked for
    ...
    agent = AIBrain(
        ...
        #                JARVIS_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via JARVIS_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via JARVIS_YOLO_MODE=1
        #   - skill secret capture → returns gracefully when no callback set
        clarify_callback=_oneshot_clarify_callback,
    )

    # Belt-and-braces: make sure AIBrain doesn't invoke any streaming
