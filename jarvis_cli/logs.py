"""``jarvis logs`` — view and filter Jarvis log files.
...
Usage examples::
    ...
    jarvis logs                    # last 50 lines of brain.log
    jarvis logs -f                 # follow brain.log in real time
    jarvis logs errors             # last 50 lines of errors.log
    jarvis logs gateway -n 100    # last 100 lines of gateway.log
    jarvis logs gui -f            # follow gui.log (dashboard/pty/ws)
    jarvis logs desktop -f        # follow desktop.log (Electron app boot/backend)
    jarvis logs --level WARNING    # only WARNING+ lines
    jarvis logs --session abc123   # filter by session ID substring
    jarvis logs --component tools  # only tool-related lines
    jarvis logs --since 1h         # lines from the last hour
    jarvis logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_jarvis_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Jarvis runs — try 'jarvis chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_jarvis_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_jarvis_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_jarvis_home()}/logs/")
        return

    print(f"Log files in {display_jarvis_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'jarvis chat' to generate logs)")
