...
``<root>`` is the **shared Jarvis root** (the parent of any active
...
``jarvis -p <profile>`` joins the same board as the dispatcher that
...
the profile's Jarvis home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``JARVIS_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``JARVIS_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``jarvis kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.jarvis``. In Docker / custom
deployments where ``JARVIS_HOME`` points outside ``~/.jarvis`` (e.g.
``/opt/jarvis``), ``<root>`` is ``JARVIS_HOME``. Legacy env-var
overrides still work:

* ``JARVIS_KANBAN_DB`` — pin the database file path directly.
* ``JARVIS_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``JARVIS_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``JARVIS_KANBAN_DB``,
``JARVIS_KANBAN_WORKSPACES_ROOT``, and ``JARVIS_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/jarvis-kanban-v1-spec.pdf`` for the full
...
# ``JARVIS_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``JARVIS_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("JARVIS_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``JARVIS_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("JARVIS_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``JARVIS_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "JARVIS_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "jarvis_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``jarvis-agent``
...
def kanban_home() -> Path:
    """Return the shared Jarvis root that anchors the kanban board.

    Resolution order:

    1. ``JARVIS_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_jarvis_root()``, which already returns ``<root>``
       when ``JARVIS_HOME`` is ``<root>/profiles/<name>``, and returns
       ``JARVIS_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``JARVIS_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``jarvis kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``JARVIS_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``jarvis kanban boards
    ...
    env = os.environ.get("JARVIS_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``jarvis kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``JARVIS_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("JARVIS_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``JARVIS_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("JARVIS_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``JARVIS_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("JARVIS_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``jarvis kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``JARVIS_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``JARVIS_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("JARVIS_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Jarvis treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``JARVIS_KANBAN_DB`` env → ``JARVIS_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See jarvis_state._WAL_INCOMPAT_MARKERS for detection logic.
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``jarvis kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        # Originating brain/chat session id, populated when the task is
        # created from within an agent loop that propagated
        # ``JARVIS_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from jarvis_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``jarvis --skills ...`` alongside the built-in
    ...
    # `jarvis --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``jarvis kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `jarvis kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``jarvis kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``jarvis kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``jarvis kanban block <id>``).  This is a deliberate handoff that
...
def release_stale_claims(
    ...
    *extended* (with a ``claim_extended`` event) instead of being
    ...
    nothing. ``_touch_activity`` (run_brain.py) bridges chunk-level
...
def complete_task(
    ...
    *,
    ...
    completion (``jarvis kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``JARVIS_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Jarvis' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Jarvis-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``jarvis kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``JARVIS_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Jarvis
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``jarvis kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``jarvis kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Jarvis profile.
    ...
    if not rows:
        return False
    try:
        from jarvis_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from jarvis_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Jarvis profile.
        # `_default_spawn` invokes ``jarvis -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Jarvis
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `jarvis kanban tail` — without
...
def _module_jarvis_argv() -> list[str]:
    """Return the interpreter-bound Jarvis CLI invocation."""
    # ``jarvis_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``jarvis`` package — there is no
    # ``jarvis`` package to import.
    return [sys.executable, "-m", "jarvis_cli.main"]


def _absolute_jarvis_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Jarvis shim."""
...
def _jarvis_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Jarvis executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_jarvis_argv()
    return [_absolute_jarvis_path(path)]


def _resolve_jarvis_argv() -> list[str]:
    """Resolve the ``jarvis`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$JARVIS_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("jarvis")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\jarvis.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m jarvis_cli.main`` — fallback for setups where
       Jarvis is launched from a venv and the ``jarvis`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_jarvis_bin`` for the same reason. Kept
    local (not imported from gateway) because ``jarvis_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("JARVIS_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _jarvis_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _jarvis_path_argv(resolved_env_bin)
        return _module_jarvis_argv()

    jarvis_bin = _safe_which_no_cwd("jarvis") if _IS_WINDOWS else shutil.which("jarvis")
    if jarvis_bin:
        return _jarvis_path_argv(jarvis_bin)
    return _module_jarvis_argv()


def _kanban_worker_skill_available(jarvis_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``jarvis -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset JARVIS_HOME means the worker falls back to the default root
    # home (``~/.jarvis``), which ships the bundled skill.
    base = _Path(jarvis_home) if jarvis_home else (_Path.home() / ".jarvis")
...
def _resolve_worker_cli_toolsets(jarvis_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``JARVIS_KANBAN_TASK`` is set.
    """
    if not jarvis_home:
        return None
    try:
        ...
        token = set_jarvis_home_override(jarvis_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for JARVIS_HOME=%r (%s)",
            jarvis_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``jarvis -p <profile> chat -q ...`` subprocess.
    ...
    ``JARVIS_KANBAN_DB`` / ``JARVIS_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject JARVIS_HOME so the worker reads the profile-scoped config.yaml
    ...
    try:
        env["JARVIS_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via JARVIS_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # JARVIS_HOME never had profiles created.
        pass
    if task.tenant:
        env["JARVIS_TENANT"] = task.tenant
    env["JARVIS_KANBAN_TASK"] = task.id
    env["JARVIS_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["JARVIS_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["JARVIS_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["JARVIS_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["JARVIS_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["JARVIS_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`jarvis -p <name>`
    # rewrites JARVIS_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_jarvis_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["JARVIS_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["JARVIS_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["JARVIS_KANBAN_BOARD"] = resolved_board
    # JARVIS_PROFILE is the author the kanban_comment tool defaults to.
    # `jarvis -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["JARVIS_PROFILE"] = profile_arg

    cmd = [
        *_resolve_jarvis_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped JARVIS_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("JARVIS_HOME")):
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("JARVIS_HOME"))
    if worker_toolsets:
    ...
    # `jarvis kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`jarvis` executable not found on PATH. "
            "Install Jarvis or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``jarvis kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled JARVIS_PROFILE values like "jarvis-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Jarvis root exists

    Reads profile paths directly so this module has no import dependency on
    ``jarvis_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from jarvis_constants import get_default_jarvis_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``jarvis kanban assignees`` for the terminal.
