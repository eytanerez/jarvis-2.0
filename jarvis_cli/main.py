...
Usage:
    ...
    jarvis gateway start       # Start gateway as service
    jarvis gateway stop        # Stop gateway service
    jarvis gateway status      # Show gateway status
    jarvis gateway install     # Install gateway service
    jarvis gateway uninstall   # Uninstall gateway service
    jarvis setup               # Interactive setup wizard
    jarvis logout              # Clear stored authentication
    jarvis status              # Show status of all components
    jarvis cron                # Manage cron jobs
    jarvis cron list           # List cron jobs
    jarvis cron status         # Check if cron scheduler is running
    jarvis doctor              # Check configuration and dependencies
    jarvis honcho setup                    # Configure Honcho AI memory integration
    jarvis honcho status                   # Show Honcho config and connection status
    jarvis honcho sessions                 # List directory → session name mappings
    jarvis honcho map <name>               # Map current directory to a session name
    jarvis honcho peer                     # Show peer names and dialectic settings
    jarvis honcho peer --user NAME         # Set user peer name
    jarvis honcho peer --ai NAME           # Set AI peer name
    jarvis honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    jarvis honcho mode                     # Show current memory mode
    jarvis honcho mode [hybrid|honcho|local]  # Set memory mode
    jarvis honcho tokens                   # Show token budget settings
    jarvis honcho tokens --context N       # Set session.context() token cap
    jarvis honcho tokens --dialectic N     # Set dialectic result char cap
    jarvis honcho identity                 # Show AI peer identity representation
    jarvis honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    jarvis honcho migrate                  # Step-by-step migration guide: OpenClaw native → Jarvis + Honcho
    jarvis version             Show version
    jarvis update              Update to latest version
    jarvis uninstall           Uninstall Jarvis
    jarvis acp                 Run as an ACP server for editor integration
    jarvis sessions browse     Interactive session picker with search
...
def _set_process_title() -> None:
    """Set the process title to 'jarvis' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``jarvis tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``jarvis.exe``).
    """
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"jarvis", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"jarvis")
        # Windows: the .exe name is already ``jarvis.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* jarvis_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".jarvis", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``JARVIS_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("JARVIS_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``JARVIS_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("JARVIS_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`jarvis --tui … >log`, CI capture):
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``jarvis --version`` before config/logging imports on Termux."""
    if os.environ.get("JARVIS_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from jarvis_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from jarvis_cli.subcommands.cron import build_cron_parser
from jarvis_cli.subcommands.gateway import build_gateway_parser
from jarvis_cli.subcommands.profile import build_profile_parser
from jarvis_cli.subcommands.model import build_model_parser
from jarvis_cli.subcommands.setup import build_setup_parser
from jarvis_cli.subcommands.postinstall import build_postinstall_parser
from jarvis_cli.subcommands.whatsapp import build_whatsapp_parser
from jarvis_cli.subcommands.slack import build_slack_parser
from jarvis_cli.subcommands.login import build_login_parser
from jarvis_cli.subcommands.logout import build_logout_parser
from jarvis_cli.subcommands.auth import build_auth_parser
from jarvis_cli.subcommands.status import build_status_parser
from jarvis_cli.subcommands.webhook import build_webhook_parser
from jarvis_cli.subcommands.hooks import build_hooks_parser
from jarvis_cli.subcommands.doctor import build_doctor_parser
from jarvis_cli.subcommands.security import build_security_parser
from jarvis_cli.subcommands.dump import build_dump_parser
from jarvis_cli.subcommands.debug import build_debug_parser
from jarvis_cli.subcommands.backup import build_backup_parser
from jarvis_cli.subcommands.import_cmd import build_import_cmd_parser
from jarvis_cli.subcommands.config import build_config_parser
from jarvis_cli.subcommands.version import build_version_parser
from jarvis_cli.subcommands.update import build_update_parser
from jarvis_cli.subcommands.uninstall import build_uninstall_parser
from jarvis_cli.subcommands.dashboard import build_dashboard_parser
from jarvis_cli.subcommands.gui import build_gui_parser
from jarvis_cli.subcommands.logs import build_logs_parser
from jarvis_cli.subcommands.prompt_size import build_prompt_size_parser
from jarvis_cli.subcommands.memory import build_memory_parser
from jarvis_cli.subcommands.acp import build_acp_parser
from jarvis_cli.subcommands.tools import build_tools_parser
from jarvis_cli.subcommands.insights import build_insights_parser
from jarvis_cli.subcommands.skills import build_skills_parser
from jarvis_cli.subcommands.pairing import build_pairing_parser
from jarvis_cli.subcommands.plugins import build_plugins_parser
from jarvis_cli.subcommands.mcp import build_mcp_parser
from jarvis_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (jarvis tools, jarvis setup, jarvis model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'jarvis {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any jarvis module import.
#
# Many modules cache JARVIS_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("JARVIS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.jarvis/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set JARVIS_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `jarvis mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Jarvis' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo jarvis -p <name>` against the invoking user's home.
        ...
        candidate = home / ".jarvis" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`jarvis chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors jarvis_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If JARVIS_HOME is already set and no explicit flag was given, trust it
    ...
    # run-script as JARVIS_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``jarvis gateway run`` to mean
    # "the root JARVIS_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("JARVIS_S6_SUPERVISED_CHILD"):
        try:
    ...
    # 3. If we found a profile, resolve and set JARVIS_HOME
    if profile_name is not None:
        try:
            ...
            jarvis_home = _resolve_sudo_user_profile_env(profile_name)
            if not jarvis_home:
            ...
            # A bug in profiles.py must NEVER prevent jarvis from starting
            print(
...
try:
    ...
    _cfg_path = get_jarvis_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from jarvis_cli import managed_scope
        ...
        if "JARVIS_REDACT_SECRETS" not in os.environ:
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["JARVIS_REDACT_SECRETS"] = str(_early_redact).lower()
...
# (chat, setup, gateway, config, etc.) write to brain.log + errors.log.
...
if _FORCE_IPV4_EARLY:
    try:
        ...
        pass  # best-effort — don't crash if jarvis_constants not importable yet
...
# Provider model-selection wizard flows extracted to jarvis_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (jarvis_cli.main._model_flow_*) keep resolving unchanged.
from jarvis_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_jarvis_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("JARVIS_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("JARVIS_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from jarvis_cli.config import get_env_path, get_jarvis_home, load_config
    from jarvis_cli.auth import get_auth_status

    # Determine whether Jarvis itself has been explicitly configured (model
    ...
    if _has_jarvis_config:
        try:
            from brain.anthropic_adapter import (
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo jarvis {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo jarvis {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  jarvis --tui --resume {target}")
    if title:
        print(f'  jarvis --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @jarvis/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@jarvis" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/jarvis-ink/package.json",
    "packages/jarvis-ink/index.js",
    "packages/jarvis-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``JARVIS_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("JARVIS_TUI_FORCE_BUILD") or "").strip().lower()
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (JARVIS_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("JARVIS_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from jarvis_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("JARVIS_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with JARVIS_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset JARVIS_TUI_DIR (e.g. `unset JARVIS_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @jarvis/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @jarvis/ink resolves through
        # packages/jarvis-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "jarvis-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from jarvis_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from jarvis_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="jarvis-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["JARVIS_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["JARVIS_PYTHON_SRC_ROOT"] = os.environ.get(
        "JARVIS_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("JARVIS_PYTHON", sys.executable)
    env.setdefault("JARVIS_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["JARVIS_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["JARVIS_MODEL"] = model
        env["JARVIS_INFERENCE_MODEL"] = model
    if provider:
        env["JARVIS_TUI_PROVIDER"] = provider
        env["JARVIS_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["JARVIS_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["JARVIS_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["JARVIS_TUI_SKILLS"] = value
    if query:
        env["JARVIS_TUI_QUERY"] = query
    if image:
        env["JARVIS_TUI_IMAGE"] = image
    if checkpoints:
        env["JARVIS_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["JARVIS_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["JARVIS_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["JARVIS_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["JARVIS_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["JARVIS_ACCEPT_HOOKS"] = "1"
    ...
    # JARVIS_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `jarvis --tui` try to
    ...
    # still set JARVIS_TUI_RESUME themselves.
    env.pop("JARVIS_TUI_RESUME", None)
    if resume_session_id:
        env["JARVIS_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `jarvis update` so
    ...
    if code == 42:
        from jarvis_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    (``jarvis kanban …``) resolve the board on different paths: the env-pin if
    ...
    """
    if os.environ.get("JARVIS_KANBAN_BOARD"):
        return
    try:
        from jarvis_cli.kanban_db import get_current_board

        os.environ["JARVIS_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.jarvis/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Jarvis — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Jarvis still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``JARVIS_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("JARVIS_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    try:
        ...
        from jarvis_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'jarvis doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Jarvis isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  jarvis setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'jarvis setup' at any time to configure.")
        sys.exit(1)
    ...
    # setup (config, rules files, plugins, MCP servers) or from Jarvis itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (JARVIS_SAFE_MODE is checked by
    # jarvis_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["JARVIS_SAFE_MODE"] = "1"
        os.environ["JARVIS_IGNORE_USER_CONFIG"] = "1"
        os.environ["JARVIS_IGNORE_RULES"] = "1"
    ...
    # Maps to AIBrain(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["JARVIS_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["JARVIS_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from jarvis_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `jarvis gateway` then paid a 30s
    ...
    session_dir = get_jarvis_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: jarvis gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `jarvis gateway` skips it cleanly instead of paying a 30s
        ...
        print()
        print("  Or install as a service: jarvis gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'jarvis whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``jarvis whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``jarvis_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from jarvis_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from jarvis_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from jarvis_cli.config import stamp_install_method
    from jarvis_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Jarvis post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from jarvis_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``jarvis model``) and the setup wizard
    ...
    from jarvis_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("JARVIS_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'jarvis model' for "
                "available providers, or run 'jarvis doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in jarvis_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.jarvis/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.jarvis/.env if the active provider is not 'custom'.
    ...
    """
    from jarvis_cli.config import get_env_value, save_env_value, load_config
...
#
# Jarvis uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `jarvis model` provider picker. It does NOT re-run credential setup — it
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`jarvis_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from jarvis_cli.config import load_config
    from jarvis_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from jarvis_cli.auth import _prompt_model_selection
    from jarvis_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from jarvis_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Jarvis URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from jarvis_cli.curses_ui import curses_radiolist
...
# downstream call sites read `jarvis_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `jarvis` invocation — including
# fast paths like `jarvis --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from jarvis_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``jarvis setup`` / ``jarvis model``.
    ...
    """
    from jarvis_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `jarvis setup` to configure {pconfig.name} again."
        )
        return "", True
...
def cmd_login(args):
    """Authenticate Jarvis CLI with a provider."""
    from jarvis_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from jarvis_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from jarvis_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from jarvis_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from jarvis_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from jarvis_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub in {None, ""}:
        ...
        print(
            ...
            "\n"
            "Run `jarvis slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from jarvis_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from jarvis_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from jarvis_cli.hooks import hooks_command
...
def cmd_security(args):
    """Dispatch `jarvis security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from jarvis_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from jarvis_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from jarvis_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from jarvis_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Jarvis home directory to a zip file."""
    if getattr(args, "quick", False):
        from jarvis_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from jarvis_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Jarvis backup from a zip file."""
    from jarvis_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from jarvis_cli.banner import format_banner_version_label
...
def cmd_uninstall(args):
    """Uninstall Jarvis (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from jarvis_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from jarvis_cli.uninstall import run_gui_uninstall
    ...
    from jarvis_cli.uninstall import run_uninstall
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``jarvis`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="jarvis-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``jarvis update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``jarvis_cli/web_dist/`` (per vite.config.ts
    outDir: "../jarvis_cli/web_dist"), NOT to ``web/dist/``, so Python
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``jarvis`` launcher present but ``jarvis_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the jarvis venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — jarvis venv python3, no nix-shell overhead
    ...
    # a jarvis venv (pip / non-managed / bare-git installs).  The resolved
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m jarvis_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (jarvis web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``jarvis update`` can unconditionally call ``jarvis desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``jarvis desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $JARVIS_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $JARVIS_HOME."""
    from jarvis_constants import get_jarvis_home
    return get_jarvis_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``jarvis update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Jarvis.app/Contents/MacOS/Jarvis"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Jarvis.exe",
            release_dir / "win-ia32-unpacked" / "Jarvis.exe",
            release_dir / "win-arm64-unpacked" / "Jarvis.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "jarvis",
            release_dir / "linux-unpacked" / "Jarvis",
            release_dir / "linux-arm64-unpacked" / "jarvis",
            release_dir / "linux-arm64-unpacked" / "Jarvis",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Jarvis'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Jarvis.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Jarvis.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Jarvis.exe: Access is
    ...
    an unrelated "Jarvis" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Jarvis is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Jarvis.app/Contents/MacOS/Jarvis  ->  app bundle = .../Jarvis.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Jarvis Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Jarvis Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from jarvis_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "ignore_existing", False):
        env["JARVIS_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "jarvis_root", None):
        env["JARVIS_DESKTOP_JARVIS_ROOT"] = str(Path(args.jarvis_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["JARVIS_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  jarvis gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Jarvis.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Jarvis.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Jarvis.exe, close any")
                    print("  running Jarvis desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> jarvis desktop --force-build")
    ...
    if source_mode:
        print("→ Launching Jarvis Desktop from source build...")
    ...
    print(f"→ Launching packaged Jarvis Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``jarvis dashboard`` processes other than ourselves.

    ``jarvis dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``jarvis update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Jarvis Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``jarvis dashboard`` as
    ...
    environment variable ``JARVIS_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "jarvis dashboard",
        "jarvis_cli.main dashboard",
        "jarvis_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "jarvis.*dashboard"`) keeps us consistent
            # with `jarvis_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `jarvis update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``jarvis update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``jarvis update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: jarvis curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``jarvis dashboard`` processes.

    Called at the end of ``jarvis update`` (default ``reason``) and also
    from ``jarvis dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Jarvis Desktop Electron app spawns this dashboard as a
    # backend child, it sets JARVIS_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("JARVIS_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    jarvis dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Jarvis by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `jarvis update "
            f"--branch {branch}`, or update against main with `jarvis update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for jarvis-agent itself
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: jarvis skills list-modified  "
    ...
    try:
        from jarvis_cli.model_catalog import seed_cache_from_checkout
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Jarvis behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Jarvis couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Jarvis couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Jarvis behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Jarvis couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Jarvis couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `jarvis update`
# =========================================================================
...
# Install-scoped breadcrumb dropped right before ``jarvis update`` mutates the
...
# survives and the next ``jarvis`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $JARVIS_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``jarvis update`` left half-done.
    ...
    protocol stream (``jarvis acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `jarvis update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from jarvis_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``jarvis update`` is still progressing even if pip/uv itself is silent.
    """
...
def _jarvis_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "jarvis.exe",
        scripts_dir / "jarvis-gateway.exe",
    ]
...
def _detect_concurrent_jarvis_instances(
    ...
    shims (``jarvis.exe`` / ``jarvis-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``jarvis update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``jarvis.exe``) is a separate process from the Python
    ...
    # setuptools-generated jarvis.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``jarvis update``
    ...
    #      jarvis.exe sitting *under* a non-Jarvis parent (e.g. a Jarvis
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "jarvis.exe"
    lines = ["✗ Another jarvis.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Jarvis Desktop, exit any open `jarvis` REPLs, and")
    lines.append("  stop the gateway (`jarvis gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `jarvis update --force` if you've already")
...
def _quarantine_running_jarvis_exe(
    ...
    when ``jarvis update`` runs, ``jarvis.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``jarvis.exe.old.<unix-ms>`` first. uv then writes
    ...
    3. Print a clear warning naming the most likely culprit (running
       Jarvis Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _jarvis_exe_shims(scripts_dir):
        ...
        print(
            "    Close Jarvis Desktop, exit other `jarvis` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `jarvis update`."
        )
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``jarvis.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``jarvis.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``jarvis`` drops off PATH. This wraps
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``jarvis.exe.old.*`` left by prior updates.

    Called early on every jarvis invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    if failed:
        ...
        print("  `jarvis update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``jarvis update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``jarvis update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``jarvis.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``jarvis`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `jarvis update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `jarvis update --force` after closing other jarvis processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from jarvis_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$JARVIS_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from jarvis_cli.managed_uv import resolve_uv
...
class _UpdateOutputStream:
    """Stream wrapper used during ``jarvis update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.jarvis/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``jarvis update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``jarvis update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.jarvis/logs/update.log`` and to silently absorb
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``jarvis update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from jarvis_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``jarvis update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from jarvis_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from jarvis_cli.config import recommended_update_command
        from jarvis_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``jarvis update`` without requiring a reinstall.  The
    ...
    ``jarvis`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/jarvis.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v jarvis'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/jarvis, code at /usr/local/lib/jarvis-agent).
    fhs_link = Path("/usr/local/bin/jarvis")
    ...
    path_comment = (
        "# Jarvis — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of JARVIS_HOME before running the update.
    ...
    # ``jarvis_cli/config.py``). Fall back to true if the key is missing
    ...
    try:
        from jarvis_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    try:
        ...
        home = get_jarvis_home()
        try:
            display_path = f"{display_jarvis_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  jarvis import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    jarvis.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Jarvis...")
    try:
    ...
    if unmapped_pids:
        ...
        print("    Restart manually after update: jarvis gateway run")
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    try:
        from jarvis_cli.gateway import launch_detached_profile_gateway_restart
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``jarvis update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `jarvis update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Jarvis via pip (for PyPI installs)."""
    from jarvis_cli import __version__
    from jarvis_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "jarvis-agent"]
        ...
        cmd = [pipx, "upgrade", "jarvis-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "jarvis-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "jarvis-agent"]
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Jarvis...")
    print()

    # On Windows, abort early if another jarvis.exe is holding the venv shim
    ...
    try:
        ...
        # though `git pull` can't touch $JARVIS_HOME, this is cheap
        ...
        try:
            from jarvis_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in jarvis_cli/config.py bricked
        # every user who ran ``jarvis update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``jarvis
        ...
        # current — so copy it straight over ~/.jarvis/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit JARVIS_HOME so
        # it is not affected by sync_skills()'s module-level JARVIS_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's JARVIS_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from jarvis_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'jarvis config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: jarvis config migrate")
            else:
                print()
                print("Skipped. Run 'jarvis config migrate' later to configure.")
        else:
        ...
        try:
            from jarvis_cli.backup import restore_cron_jobs_if_emptied
        ...
        # Tying the refresh to ``jarvis update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from jarvis_cli.tools_config import install_cua_driver
        ...
        # When running as ``jarvis update --gateway`` (spawned by the gateway's
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... jarvis-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... jarvis-gateway*)
            ...
            # Discover all jarvis-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. jarvis-gateway.service or jarvis-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `jarvis update` restart it automatically, allow\n"
                            ...
                            # path in `jarvis gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: jarvis gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: jarvis gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: jarvis -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Jarvis gateway unit files are still installed.
        # When both jarvis.service (from a pre-rename install) and the
        # current jarvis-gateway.service are enabled, they SIGTERM-fight
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_jarvis_units():
                ...
                print()
                print("  These pre-rename units (jarvis.service) fight the current")
                print("  jarvis-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    jarvis gateway migrate-legacy")
                print()
        ...
        print("  jarvis model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``jarvis -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `jarvis profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → jarvis -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.jarvis)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Or access via flag:     jarvis -p {name} chat")
                else:
        ...
        from jarvis_cli import profiles as _profiles_mod
        ...
        from jarvis_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `jarvis profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → jarvis -p {name}  ({wrapper})")
        print()
        ...
        from jarvis_cli.profiles import profile_exists
        ...
        from jarvis_cli.profiles import rename_profile
        ...
        from jarvis_cli.profiles import export_profile
        ...
        from jarvis_cli.profiles import import_profile
        ...
        import tempfile
        from jarvis_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  jarvis -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      jarvis -p {plan.manifest.name} chat")
        ...
        from jarvis_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `jarvis profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"jarvis -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from jarvis_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from jarvis_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _report_dashboard_status() -> int:
    """Print ``jarvis dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``jarvis dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No jarvis dashboard processes running.")
        return 0

    print(f"{len(pids)} jarvis dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No jarvis dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `jarvis update`.
    ...
    # launches the dashboard (`worker dashboard` → JARVIS_HOME points into
    ...
    if (
        ...
        and os.environ.get("JARVIS_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "jarvis_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # JARVIS_HOME.  We must resolve the root explicitly instead of just
        # dropping JARVIS_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV JARVIS_HOME=/opt/data`), so an unset
        # JARVIS_HOME falls back to $HOME/.jarvis = /opt/data/.jarvis — an
        ...
        # update-button guard also misfires).  get_default_jarvis_root()
        # returns the root for both layouts: ~/.jarvis for a standard install
        ...
        try:
            from jarvis_constants import get_default_jarvis_root
            env["JARVIS_HOME"] = str(get_default_jarvis_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop JARVIS_HOME) rather than block the reroute.
    ...
    # the same logs directory as every other Jarvis surface.
    try:
        from jarvis_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "JARVIS_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["JARVIS_WEB_DIST"])
            if "JARVIS_WEB_DIST" in os.environ
            else PROJECT_ROOT / "jarvis_cli" / "web_dist"
        )
    ...
    try:
        from jarvis_cli.mcp_startup import start_background_mcp_discovery
...
def cmd_dashboard_register(args):
    ...
    from jarvis_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from jarvis_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from jarvis_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from jarvis_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Jarvis log files."""
    from jarvis_cli.logs import tail_log, list_logs
...
# so that in ``jarvis -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``jarvis_cli/_parser.py``.
#
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``jarvis`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("JARVIS_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        from jarvis_cli.config import load_config
        from brain.shell_hooks import register_from_config
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["JARVIS_DEFER_AGENT_STARTUP"] = "1"
            os.environ["JARVIS_FAST_STARTUP_BANNER"] = "1"
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `jarvis --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_jarvis_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_jarvis_home()}/memories/\n")
    else:
        from jarvis_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Jarvis as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from jarvis_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from jarvis_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from jarvis_cli.tools_config import tools_command
...
def cmd_insights(args):
    try:
        from jarvis_state import SessionDB
        from brain.insights import InsightsEngine
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from jarvis_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from jarvis_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from jarvis_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from jarvis_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from jarvis_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from jarvis_cli.claw import claw_command
...
def main():
    ...
    # Cosmetic: make the process show up as 'jarvis' instead of 'python3.11'
    ...
    # Self-heal a venv left half-built by an interrupted ``jarvis update``
    ...
    # ``jarvis skills install update``) merely defers recovery one launch;
    # under-matching (missing ``jarvis -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in jarvis_cli/subcommands/model.py)
    # =========================================================================
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `jarvis model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    # migrate command
    # =========================================================================
    from jarvis_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in jarvis_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    try:
        from brain.lsp.cli import register_subparser as _lsp_register
    ...
    # =========================================================================
    # setup command  (parser built in jarvis_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in jarvis_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in jarvis_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `jarvis whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in jarvis_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    # login command  (parser built in jarvis_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in jarvis_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in jarvis_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in jarvis_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in jarvis_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in jarvis_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from jarvis_cli.portal_cli import add_parser as _add_portal_parser
    ...
    # =========================================================================
    # hooks command  (parser built in jarvis_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in jarvis_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in jarvis_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in jarvis_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in jarvis_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in jarvis_cli/subcommands/backup.py)
    # =========================================================================
    ...
    from jarvis_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in jarvis_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in jarvis_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in jarvis_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in jarvis_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from jarvis_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in jarvis_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``jarvis --help``, ``jarvis version``, ``jarvis logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from jarvis_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from jarvis_cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # memory command  (parser built in jarvis_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in jarvis_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `jarvis computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `jarvis tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: jarvis computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: jarvis computer-use install")
            return
    ...
    # =========================================================================
    # mcp command  (parser built in jarvis_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_jarvis_home() / "sessions"
            ...
            # Launch jarvis --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in jarvis_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in jarvis_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in jarvis_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in jarvis_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in jarvis_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in jarvis_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in jarvis_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in jarvis_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Jarvis-Setup.exe success screen tells users to
    # run `jarvis desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in jarvis_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in jarvis_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in jarvis_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``jarvis -c Pokemon Agent Dev`` → ``jarvis -c 'Pokemon Agent Dev'``
    ...
    from jarvis_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'jarvis -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (jarvis hooks list, cron
