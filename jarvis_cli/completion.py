"""Shell completion script generation for jarvis CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Jarvis bash completion
# Add to ~/.bashrc:
...
_jarvis_profiles() {{
    ...
    if [[ -d "$HOME/.jarvis/profiles" ]]; then
        profiles+=($HOME/.jarvis/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'jarvis command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
