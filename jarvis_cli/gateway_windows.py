...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m jarvis_cli.main gateway run --replace`` invocation) and
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Jarvis profile."""
    from jarvis_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "jarvis_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("JARVIS_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("JARVIS_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("JARVIS_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["JARVIS_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["JARVIS_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["JARVIS_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("JARVIS_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("JARVIS_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("JARVIS_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    from jarvis_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\jarvis\\gateway-service\\<task_name>.cmd``
    (or ``<JARVIS_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Jarvis installs stay self-contained).
    """
    _assert_windows()
    from jarvis_cli.config import get_jarvis_home

    script_dir = Path(get_jarvis_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``JARVIS_HOME`` whenever it
    ...
    source checkout only if ``JARVIS_HOME`` cannot be resolved yet.
    """
    from jarvis_cli.config import get_jarvis_home
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports JARVIS_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m jarvis_cli.main [--profile X] gateway run``
    ...
    prog_args = [pythonw_path, "-m", "jarvis_cli.main"]
    if profile_arg:
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Jarvis builds and failed
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m jarvis_cli.main gateway run``
    ...
    log_dir = Path(get_jarvis_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("JARVIS_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("JARVIS_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from jarvis_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `jarvis -p <profile> gateway install` must be safe.
    ...
    from jarvis_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"jarvis {profile_arg} gateway start" if profile_arg else "jarvis gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: jarvis gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Jarvis gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: jarvis gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: jarvis gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from jarvis_cli.config import get_jarvis_home
        print(f"    type {Path(get_jarvis_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_jarvis_home()).resolve()}\\logs\\gateway-stdio.log")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Jarvis gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: jarvis gateway status")
                    return
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_jarvis_home()).resolve()
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: jarvis gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: jarvis gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from jarvis_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
...
def restart() -> None:
    ...
    """
    _assert_windows()
    from jarvis_cli.gateway import kill_gateway_processes
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `jarvis gateway status`."
        )
