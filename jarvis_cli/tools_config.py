"""
Unified tool configuration for Jarvis.

`jarvis tools` and `jarvis setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.jarvis/config.yaml under
...
from jarvis_cli.nous_account import format_nous_portal_entitlement_message
...
from jarvis_cli.cli_output import (  # noqa: E402 — late import block
...
# who want it opt in via `jarvis tools` → Video Generation, which walks
...
# `jarvis tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from jarvis_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `jarvis tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``jarvis tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from jarvis_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``jarvis tools`` checklist actually offers
    for ``platform``.
    ...
    so ``jarvis tools`` never claims to add or remove a toolset the user was
...
from jarvis_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Jarvis routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. JARVIS_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "JARVIS_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "JARVIS_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "JARVIS_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "JARVIS_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "JARVIS_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("JARVIS_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``jarvis update`` and
      by ``jarvis computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `jarvis update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Jarvis process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_jarvis_home()}/jarvis-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in jarvis-agent directory)")
            return
        ...
        # Run the full `jarvis auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.jarvis/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from jarvis_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: jarvis auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `jarvis auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: jarvis auth spotify")
        except Exception as exc:
        ...
        try:
            from jarvis_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: jarvis plugins enable observability/langfuse")
        _print_info("    Restart Jarvis for tracing to take effect.")
        _print_info("    Verify: jarvis plugins list")
        ...
        try:
            ...
            _print_info("    Run later: jarvis auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `jarvis auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: jarvis auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: jarvis auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``jarvis tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``jarvis tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: jarvis tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # ``[jarvis-cli, spotify]`` after enabling Spotify via ``jarvis
        ...
        # (e.g. "jarvis-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``jarvis tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `jarvis tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"jarvis-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("jarvis-")}
    ...
    for ts_key, ts_def in TOOLSETS.items():
        ...
        # by brain/coding_context.py — not per-platform capabilities to recover.
    ...
    # shipped as a bundled plugin but user must opt in via `jarvis tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `jarvis tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by jarvis tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (jarvis-cli, jarvis-telegram, etc.)
    ...
    # Opening `jarvis tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key == "vision":
        try:
            from brain.auxiliary_client import resolve_vision_provider_client
...
def _plugin_web_search_providers() -> list[dict]:
    ...
    """
    try:
        from brain.web_search_registry import list_providers as _list_web_providers
        from jarvis_cli.plugins import _ensure_plugins_discovered
...
def _plugin_browser_providers() -> list[dict]:
    ...
    """
    try:
        from brain.browser_registry import list_providers as _list_browser_providers
        from jarvis_cli.plugins import _ensure_plugins_discovered
...
def _plugin_tts_providers() -> list[dict]:
    ...
    """
    try:
        from brain.tts_registry import _BUILTIN_NAMES, list_providers
        from jarvis_cli.plugins import _ensure_plugins_discovered
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `jarvis tools` gets a silent no-op
...
def _plugin_image_gen_catalog(plugin_name: str):
    ...
    """
    try:
        from brain.image_gen_registry import get_provider
        from jarvis_cli.plugins import _ensure_plugins_discovered
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `jarvis model`).
    if managed_feature:
        from jarvis_cli.nous_subscription import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `jarvis tools` and `jarvis setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Jarvis Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://jarvis-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from jarvis_constants import display_jarvis_home
    print(color(f"  Tool configuration saved to {display_jarvis_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'jarvis' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, jarvis mcp
