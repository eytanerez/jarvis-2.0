"""
Profile management for multiple isolated Jarvis instances.

Each profile is a fully independent JARVIS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.jarvis/profiles/<name>/`` by default.

The "default" profile is ``~/.jarvis`` itself — backward compatible,
zero migration needed.

Usage::

    jarvis profile create coder          # fresh profile + bundled skills
    jarvis profile create coder --clone  # also copy config, .env, SOUL.md, skills
    jarvis profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    jarvis -p coder chat                 # or via flag
    jarvis profile use coder             # set as sticky default
    jarvis profile delete coder          # remove profile + alias + service
"""
...
from brain.skill_utils import is_excluded_skill_path
...
_PROFILE_DIRS = [
    ...
    # See jarvis_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.jarvis``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   jarvis-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `jarvis backup` archives
...
# Marker file written by `jarvis profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `jarvis
...
# `jarvis skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Jarvis infrastructure directories that only the default profile
         (``~/.jarvis``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.jarvis) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "jarvis-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "jarvis", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the jarvis root, NOT to the current JARVIS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where JARVIS_HOME points outside
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Jarvis installation itself or a common system binary.  "
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, jarvis subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            jarvis_exe = shutil.which("jarvis") or "jarvis"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(jarvis_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Jarvis. If we
    ...
    try:
        ...
        # not be migrated. The next `jarvis doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``jarvis -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``jarvis profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``jarvis profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Jarvis config
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, jarvis_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own JARVIS_HOME.
    ...
    The returned ``jarvis_home`` is the path to pass to
    ``set_jarvis_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_jarvis_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``jarvis update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.jarvis)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.jarvis/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `jarvis -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Jarvis profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `jarvis update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`jarvis profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Jarvis (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `jarvis profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `jarvis -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches JARVIS_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``jarvis profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "JARVIS_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_jarvis_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.jarvis).\n"
            "To remove everything, use: jarvis uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``JARVIS_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from jarvis_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set JARVIS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("JARVIS_HOME")
    try:
        os.environ["JARVIS_HOME"] = str(profile_dir)
        from jarvis_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["JARVIS_HOME"] = old_home
        elif "JARVIS_HOME" in os.environ:
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.jarvis/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: jarvis profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from JARVIS_HOME.

    Returns ``"default"`` if JARVIS_HOME is not set or points to ``~/.jarvis``.
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.jarvis itself — its parent is ~/ and its
        # directory name is ".jarvis", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: jarvis profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.jarvis itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.jarvis). "
            "Specify a different name: jarvis profile import <archive> --name <name>"
        )
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"jarvis_{old_name}"
    legacy_old_host = f"jarvis.{old_name}"
    new_host = f"jarvis_{new_name}"
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("jarvis_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a JARVIS_HOME path string.

    Called early in the CLI entry point, before any jarvis modules
    are imported, to set the JARVIS_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
