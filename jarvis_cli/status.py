...
Shows the status of all Jarvis components.
"""
...
from jarvis_cli.auth import AuthError, resolve_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``jarvis config``'s
...
def show_status(args):
    """Show status of all Jarvis components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Jarvis Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: jarvis portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: jarvis model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: jarvis auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: jarvis auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: jarvis model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from jarvis_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from jarvis_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   jarvis gateway")
    ...
    jobs_file = get_jarvis_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_jarvis_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'jarvis doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'jarvis setup' to configure", Colors.DIM))
    print()
