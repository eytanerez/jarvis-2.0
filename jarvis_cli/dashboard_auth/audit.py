...
Profile-aware location: ``$JARVIS_HOME/logs/dashboard-auth.log``.
...
from ``jarvis_constants`` or other jarvis_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$JARVIS_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``jarvis_constants.get_jarvis_home`` semantics: env var wins,
    else ``~/.jarvis``. A local copy avoids an import cycle with the
    middleware which lives below ``jarvis_cli``.
    """
