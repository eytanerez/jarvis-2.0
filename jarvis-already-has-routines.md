# Jarvis Has Had "Routines" Since March
...
Jarvis equivalent — works today:
```bash
jarvis webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Jarvis implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Jarvis |
|---|---|---|
...
## Things Jarvis Does That Routines Can't
...
Jarvis has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Jarvis is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
jarvis gateway setup    # enable webhooks
jarvis webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [jarvis-agent.nousresearch.com/docs/reference/automation-blueprints-catalog](https://jarvis-agent.nousresearch.com/docs/reference/automation-blueprints-catalog)
