"""CLI entry point for the jarvis-agent ACP adapter.

Loads environment variables from ``~/.jarvis/.env``, configures logging
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="jarvis-acp",
        description="Run Jarvis as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Jarvis version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Jarvis provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.jarvis/node/ "
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``jarvis postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Ensure the project root is on sys.path so ``from run_brain import AIBrain`` works
