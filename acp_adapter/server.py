"""ACP agent server — exposes Jarvis via the Agent Client Protocol."""
...
# Thread pool for running AIBrain (synchronous) in parallel.
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Jarvis running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Jarvis cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Jarvis/OpenAI-compatible user content payload."""
...
class JarvisACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Jarvis AIBrain."""
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Jarvis maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from jarvis_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from jarvis_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Jarvis estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_jarvis_session_id: str,
        previous_jarvis_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.jarvis.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_jarvis_session_id,
                previous_jarvis_session_id=previous_jarvis_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_jarvis_session_id: Optional[str] = None,
        previous_jarvis_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Jarvis changes it.

        When the internal Jarvis head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_jarvis_session_id`` so the
        attached ``_meta.jarvis.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        meta = self._provenance_meta(
            session_id,
            current_jarvis_session_id or session_id,
            previous_jarvis_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            from brain.memory_manager import inject_memory_provider_tools

            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["jarvis-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="jarvis-agent", version=JARVIS_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Jarvis' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Jarvis setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Jarvis
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Jarvis on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # still running, queue it instead of racing two AIBrain loops against
        ...
        # ACP thought panes should not receive Jarvis' local kawaii waiting/status
        ...
        # Set it INSIDE _run_brain so the TLS write happens in the executor
        # thread — setting it here would write to the event-loop thread's TLS,
        # not the executor's. Also set JARVIS_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue JARVIS_EXEC_ASK path,
        ...
        def _run_brain() -> dict:
            ...
            # Bind JARVIS_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("JARVIS_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("JARVIS_SESSION_ID")
            os.environ["JARVIS_SESSION_ID"] = session_id
            try:
                ...
                # Restore JARVIS_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("JARVIS_INTERACTIVE", None)
                else:
                    os.environ["JARVIS_INTERACTIVE"] = previous_interactive
                # Restore JARVIS_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("JARVIS_SESSION_ID", None)
                else:
                    os.environ["JARVIS_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Jarvis DB session id before the turn so we
            ...
            pre_turn_jarvis_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (JARVIS_SESSION_KEY in
            # particular — used by the interactive sudo password cache scope).
            ctx = contextvars.copy_context()
            result = await loop.run_in_executor(_executor, ctx.run, _run_brain)
        except Exception:
        ...
        # _meta.jarvis.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_jarvis_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_jarvis_id
            and pre_turn_jarvis_id
            and post_turn_jarvis_id != pre_turn_jarvis_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_jarvis_session_id=post_turn_jarvis_id,
                    previous_jarvis_session_id=pre_turn_jarvis_id,
                )
            except Exception:
                logger.debug(
        ...
        # Jarvis' local "waiting for model response" interrupt status is metadata,
        # not assistant prose — clients get cancellation from stop_reason instead.
        from brain.conversation_loop import INTERRUPT_WAITING_FOR_MODEL_PREFIX
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Jarvis v{JARVIS_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Jarvis has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
