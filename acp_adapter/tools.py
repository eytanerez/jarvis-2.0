"""ACP tool-call helpers for mapping jarvis tools to ACP ToolKind and building content."""
...
# ---------------------------------------------------------------------------
# Map jarvis tool names -> ACP ToolKind
# ---------------------------------------------------------------------------
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a jarvis tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some Jarvis tools append a human hint after a JSON payload, e.g.
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Jarvis tool result clearly failed.
    ...
    # brain/tool_executor.py around the try/except). That prefix is uniquely
    ...
    # Jarvis core/polished tools commonly report tool-level failures as a
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # Jarvis read_file output is line-numbered with `|`. If we send it as raw
...
def _build_tool_complete_content(
    ...
    *,
    ...
    if tool_name == "skill_manage":
        try:
            from brain.display import extract_edit_diff
...
def build_tool_start(
    ...
    *,
    ...
    """Create a ToolCallStart event for the given jarvis tool invocation."""
