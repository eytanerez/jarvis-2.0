"""ACP auth helpers — detect and advertise Jarvis authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Jarvis runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Jarvis can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Jarvis.
    ...
    not have Jarvis provider credentials configured yet, so Jarvis always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Jarvis using the currently configured "
