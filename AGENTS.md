...
Jarvis is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Jarvis ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`jarvis tools`, `jarvis setup`, auto-install) rather than bolting on a raw
  env var.
- **Refactor god-files into clean modules.** Extracting a multi-thousand-line
  cluster out of `cli.py` / `run_brain.py` / `gateway/run.py` into a focused
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `JARVIS_HOME`. Mocks hide integration bugs.
...
- **New `JARVIS_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `jarvis tools`
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `jarvis <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `jarvis webhook`, `jarvis cron`, `jarvis tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.jarvis/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.jarvis/jarvis-brain/venv` (for worktrees that share a venv with the
main checkout).
...
```
jarvis-brain/
├── run_brain.py          # AIBrain class — core conversation loop (~12k LOC)
├── model_tools.py        # Tool orchestration, discover_builtin_tools(), handle_function_call()
├── toolsets.py           # Toolset definitions, _JARVIS_CORE_TOOLS list
├── cli.py                # JarvisCLI class — interactive CLI orchestrator (~11k LOC)
...
├── jarvis_logging.py     # setup_logging() — brain.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── brain/                # Agent internals (provider adapters, memory, caching, compression, etc.)
...
│   ├── jarvis-achievements/  # Gamified achievement tracking
...
**User config:** `~/.jarvis/config.yaml` (settings), `~/.jarvis/.env` (API keys only).
**Logs:** `~/.jarvis/logs/` — `brain.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_jarvis_home()`.
Browse with `jarvis logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Jarvis: desktop, TUI, website, and future TS packages.
...
model_tools.py  (imports tools/registry + triggers tool discovery)
       ↑
run_brain.py, cli.py, batch_runner.py, environments/
```

---

## AIBrain Class (run_brain.py)

The real `AIBrain.__init__` takes ~60 parameters (credentials, routing, callbacks,
session context, budget, credential pool, etc.). The signature below is the
minimum subset you'll usually touch — read `run_brain.py` for the full list.
...
- **KawaiiSpinner** (`brain/display.py`) — animated faces during API calls, `┊` activity feed for tool results
...
- `process_command()` is a method on `JarvisCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `brain/skill_commands.py` scans `~/.jarvis/skills/`, injects as **user message** (not system prompt) to preserve prompt caching
...
- **Slack** — `slack_subcommand_map()` generates `/jarvis` subcommand routing
...
```
2. Add handler in `JarvisCLI.process_command()` in `cli.py`:
```python
...
A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). It does NOT embed `jarvis --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `jarvis-desktop-app-work` skill, not `jarvis-dashboard-work`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Jarvis built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Jarvis core.
Use the plugin route instead: create `~/.jarvis/plugins/<name>/plugin.yaml`
and `~/.jarvis/plugins/<name>/__init__.py`, then register tools with
...
**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_jarvis_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `JARVIS_HOME`.
...
**Agent-level tools** (todo, memory): intercepted by `run_brain.py` before `handle_function_call()`. See `tools/todo_tool.py` for the pattern.
...
`brain/auxiliary_client.py::_resolve_auto` for resolution order.
...
```
jarvis_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.jarvis/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Jarvis gold/kawaii (the current look)
...
Users create `~/.jarvis/skins/<name>.yaml`:
...
Hooks are invoked from `model_tools.py` (pre/post tool) and `run_brain.py`
...
Each provider implements the `MemoryProvider` ABC (see `brain/memory_provider.py`)
and is orchestrated by `brain/memory_manager.py`. Lifecycle hooks include
...
(`run_brain.py`, `cli.py`, `gateway/run.py`, `jarvis_cli/main.py`, etc.).
...
plug into `brain/context_engine.py`; image-gen providers into
`brain/image_gen_provider.py`. Reference / docs-companion plugins
(`example-dashboard`, `strike-freedom-cockpit`, `plugin-llm-example`,
`plugin-llm-async-example`) live in the
[`jarvis-example-plugins`](https://github.com/NousResearch/jarvis-example-plugins)
...
`metadata.jarvis.tags`, `metadata.jarvis.category`,
`metadata.jarvis.related_skills`, `metadata.jarvis.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.jarvis.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Jarvis tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Jarvis" is the secondary collaborator. If the
   contributor's commit shows "Jarvis" as author (because they
   used Jarvis to draft the skill), replace it with their actual name
...
Enable/disable per platform via `jarvis tools` (the curses UI) or the
...
- **Core:** `brain/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `brain/curator_backup.py` (pre-run tar.gz snapshots).
...
collaborate on shared tasks. Users drive it via `jarvis kanban <verb>`;
...
- **Board** is the hard boundary — workers are spawned with
  `JARVIS_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Jarvis Brain ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `JARVIS_BACKGROUND_NOTIFICATIONS` env var):
...
2. **Use `display_jarvis_home()` for user-facing messages.** Import from `jarvis_constants`.
   ...
   print("Config saved to ~/.jarvis/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_jarvis_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_jarvis_home()`,
   not `Path.home() / ".jarvis"`.

4. **Tests that mock `Path.home()` must also set `JARVIS_HOME`** — since code now uses
   `get_jarvis_home()` (reads env var), not `Path.home() / ".jarvis"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"JARVIS_HOME": str(tmp_path / ".jarvis")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not JARVIS_HOME-anchored** — `_get_profiles_root()`
   ...
   This is intentional — it lets `jarvis -p coder profile list` see all profiles regardless
...
scripts/run_tests.sh tests/brain/test_foo.py::test_x  # one test
...
- The plugin disables itself in child processes (sentinel envvar
  `JARVIS_ISOLATE_CHILD=1`), so there's no fork-bomb risk.
...
| HOME / `~/.jarvis/` | Your real config+auth.json | Temp dir per test |
...
```bash
python -m pytest tests/brain/test_foo.py -q --no-isolate
```
