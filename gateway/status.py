...
The PID file lives at ``{JARVIS_HOME}/gateway.pid``.  JARVIS_HOME defaults to
``~/.jarvis`` but can be overridden via the environment variable.  This means
separate JARVIS_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting JARVIS_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("JARVIS_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "jarvis" / _LOCKS_DIRNAME
...
def looks_like_gateway_command_line(command: str | None) -> bool:
    ...
    test also matched ``jarvis_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\jarvis-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Jarvis's
    ...
    for token in tokens:
        ...
        if basename in ("jarvis-gateway", "jarvis-gateway.exe"):
            return True

    joined = " ".join(tokens)
    has_gateway_entry = (
        "jarvis_cli.main" in joined
        or "jarvis_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("jarvis", "jarvis.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return True  # bare `jarvis gateway` defaults to `run`
        return filtered[i + 1] == "run"
    return False


def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Jarvis gateway."""
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `jarvis status` show per-profile
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different JARVIS_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``jarvis.service`` + ``jarvis-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``jarvis gateway stop`` on Windows would be misclassified as an
