...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Jarvis-Session-Id header; opt-in long-term memory scoping via X-Jarvis-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Jarvis-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists jarvis-agent as an available model
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Jarvis sessions
- POST /api/sessions               — create an empty Jarvis session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to jarvis-agent
...
def _jarvis_version() -> str:
    """Return the jarvis-agent version string, or "dev" if it can't be resolved.

    Tries the installed package metadata first (authoritative for a pip/uv
    install), then the in-tree ``jarvis_cli.__version__`` (covers editable /
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_jarvis_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
...
def _derive_chat_session_id(
    ...
    the same Jarvis session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through jarvis-agent's AIBrain.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Active run brain/task references for stop support
        self._active_run_brains: Dict[str, Any] = {}
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Jarvis-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Jarvis-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Jarvis-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Jarvis-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Jarvis-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``jarvis sessions list``
    ...
    def _create_agent(
        ...
        """
        Create an AIBrain instance using the gateway's runtime config.

        Uses _resolve_runtime_agent_kwargs() to pick up model, api_key,
        base_url, etc. from config.yaml / env vars.  Toolsets are resolved
        from config.yaml platform_toolsets.api_server (same as all other
        gateway platforms), falling back to the jarvis-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Jarvis-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "jarvis-agent", "version": _jarvis_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return jarvis-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Jarvis version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Jarvis AIBrain; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Jarvis-Session-Id",
                "session_key_header": "X-Jarvis-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Jarvis sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty Jarvis session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({"object": "jarvis.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "jarvis.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "jarvis.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "jarvis.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Jarvis-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Jarvis-Session-Key"] = gateway_session_key
        return web.json_response(
            {
                "object": "jarvis.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        """POST /api/sessions/{session_id}/chat/stream — SSE wrapper over _run_brain."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        gateway_session_key, key_err = self._parse_session_key_header(request)
        ...
        headers = {
            ...
            "X-Jarvis-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Jarvis-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Jarvis-Session-Key.  This
        # is independent of X-Jarvis-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Jarvis-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Jarvis-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Jarvis-Session-Id rejected: "
            ...
            # conversation map to the same Jarvis session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``jarvis.tool.progress`` with ``status: running``.
                ...
                from brain.display import build_tool_preview, get_tool_emoji
            ...
            # it would duplicate every emit because ``run_brain`` fires it
            # side-by-side with ``tool_start_callback``/``tool_complete_callback``.
            # The structured callbacks are strictly richer (they carry the
            # tool_call id), so they own the chat-completions SSE channel.
            agent_ref = [None]
            agent_task = asyncio.ensure_future(self._run_brain(
                user_message=user_message,
                conversation_history=history,
                ephemeral_system_prompt=system_prompt,
                session_id=session_id,
        ...
        response_headers = {
            "X-Jarvis-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Jarvis-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["jarvis"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-Jarvis-Completed"] = "false"
            response_headers["X-Jarvis-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Jarvis-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Jarvis-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Jarvis-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Jarvis-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Jarvis-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: jarvis.tool.progress`` SSE event so
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # Build output items from the current turn only.  AIBrain returns a
        ...
        response_headers = {"X-Jarvis-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Jarvis-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_brain(
        ...
        If *agent_ref* is a one-element list, the AIBrain instance is stored
        ...
        def _run():
            ...
            try:
                ...
                # (e.g. X-Jarvis-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        async def _run_and_close():
            try:
                ...
                self._active_run_brains[run_id] = agent
                ...
                try:
                    ...
                    pass
                self._active_run_brains.pop(run_id, None)
        ...
        response_headers = (
            {"X-Jarvis-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "jarvis.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def _handle_stop_run(self, request: "web.Request") -> "web.Response":
        ...
        run_id = request.match_info["run_id"]
        agent = self._active_run_brains.get(run_id)
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Session/client control surface (thin wrappers over SessionDB + _run_brain)
            ...
            # Store the adapter after native routes are registered. Local Jarvis-Relay
