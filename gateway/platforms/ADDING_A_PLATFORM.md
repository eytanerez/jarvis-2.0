...
There are two ways to add a platform to the Jarvis gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.jarvis/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Jarvis code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `jarvis gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `jarvis_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Jarvis core.
...
## 6. System Prompt Hints (`brain/prompt_builder.py`)
...
And add it to the `jarvis-gateway` composite:

```python
"jarvis-gateway": {
    "includes": [..., "jarvis-your-platform"]
}
```
...
## 12. Status Display (`jarvis_cli/status.py`)
...
## 13. Gateway Setup Wizard (`jarvis_cli/gateway.py`)
...
## 14. Phone/ID Redaction (`brain/redact.py`)

If your platform uses sensitive identifiers (phone numbers, etc.), add a
regex pattern and redaction function to `brain/redact.py`. This ensures
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ brain/ cron/ jarvis_cli/ toolsets.py \
  --include="*.py" -l | sort -u
