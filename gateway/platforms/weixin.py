...
Connects Jarvis to WeChat personal accounts via Tencent's iLink Bot API.
...
def _account_dir(jarvis_home: str) -> Path:
    path = Path(jarvis_home) / "weixin" / "accounts"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _account_file(jarvis_home: str, account_id: str) -> Path:
    return _account_dir(jarvis_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(jarvis_home, account_id)
...
def load_weixin_account(jarvis_home: str, account_id: str) -> Optional[Dict[str, Any]]:
...
class ContextTokenStore:
    ...
    def __init__(self, jarvis_home: str):
        self._root = _account_dir(jarvis_home)
...
def _sync_buf_path(jarvis_home: str, account_id: str) -> Path:
    return _account_dir(jarvis_home) / f"{account_id}.sync.json"


def _load_sync_buf(jarvis_home: str, account_id: str) -> str:
    path = _sync_buf_path(jarvis_home, account_id)
    if not path.exists():
        return ""
    try:
...
def _save_sync_buf(jarvis_home: str, account_id: str, sync_buf: str) -> None:
    path = _sync_buf_path(jarvis_home, account_id)
...
class WeixinAdapter(BasePlatformAdapter):
    """Native Jarvis adapter for Weixin personal accounts."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._token_store = ContextTokenStore(jarvis_home)
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(jarvis_home, self._account_id)
            if persisted:
    ...
    async def connect(self) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "these accounts, so group messages may never reach Jarvis regardless of this "
    ...
    async def send(
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                client_id = f"jarvis-weixin-{uuid.uuid4().hex}"
    ...
    async def _send_file(
        ...
        if caption:
            last_message_id = f"jarvis-weixin-{uuid.uuid4().hex}"
            await _send_message(
                self._send_session,
                base_url=self._base_url,
                token=self._token,
