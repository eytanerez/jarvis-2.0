...
# Inbound media cache lives under the user's jarvis dir so it survives
...
def check_whatsapp_cloud_requirements() -> bool:
    ...
    for Graph API calls (outbound). Both ship with jarvis-agent's default
...
class WhatsAppCloudAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        a generic audio attachment. Jarvis TTS produces MP3, so we try
    ...
    async def _build_message_event_from_cloud(
        ...
        """Convert a Cloud-API message object into a Jarvis MessageEvent.
