...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Jarvis does not currently need to route them into the agent loop.
            ...
            # /model work directly without /jarvis prefix). A single regex
            ...
            # manifest (see `jarvis slack manifest`). In Socket Mode, Slack
            ...
            from jarvis_cli.commands import slack_native_slashes
            import re as _re
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Jarvis handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Jarvis session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                ...
                # forms like ``!stop@jarvis`` still resolve.
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/jarvis <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/jarvis
        ...
        if slash_name in {"jarvis", ""}:
            # Legacy /jarvis <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from jarvis_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/jarvis   ``).
