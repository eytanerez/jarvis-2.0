...
JARVIS_CFG="${JARVIS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Jarvis config has twozero_td MCP entry ──
if [[ ! -f "$JARVIS_CFG" ]]; then
    echo -e " ${FAIL} Jarvis config not found at ${JARVIS_CFG}"
    manual_steps+=("Create ${JARVIS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$JARVIS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Jarvis config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Jarvis config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${JARVIS_CFG} manually"); }
    manual_steps+=("Restart Jarvis session to pick up config change")
fi
