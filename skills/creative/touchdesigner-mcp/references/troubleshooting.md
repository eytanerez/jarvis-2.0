...
`$JARVIS_HOME/config.yaml` (defaults to `~/.jarvis/config.yaml` when `JARVIS_HOME` is unset)
...
Restart the Jarvis session for changes to take effect. The MCP connection is
