#!/usr/bin/env bash
# GitHub environment detection helper for Jarvis skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _jarvis_env="${JARVIS_HOME:-$HOME/.jarvis}/.env"; [ -f "$_jarvis_env" ] && grep -q "^GITHUB_TOKEN=" "$_jarvis_env" 2>/dev/null; then
