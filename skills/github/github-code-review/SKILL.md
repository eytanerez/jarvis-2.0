...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _jarvis_env="${JARVIS_HOME:-$HOME/.jarvis}/.env"; [ -f "$_jarvis_env" ] && grep -q "^GITHUB_TOKEN=" "$_jarvis_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_jarvis_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
