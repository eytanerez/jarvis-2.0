#!/usr/bin/env python3
"""Google Workspace API CLI for Jarvis.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Jarvis-facing JSON contract and falls back to the Python client
...
from _jarvis_home import get_jarvis_home
...
def _gws_binary() -> str | None:
    override = os.getenv("JARVIS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Jarvis")
