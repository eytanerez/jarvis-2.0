...
declare global {
  interface Window {
    ...
    __JARVIS_AUTH_REQUIRED__?: boolean;
  }
}
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``jarvis update``, ``jarvis gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__JARVIS_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__JARVIS_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("jarvis.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("jarvis.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__JARVIS_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("jarvis.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
 ...
 *
 * The dashboard's gated-mode WS auth (``jarvis_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__JARVIS_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Jarvis-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``jarvis_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__JARVIS_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  checkJarvisUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/jarvis/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface DebugShareResponse {
  ok: boolean;
  // label -> paste URL, e.g. { Report: "https://paste.rs/abc", "brain.log": "..." }
...
export interface SkillHubSource {
  ...
  /** jarvis-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``jarvis update``. */
  can_update_jarvis?: boolean;
  config_path: string;
