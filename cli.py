...
A beautiful command-line interface for the Jarvis, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills jarvis-agent-dev,github-auth
...
os.environ["JARVIS_QUIET"] = "1"  # Our own modules

import yaml

from jarvis_cli.fallback_config import get_fallback_chain
from jarvis_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from jarvis_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from jarvis_cli.pt_input_extras import (
...
def CanonicalUsage(*args, **kwargs):
    from brain.usage_pricing import CanonicalUsage as _CanonicalUsage

    return _CanonicalUsage(*args, **kwargs)


def estimate_usage_cost(*args, **kwargs):
    from brain.usage_pricing import estimate_usage_cost as _estimate_usage_cost
...
def is_table_divider(*args, **kwargs):
    from brain.markdown_tables import is_table_divider as _is_table_divider

    return _is_table_divider(*args, **kwargs)


def looks_like_table_row(*args, **kwargs):
    from brain.markdown_tables import looks_like_table_row as _looks_like_table_row

    return _looks_like_table_row(*args, **kwargs)


def realign_markdown_tables(*args, **kwargs):
    from brain.markdown_tables import realign_markdown_tables as _realign_markdown_tables

    return _realign_markdown_tables(*args, **kwargs)
# NOTE: `from brain.account_usage import ...` is deliberately NOT at module
# top — it transitively pulls the OpenAI SDK chain (~230 ms cold) and is only
# needed when the user runs `/limits`. Lazy-imported inside the handler below.
from jarvis_cli.banner import _format_context_length, format_banner_version_label
...
def _strip_reasoning_tags(text: str) -> str:
    ...
    ``run_brain.py::_strip_think_blocks`` and the stream consumer's
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.jarvis/.
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("JARVIS_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.jarvis/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If JARVIS_IGNORE_USER_CONFIG=1 is set (via ``jarvis chat --ignore-user-config``),
    the user config at ``~/.jarvis/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({JARVIS_HOME}/config.yaml)
    user_config_path = _jarvis_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("JARVIS_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Jarvis, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Jarvis - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see jarvis_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
        ...
        "onboarding": {
            # First-touch hint flags (see brain/onboarding.py).  Each hint is
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from jarvis_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and JarvisCLI.__init__ checks "default" first.
    ...
    from jarvis_cli.config import _expand_env_vars
    ...
    # jarvis_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `jarvis config`/`doctor`/guards (which use load_config) honor it. The
    ...
    # Normalize config key: the new config system (jarvis_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && jarvis` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _JARVIS_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_JARVIS_GATEWAY") == "1"
    ...
    # Compression config is read directly from config.yaml by run_brain.py and
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["JARVIS_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — brain.log + errors.log in ~/.jarvis/logs/.
# This ensures CLI sessions produce a log trail even before AIBrain is instantiated.
try:
...
try:
    from jarvis_cli.skin_engine import init_skin_from_config
...
try:
    from brain.display import set_tool_preview_max_len
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``jarvis --help`` paths inside the chat command flow).  See
...
# prompt; the full brain/tool registry is initialized on first use.
def AIBrain(*args, **kwargs):
    from run_brain import AIBrain as _AIBrain

    return _AIBrain(*args, **kwargs)


def get_tool_definitions(*args, **kwargs):
    from jarvis_cli.mcp_startup import wait_for_mcp_discovery
...
from jarvis_cli.banner import build_welcome_banner
from jarvis_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from jarvis_cli.callbacks import prompt_for_secret
...
# Weak reference to the active AIBrain for memory provider shutdown at exit
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("JARVIS_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("JARVIS_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _run_cleanup(*, notify_session_finalize: bool = True):
    ...
    try:
        from brain.auxiliary_client import shutdown_cached_clients
    ...
    try:
        if _active_agent_ref and hasattr(_active_agent_ref, 'shutdown_memory_provider'):
            ...
            # ``AIBrain.__init__`` and refreshed every turn via
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run jarvis -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"jarvis-{short_id}"
    branch_name = f"jarvis/{wt_name}"
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"jarvis pid={os.getpid()}", str(wt_path)],
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`jarvis_cli.config.load_config` (the authoritative loader that
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_jarvis_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`jarvis_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``jarvis/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("jarvis-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``jarvis/jarvis-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``jarvis -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("jarvis/jarvis-") or b.startswith("pr-"))
    ]
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from jarvis_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_jarvis_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._jarvis_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from jarvis_cli.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Jarvis output it's common to display cron schedules verbatim.
    #
...
def _resolve_attachment_path(raw_path: str) -> Path | None:
    ...
    # is silently lost (the warning ends up in brain.log but the user sees
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_jarvis_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_jarvis_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._jarvis_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_jarvis_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._jarvis_bp_start = now
        ...
        _vt100_mod._jarvis_bp_timeout_patched = True
...
# ASCII Art - JARVIS-AGENT logo (full width, single line - requires ~95 char terminal)
JARVIS_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - Jarvis Caduceus (compact, fits in left panel)
JARVIS_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if os.environ.get("JARVIS_FAST_STARTUP_BANNER") == "1":
        from jarvis_cli import __release_date__ as _release_date
        from jarvis_cli import __version__ as _version

        version_line = f"Jarvis v{_version} ({_release_date})"
    else:
...
def _ensure_skill_commands() -> dict:
    ...
    if _skill_commands is None:
        from brain.skill_commands import scan_skill_commands
...
def build_skill_invocation_message(*args, **kwargs):
    from brain.skill_commands import build_skill_invocation_message as _impl

    return _impl(*args, **kwargs)


def build_preloaded_skills_prompt(*args, **kwargs):
    from brain.skill_commands import build_preloaded_skills_prompt as _impl
...
def get_skill_bundles() -> dict:
    ...
    if _skill_bundles is None:
        from brain.skill_bundles import get_skill_bundles as _impl
...
def build_bundle_invocation_message(*args, **kwargs):
    from brain.skill_bundles import build_bundle_invocation_message as _impl
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from jarvis_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.jarvis/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    
    Args:
        key_path: Dot-separated path like "brain.system_prompt"
        value: Value to save
    ...
    try:
        # Ensure parent directory exists (for ~/.jarvis/config.yaml on first use)
...
class JarvisCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Jarvis.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: JARVIS_MAX_TOKENS)
        _env_mt = os.environ.get("JARVIS_MAX_TOKENS")
        if _env_mt:
            try:
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from jarvis_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("JARVIS_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("JARVIS_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("JARVIS_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `jarvis chat --ignore-rules` in jarvis_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIBrain so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("JARVIS_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("JARVIS_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `jarvis update` to rebuild the venv) to restore persistence."
                )
            except Exception:
        ...
        # it's shared across all Jarvis processes for this JARVIS_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.jarvis/checkpoints/.  Opt-in via
        ...
        self._history_file = _jarvis_home / ".jarvis_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from jarvis_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Jarvis'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from jarvis_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from jarvis_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from jarvis_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from jarvis_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - JarvisCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``jarvis doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``jarvis doctor`` so the banner stays
        small.
        """
        try:
            from jarvis_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        from brain.model_metadata import MINIMUM_CONTEXT_LENGTH
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Jarvis needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        from jarvis_cli.model_switch import is_nous_jarvis_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_jarvis_non_agentic(model_name):
            self._console_print()
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.jarvis/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from jarvis_cli.clipboard import save_clipboard_image

        img_dir = get_jarvis_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'jarvis setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        try:
            from jarvis_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Jarvis!{_RST}")
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from jarvis_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from jarvis_cli.main import _relative_time
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``jarvis sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `jarvis sessions list` (gemini-cli#27770 port).
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("JARVIS_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.jarvis/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``jarvis --resume <id>``
        ...
        saved_dir = get_jarvis_home() / "sessions" / "saved"
        try:
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: jarvis --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from jarvis_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        if not getattr(result, "success", False):
            return True
        try:
            from jarvis_cli.model_cost_guard import expensive_model_warning
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Jarvis session."),
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `jarvis model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from jarvis_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from jarvis_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from jarvis_cli.model_switch import (
            switch_model,
            parse_model_flags,
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_jarvis_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_jarvis_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in jarvis_cli/commands.py instead of touching every dispatch site.
        from jarvis_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from jarvis_cli.main import _print_version_info
            ...
            from jarvis_cli.config import reload_env
            count = reload_env()
            ...
            try:
                # Discover from disk (bundled + user), matching `jarvis plugins
                ...
                from jarvis_cli.plugins_cmd import (
                ...
                # catalog lives behind `jarvis plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: jarvis plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_jarvis_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: jarvis plugins list)")
                else:
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: jarvis plugins list)")
                    print("  Enable/disable: jarvis plugins enable/disable <name>")
            except Exception as e:
                ...
                if qcmd.get("type") == "exec":
                    ...
                    if exec_cmd:
                        try:
                            ...
                            # shell snippets from config.yaml — not brain/LLM controlled.
                            result = subprocess.run(
    ...
    def _toggle_verbose(self):
        ...
        from jarvis_cli.colors import Colors as _Colors
        labels = {
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``JARVIS_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``JARVIS_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from jarvis_cli.partial_compress import (
        ...
        with self._busy_command("Compressing context..."):
            try:
                from brain.model_metadata import estimate_request_tokens_rough
                from brain.manual_compression_feedback import summarize_manual_compression
                ...
                # session on the agent (run_brain.py::_compress_context). Sync the
    ...
    def _show_usage(self):
        ...
        if rl_state and rl_state.has_data:
            from brain.rate_limit_tracker import format_rate_limit_display
            print()
        ...
        from brain.account_usage import fetch_account_usage, render_account_usage_lines
        ...
        if self.verbose:
            ...
            # tools/run_brain/etc. in quiet mode. Setting logger.setLevel
            # above the file handler level filters records before they
            # reach handlers, so brain.log / errors.log lose visibility
            # into stream-retry events, credential rotations, etc.
            # Console quietness is enforced by jarvis_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors JARVIS_DEV_CREDITS_FIXTURE
        for offline testing — same behavior as every other surface.
        """
        from brain.account_usage import nous_credits_lines
    ...
    def _show_credits(self):
        ...
        """
        from brain.account_usage import build_credits_view
        ...
        if not view.logged_in:
            ...
            print("  Run `jarvis portal` to log in, then /credits.")
            return
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        """
        from brain.billing_view import build_billing_state
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `jarvis portal` to log in, then /billing.")
            return
    ...
    def _billing_overview(self, state):
        ...
        from brain.billing_view import format_money
    ...
    def _billing_buy_flow(self, state):
        ...
        from brain.billing_view import format_money, validate_charge_amount
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        from brain.billing_view import format_money, new_idempotency_key
        ...
        from jarvis_cli.nous_billing import (
            BillingError,
    ...
    def _billing_poll_charge(self, state, charge_id, amount):
        ...
        while _time.time() < deadline:
            ...
            if state_str == "settled":
                ...
                from brain.billing_view import parse_money
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from jarvis_cli.nous_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `jarvis portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
        ...
        if choice != "yes":
            ...
            return
        try:
            from jarvis_cli.auth import step_up_nous_billing_scope
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from jarvis_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.jarvis/skills/ and queue a note for the
        next user turn.
        ...
        """
        try:
            from brain.skill_commands import reload_skills, get_skill_commands
    ...
    def _on_tool_progress(self, event_type: str, function_name: str = None, preview: str = None, function_args: dict = None, **kwargs):
        ...
        if event_type == "tool.completed":
            ...
            if function_name and self.tool_progress_mode in {"all", "new"}:
                ...
                try:
                    from brain.display import get_cute_tool_message
        ...
        if function_name and not function_name.startswith("_"):
            ...
            from brain.display import get_tool_preview_max_len
    ...
    def _on_tool_complete(self, tool_call_id: str, function_name: str, function_args: dict, function_result: str):
        ...
        try:
            from brain.display import render_edit_diff_with_delta
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        # See brain/image_routing.py for the decision table.
        if images:
            try:
                from brain.image_routing import (
        ...
        if isinstance(message, str) and "@" in message:
            try:
                from brain.context_references import preprocess_context_references
        ...
        if isinstance(message, str):
            from run_brain import _sanitize_surrogates
        ...
        try:
            ...
            agent_thread = threading.Thread(target=run_brain, daemon=True)
            agent_thread.start()
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _jarvis_home / "interrupt_debug.log"
            ...
            try:
                from brain.auxiliary_client import cleanup_stale_async_clients
            ...
            if response and result and not result.get("failed") and not result.get("partial"):
                try:
                    from brain.title_generator import maybe_auto_title
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard JARVIS_HOME, so no -p needed.
            try:
            ...
            print(f"  jarvis --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  jarvis -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from jarvis_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from jarvis_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from jarvis_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Jarvis! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Jarvis! Type your message or /help for commands."
        ...
        try:
            from jarvis_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            _redact_raw = os.getenv("JARVIS_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(JARVIS_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Jarvis migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from jarvis_cli.config import get_config_path as _get_cfg_path_resid
        ...
        try:
            from brain.curator import maybe_run_curator
            maybe_run_curator(
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in jarvis_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from jarvis_cli.model_switch import resolve_persist_behavior
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Jarvis binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from jarvis_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Jarvis")
        ...
        # Dynamic prompt: shows Jarvis symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Jarvis needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Jarvis needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Jarvis needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = JarvisCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_jarvis_osd_patched", False):
                ...
                _pt_renderer._jarvis_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``JARVIS_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("JARVIS_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the jarvis process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: jarvis setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `jarvis sessions list`
...
def _run_kanban_goal_loop_q(cli: "JarvisCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``JARVIS_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("JARVIS_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from jarvis_cli import kanban_db as _kb
    from jarvis_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Jarvis CLI - Interactive AI Assistant
    ...
    os.environ["JARVIS_INTERACTIVE"] = "1"
    ...
    # Default to jarvis-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Jarvis): with no explicit --toolsets, collapse
        # to the coding toolset (+ enabled MCP servers) when sitting in a code
        # workspace. See brain/coding_context.py.
        _coding = None
        try:
            from brain.coding_context import coding_selection
    ...
    # mode registers its own inside JarvisCLI.run(), but `-q` runs
    # cli.agent.run_conversation() below and AIBrain spawns worker threads
    ...
    # so main unwinds normally.  JARVIS_SIGTERM_GRACE overrides the 1.5 s
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``jarvis chat -q "work kanban task <id>"``;
            ...
            _kanban_task_id = os.environ.get("JARVIS_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
                    from jarvis_cli import kanban_db as _kb
                    from brain.image_routing import extract_image_refs as _extract_refs
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if single_query_images or single_query_image_urls:
                        ...
                        try:
                            ...
                            from brain.image_routing import decide_image_input_mode
                            from jarvis_cli.config import load_config
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Jarvis" box, no tool-gen
                        ...
                        if os.environ.get("JARVIS_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("JARVIS_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from jarvis_cli.kanban_db import (
                ...
                # Single-query mode (`jarvis chat -q "…"`): skip the welcome
