...
load_jarvis_dotenv(
    jarvis_home=_jarvis_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.jarvis/logs/tui_gateway_crash.log
...
try:
    _slash_timeout = float(os.environ.get("JARVIS_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("JARVIS_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("JARVIS_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent JarvisCLI subprocess for slash commands."""
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    # Use session_id (from brain.session_id) not session_key — after compression,
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `jarvis --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("JARVIS_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `jarvis --tui`, so it must NOT count as dead here (doing
...
# ``profile`` on those calls; we open that profile's db and bind its JARVIS_HOME
...
def _status_update(sid: str, kind: str, text: str | None = None):
    ...
    if out_kind == "lifecycle":
        from brain.conversation_compression import COMPACTION_STATUS_MARKER
...
def _start_agent_build(sid: str, session: dict) -> None:
    """Start building the real AIBrain for a TUI session, once.

    Classic `jarvis` shows the prompt before constructing AIBrain; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # JARVIS_HOME so config/skills/model resolve to it, and hand the
            ...
            try:
                from brain.credits_tracker import seed_credits_at_session_start
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    Uses INSERT OR IGNORE under the hood, so re-calls (and the AIBrain's own
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _jarvis_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_jarvis_home_override()
        home = override if isinstance(override, str) and override else _jarvis_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    jarvis_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["JARVIS_GATEWAY_SESSION"] = "1"
...
def _resolve_model() -> str:
    env = (
        ...
        or os.environ.get("JARVIS_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _config_model_target() -> tuple[str, str]:
    ...
    config.yaml wins over JARVIS_MODEL / JARVIS_INFERENCE_MODEL here, the
    reverse of `_resolve_model()`'s startup order. Those env vars are a
    provision-time seed (hosted instances set JARVIS_INFERENCE_MODEL in the
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    model = _resolve_model()
    explicit_provider = os.environ.get("JARVIS_TUI_PROVIDER", "").strip()
    ...
    try:
        ...
        current_provider = (
            ...
            or os.environ.get("JARVIS_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("JARVIS_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("JARVIS_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Jarvis): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `jarvis --tui` both land here. See
    # brain/coding_context.py. No config is loaded yet at this point, so we let
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] JARVIS_TUI_TOOLSETS=all enables every toolset; "
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown JARVIS_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in JARVIS_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid JARVIS_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid JARVIS_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    #
    # We deliberately do NOT write process-global env vars (JARVIS_MODEL /
    # JARVIS_INFERENCE_MODEL / JARVIS_TUI_PROVIDER / JARVIS_INFERENCE_PROVIDER)
...
def _compress_session_history(
    ...
    # Pass system_message=None so AIBrain._compress_context rebuilds the
...
def _sync_session_key_after_compress(
    ...
    *,
    ...
    """Re-anchor session_key when AIBrain._compress_context rotates session_id.

    AIBrain._compress_context ends the current SessionDB session and creates
...
def _get_usage(agent) -> dict:
    ...
    try:
        from brain.usage_pricing import CanonicalUsage, estimate_usage_cost
    ...
    # JARVIS_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("JARVIS_DEV_CREDITS")):
        try:
...
def _tool_ctx(name: str, args: dict) -> str:
    try:
        from brain.display import build_tool_preview
...
def _tool_result_text(result: object) -> str:
    try:
        from brain.tool_dispatch_helpers import _multimodal_text_summary
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from jarvis_cli.config import load_config as _load_full_cfg
...
def _prompt_text(value) -> str:
    """Normalize config prompt values from YAML before handing them to AIBrain."""
    if value is None:
        return ""
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("JARVIS_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("JARVIS_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``JarvisCLI.__init__`` and ``gateway/run.py``:
...
def _schedule_mcp_late_refresh(sid: str, agent) -> None:
    ...
    the registry (run_brain/agent_init). ``_make_agent`` briefly joins the
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses jarvis_cli.mcp_startup; TUI stdio keeps
    ...
    if startup_skills:
        from brain.skill_commands import build_preloaded_skills_prompt
    ...
    return AIBrain(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("JARVIS_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("JARVIS_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("JARVIS_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("JARVIS_IGNORE_RULES")),
...
def _init_session(
    ...
    try:
        ...
        # Bare AIBrains that don't expose the attribute (unlikely, but keep
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind JARVIS_HOME. None/own profile → launch (unchanged).
    ...
    # and the AIBrain's own INSERT-OR-IGNORE persists it on the first turn too.

    # Return the lightweight session immediately so Ink can paint the composer
    # + skeleton panel, then build the real AIBrain just after this response is
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `JARVIS_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    # (MCP discovery, prompt/skill build, AIBrain construction). Holding
    ...
    _enable_gateway_prompts()
    home_token = (
        set_jarvis_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_jarvis_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_jarvis_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds JARVIS_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # is dead — its brain/worker are being released and it is no longer
    ...
    # ``jarvis --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``jarvis gateway`` process, whose
...
# (brain/billing_view.py + jarvis_cli/nous_billing.py) — same as /credits.
...
def _(rid, params: dict) -> dict:
    ...
    """
    from jarvis_cli.nous_billing import BillingError, post_charge
    from brain.billing_view import new_idempotency_key
...
def _(rid, params: dict) -> dict:
    ...
    """
    from jarvis_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from jarvis_cli.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Jarvis profile home
    # (~/.jarvis/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $JARVIS_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _(rid, params: dict) -> dict:
    ...
    Mirrors AIBrain.steer(). Safe to call while a turn is running — the text
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn JARVIS_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_jarvis_home_override(_profile_home_str)
            ...
            # See brain/image_routing.py for the full decision table.
            ...
            if images:
                try:
                    ...
                    from jarvis_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: jarvis_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from jarvis_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _jarvis_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".jarvis" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.jarvis/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.jarvis/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Jarvis Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Jarvis/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Jarvis Desktop. If it serves Jarvis/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from jarvis_cli.model_switch import parse_model_flags
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("JARVIS_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["JARVIS_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("JARVIS_YOLO_MODE", None)
                    nv = "0"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from jarvis_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_jarvis_home), "display": display_jarvis_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from jarvis_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from jarvis_cli.runtime_provider import resolve_runtime_provider
        from jarvis_cli.auth import has_usable_secret
        from jarvis_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Jarvis provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.jarvis/.env`` into the gateway process via
    ``jarvis_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `jarvis` is interactive — use `/jarvis chat -q …` or run `jarvis` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`jarvis setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`jarvis gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`jarvis sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`jarvis config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m jarvis_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "jarvis_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from brain.skill_commands import get_skill_commands
        from brain.skill_bundles import get_skill_bundles
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from jarvis_cli.config import is_managed, save_env_value
        from jarvis_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `jarvis model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.jarvis/.env
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from jarvis_cli.config import remove_env_value
...
def _mirror_slash_side_effects(sid: str, session: dict, command: str) -> str:
    ...
    # all do read-then-mutate on live brain/session state that the
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action == "tts":
        ...
        os.environ["JARVIS_VOICE_TTS"] = "1" if new_value else "0"
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from jarvis_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from jarvis_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from jarvis_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("JARVIS_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("JARVIS_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_jarvis_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from jarvis_cli.skills_hub import do_install
        ...
        if action == "browse":
            from jarvis_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from jarvis_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    try:
        from brain.skill_commands import reload_skills
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``jarvis plugins`` / the dashboard, so the three surfaces
