...
When running Jarvis inside Docker, the default `network_mode: host` gives the
...
The Jarvis [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   JARVIS_UID=$(id -u) JARVIS_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=jarvis,jarvis-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://jarvis-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Jarvis trust model and vulnerability reporting
