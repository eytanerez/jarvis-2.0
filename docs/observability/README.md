# Jarvis Observer Hooks

Jarvis observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Jarvis' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Jarvis catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Jarvis only builds
