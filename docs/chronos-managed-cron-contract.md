...
Chronos lets a hosted Jarvis gateway **scale to zero** while idle and still
...
- **Auth:** `Authorization: Bearer <agent Nous access token>`. NAS validates via
  its normal agent-token path and scopes the row to the calling brain/org.
- **Request body:**
  ```json
  {
    "job_id": "ab12cd34",
...
**dashboard app** (`jarvis_cli/web_server.py`) — the agent's always-reachable
...
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `JARVIS_HOME` — exactly one replica runs each
  fire.
