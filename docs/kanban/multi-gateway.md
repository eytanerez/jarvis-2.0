...
Jarvis supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.jarvis/config.yaml`:
...
Or set the env var: `JARVIS_KANBAN_DISPATCH_IN_GATEWAY=false`
