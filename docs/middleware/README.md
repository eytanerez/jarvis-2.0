...
Jarvis middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Jarvis'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Jarvis calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Jarvis retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Jarvis guardrails,
...
- `telemetry_schema_version`: currently `jarvis.observer.v1`
- `middleware_schema_version`: currently `jarvis.middleware.v1`
...
Jarvis stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Jarvis runs
them as a nested chain in registration order. Middleware failures are fail-open:
Jarvis logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Jarvis applies middleware in this order:
...
For each tool call, Jarvis applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Jarvis pre-execution path against the effective arguments:
...
```bash
jarvis plugins enable <plugin-name>
```

For isolated local testing, use one `JARVIS_HOME` for plugin enablement and the
agent run:

```bash
export JARVIS_HOME=/tmp/jarvis-middleware-test
mkdir -p "$JARVIS_HOME"
jarvis plugins enable <plugin-name>
jarvis chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run jarvis plugins enable <plugin-name>
uv run jarvis chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["jarvis_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/jarvis-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Jarvis expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Jarvis treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Jarvis preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Jarvis does not convert downstream
