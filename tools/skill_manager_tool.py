...
~/.jarvis/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `jarvis config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.jarvis/skills/ (single source of truth)
...
def _containing_skills_root(skill_path: Path) -> Path:
    ...
    ``_find_skill`` first).
    """
    from brain.skill_utils import get_all_skills_dirs
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`jarvis curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.jarvis/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Jarvis profiles.
    ...
    # Default profile (~/.jarvis/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.jarvis/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`jarvis -p "
            ...
            base += (
                ...
                f"Switch profiles (`jarvis -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_jarvis_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `jarvis curator unpin <name>`. "
