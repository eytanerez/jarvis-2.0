...
_FORCE_SYNC_ENV = "JARVIS_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.jarvis") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.jarvis to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``JARVIS_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, jarvis_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.jarvis/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="jarvis-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.jarvis/skills/a.md`` →
        ``~/.jarvis/skills/a.md``, a new remote file at
        ``/root/.jarvis/skills/b.md`` maps to ``~/.jarvis/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
