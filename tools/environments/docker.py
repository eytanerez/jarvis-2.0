...
def _get_active_profile_name() -> str:
    """Return the active Jarvis profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale jarvis-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=jarvis-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Jarvis process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=jarvis-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a jarvis process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Jarvis
    process leaves its container behind permanently because there's no
    subsequent Jarvis process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=jarvis-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=jarvis-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``JARVIS_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("JARVIS_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using JARVIS_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'jarvis'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``jarvis-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.jarvis/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. jarvis-agent:latest) already use /init as PID 1
        ...
        container_name = f"jarvis-{uuid.uuid4().hex[:8]}"
        # Labels make jarvis-created containers identifiable to:
        #   * the orphan reaper (`jarvis-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`jarvis-task-id`, `jarvis-profile`)
        #   * operators running `docker ps --filter label=jarvis-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Jarvis profile is captured at
        ...
        label_args = [
            "--label", "jarvis-agent=1",
            "--label", f"jarvis-task-id={task_label}",
            "--label", f"jarvis-profile={profile_name}",
        ]
        ...
        self._labels = {
            "jarvis-agent": "1",
            "jarvis-task-id": task_label,
            "jarvis-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Jarvis process
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("jarvis-task-id", "")
        profile_label = self._labels.get("jarvis-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"jarvis-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``jarvis-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=jarvis-agent=1",
                    "--filter", f"label=jarvis-task-id={task_label}",
                    "--filter", f"label=jarvis-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Jarvis process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Jarvis exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Jarvis process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Jarvis startup. That covers the
        ...
        # outlives Jarvis processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"jarvis-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``jarvis /quit``
