"""Base class for all Jarvis execution environment backends.
...
# JARVIS_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
...
if _DEBUG_INTERRUPT:
    # AIBrain's quiet_mode path (run_brain.py) forces the `tools` logger to
    ...
    # brain.log regardless of quiet-mode.  Scoped to the opt-in case only.
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {JARVIS_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_jarvis_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__JARVIS_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Jarvis backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/jarvis-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/jarvis-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        # caused ``C:/Users/.../jarvis-snap-*.sh: No such file or directory``
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"JARVIS_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        # --- Debug tracing (opt-in via JARVIS_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __JARVIS_CWD_{session}__ marker from stdout output.
