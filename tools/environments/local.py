...
# Jarvis-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Jarvis inference secret exactly
...
def _inject_context_jarvis_home(env: dict) -> None:
    ...
    try:
        ...
        value = get_jarvis_home_override()
        if value:
            env["JARVIS_HOME"] = value
    except Exception:
        pass


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Jarvis-managed secrets from a subprocess environment."""
    try:
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("JARVIS_GIT_BASH_PATH")
    ...
    raise RuntimeError(
        "Git Bash not found. Jarvis requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set JARVIS_GIT_BASH_PATH to your bash.exe location."
    )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Jarvis trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``JARVIS_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under JARVIS_HOME.  Using
