...
Trust levels:
  - builtin:   Ships with Jarvis. Never scanned, always trusted.
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.jarvis/\.env|\~/\.jarvis/\.env',
     "jarvis_env_access", "critical", "exfiltration",
     "directly references Jarvis secrets file"),
    ...
    (r'launchctl\s+load|LaunchAgents|LaunchDaemons',
     ...
     "macOS launch brain/daemon persistence"),
    ...
    (r'\.jarvis/config\.yaml|\.jarvis/SOUL\.md',
     "jarvis_config_mod", "critical", "persistence",
     "references Jarvis configuration files directly"),
...
# scan. `.skillignore` is the Jarvis-native name; `.clawhubignore` is honored
