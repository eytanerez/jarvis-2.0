#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Jarvis Skills Hub.
...
Used by jarvis_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            jarvis_meta = metadata.get("jarvis", {})
            if isinstance(jarvis_meta, dict):
                tags = jarvis_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.jarvis/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                jarvis_meta = meta_block.get("jarvis", {})
                if isinstance(jarvis_meta, dict):
                    tags = jarvis_meta.get("tags", [])
...
def _load_jarvis_index() -> Optional[dict]:
    ...
    """
    # Check local cache
    if JARVIS_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - JARVIS_INDEX_CACHE_FILE.stat().st_mtime
            if age < JARVIS_INDEX_TTL:
                return json.loads(JARVIS_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("Jarvis index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Jarvis index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        JARVIS_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        JARVIS_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class JarvisIndexSource(SkillSource):
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_jarvis_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "jarvis-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "jarvis-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        JarvisIndexSource(auth=auth), # Centralized index (search + resolved install paths)
