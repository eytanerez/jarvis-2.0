...
from jarvis_cli.config import DEFAULT_CONFIG, cfg_get
...
from brain.browser_provider import BrowserProvider as CloudBrowserProvider  # noqa: F401  (legacy alias)
from brain.browser_registry import (  # noqa: F401  (test-patchable surface)
...
# plugins drop in under ``~/.jarvis/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.jarvis/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Jarvis silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Jarvis CLI where sys.stdout and sys.stderr both report
...
def _emergency_cleanup_all_sessions():
    ...
    crashed jarvis processes — this way every clean jarvis exit sweeps
    ...
    # Sweep orphans from other crashed jarvis processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live jarvis processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current jarvis PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live jarvis process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning jarvis process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced jarvis PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent jarvis instances won't reap each
         other's daemons.
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live jarvis process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Jarvis-managed
    ...
    # Build an extended search PATH including Jarvis-managed Node, macOS
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this jarvis PID as the session owner (cross-process safe
...
def _extract_relevant_content(
    ...
    # secrets to the extraction model before run_brain.py's general
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Jarvis
    ...
    screenshots_dir = get_jarvis_dir("cache/screenshots", "browser_screenshots")
