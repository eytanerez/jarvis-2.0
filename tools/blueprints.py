...
`jarvis skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.jarvis.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.jarvis.blueprint`` is a mapping containing
    ...
    jarvis = meta.get("jarvis") if isinstance(meta, dict) else None
    blueprint = jarvis.get("blueprint") if isinstance(jarvis, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.jarvis.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.jarvis.blueprint`` block) they can hand
    to ``jarvis skills publish`` to share. ``body`` is the plain-language
