...
Lets the LLM write a Python script that calls Jarvis tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `jarvis_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `jarvis_tools.py` with file-based RPC stubs
...
# available on every platform Jarvis itself runs on.
...
# match a safe prefix, the operational JARVIS_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "JARVIS_" prefix was deliberately removed (#27303) — it leaked
# JARVIS_*-named config that lacks a secret substring (e.g. JARVIS_BASE_URL,
# JARVIS_KANBAN_DB, JARVIS_*_WEBHOOK).  The child only needs the few
# location/profile vars in _JARVIS_CHILD_ALLOWED below; JARVIS_RPC_SOCKET /
# JARVIS_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational JARVIS_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set jarvis_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_JARVIS_CHILD_ALLOWED = frozenset({
    "JARVIS_HOME",
    "JARVIS_PROFILE",
    "JARVIS_CONFIG",
    "JARVIS_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational JARVIS_* vars (_JARVIS_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret JARVIS_* vars dropped by the tightened allowlist (#27303). The
    # broad "JARVIS_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like JARVIS_KANBAN_DB / JARVIS_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if k.startswith("JARVIS_"):
            ...
            # allowlist — a deliberately-dropped JARVIS_* var.
            _dropped_jarvis.append(k)
    if _dropped_jarvis:
        logger.debug(
            "execute_code: dropped %d non-allowlisted JARVIS_* var(s) from the "
...
# ---------------------------------------------------------------------------
# jarvis_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Jarvis-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Jarvis-profile soft guard."""',
...
def generate_jarvis_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the jarvis_tools.py stub module.
...
def _connect():
    ...
    JARVIS_RPC_SOCKET can be either:
...
"""Auto-generated Jarvis tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("JARVIS_RPC_DIR") or os.path.join(tempfile.gettempdir(), "jarvis_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated jarvis_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/jarvis_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_jarvis_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/jarvis_tools.py", tools_src)
        ...
        env_prefix = (
            f"JARVIS_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
...
def execute_code(
    ...
    # --- Set up temp directory with jarvis_tools.py and script.py ---
    ...
    # selector from JARVIS_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"jarvis_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated jarvis_tools module.
        ...
        tools_src = generate_jarvis_tools_module(list(sandbox_tools))
        ...
        #   connect.  JARVIS_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # Ensure the jarvis-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from jarvis_tools import ...`` resolves even
        ...
        _pp_parts = [tmpdir, _jarvis_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # JARVIS_TIMEZONE is an internal Jarvis setting and must not leak
        # into child processes.
        _tz_name = os.getenv("JARVIS_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("JARVIS_TIMEZONE", None)

        from jarvis_constants import apply_subprocess_home_env
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.jarvis/.env')).
...
def _load_config() -> dict:
    ...
    ``jarvis --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (jarvis-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``jarvis tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and jarvis-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.jarvis/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        ...
        f"Available via `from jarvis_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from jarvis_tools import {import_str}` "
