...
This module provides the glue:
    - ``JarvisTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Jarvis"
"""
...
_USER_SKIPPED_SENTINEL = "__jarvis_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses JARVIS_HOME so each profile gets its own OAuth tokens.
...
def _write_json(path: Path, data: dict) -> None:
    ...
    in ``brain/google_oauth.py`` (#19673).
    """
...
# ---------------------------------------------------------------------------
# JarvisTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class JarvisTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        JARVIS_HOME/mcp-tokens/<server_name>.json         -- tokens
        JARVIS_HOME/mcp-tokens/<server_name>.client.json   -- client info
        JARVIS_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Jarvis records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Jarvis.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://jarvis-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `jarvis mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Jarvis")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`jarvis mcp login {server_name}` interactively first to complete "
