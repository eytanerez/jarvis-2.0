...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.jarvis",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``JARVIS_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside JARVIS_HOME so that a malicious
    ...
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to JARVIS_HOME)",
            relative_path,
        )
        return False
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.jarvis/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="jarvis-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching jarvis_constants.get_jarvis_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_jarvis_dir()`` for backward compatibility with old directory layouts.
    """
    from jarvis_constants import get_jarvis_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_jarvis_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.jarvis``,
    SSH ``<remote_home>/.jarvis``, etc.) and whether translation is wanted.
