...
selectable via ``jarvis tools`` → Image Generation; the active model is
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.jarvis"
    ...
    # Jarvis cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `jarvis tools` → Image Generation."
            )
...
def _build_no_backend_setup_message() -> str:
    ...
    lines.append(
        "  3. Configure a different image_gen provider via `jarvis tools` "
        "→ Image Generation (run `jarvis plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping jarvis with a FAL key configured
...
def _dispatch_to_plugin_provider(
    ...
    try:
        ...
        if reference_image_urls is not None:
            from brain.image_gen_provider import normalize_reference_images
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `jarvis tools` → Image Generation."
                ),
...
# model/provider via `jarvis tools` or `/skills`.
