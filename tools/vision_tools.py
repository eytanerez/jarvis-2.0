...
import httpx
from brain.auxiliary_client import async_call_llm, extract_content_or_reasoning
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("JARVIS_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _should_use_native_vision_fast_path() -> bool:
    ...
    """
    try:
        from brain.auxiliary_client import _read_main_provider, _read_main_model
        from brain.image_routing import decide_image_input_mode, _lookup_supports_vision
...
async def _vision_analyze_native(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            temp_dir = get_jarvis_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $JARVIS_HOME/cache/vision/ and cleaned up
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_jarvis_dir("cache/video", "temp_video_files")
