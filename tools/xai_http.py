...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``jarvis tools`` repaint, tool-registration scans,
    ...
    2. ``~/.jarvis/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.jarvis/.env`` first, then ``os.environ``.

    Wraps :func:`jarvis_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from jarvis_cli.config import get_env_value as _jarvis_get_env_value

        value = _jarvis_get_env_value(name)
...
def jarvis_xai_user_agent() -> str:
    """Return a stable Jarvis-specific User-Agent for xAI HTTP calls."""
    try:
        from jarvis_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Jarvis-Brain/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Jarvis-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``jarvis_cli.config.get_env_value`` so keys
    stored in ``~/.jarvis/.env`` (the standard Jarvis location) are honored —
