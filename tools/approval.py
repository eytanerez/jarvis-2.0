...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("JARVIS_YOLO_MODE", ""))
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from jarvis_cli.plugins import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("JARVIS_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("JARVIS_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("JARVIS_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set JARVIS_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind JARVIS_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds JARVIS_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("JARVIS_CRON_SESSION"):
        return False
    if env_var_enabled("JARVIS_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $JARVIS_HOME, or by the resolved absolute
# active profile home path such as /home/jarvis/.jarvis/config.yaml. The
# resolved-absolute form is folded into the ~/.jarvis/ patterns at detection
...
# go stale when JARVIS_HOME is set after this module is imported, e.g. under the
...
# ~/.jarvis/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _JARVIS_ENV_PATH; matches the JARVIS_HOME override form as
# well as ~/.jarvis/.
_JARVIS_CONFIG_PATH = (
    r'(?:~\/\.jarvis/|'
    r'(?:\$home|\$\{home\})/\.jarvis/|'
...
DANGEROUS_PATTERNS = [
    ...
    # that should always require user consent, just like `jarvis gateway restart`
    ...
    (r'\b(pkill|killall)\b.*\b(jarvis|gateway|cli\.py)\b', "kill jarvis/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill jarvis` but not
    # `kill -9 $(pgrep -f jarvis)` because the substitution is opaque
    ...
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Jarvis file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.jarvis/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Jarvis-managed security file (~/.jarvis/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_JARVIS_CONFIG_PATH}|{_JARVIS_ENV_PATH})', "in-place edit of Jarvis config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_JARVIS_CONFIG_PATH}|{_JARVIS_ENV_PATH})', "in-place edit of Jarvis config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_JARVIS_CONFIG_PATH}|{_JARVIS_ENV_PATH})', "in-place edit of Jarvis config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.jarvis/ form so the Jarvis config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/jarvis/.jarvis/config.yaml`) rather than
    # ~, $HOME, or $JARVIS_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live JARVIS_HOME even when that is set
    # after this module is imported — as the hermetic test conftest does.
    command = _rewrite_resolved_jarvis_home(command)
    return command
...
def _rewrite_resolved_jarvis_home(command: str) -> str:
    """Rewrite the resolved absolute Jarvis home prefix to ``~/.jarvis/``.

    Resolves the active ``JARVIS_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.jarvis/`` so the static ``_JARVIS_CONFIG_PATH`` / ``_JARVIS_ENV_PATH``
    patterns match. No-op when the path can't be resolved or doesn't appear.
    """
    try:
        from jarvis_constants import get_jarvis_home
        home = get_jarvis_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate JARVIS_HOME (e.g. "/" or "") rewriting
        ...
        # /home/jarvis/.jarvis or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.jarvis/")
    return command
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    # before reaching this point (see delegate_tool.py, run_brain.py
    ...
    os.environ["JARVIS_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "JARVIS_SPINNER_PAUSE" in os.environ:
            del os.environ["JARVIS_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("JARVIS_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set JARVIS_INTERACTIVE or JARVIS_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("JARVIS_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("JARVIS_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("JARVIS_EXEC_ASK")
