...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Jarvis exit
...
# display_jarvis_home imported lazily at call site (stale-module safety during jarvis update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of jarvis directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "jarvis-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("JARVIS_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("JARVIS_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("JARVIS_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (JARVIS_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (JARVIS_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive Jarvis password prompt or the sudo -S password-pipe path.
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Jarvis can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``jarvis-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Jarvis processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Jarvis processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Jarvis processes a generous grace
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.jarvis/.env or environment variables."
        )
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for jarvis-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Jarvis processes that hit SIGKILL / OOM / a closed terminal
...
def cleanup_vm(task_id: str, *, force_remove: bool = False):
    ...
    *force_remove* (default False) is forwarded to backends that accept it
    ...
    ``AIBrain.close()`` (TUI session close, gateway session teardown) and the
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Jarvis can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/jarvis
        # restart|stop targeting jarvis-gateway) must never run inside the
        ...
        # never restart. This mirrors the `jarvis gateway restart` guard in
        # jarvis_cli/gateway.py and the cron-path guard in jarvis_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_JARVIS_GATEWAY") == "1":
            from jarvis_cli.cron import _contains_gateway_lifecycle_command
            if _contains_gateway_lifecycle_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Run `jarvis gateway restart` from a separate shell outside "
                        "the running gateway."
                    ),
                    "status": "error",
                }, ensure_ascii=False)
        ...
        if background:
            ...
            try:
                ...
                # jarvis-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in jarvis-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("JARVIS_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("JARVIS_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("JARVIS_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("JARVIS_SESSION_USER_ID", "")
                        _gw_user_name = _gse("JARVIS_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("JARVIS_SESSION_MESSAGE_ID", "")
