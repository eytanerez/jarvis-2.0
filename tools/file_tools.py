...
from brain.file_safety import get_read_block_error
...
_jarvis_config_resolved_loaded = False


def _get_jarvis_config_resolved() -> str | None:
    """Return the resolved absolute path of the Jarvis config file (cached)."""
    global _jarvis_config_resolved, _jarvis_config_resolved_loaded
    if _jarvis_config_resolved_loaded:
        return _jarvis_config_resolved
    _jarvis_config_resolved_loaded = True
    try:
        from jarvis_cli.config import get_config_path
        _jarvis_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _jarvis_config_resolved = str(Path("~/.jarvis/config.yaml").expanduser().resolve())
        except Exception:
            _jarvis_config_resolved = None
    return _jarvis_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    # this file.
    jarvis_config = _get_jarvis_config_resolved()
    if jarvis_config and (resolved == jarvis_config or normalized == jarvis_config):
        return (
            f"Refusing to write to Jarvis config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.jarvis/config.yaml directly or use 'jarvis config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Jarvis mirror prefix for Docker file tools."""
    try:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Jarvis
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Jarvis state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.jarvis/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Jarvis process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.jarvis/…`` path.

    Returns ``None`` when the write is in-scope or outside Jarvis scope.
    ...
    directly. See ``brain/file_safety.classify_cross_profile_target``,
    ...
    # in a session that cd'd into ``~/.jarvis/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Jarvis internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under JARVIS_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == JARVIS_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Jarvis-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Jarvis-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Jarvis profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Jarvis profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with jarvis_tools.write_file() for very "
            "large files."
        )
