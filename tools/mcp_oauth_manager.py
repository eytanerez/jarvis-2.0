...
`mcp_tool.py`, and `jarvis_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# JarvisMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_jarvis_provider_class() -> Optional[type]:
    ...
    class JarvisMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._jarvis_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`JarvisTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, JarvisTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._jarvis_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import JarvisTokenStorage
                        if isinstance(storage, JarvisTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._jarvis_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import JarvisTokenStorage
            if not isinstance(storage, JarvisTokenStorage):
                return
    ...
    return JarvisMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_JARVIS_PROVIDER_CLS: Optional[type] = _make_jarvis_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`JarvisMCPOAuthProvider` directly using the helpers
        ...
        storage = JarvisTokenStorage(server_name)

        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                "MCP OAuth for "
                f"'{server_name}': non-interactive environment and no "
                "cached tokens found. Run `jarvis mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``jarvis mcp remove <name>`` and (indirectly) by
        ``jarvis mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
