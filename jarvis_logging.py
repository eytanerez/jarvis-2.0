...
``~/.jarvis/logs/`` (profile-aware via ``get_jarvis_home()``).

Log files produced:
    brain.log   — INFO+, all brain/tool/session activity (the main log)
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``jarvis_cli.web_server``,
    ``jarvis_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
    brain.log remains the catch-all (everything goes there).
...
# another process holds an append-mode handle on ``brain.log`` — which is
# essentially always in Jarvis (TUI, gateway, ``hy_memory`` server, MCP
# servers, and on-demand CLI commands all log from separate processes),
# pinning ``brain.log`` at the 5 MiB threshold and spamming stderr with
...
from jarvis_constants import get_config_path, get_jarvis_home
...
class _ComponentFilter(logging.Filter):
    ...
    keeping ``brain.log`` as the catch-all.
    """
...
COMPONENT_PREFIXES = {
    ...
    "cron": ("cron",),
    "gui": (
        "jarvis_cli.web_server",
        "jarvis_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Jarvis logging subsystem.
    ...
    *force* is ``True``.
    ...
    log_level
        Minimum level for the ``brain.log`` file handler.  Accepts any
    ...
    # --- brain.log (INFO+) — the main activity log -------------------------
...
def setup_verbose_logging() -> None:
    ...
    Called by ``AIBrain.__init__()`` when ``verbose_logging=True``.
    """
    from brain.redact import RedactingFormatter
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the jarvis group. However, both ``_open()`` (initial creation)
