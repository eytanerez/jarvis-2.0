# OpenAI-Compatible API Server for Jarvis
...
SSE streaming. By exposing this endpoint, jarvis-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  jarvis-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.jarvis/.env`
2. Run `jarvis gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with jarvis-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns jarvis-agent as a model) |
...
AIBrain instances directly. Simpler but duplicates session/auth logic.
...
Authorization: Bearer jarvis-api-key-here
...
But jarvis-agent sessions have persistent state (memory, skills, tool context).
...
**Phase 2:** Add a response callback to AIBrain that emits text chunks as the
...
Frontends send `"model": "jarvis-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured jarvis-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of jarvis-agent's core prompt
...
2. Add `stream_callback` parameter to `AIBrain.run_conversation()`
...
Once implemented, jarvis-agent works as a drop-in backend for:
